...
Kiem Vong Giang Ho: StillObject.mDropLightObj cot sang theo pham chat + cmn_drop_flash loe luc cham dat):
  * spr\vatroi\cotsang.spr : 1 khung 48x120, cot sang TRANG mo dan len tren + vung sang o chan; neo (cx, cy) = (12, 88): chan cot (24, 112)
                             roi vao giua-duoi icon vat pham 24x24 (icon co cx=cy=0, goc trai tren tai diem dat). Mau: KObj::Draw ve bang
...
def main():
    ...
    for dich in DICH:
        ...
        n1 = ghi_spr([cot], os.path.join(dich, "cotsang.spr"), 12, 88)
        n2 = ghi_spr(loe, os.path.join(dich, "loe.spr"), 20, 8)
