...
function callPlayerFunction(pid,fun,...) local arg = {n = select("#", ...), ...};
	local oldPid = PlayerIndex
	PlayerIndex = pid
...
function Player:Pack(...) local arg = {n = select("#", ...), ...};
	return arg
end

function Player:Call(pFun, ...) local arg = {n = select("#", ...), ...};
	if not self.m_PlayerIndex or self.m_PlayerIndex <= 0 then
		return
	end
	local nOldPlayer = PlayerIndex;
...
function Player:CastSkill(...) local arg = {n = select("#", ...), ...};
	return self:CallWithParam(CastSkill, arg)
end

function Player:OpenProgressBar(...) local arg = {n = select("#", ...), ...};
...
function Player:SetTempRevPos(...) local arg = {n = select("#", ...), ...};
	return self:CallWithParam(SetTempRevPos, arg)
end

function Player:SetRevPos(...) local arg = {n = select("#", ...), ...};
