...
if MODEL_RELAY == 1 then
	Include("\\script\\tong\\tong_blacklist.lua")
end
...
function MONEYFUND_ADD_R(nTongID, nAdd)
	if IsInBlackList(nTongID) == 1 then
		if ExecutorId ~= 0 then
			local szMember = TONGM_GetName(nTongID, ExecutorId);
			Msg2PlayerByName(szMember, "Tr¹ng th¸i bÊt th­êng xin mêi liªn l¹c víi nhµ ®iÒu hµnh xö lý.")
			cTongLog:WriteInfTB("SYS", "moneyaddfail", nTongID, {nMoney = nAdd, Executor=szMember})
		end
		return 0
	end
	local nMoney = TONG_GetMoney(nTongID)
	if (nMoney + nAdd < 0)then
		return 0
	end
	return 1
end

function AFTER_MONEYFUND_ADD_R(nTongID, nMoneyBeforChange, nAdd)
	local szKey = "MoneyFundAddSys"
	...
	local nAfterChanged = TONG_GetMoney(nTongID)
	cTongLog:WriteTongMoneyChangeLog(nTongID, szKey, nMoneyBeforChange, nAdd, nAfterChanged)
end

function MONEYFUND_SET_R(nTongID, nMoney)
	if IsInBlackList(nTongID) == 1 then
		return 0
	end
	return 1
...
function AFTER_MONEYFUND_SET_R(nTongID, nMoneyBeforChange, nChange)
	local nAfterChanged = TONG_GetMoney(nTongID)
	cTongLog:WriteTongMoneyChangeLog(nTongID, "MoneyFundSet", nMoneyBeforChange, nChange, nAfterChanged)
end
...
function MONEYFUND2BUILDFUND_R(nTongID, nOffer)
	...
	local nBeforeChanged = TONG_GetMoney(nTongID)
	if (nBeforeChanged < nOffer)then
		return 0
	end
	...
	if (TONG_ApplyAddMoney(nTongID, -nOffer) == 1 and TONG_ApplyAddBuildFund(nTongID, nAdd) == 1)then
		...
		cTongLog:WriteTongMoneyChangeLog(nTongID, "MoneyFundToBuildFund", nBeforeChanged, -nOffer, nAfterChanged)
		return 1
	end
	return 0
end
...
function MEMBER_RETIRE_R(nTongID, nMemberID, bRetireOrNot)
	...
	if (bRetireOrNot == 1) then
		...
		if (ExecutorId ~= nMemberID) then
			...
			szMsg = szExecutorName.." nãi víi "..szMemberName.."TiÕn hµnh thao t¸c tho¸i Èn"
		else
...
--					TB_STUNTID_INFO[6].name.."/#StuntConfirm(6)",
		...
		local szMsg = "Kü n¨ng bang héi hiÖn t¹i: "..
...
function CHANGE_STUNT_G_1(nTongID)
	...
	if (nStuntID ~= 0) then	
		local szMsg = "Kü n¨ng bang héi hiÖn t¹i: "..TB_STUNTID_INFO[nStuntID].name
