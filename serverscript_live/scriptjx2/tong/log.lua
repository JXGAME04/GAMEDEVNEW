...
function cTongLog:TB2StrMsg(szAction, nTongID, tbMsg)
	local szMag	= ""
	for key, value in pairs(tbMsg) do
...
function cTongLog:WriteTongMoneyChangeLog(nTongID, szKey, nMoneyBefore, nMoneyChange, nMoneyAfter)
	self:WriteInfTB("FUND", "MoneyChanged", nTongID, {key=szKey, Before=nMoneyBefore, Change=nMoneyChange, After=nMoneyAfter})
end
