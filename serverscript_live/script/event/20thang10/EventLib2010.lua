...
TAB_NEED = {
{1,3735,"Hoa HÂng Træng ",},
{2,3736,"Hoa HÂng Xanh ",},
{3,3737,"Hoa HÂng Vµng",},
{4,3738,"Hoa hÂng Æ·",},
{5,3741,"Gi· ß˘ng Hoa",},
{6,3739,"B„ HÂng Tam Sæc ",},
{7,3740,"B„ HÂng T¯ Sæc ",},
{8,3745,"Gi· Hoa HÂng V‹nh Cˆu ",},
}
...
local nTaskNum = 0;
	if (nNum == 1) then
		nTaskValue = GetTask(TASK_EVENT2011_1);
		nTaskNum = GetNumber(4,nTaskValue,1);
		if nTaskNum > 2000 then
		return
		end
		Exp = 1E6;
		SetTask(TASK_EVENT2011_1,SetNumber(4,nTaskValue,1,nTaskNum+1));
		
	elseif (nNum == 2) then
		nTaskValue = GetTask(TASK_EVENT2011_2);
		nTaskNum = GetNumber(4,nTaskValue,1);
		SetTask(TASK_EVENT2011_2,SetNumber(4,nTaskValue,1,nTaskNum+1));
		Exp = 2E6;
	...
	elseif (nNum == 3) then
		...
		nTaskNum = GetNumber(4,nTaskValue,1);
		SetTask(TASK_EVENT2011_3,SetNumber(4,nTaskValue,1,nTaskNum+1));
		Exp = 5E6;
		nRan = random(1,getn(TAB_Bonus[3]));
		if random(1,100) > 99 then
			local nRanF = random(1,8);
			if (nRanF == 1) then
			nIndex = AddGoldItem(random(168,176),0);--nhu tinh
			-- AddTimeItem(nIndex,7*24*60*60);
			elseif (nRanF == 2) then
			nIndex = AddGoldItem(random(159, 163),0);--nhu tinh
			-- AddTimeItem(nIndex,7*24*60*60);
			elseif (nRanF == 3) then
			nIndex = AddGoldItem(random(168,176),0);--hiepcot
			-- AddTimeItem(nIndex,7*24*60*60);
			elseif (nRanF == 4) then
			nIndex = AddGoldItem(random(168,176),0);--hiepcot
			-- AddTimeItem(nIndex,7*24*60*60);
			...
			-- AddTimeItem(nIndex,7*24*60*60);
			elseif (nRanF == 6) then
			AddItem(6,1,2405,0,0,0,0 );--phi van
			elseif (nRanF == 7) then
			AddItem(6,1,1126,0,0,0,0 );--bk 12x
			elseif (nRanF == 8) then
				if random(1,10) > 8 then
					nIndex =	AddGoldItem(random(164, 167),0); --dinhquoc
				else
					nIndex = AddGoldItem(random(159, 163),0); --dinhquoc
