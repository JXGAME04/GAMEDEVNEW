...
tbLog = {}
function tbLog:TabFormatLog(...) local arg = {n = select("#", ...), ...};
	local szLog = arg[1]
	for i=2, getn(arg) do
...
function tbLog:PlayerActionLog(szKeyLog, ...) local arg = {n = select("#", ...), ...};
	self:TabFormatLog(
...
function tbLog:VngGetItemID(tbItem,nAwardCount)
	for k, v in pairs(self.tbLogItemCount) do
		if(tbItem[k]) then
...
function tbLog:VngActionFormatLog(tbLogTitle,szLogItemId)
	...
	local szFaction = tbFactionNumber2Name[GetLastFactionNumber()] or "Kh«ng ph¸i";
