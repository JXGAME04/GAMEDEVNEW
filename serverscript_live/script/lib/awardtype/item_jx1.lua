...
function ItemJX1Type:Give(tbItem, nAwardCount, tbLogTitle)
	...
	local nDaNhan = 0	-- [MSGAWARD 01/09]
	local i
	for i = 1, nAmount do
		local nItemIdx
	...
	-- [MSGAWARD 01/09] thong bao nhu Linux lib\awardtype\item.lua:124 - khong co thi nguoi choi
	-- tuong thuong (vd Tinh Ngoc Nguyen Thach) khong roi.
	if nDaNhan > 0 then
		Msg2Player(format("NhËn ®­îc %d %s.", nDaNhan, tbItem.szName or "vËt phÈm"))
	end
	return 1
end
