...
function AUC_Broadcast(nType, szAct, szEnum, fnPush, nTongLoc)
    for nIdx, v in pairs(AUC_Viewers) do
...
function AUC_ConLai(nEnd)
    ...
    if nPhut < 60 then
        return "cßn "..nPhut.." phót"
    end
    if mod(nPhut, 60) == 0 then
        return "cßn "..floor(nPhut / 60).." giê"
    end
    return "cßn "..floor(nPhut / 60).." giê "..mod(nPhut, 60).." phót"
end
...
function AUC_ChatHuongDan(nType)
    if nType == AUCTION_DEF.tbAuctionTypeEnum.eType_TONG then
        return " Më cöa sæ §Êu gi¸, thÎ Bang héi ®Ó tr¶ gi¸."
    end
    return " Më cöa sæ §Êu gi¸, thÎ ThÕ giíi ®Ó tr¶ gi¸."
end
...
function AUC_ChatGui(nType, nTong, sz)
    if nType == AUCTION_DEF.tbAuctionTypeEnum.eType_WORLD then
        Msg2SubWorld(AUC_ChatCat("[§Êu gi¸ thÕ giíi] "..sz))
        return 1
    end
    if nType ~= AUCTION_DEF.tbAuctionTypeEnum.eType_TONG or (nTong or 0) <= 0 then
        return 0
    end
    sz = AUC_ChatCat("[§Êu gi¸ bang] "..sz)
    if AUC_MsgTong then
...
function AUC_ChatLenSan(nType, nTong, szAct, szNguoi, szTen, nCur, nGuar, nBase, nCurP, nKind, nEnd)
    ...
    local sz = szNguoi.." ®­a "..szTen.." lªn sµn ("..szAct.."): "
    if nKind == AUCTION_DEF.tbItemTypeEnum.eType_DUTCH then
        sz = sz.."gi¸ më "..AUC_GiaTien(nCurP, nCur)..", h¹ dÇn tíi "..AUC_GiaTien(nGuar, nCur)
    else
        sz = sz.."khëi ®iÓm "..AUC_GiaTien(nGuar, nCur)
        if (nBase or 0) > (nGuar or 0) then
            sz = sz..", mua ngay "..AUC_GiaTien(nBase, nCur)
        end
    end
    sz = sz..", "..AUC_ConLai(nEnd).." (kÕt thóc "..AUC_GioDongHo(nEnd)..")."..AUC_ChatHuongDan(nType)
...
function AUC_ChatTraGia(r)
    ...
    local sz = r.buyer.." tr¶ "..AUC_GiaTien(r.cur, r.currency).." cho "..r.name.." - muèn v­ît ph¶i tr¶ tõ "..AUC_GiaTien(r.cur + nStep, r.currency)
...
function AUC_ChatBan(r, szBuyer, nPrice, nNet, bMuaNgay)
    ...
    local sz
    if bMuaNgay == 1 then
        sz = szBuyer.." mua ngay "..r.name.." víi gi¸ "..AUC_GiaTien(nPrice, r.currency)..", vËt phÈm göi qua th­."
    else
        sz = "HÕt giê: "..szBuyer.." th¾ng "..r.name.." víi gi¸ "..AUC_GiaTien(nPrice, r.currency)..", vËt phÈm göi qua th­."
    end
    if r.atype == AUCTION_DEF.tbAuctionTypeEnum.eType_TONG then
        if r.currency == AUCTION_DEF.tbCurrency.MONEY then
            sz = sz.." Quü bang nhËn "..AUC_GiaTien(nNet, r.currency).."."
        else
            sz = sz.." Ng­êi b¸n nhËn "..AUC_GiaTien(nNet, r.currency).."."
        end
    end
    AUC_ChatGui(r.atype, r.tong, sz)
    return 1
...
function AUC_ChatE(r)
    ...
    local sz = "HÕt giê: "..r.name.." kh«ng ai tr¶ gi¸, "
    if r.atype == AUCTION_DEF.tbAuctionTypeEnum.eType_WORLD then
        sz = sz.."®· thu håi."
    else
        sz = sz.."tr¶ vÒ ng­êi b¸n qua th­."
    end
    AUC_ChatGui(r.atype, r.tong, sz)
    return 1
end
...
function AUC_ChatRut(r)
    ...
    AUC_ChatGui(r.atype, r.tong, r.name.." ®· ®­îc rót khái sµn (ch­a ai tr¶ gi¸).")
    return 1
end
...
function AUC_ChatHaGia(r)
    ...
    local sz = r.name.." h¹ gi¸ cßn "..AUC_GiaTien(r.cur, r.currency)
    if (r.dropleft or 0) > 0 then
        sz = sz.." (cßn "..r.dropleft.." lÇn h¹, thÊp nhÊt "..AUC_GiaTien(r.guar, r.currency)..")"
    else
        sz = sz.." (gi¸ thÊp nhÊt)"
    end
    sz = sz..", "..AUC_ConLai(r.endtime).."."
...
function AucWeb_ChatDot(szAct, nRound, nEnd, tbBao)
    ...
    AUC_ChatGui(nType, 0, szAct.." më: "..n.." mãn míi, "..AUC_ConLai(nEnd).." (kÕt thóc "..AUC_GioDongHo(nEnd).."). Ai tr¶ cao nhÊt khi hÕt giê ®­îc nhËn qua th­."..AUC_ChatHuongDan(nType))
    for i = 1, n do
        local m = tbBao[i]
        local sz = i..". "..m.ten..": khëi ®iÓm "..AUC_GiaTien(m.kd, m.tien)
        if (m.mn or 0) > 0 then
...
function AucWeb_Bao(szAct, nId)
    AUC_NotifyNewItem(AUCTION_DEF.tbAuctionTypeEnum.eType_WORLD, szAct, nId, 0)
    return 1
end
...
function AUC_Tick()
    ...
    for _, nType in ipairs({AUCTION_DEF.tbAuctionTypeEnum.eType_WORLD, AUCTION_DEF.tbAuctionTypeEnum.eType_TONG}) do	-- [LUA54 05/09] Lua 4 'in {..}' -> ipairs (loi 12:42 'attempt to call a table value')
        ...
        for k, v in pairs(AUC_LastCount) do
