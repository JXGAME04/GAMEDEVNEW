...
function g_GiveItemUI(szTitle, szContent, pFunData, pCancelData, nParam, pCheckData, bConfirm)
	G_PlayerGiveItemUIData[PlayerIndex] = {pFunData, pCancelData, pCheckData}
	local nParam2 = bConfirm or 0
	GiveItemUI(szTitle, szContent, "g_GiveItemUIBack", "g_GiveItemUICancel", nParam, "g_GiveItemUICheck", nParam2)
end
...
function g_GiveItemUICheck(nCount)
	local pData = G_PlayerGiveItemUIData[PlayerIndex];
...
function g_GiveMantleInlayUI()
    local handle = OB_Create(); 
    ObjBuffer:PushByType(handle, OBJTYPE_NUMBER, 0)
    ScriptProtocol:SendData("emSCRIPT_PROTOCOL_MANTLE_OPENUI", handle);
    OB_Release(handle);
end

function g_ReturnInlayResult(nSuccess)
    local handle = OB_Create();
    ObjBuffer:PushByType(handle, OBJTYPE_NUMBER, nSuccess);
    ScriptProtocol:SendData("emSCRIPT_PROTOCOL_MANTLE_INLAYRESULT", handle);
    OB_Release(handle);
end
