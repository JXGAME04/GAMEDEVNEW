...
--发放奖励
	...
	if ( tonumber( TabFile_GetCell( "Tollgateprize" ,prize_givefinish , "prize_Quality" )) == 1 ) then
		-- [FIX 25/08] DropItem/Ex JX1 KHAC CHU KY Linux (JX1: tham so 1 = NPC index, 2 = genre
		-- - ScriptFuns.cpp:4416; Linux: world,x,y,player,...). Goi kieu Linux la JX1 doc TOA DO
		-- lam GENRE -> khong bao gio ra do (khuon 44c38de3 cua 3HD). Ham nay hien MA CHET
		-- (khong ai goi messenger_giveprize) - va toi thieu: roi tai nguoi goi (tham so 1 = 0).
		DropItemEx(0,4,0,1,0,prize_numberone,0,0,0,0,0)	
	else 
		DropItem(0,prize_numberone,prize_numbertwo,prize_numberthree,prize_numberfour,prize_numberfive,prize_numbersix,prize_numberseven)	-- [FIX 25/08] chu ky JX1
	end	
