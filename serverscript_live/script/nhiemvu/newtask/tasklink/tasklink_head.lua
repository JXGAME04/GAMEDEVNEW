...
function DT_CFG(szKhoa, macdinh)
	if (G_CFG ~= nil) then
...
TL_MAXTIMES = DT_CFG("DT_SO_NV_MOI_VONG", 20) -- Ã¿¸öÁ´ÓĞ 20 ¸öÈÎÎñ
TL_MAXLINKS = DT_CFG("DT_SO_VONG_MOI_CHU_KY", 20) -- Ã¿¸ö»·ÓĞ 20 ¸öÁ´
TL_MAXLOOPS = DT_CFG("DT_SO_CHU_KY_TOI_DA", 20) -- Íæ¼Ò×Ü¹²¿ÉÒÔ×ö 20 »·µÄÈÎÎñ
...
-- ¶ÔÓ¦²»Í¬µÄÈÎÎñ×÷²»Í¬µÄ±äÁ¿´æ´¢´¦Àí
	...
	elseif ( myTaskType == 5 ) then -- Èç¹ûÊÇÊôÓÚÊıÖµÉı¼¶µÄÈÎÎñµÄ»°£¬ÔòÅĞ¶ÏÊÇÄÄÖÖÊıÖµ
		...
		elseif (myTaskValueType == 4) then -- Èç¹ûÊÇÕÇ¸£Ôµ
			nt_setTask(1026,GetFuYuan()) -- [JX1 PORT] goc: nt_getTask(151) - kho phuc duyen JX2, JX1 khong ghi 151
		elseif (myTaskValueType == 5) then -- Èç¹ûÊÇÕÇ PK Öµ
			nt_setTask(1026,GetPK())
		elseif (myTaskValueType == 6) then -- Èç¹ûÊÇÕÇËÎ¾©»ı·Ö
			nt_setTask(1026,GetTask(38)) -- [JX1 PORT] goc: nt_getTask(747) - tich luy TK ben JX1 nam o task 38
		end
...
local myTaskID = tl_gettasktablecol()
	...
	elseif (myTaskType == 5) then
		...
		elseif (myTaskValueType == 4) then -- ¸£ÔµÖµ
			myOrgValue = GetFuYuan() -- [JX1 PORT] goc: nt_getTask(151)
			if (myOrgValue-(nt_getTask(1026)) >= myTaskValue) then
				return 1
			else
				return 0
		...
		elseif (myTaskValueType == 6) then -- ËÎ½ğ»ı·Ö
			myOrgValue = GetTask(38) -- [JX1 PORT] goc: nt_getTask(747)
			if (myOrgValue-(nt_getTask(1026)) >= myTaskValue) then
				return 1
			else
				return 0
...
local myTableRow = TabFile_GetRowCount(myTableName) -- »ñÈ¡TABLEÎÄ¼şµÄ×ÜĞĞÊı
	...
	for i = 3,myTableRow-1,2 do	--[LUA54] than vong co 'i = i + 1' (Lua 4 = buoc 2); 5.4 phai ghi buoc 2 o day
		myReadRate = tonumber(TabFile_GetCell(myTableName,myTableRow,myTableCol))
		myTotalRate = myTotalRate + myReadRate
		--[LUA54] bo 'i = i + 1': da chuyen thanh buoc 2 o dong for
	end
