...
local nCanO = 0
	...
	-- [MAIL 03/09 D9] thuong MOC Da Tau gui QUA THU: giu dung so luong / khoa / han tung dong, khong can cho trong tui
	tl_linkaward_mail(tbCol, nTask)
end
-- [MAIL 03/09 D9] gom cac dong cua moc thanh 1 thu (gold: hoang kim theo dong goldequip.txt nhu tl_linkaward_give)
function tl_linkaward_mail(tbCol, nTask)
	...
	local nId = MailManager_SendRewardTemplet("datau", nil, "Th­ëng mèc "..nTask.." nhiÖm vô D· TÈu", "Chóc mõng ®¹i hiÖp hoµn thµnh mèc "..nTask.." nhiÖm vô D· TÈu, phÇn th­ëng ®İnh kÌm trong th­.<enter>Tr©n träng", tb, 30)
	if (nId > 0) then
		Msg2Player("PhÇn th­ëng mèc "..nTask.." D· TÈu ®· göi vµo hép th­ ("..getn(tb).." môc).")
...
local szSL = ""
	...
	Msg2Region(SubWorldIdx2ID(SubWorld),
		" <color=Yellow>"..GetName().." <color>".."nhËn ®­îc"..
		" <color=Gold>"..szTen..szSL.." <color>".."- D· TÈu lÇn"..
