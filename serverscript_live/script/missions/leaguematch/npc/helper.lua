...
WLLS_REG_TABLE	= {
	...
	{"m«n ph¸i",	GetLastFactionNumber,	{"ThiÕu L©m", "Thiªn V­¬ng", "§­êng M«n", "Ngò §éc", "Nga Mi", "Thóy Yªn", "C¸i Bang", "Thiªn NhÉn", "Vâ §ang", "C«n L«n", " Hoa S¬n"},""},
	{"®¼ng cÊp",	GetLevel,				{{80,99}, {100,119}, {120,129}, {130,139}, {140,149}, {150,nil}},	""},
...
function main()
	...
	Say("Sø gi¶ liªn ®Êu: Mïa nµy thi ®Êu theo thÓ thøc " .. WLLS_TAB[n_ntype].name .. ". GÆp ta ®Ó t×m hiÓu th«ng tin c¸c tuyÓn thñ kh¸c, hoÆc ®¨ng ký th«ng tin cña m×nh ®Ó tiÖn ghÐp ®éi.", getn(tb_option), tb_option)
end
...
function wlls_save_myinfo()
	local tb_info = {}
	for n_idx, tb_fmt in pairs(WLLS_REG_TABLE) do
...
function wlls_reg_query_single_do(n_idx, n_need)
	local tb_info	= {}
	for n, tb_fmt in pairs(WLLS_REG_TABLE) do
		if (n == n_idx) then
			tb_info[n]	= n_need
		else
			tb_info[n]	= 0
...
function wlls_get_infostr(str_name)
	...
	if FALSE(n_lid) then
		return "Sø gi¶ liªn ®Êu: Xin lçi! thµnh viªn '"..safeshow(str_name).."' kh«ng tån t¹i, hoÆc ®·   xãa bá."
	end
