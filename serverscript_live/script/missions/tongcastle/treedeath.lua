...
JOIN_TONG_TIME = HD_CFG("TC_VAO_BANG_PHUT", 1440)	-- [HD CONFIG 24/08] FIX: file goc thieu -> %JOIN_TONG_TIME capture nil
...
function CheckNearTreeNpcEx(nNpcIndex)
	local tbNpc, nCount = GetNpcAroundNpcList(nNpcIndex, HD_CFG("TC_BANKINH_BUA", 15))
	local nRes = 0 
	for i = 1, nCount do 
...
function RegisterNpc()
	for szNpcName, _ in pairs(TongCastle.tbNpcTypeList) do	
