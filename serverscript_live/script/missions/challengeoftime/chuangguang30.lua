...
tbVnItemAwardEx = {
	[1] = {--PhÇn 1
		{szName="Tinh Tinh Kho¸ng",tbProp={6,1,4429,1,0,0},nCount=1,nRate=20},
	...
	[3] = {--PhÇn 3
		...
		{szName = "§»ng Long Th¹ch - H¹", tbProp = {6,1,4866,1,0,0}, nRate = 6,nCount=5},
...
function ChuangGuan30:Regist(szType, pFun, ...) local arg = {n = select("#", ...), ...};
...
function ChuangGuan30:Unregist()
	if self.tbRegist then
		for szType, tbId in pairs(self.tbRegist) do
...
function ChuangGuan30:OnNpcDeath(nNpcIndex, nPlayerIndex)
	...
	for _, szName in pairs((self.tbGroup[nGroup])) do
...
function ChuangGuan30:KickOutAll()
	for szName, bFlag in pairs(self.tbPlayer) do
		if bFlag == 1 then
			local nPlayerIndex = SearchPlayer(szName)
...
-- [3HD 25/08] FORBITMAP_LIST khai o item\heart_head.lua ban Linux - tep do KHONG duoc chep (JX1 da co ban khac) => nil => chunk dut o dong 273, Init()+RegistAll() khong chay => Vuot Ai chet. Khuon va y het TONGCASTLE 23/08 (tongcastle\game.lua:164).
