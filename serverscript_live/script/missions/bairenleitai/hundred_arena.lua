Include("\\script\\header\\cauhinh_hoatdong.lua")	-- [HD CONFIG 24/08] gio mo / exp / nguong chinh duoc (LIVE)
-- ====================== ÎÄ¼þÐÅÏ¢ ======================
...
function HundredArena:UnReg()
	if self.tbRegId then
		for szType, nId in pairs(self.tbRegId) do
...
function HundredArena:RegMap(szType, varFun, ...) local arg = {n = select("#", ...), ...};
...
function HundredArena:OnTime()
	...
	if (nHour == HD_CFG("BR_GIO_DONG_H", 0)) then
		self:Close();
		print("HundredArena:Close")
		return 1
	end
	
	if (nHour >= HD_CFG("BR_GIO_DONG_H", 0) and nHour < floor(HD_CFG("BR_GIO_MO", 1200)/100)) then
...
--	Msg2SubWorld("HundredArena:OnTime:"..t)
	...
	if (nMin == 0 or mod(nMin, HD_CFG("BR_PHUT_CHUKY_EXP", 5)) == 0) then
		for szname, tb in pairs(self.tbPlayerList) do
			
			if (tb.nLastServerTime > nCurSvrTime) then
				--Change request 14/07/2011 - Modified By DinhHQ - 2011
				--local nExp = 122400
				local nExp = HD_CFG("BR_EXP_TICK", 1000000)
				...
				if (tb.nDouble and tb.nDouble > GetCurServerTime()) then					
					...
					if PlayerFunLib:GetTaskDailyCount(self.nTaskExpLimit) < HD_CFG("BR_TRAN_LUOT_NGAY", 50) then
				...
				--local nExpEx = 204000;
				local nExpEx = HD_CFG("BR_EXP_LOICHU", 2000000);
				if (tb.nRight == 1) then					
					if PlayerFunLib:GetTaskDailyCount(self.nTaskExpLimit) < HD_CFG("BR_TRAN_LUOT_NGAY", 50) then
						PlayerFunLib:AddTaskDaily(self.nTaskExpLimit, 2)
	...
	if (nMin == 0 or mod(nMin, HD_CFG("BR_PHUT_COTHU", 30)) == 0) then
...
--		end
		...
		WriteLog(format("BaiRenLeiTai\tCallNPC_GuShou:%d,%d,%d",
	 	...
	 	self.nDoubleCount = floor(nPlayerCount * HD_CFG("BR_TILE_BUFFX2", 0.2))--0.2;
...
function HundredArena:Announce(szMsg)
	for name, tb in pairs(self.tbPlayerList) do
...
function HundredArena:IsOpen()
	...
	if (ntime >= HD_CFG("BR_GIO_MO", 1200)) then
		
		if (GetLevel() >= HD_CFG("BR_CAP_TOITHIEU", 90)) then
			-- [24/08 hau phan bien] doc mot lan de ham tu ghi lai NGAY MOI va day gia tri da
			-- reset xuong client; khong the bang chi nam (F11) con doc so luot cua ngay cu
			-- va bao "het luot" oan.
			PlayerFunLib:GetTaskDailyCount(self.nTaskExpLimit)	-- [JX1 23/08] chinh sach cap 90
			return 1;
		else
			Talk(1, "", "<color=yellow>CÊp "..HD_CFG("BR_CAP_TOITHIEU", 90).." vµ "..HD_CFG("BR_CAP_TOITHIEU", 90).." trë lªn<color>míi cã thÓ tham gia.")
		end
		
	else
		Talk(1, "", "Thêi gian më L«i §µi mçi ngµy<color=yellow> vµo lóc "..HD_GioPhut(HD_CFG("BR_GIO_MO", 1200)).." ~ 24:00<color>, xin h·y quay l¹i sau!")
	end
	return 0;
end
...
function ArenaField:CallFightNpc()
	...
	tb_npc.nLevel = HD_CFG("BR_CAP_NPC_GOC", 90) + nLevel;
...
function ArenaField:IsMaxGrade(nGrade)
	...
	if (n_last_grade < HD_CFG("BR_TRAN_CHUOI", 100)) then
		...
		if (self.Master) then
			...
			if (self.nArenaId == 1) then
				local sz_msg = format("<color=yellow>%s<color>Liªn tôc chiÕn th¾ng t¹i L«i §µi Hoµng Thµnh T­<color=yellow>%d<color>ng­êi khiªu chiÕn, ®· trë thµnh mét truyÒn thuyÕt ®ån ®¹i trªn giang hå.",
								self.Master.szPlayerName, HD_CFG("BR_TRAN_CHUOI", 100) );
...
-- ===== -- [JX1 23/08] cau noi 1 state/tep: npc_death.lua & player_death.lua goi sang state nay
