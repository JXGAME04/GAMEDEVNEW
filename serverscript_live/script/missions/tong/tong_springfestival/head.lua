if (1) then -- not __FESTIVAL_HEAD__) then
	...
	SF_LEVELLIMIT = 90;	-- [21/08] chu game: goc Linux 50 -> cap 90 chung cho moi hoat dong --Íæ¼Ò²Î¼ÓĞèÒªµÄ×îĞ¡¼¶±ğ
	
	SF_PREPARETIME = 68; --×¼±¸³¡¼ÆÊ±Æ÷
	SF_FAIRY_SMALLTIMERID = 69;	--Ë¢¹Ö¼ÆÊ±Æ÷
	SF_REPORT_SMALLTIMERID = 70;	--RunMissionÓÃµÄ¼ÆÊ±Æ÷£»
	SF_INTERVER = 5 * SF_FRAME;	--5ÃëÖÓ´¥·¢Ë¢¹ÖºÍÇå³ş¹Ö´¥·¢Æ÷
	...
	SF_SMALLMAPCOUNT = 19;	--Ò»¹²ÓĞ12ÕÅĞ¡µØÍ¼
	SF_MOUSECOUNT_MIN = 10;	--Ò»¸öĞ¡µØÍ¼Ò»´Î×îÉÙË¢µØÊóÊıÄ¿
	SF_MOUSECOUNT_MAX = 10;	--Ò»¸öĞ¡µØÍ¼Ò»´Î×î¶àË¢µØÊóÊıÄ¿
	SF_MAXPLAYERCOUNT = 10;	--Ã¿¸öÓÎÏ·×î¶à10¸öÈË£»
	SF_NIANSHOU_XUANYUN = 10;   --ÄêÊŞÃ¿±»¹¥»÷10´Î¾Í±»Ñ£ÔÎ
...
--	
	...
	--MS²¿·Ö£»
	MS_SF_STATE = 1;
	MS_SF_TIMERSTATE = 2; --ÓÃÀ´¼ÇÂ¼ÓÃÓÚ½»ÌæµÄ½øĞĞË¢¹ÖºÍÇå³ı¹Ö£º0£­Çå³ı¹Ö£»1£­Ë¢³ö¹Ö
	MS_SF_NEWSVALUE = 3; --
	MS_SF_BEGINGMAPID = 10; --¼ÇÂ¼¿ªÊ¼µÄµØÍ¼µÄID
	MS_SF_PREPARENEWSVALUE = 5; --×¼±¸³¡¼ÆÊ±±äÁ¿
	MS_SF_GAMECOUNT = 6;    --  ¼ÇÂ¼Ò»³¡¿ªÊ¼µÄÓÎÏ·µÄ¸öÊı
	--³£Á¿²¿·Ö£»
	FESTIVAL_SHREWMOUSE = "TONG_SPFESTIVAL";
	SF_ANIMALNAME = "Qu¸i thó ";
	SF_MOUSENAME = "®øa trÎ mÊt tİch"; --Ê¥µ®¾«ÁéµÄÃû×Ö
	...
	function festival_clearfairy(szNPCName)
		...
		local OldWorld = SubWorld;
		SubWorld = SubWorldID2Idx(SF_MAPID[2]);
		ClearMapNpcWithName(SF_MAPID[2], szNPCName);     --½«ËùÓĞµÄÄêÊŞÇå³ı
		SubWorld = OldWorld;
	end;
	...
	function festival_check_pl(szPlayName)
		...
		if (nPlIdx > 0) then
			local nPlMsIdx = PIdx2MSDIdx(SF_MISSIONID, nPlIdx)	--Íæ¼ÒÊÇ·ñÔÚÖ¸¶¨MissionÖĞ
			if (nPlMsIdx > 0) then
				return nPlIdx
			end
		end
