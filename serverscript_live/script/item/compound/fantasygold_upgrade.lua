...
function Compound(arynNecessaryItemIdx, arynAlternativeItemIdx, nExParam)
	...
	if nResultDesItemIdx > 0 then
		...
		WriteLog(format("[N©ng cÊp HuyÒn Kim]\ttµi kho¶n:%s\tnh©n vËt:%s\ttiÒn v¹n tiªu hao%d\tnhËn ®­îc mét c¸i%dcÊp HuyÒn Kim thµnh c«ng", GetAccount(), GetName(), nPay, nWantLevel));
		return nResultDesItemIdx, RESULT_SUCCEED;
	else
		WriteLog(format("[N©ng cÊp HuyÒn Kim]\ttµi kho¶n:%s\tnh©n vËt:%s\ttiÒn v¹n tiªu hao%d\tnhËn ®­îc mét c¸i%dcÊp HuyÒn Kim thÊt b¹i", GetAccount(), GetName(), nPay, nWantLevel));
