...
function G_ACTIVITY:DebugMsg(...) local arg = {n = select("#", ...), ...};
	if self.bDebug then
		call(print, arg)
	end
end
...
function G_ACTIVITY:OnMessage(szKey, ...) local arg = {n = select("#", ...), ...};
	self:DebugMsg("G_ACTIVITY:OnMessage",szKey, arg)
	if self.FunSet[szKey] then
		for varId, pData in pairs(self.FunSet[szKey]) do
...
function G_ACTIVITY:GetItemScript(tbItem)
	...
	if self.tbItemScript[varKey] then
		...
		for varId, pData in pairs(self.tbItemScript[varKey]) do
