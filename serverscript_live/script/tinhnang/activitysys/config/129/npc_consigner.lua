...
function pActivity:OpenTongState()
    ...
    if (nWeekIdx ~= 0 and nWeekIdx ~= 6) and VT_TestBoQua(1) ~= 1 then	-- [VTCN] co test 1
        Talk(1, "", "Kh«ng trong thêi gian më ¸p Tiªu Bang (12:00-23:00 Thø b¶y, Chñ nhËt).");
        return nil;
    end
    -- ¼ì²éÑºïÚÊ±¼ä
    local nCurTime = tonumber(GetLocalDate("%H%M"));
    if (nCurTime < 1200 or nCurTime > 2300) and VT_TestBoQua(1) ~= 1 then	-- [VTCN] co test 1
        Talk(1, "", "Kh«ng trong thêi gian më ¸p Tiªu Bang (12:00-23:00 Thø b¶y, Chñ nhËt).");
        return nil;
    end
    -- ¼ì²éÕ¼³ÇÊıÁ¿
...
function pActivity:RecvTongBiaoChe()
    --µÈ¼¶ÅĞ¶Ï
    if PlayerFunLib:CheckTotalLevel(90, "Ch­a ®¹t cÊp 90 vµ ch­a chuyÓn sinh, kh«ng thÓ nhËn ¸p Tiªu Bang.", ">=") ~= 1 then	-- [VTCN 06/09] chu: cap ha xuong 90 (Linux 150)
	    return nil;
    end
    -- ÓĞÃ»ÓĞ¿ªÆô°ï»áÑºïÚ
    local szTongName, nTongId = GetTongName();
