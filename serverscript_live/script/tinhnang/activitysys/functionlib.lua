...
function lib:OptionFunction(nValue1, nValue2, szOption)
	-- [TOIUU 05/09] truoc: dostring("return "..a..op..b) = bien dich mot chunk MOI LAN kiem dieu kien
	-- hoat dong. Nay so truc tiep khi ca hai la so; truong hop khac giu nhu cu.
	if (type(nValue1) == "number" and type(nValue2) == "number") then
...
function lib:Pack(...) local arg = {n = select("#", ...), ...};
	return arg
end


function lib:DoFunByPlayer(nPlayerIndex, fun, ...) local arg = {n = select("#", ...), ...};	
	local nOldPlayer = PlayerIndex;
	PlayerIndex = nPlayerIndex
	local re = self:Pack(call(fun, arg));
	PlayerIndex = nOldPlayer;
	-- [LUA4 28/08] unpack la ham Lua 5, Lua 4.0.1 cua JX1 khong co (goi la sap
...
--
function lib:DoFunInWorld(nSubWorld, fun, ...) local arg = {n = select("#", ...), ...};
...
-- [LUA4 28/08] CANH BAO: ham duoi van dung unpack(arg) (Lua 5) - tren JX1 goi la
-- sap. Chua co ai dung nen de nguyen; muon dung phai doi sang call(...).
function lib:DoFunByTeam(nPlayerIndex, fun, ...) local arg = {n = select("#", ...), ...};
...
function lib:GetTimeDelta(nTime)
	if nTime == 0 then
		return -1
	end
	second = mod(nTime,100)
	minute = mod(floor(nTime/100), 100)
	hour = mod(floor(nTime/10000),  100)
	return self:RemainTime({nHour=hour, nMinute=minute, nSecond=second})
end

function lib:SetTimedTask(tbTaskTime, nIntervalSec, szfnTimeOut, nCallbackParam)
	if tbTaskTime == nil then
		return
	end
	
	local nYear = tbTaskTime.nYear or tonumber(GetLocalDate("%Y"))
	local nMonth = tbTaskTime.nMonth or tonumber(GetLocalDate("%m"))
	local nDay = tbTaskTime.nDay or tonumber(GetLocalDate("%d"))
	local nHour =	tbTaskTime.nHour or tonumber(GetLocalDate("%H"))
	local nMinute = tbTaskTime.nMinute or tonumber(GetLocalDate("%M"))
	local nSecond = tbTaskTime.nSecond or tonumber(GetLocalDate("%S"))
	nIntervalSec = tonumber(nIntervalSec)
	local nTaskTime = Tm2Time(nYear, nMonth, nDay, nHour, nMinute, nSecond)
	local nCurrTime = GetCurServerTime()
	while nCurrTime > nTaskTime do
		nTaskTime = nTaskTime + nIntervalSec
	end
	local nTimeOut = nTaskTime - nCurrTime
	AddTimer(nTimeOut*18, szfnTimeOut, nCallbackParam)
end

function lib:RemainTime(tbTaskTime)
	local nYear = tbTaskTime.nYear or tonumber(GetLocalDate("%Y"))
	local nMonth = tbTaskTime.nMonth or tonumber(GetLocalDate("%m"))
	local nDay = tbTaskTime.nDay or tonumber(GetLocalDate("%d"))
	local nHour =	tbTaskTime.nHour or tonumber(GetLocalDate("%H"))
	...
	local nTaskTime = Tm2Time(nYear, nMonth, nDay, nHour, nMinute, nSecond)
	local nCurrTime = GetCurServerTime()
	if nTaskTime >= nCurrTime then
		return nTaskTime - nCurrTime
	else
		return 24*3600 + nTaskTime - nCurrTime
	end
end