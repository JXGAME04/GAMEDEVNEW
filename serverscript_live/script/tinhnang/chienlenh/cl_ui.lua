...
function CLUI_GuiXuong(szEnum, ...) local arg = {n = select("#", ...), ...};
	local h = OB_Create()
	for i = 1, arg.n do
...
function CLUI_OnOpen(n)
	...
	if (n == 2) then
		...
		if (tt and tt.vip == 1) then
			CLUI_Bao("Nh©n vÀt Æ∑ k›ch hoπt Chi’n L÷nh Hµo Hoa rÂi")
			return
		end
		if (GetFightState() >= 1) then
			CLUI_Bao("K˙ Tr©n C∏c chÿ mÎ Æ≠Óc khi kh´ng Î trπng th∏i chi’n Æ u")
			return
		end
		NewSale(0, 1, 5, 95, 96, 97, 98, 101, 100)
		CLUI_Bao("Mua thŒ Hµo Hoa (500 xu) Î thŒ S˘ Ki÷n, mua xong dÔng thŒ")
		return
	end
	if (n == 3) then
		CLUI_Bao("Lµm nhi÷m vÙ l y Æi”m, ÆÒ Æi”m b m ´ s∏ng Æ” nhÀn th≠Îng qua th≠")
		return
	end
	CLUI_DayTronBo(1)
end
