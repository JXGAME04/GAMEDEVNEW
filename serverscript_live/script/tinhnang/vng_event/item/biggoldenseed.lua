...
local huihuang_checkdate
Include("\\script\\header\\cauhinh_hoatdong.lua")	-- [HD CONFIG 24/08] exp Qua Dai Hoang Kim chinh duoc
IncludeLib("ITEM");
...
-- [TONGWAR 23/08] 2 global Linux nam o item\goldenseed.lua (khong port) - dinh nghia tai cho
TASKIDDAY = 2321
TASKEATCOUNT = 2322

function main(ItemIndex)
	if (GetLevel() < 90) then	-- [TONGWAR 23/08] chinh sach cap 90
	...
	local tbExpAward = {szName = floor(HD_CFG("QDHK_EXP", 200000000)/1e6).." triÖu ®iÓm kinh nghiÖm", nExp = HD_CFG("QDHK_EXP", 200000000)}
...
function huihuang_checkdate(ItemIndex)
	...
	if (nItemdate == 0) then
		-- [TONGWAR 23/08] khong dat han (ITEM_GetExpiredTime khong co tren JX1) -> luon hop le (lech co chu dich, nhu 'GetItemLife' Dot E)
		return 1;
	end
