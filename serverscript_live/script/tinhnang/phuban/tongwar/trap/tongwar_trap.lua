...
local tongwar_checkjoin, tongwar_signerror, tongwar_cleartask
Include("\\script\\header\\cauhinh_hoatdong.lua")	-- [HD CONFIG 24/08] cap + so mang chinh duoc
...
function tongwar_signerror(camp, cityname)
	--VLDNB 15- ®iÒu kiÖn tham gia: ®· vµo bang tr­íc 0h ngµy 29/10 - Modified By ThanhLD - 20140319
	-- [TONGWAR 23/08] bo dieu kien 'vao bang truoc 0h 29/03/2014' (DIEUKIEN #18 - moc mua VNG)
	--VLDNB15- Translife >= 4	
	-- [TONGWAR 23/08] chinh sach chu game: bo trung sinh, cong cap >= 90 (DIEUKIEN #20)
	if (GetLevel() < HD_CFG("TW_CAP_TOITHIEU", 90)) then
		Talk(1, "", "Ph¶i ®¹t cÊp "..HD_CFG("TW_CAP_TOITHIEU", 90).." trë lªn míi ®­îc vµo ®Êu tr­êng.")
		return
	end
...
function tongwar_signup(camp)
	-- [FIX 25/08] bay OpenMission=TAO (khuon 86655be2 cua 3HD): mission chua ton tai thi
	-- GetMissionV tra 0 het -> NewWorld toi toa do (0,0) ket goc map. Chan bang IsMission.
	if (IsMission(MISSIONID) ~= 1) then
		Say("TrËn ch­a khai cuéc, h·y chê tíi giê khai chiÕn.", 0)
		return
	end
	if (tongwar_getdata(TONGWAR_RLTASK_KEYNUMBER) == GetMissionV(MS_KEYNUMBER)) then
...
function tongwar_cleartask()
	...
	tongwar_setdata(TONGWAR_RLTASK_MAXDEATH, HD_CFG("TW_SO_MANG", 10))
	-- [24/08 hau phan bien] 2378 (tong diem ca mua) khong duoc dong bo lai sau khi dang
	-- nhap (client bi xoa sach bang task luc vao game) nen bang chi nam luon hien 0.
	SyncTaskValue(TONGWAR_RLTASK_TOTALPOINT)
