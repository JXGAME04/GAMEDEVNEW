...
function OnDeath(index)
	...
	local nteams = GetTmpCamp()
	local szname1 = GetName()
	local szplayname1 = GetMissionS(YDBZ_TEAM_NAME[nteams])
		print("PlayerDeath", nteams, szname1, SubWorldName(SubWorld),szplayname1)
	...
	if nidx > 0 then
		...
		AddOwnExp(nexp)
		--Msg2Player(format("%s<color=yellow>%s<color>",szdstr,nexp))
...
--		Msg2MSAll(YDBZ_MISSION_MATCH,format("<color=yellow>%s<color>",pname))
...
--		Msg2MSAll(YDBZ_MISSION_MATCH,"<color=yellow><color>")
