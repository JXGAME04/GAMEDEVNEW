...
YDBZ_READY_TIMER 				= 88		--
YDBZ_READY_LIMIT_SIGNUP	= 5 * 60--5
YDBZ_READY_LIMIT_BROAD	= 1 * 60--1,
YDBZ_READY_LIMIT_WAIT		= 1 * 30--30
YDBZ_READY_TEAM 				= 1 		--
YDBZ_READY_TEAM_MAX			= 2			--
YDBZ_READY_STATE				= 4			-- mission12



YDBZ_TEAM_COUNT_LIMIT		= 4			--
YDBZ_TEAM_COUNT_MAXLIMIT	= 6			--
YDBZ_TEAM_START_LIMIT 	= 2			--
...
-- [VATBREADY 30/08] GUARD. Truoc day dong nay ghi thang tbReady = {}, nen khi
-- tep duoc nap SAU readymap\ready.lua thi no XOA SACH 18 phuong thuc ma
-- ready.lua vua gan (InitMatchMission, ToMatch...). Bang van khac nil nen
-- moi chot 'if tbReady == nil' deu qua, roi goi phuong thuc moi bao nil.
-- Trieu chung: bo test Viem De bam 'vao tran mot minh' bao ScriptError 4
-- (co tu 26/08, 22 lan trong ScriptError.log). Cung loi da tung gap voi
-- bang PetSys - va bang dung cach guard nay.
if (tbReady == nil) then
	tbReady = {}
end
tbReady.nReadyBroadState = 0
tbReady.nMissionKey = 0
tbReady.ReadySignUpWorld = 0
tbReady.ReadySignUpPosX = 0
tbReady.ReadySignUpPosY = 0
...
function YDBZ_ready_kickout(group)
	...
	local world = tbReady.ReadySignUpWorld;
	local pos_x = tbReady.ReadySignUpPosX;
	local pos_y = tbReady.ReadySignUpPosY; 
...
function YDBZ_ready_joinmap(RoleIndex)
		...
		local nkey = tbReady.nMissionKey
...
function YDBZ_joinmap(RoleIndex,readymap,matchmap,camp)
		...
		local g = YDBZ_LIMIT_DOUBEL_ITEM[1][1]									--
		...
		if numbeibao > 0 then
	 	...
	 	elseif numzimu > 0 then
			...
			if ( (numzimu+numbeibao) < YDBZ_LIMIT_ITEM[2]) then
					Msg2Player("Trªn hµnh trang kh«ng ®ñ anh hïng thiÕp, kh«ng thÓ tham gia tranh ®o¹t Viªm §Õ")
					YDBZ_restore(PlayerIndex,YDBZ_READY_MISSION,nteam);
					NewWorld(world, pos_x, pos_y);
					SetLogoutRV(0);
					PlayerIndex = OldPlayerIndex;
