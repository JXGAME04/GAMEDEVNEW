...
local wlls_query_do, wlls_lamsach
IL("RELAYLADDER");
...
function wlls_query_type(n_level)
	...
	local tb_option = {}
	local n
	for n_mtype, tb_mtaype in pairs(WLLS_TAB[n_type].match_type) do
...
function wlls_query_top10(n_mtype)
	...
	for i = 1, 10 do
		...
		if (not FALSE(n_order)) then
			tb_option[getn(tb_option)+1] = strfill_left("thø "..i.."tªn", 7)..str_lgname.."/#wlls_query_top("..n_ladder..","..i..")"
		end
	end
...
function wlls_query_plnamein()
	AskClientForString("wlls_query_plname", "", 1, 16, " Xin nhËp tªn nh©n vËt cÇn t×m hiÓu")
end
...
--Describe¶Ô»°
-- [21/08] client TEncodeText hiÓu byte cao lµ ch÷ GBK 2 byte: chuçi lÎ byte
-- cao ®øng s¸t "<" sÏ nuèt mÊt dÊu "<" lµm thÎ <color> hiÖn nguyªn v¨n, ch÷
-- x« lÖch. Tù chÌn mét kho¶ng tr¾ng tr­íc "<" mçi khi ký tù tr­íc lµ byte cao.
function wlls_lamsach(str)
	if (type(str) ~= "string") then return str end
	return gsub(str, "([€-ÿ])(<)", "%1 %2")
end
function wlls_descript(str, ...) local arg = {n = select("#", ...), ...};
	str = ""..wlls_npcname()..""..str
	if (type(arg[1]) == "table") then
		arg = arg[1]
	end
	str = wlls_lamsach(str)
	if (getn(arg) <= 0) then
