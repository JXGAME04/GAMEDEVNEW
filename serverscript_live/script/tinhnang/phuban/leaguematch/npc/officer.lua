...
function main()
	...
	local tb_option = {}
	tb_option[getn(tb_option) + 1] = "Ta muèn ®Õn khu thi ®Êu h¹ng "..str_des.."/wlls_want2signmap"
	tb_option[getn(tb_option) + 1] = "ChiÕn ®éi h¹ng "..str_des.."cña ta/wlls_mylg"
	...
	tb_option[getn(tb_option) + 1] = "BËt t¾t nhËn kinh nghiÖm khi thi ®Êu/wlls_show_expswitch"
...
function wlls_getaward_rank()
	...
	if (n_award) then
		...
		if tb_award[4] and PlayerFunLib:CheckFreeBagCell(tb_award[4].nCount, "default") ~= 1 then
			return
		end
		...
		Msg2Player("Chóc mõng b¹n ®¹t ®­îc"..str_des.."PhÇn th­ëng xÕp h¹ng: ®iÓm vinh dù"..nPoint.." ®iÓm vinh dù")
		...
		if (tb_award[3]) then	--ÓĞ¶Ó³¤¶îÍâ½±Àø
			...
			if (LG_GetMemberTask(WLLS_LGTYPE, str_lgname, GetName(), WLLS_LGMTASK_JOB) == 1) then
				...
				Msg2Player("Chóc mõng b¹n ®¹t ®­îc"..str_des.."PhÇn th­ëng xÕp h¹ng (®éi tr­ëng): ®iÓm vinh dù"..tb_award[3].." ®iÓm vinh dù")
		...
		if tb_award[4]  then
			tbAwardTemplet:Give(tb_award[4], 1, {"WLLS_RANK_AWARD", ""})
		end
		
		if tb_award[5]  then
			tbAwardTemplet:Give(tb_award[5], 1, {"WLLS_RANK_AWARD", "OtherWard"})
		end
...
function wlls_set_expswitch(b_exps)
	...
	Talk(1, "wlls_show_expswitch", "Thµnh c«ng"..iif(b_exps == 0, "Khai më", " §ãng").."phÇn th­ëng kinh nghiÖm liªn ®Êu!")
end
...
function wlls_buy_honour()
	...
	if (GetBoxLockState() == 0) then
		NewSale(1, 5, 1, 93) -- [WLLS port] shop vinh du cua du an (goc Sale(146,11)) -- ÉÌµêµÄÈÙÓş»ı·Ö±àºÅÎª11
	else
...
--	if (1 <= nDate and nDate < 8) or (29 <= nDate and nDate <= 31) then
--		Talk(1, "", "Ä¿Ç°²»ÄÜ¹ºÂòÍşÍûµÀ¾ß¡£")
--		return 
--	end
