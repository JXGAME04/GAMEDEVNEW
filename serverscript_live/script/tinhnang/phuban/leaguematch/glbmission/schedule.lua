...
function wlls_addplayer_combat(tbLGs, nOrgCamp, nEmyCamp, nNewCamp, tbNewPos)
	...
	for _, nIdx in pairs(tbLGs[nOrgCamp].tbPlayer) do
		PlayerIndex = nIdx
	...
	for _, nIdx in pairs(tbLGs[nEmyCamp].tbPlayer) do
		PlayerIndex = nIdx
...
function OnTimer()
	...
	if (n_timer <= 0) then	--×¼±¸½áÊø£¬¿ªÊ¼±ÈÈü
		...
		--´¦ÀíÃ¿¸ö×¼±¸³¡
		for n_idx, nGroupIdx in pairs(tb_sub) do
			SubWorld = n_idx
			...
			local tbLGs	= {}
			local tbPlayer	= {}
			for _, nCamp in pairs(tb_mstroop) do
		...
		local tbClose = {GetBit(nClose, 1), GetBit(nClose, 2)}
		
		for n_idx, nGroupIdx in pairs(tb_sub) do
			SubWorld = n_idx
			local nMType	= wlls_get_mapinfo()
			local nLevel	= wlls_get_level(nMType)
			if (FALSE(tbClose[nLevel])) then	-- Ã»ÓÐ¹Ø±ÕµÄ»°
