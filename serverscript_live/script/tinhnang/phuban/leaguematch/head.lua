...
WLLS_TASKID_HONOUR	= 37	-- [WLLS port] goc 2501; dung 37 = TASKVALUE_STATTASK_HONOR cua engine (shop/UI co san)		--¸öÈËÈÙÓşµãÊı£¨¿ÉÏûºÄ»ı·Ö£©
WLLS_TASKID_SERIES_WIN = 4125	--¸öÈËÁ¬Ê¤´ÎÊı
...
WLLS_LEVEL_DESC = {"KiÖt xuÊt ", "Vâ l©m "}	-- [21/08] gèc trïng tªn c¶ 2 h¹ng, ®Æt l¹i cho râ
...
function wlls_record_one_match_result(n_level, str_lgname, n_result, n_usedtime)
	...
	for i = 0, n_memcount-1 do
		...
		if (n_plidx > 0) then
			...
			SyncTaskValue(WLLS_TASKID_POINT) SyncTaskValue(WLLS_TASKID_HONOUR)	-- [WLLS 21/08] HONOUR=37 lam range nguoc -> tach 2 lenh
		else	--ÈË²»ÔÚ£¬²¹¼Ó²ÎÈü¼ÇÂ¼
			if (n_result == 1) then	--Ê¤
...
function wlls_award_lg(n_level, str_lgname, n_result, n_usedtime)
	...
	for i = 0, n_memcount-1 do
		...
		if (n_plidx > 0) then
			...
			if n_result == 1 then
				SetTask(WLLS_TASKID_SERIES_WIN, GetTask(WLLS_TASKID_SERIES_WIN) + 1)
			else
				SetTask(WLLS_TASKID_SERIES_WIN, 0)
			end
			
			local nMatchType = GetGlbValue(GLB_WLLS_TYPE)
			print("LeagueMatch:MatchResult", nMatchType, n_level, n_result)
			G_ACTIVITY:OnMessage("LeagueResult", n_level, n_result)
			EventSys:GetType("LeagueMatch"):OnPlayerEvent("MatchResult", PlayerIndex, nMatchType, n_level, n_result)
			if (n_total > 48 ) then
...
function wlls_en_check()
	...
	local keyitem_d = {}
	local keyitem_p = {}
	for i = 1, 9 do	-- [WLLS 21/08] du an 9 o dung-ngay (Linux 3)
...
--´íÎóÈÕÖ¾
function wlls_error_log(...) local arg = {n = select("#", ...), ...};
...
--µ÷ÊÔÓÃÊä³ö
function _M(...) local arg = {n = select("#", ...), ...};
	--local str = join(arg)
...
-- [WLLS port 20/08/2026] xem THICONG_LIENDAU_PORT.md - keo award.lua vao
-- cung state (engine goc 1-state; engine ta moi file 1 state).
Include("\\script\\leaguematch\\wlls_config.lua")	-- [21/08] cau hinh tap trung
Include("\\script\\missions\\leaguematch\\award.lua")
