...
local wlls_add_citynpc, wlls_add_matchnpc
Include("\\script\\missions\\leaguematch\\head.lua")

--³ÇÊÐNPC
WLLS_NPC_CITY = {
	{308,	"Sø gi¶ kiÖt xuÊt",	"\\script\\missions\\leaguematch\\npc\\officer.lua",	{{176, 1463, 3247}, {37, 1777, 3063}, {78,1546,3117}}},
...
function wlls_autoexe()
	--³ÇÊÐNPC
	for _, tb in pairs(WLLS_NPC_CITY) do
	...
	local n_npcidx = 0
	local n_mapidx = 0
	for _, tb_data in pairs(WLLS_TAB) do
		for _, tb_mtype in pairs(tb_data.match_type) do
			for _, tb_group in pairs(tb_mtype.map) do
...
function wlls_add_citynpc(n_id, str_name, str_script, tb_poss)
	for _, tb_pos in pairs(tb_poss) do
...
function wlls_add_matchnpc(n_mapid, tb_npcs)
	...
	if (n_mapidx >= 0) then
		for _, tb_npc in pairs(tb_npcs) do 
