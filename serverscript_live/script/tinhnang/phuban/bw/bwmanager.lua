...
szCaptainName = {};
function main()
-- [BW 23/08] bo dong khoa 'Chuc nang da dong' (DIEUKIEN #1)
	--ÉèÖÃ·µ»Øµã
	x,y,z = GetWorldPos();
...
function OnRegister()
	...
	-- [BW 23/08] chinh sach cap 90 (DIEUKIEN #2)
	if (GetLevel() < HD_CFG("BW_CAP_TOITHIEU", 90)) then
		Say("Ph¶i ®¹t cÊp "..HD_CFG("BW_CAP_TOITHIEU", 90).." trë lªn míi ®­îc tham gia L«i ®µi thi ®Êu.", 0)
		return
	end;
...
function SignUpFinal(MemberCount)
	...
	if (MemberCount <= 0 or MemberCount > HD_CFG("BW_DOI_TOIDA", 8)) then
		return
	end
	...
	for i = 1, 2 do
		...
		if (PlayerIndex <= 0 or GetLevel() < HD_CFG("BW_CAP_TOITHIEU", 90)) then
			PlayerIndex = nOldPlv;
			Say("Ph¶i ®¹t cÊp "..HD_CFG("BW_CAP_TOITHIEU", 90).." trë lªn míi ®­îc tham gia L«i ®µi thi ®Êu.", 0)
			return
		end;
...
function OnEnterKey()
	AskClientForNumber("OnEnterKey1", 0, 10000, "NhËp sè thø tù vµo ®Êu tr­êng:");	-- [BW 23/08] S2C_INPUT_BOX.Value[32] cat o 31 byte
end
...
function OnJoin(group)
	-- [BW 23/08] chinh sach cap 90 (khan gia group 3 khong chan)
	if (group ~= 3 and GetLevel() < HD_CFG("BW_CAP_TOITHIEU", 90)) then
		Say("Ph¶i ®¹t cÊp "..HD_CFG("BW_CAP_TOITHIEU", 90).." trë lªn míi ®­îc tham gia L«i ®µi thi ®Êu.", 0)
		return
	end;
	idx = SubWorldID2Idx(BW_COMPETEMAP[1]);
