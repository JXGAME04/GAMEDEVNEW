...
function OnTimer()
	...
	if (t == UPBOSS_TIME) then		-- 15·ÖÖÓ
		...
		npcindex = HD3_AddNpc(725, 85, SubWorld, posx, posy, 1, "Thñy tÆc ®Çu lÜnh", 1)
	...
	if (t == UPBOSS_TIME2) then		-- 20·ÖÖÓ
		...
		if (check_new_shuizeitask() == 1) then
			for i = 1, 2 do
				posx, posy = fld_getadata(npcthiefpos)
				npcindex = HD3_AddNpc(1692, 85, SubWorld, posx, posy, 1, "Thñy TÆc §¹i §Çu LÜnh.", 1)
