...
local fld_TakeBoat, exchange_lingpai
...
function exchange_juanzhou(ncount)
	...
	for i=1, ncount do
		...
		if (itemgenre == 6 and detailtype == 1 and parttype ==195) then	
			y = y + 1
...
function exchange_lingpai(ncount, ntype)
	...
	if (ntype ==1) then
		...
		if (itemgenre ~= 6 or particular ~= 3363) then
	...
	if particular == 3363 and CalcFreeItemCellCount() < 1 then
