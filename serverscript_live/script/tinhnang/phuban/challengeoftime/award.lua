...
map_random_awards = {
	...
	{0.19587, 		{"Phóc Duyªn Lé (Trung) ", 	6, 1, 122, 1, 0, 0}},	-- ¸£ÔµÂ¶£¨ÖĞ£©
	{0.18, 		{"Phóc Duyªn Lé (TiÓu) ",	6, 1, 121, 1, 0, 0}},	-- ¸£ÔµÂ¶£¨Ğ¡£©
...
local tbPro = {szName="B¶o R­¬ng V­ît ¶i",tbProp={6, 1, 3360, 1, 0, 0},}
...
function award_item(item)
	...
	elseif (getn(item) == 7) then
		-- [MAIL 03/09 D9] vat pham (ai an / ket thuc) gui qua thu
		Include("\\script\\mail\\mailmanager.lua")
		MailManager_SendRewardTemplet("vuotai", nil, "Th­ëng V­ît ¶i: "..name, "PhÇn th­ëng V­ît ¶i ®İnh kÌm trong th­.<enter>Tr©n träng", {{tbProp = {item[2], item[3], item[4], item[5], item[6], item[7]}, nCount = 1}}, 30)
	end
...
--		end;
		...
		-- [MAIL 03/09 D9] kinh nghiem hoan thanh Vuot Ai gui qua thu
		Include("\\script\\mail\\mailmanager.lua")
		MailManager_SendRewardTemplet("vuotai", nil, "Th­ëng hoµn thµnh V­ît ¶i", "Chóc mõng ®¹i hiÖp ®· v­ît ¶i, kinh nghiÖm ®İnh kÌm trong th­.<enter>Tr©n träng", {{szKind = "exp", nCount = floor(point)}}, 30)
		Msg2Player("<#> PhÇn th­ëng " .. floor(point) .. " ®iÓm kinh nghiÖm V­ît ¶i ®· göi vµo hép th­.");
	end
