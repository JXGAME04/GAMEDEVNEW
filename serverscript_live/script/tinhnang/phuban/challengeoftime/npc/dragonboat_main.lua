...
local COT_CheckTeamRequirement, COT_GetFreeMap, COT_TeamMemberJoin, COT_TeamJoin
...
function COT_CheckTeamRequirement()
	...
	for i = 1, GetTeamSize() do 
		...
		local nTaskItem = CallPlayerFunction(nPlayerIndex, CalcItemCount, 3, 6, 1, 399, 90)
		...
		if nMatchLevel == 1 then
			for i=20,80,10 do
				if CallPlayerFunction(nPlayerIndex, CalcEquiproomItemCount, 6, 1, 399, i) > 0 then
					bHaveItem = 1
					break
				end
			end	
		elseif nMatchLevel == 2 then
			bHaveItem = CallPlayerFunction(nPlayerIndex, CalcEquiproomItemCount, 6, 1, 399, 90)
		end
...
function COT_TeamMemberJoin(nMatchLevel)
	...
	if nMatchLevel == 1 then
		for i=20,80,10 do
			if(CalcEquiproomItemCount( 6, 1, 399, i ) > 0) then
				nConsumed = ConsumeEquiproomItem( 1, 6, 1, 399, i)
				if nConsumed > 0 then break end
			end
		end
	elseif nMatchLevel == 2 then
		nConsumed = ConsumeItem(3, 1, 6,1,399,90)
	end
