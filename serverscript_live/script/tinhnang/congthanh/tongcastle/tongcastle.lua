Include("\\script\\header\\cauhinh_hoatdong.lua")	-- [HD CONFIG 24/08] lich cay / diem cay chinh duoc (relay)
...
-- [HD CONFIG 24/08] de bang diem cay tu config (thay ca bang neu co)
if (tbCHD and tbCHD.TC_DIEM_CAY) then
	tbS3TongCastle.tbTreePoint = tbCHD.TC_DIEM_CAY
end
...
function tbS3TongCastle:SavePointData(nKey, tbData)
	...
	local nCount = 0
	for szName, nPoint in pairs(tbData) do
		nCount = nCount + 1
		local hData = OB_Create()
...
function tbS3TongCastle:TestShoPoint()
	OutputMsg("[Player]")
	for k, v in pairs(self.tbPlayerTreePoint) do
		OutputMsg(format("%s : %d", k, v))
	end
	
	OutputMsg("[Tong]")
	for k, v in pairs(self.tbTongTreePoint) do
		OutputMsg(format("%s : %d", k, v))
	end
end
