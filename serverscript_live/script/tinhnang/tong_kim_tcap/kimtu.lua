--Author: Fong KiÒu
--Date: 2021
--Function: Player phe kim tö trËn
...
TONGKILLKIM = "B¹n ®¸nh träng th­¬ng Kim %s%s<color> %s nhËn ®­îc <color=green>%d<color> tÝch lòy."
...
function OnPlayerDeath(nPlayerIndex,nLastDamageIndex) -- player, npc
	--dofile("script/tinhnang/tong_kim_tcap/kimtu.lua")
	...
	local nPThuc  = GetMissionV(M_HINHTHUC) -- 1: cuu sat, 2: bao ve nguyen soai, 3: doat co
	...
	--Msg2Player(format("kimtu nPhe_playerdie=%d nPhe_playerkiller=%d",nPhe_playerdie,nPhe_playerkiller))
	...
	if(nDesPlayerIdx <= 0) then --quai'
		if(nPlayerDataIdx > 0 and nOnAttack == 1) then
			...
			if (nFlag == 2) then
				...
				local nNpcIndex = AddNpcEx1({TAB_PHE_TONGKIM[2][7]},1,{0},MAP_TK_TC,nX*32,nY*32,nil,DOSCRIPTCO,nil,6)
	...
	--Msg2Player(format("kimtu nDesPlayerData:%d nOnAttack:%d nDesOnAttack:%d nPlayerDataIdx:%d",nDesPlayerData,nOnAttack,nDesOnAttack,nPlayerDataIdx))
	if(nPlayerDataIdx > 0 and nDesPlayerData > 0 and nOnAttack == 1 and nDesOnAttack == 1) then	--co ton tai. va ca 2 dang online trong chien truong tr¹ng th¸i chiÕn ®Êu
		...
		SetPMParam(MS_TONGKIM,nPlayerDataIdx,4,GetPMParam(1,nPlayerDataIdx,4)+1)--tu vong
		SetPMParam(MS_TONGKIM,nPlayerDataIdx,7,0)--lien tram hien tai
		SetPMParam(MS_TONGKIM,nPlayerDataIdx,1,0)--set l¹i tr¹ng th¸i kh«ng chiÕn ®Êu vÒ hËu doanh tèng kim
		...
		SetMission(M_TICHLUYA,GetMissionV(M_TICHLUYA)+nPoint) -- cong diem tich luy
		...
		Msg2MSAll(MS_TONGKIM,format("%s ®¸nh träng th­¬ng Kim %s%s<color> %s nhËn ®­îc <color=green>%d<color> tÝch lòy.",szDesName, QUANHAMTK[szRank][5],QUANHAMTK[szRank][1],szName,nPoint),nDesPlayerData)
		...
		if (szDesRank ~= nCurDescRank) then
			...
			SetRankBattle(nCurDescRank, -1, 0) --ntime = -1 chet khong mat overlook = 0 thoat ra vao lai mat
...
function OnPlayerTimerIdle(nPlayerIndex)
	...
	SetTempRevPos(324, 50240, 98720)--ket thuc thiet lap diem hoi sinh o diem bao danh Kim
