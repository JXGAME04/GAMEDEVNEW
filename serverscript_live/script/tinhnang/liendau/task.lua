...
function wlls_calc_loop(tb_calendar, n_mins)
	local tb_cal, n
	local n_loop = 1
	for _, tb_cal in pairs(tb_calendar) do
...
function wlls_calc_phase()
	...
	local n_sid = 0
	for i, tb in pairs(WLLS_SEASON_TB) do
	...
	if (n_date <= tb[3]) then	--±ÈÈüÆÚ
		...
		n_time = floor(n_time/100)*60 + mod(n_time,100)
		local n_loop
		...
		if tb[5][n_wday] then
			n_loop = wlls_calc_loop(tb[5][n_wday], n_time)
			if n_loop then
...
function TaskShedule()
	...
	--ÐèÒª³õÊ¼»¯
	for i = RLGLB_WLLS_PHASE, RLGLB_WLLS_SVRCOUNT+22 do
		SetGblInt(i, 0)
	end
...
function TaskContent()
	...
	if (n_matchid > 0) then	--ÓÐ±ÈÈü£¬Çå³ý×¼±¸³¡¼ÆÊý
		for i = RLGLB_WLLS_SVRCOUNT+1, RLGLB_WLLS_SVRCOUNT+22 do
			SetGblInt(i, 0)
		end
	end
