...
function LongMenBiaoJu:AddTaskNpc()
	...
	for _, tbPos in pairs(LongMenBiaoJu.ZhangGuiPos) do
	...
	for _, tbPos in pairs(LongMenBiaoJu.BiaoShiPos) do
		local nMapIndex = SubWorldID2Idx(tbPos[1])
		if nMapIndex >= 0 then
			local nNpcIndex = AddNpcEx(tbNpc.nNpcId, 1, 0, nMapIndex, tbPos[2]*32, tbPos[3]*32, 1, tbNpc.szName, 0)
			if nNpcIndex > 0 then
...
-- [SAPXEP 06/09] JX1 moi tep mot lua_State: AutoFunctions (global/autoexec_head.lua) khong co o day -> chan nil (ScriptError 4 luc boot)
if AutoFunctions ~= nil then AutoFunctions:Add(LongMenBiaoJu.AddTaskNpc) end
