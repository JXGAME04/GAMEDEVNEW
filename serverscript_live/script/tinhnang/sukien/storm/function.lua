...
local storm_calc_today, storm_calc_point
--Storm »î¶¯¿ªÊ¼
...
function storm_clear(gameid)
	-- [CHANMIN 30/08] CHOT AN TOAN. Nhanh duoi day xoa trang MOI task tu 1661
	-- toi tran cua nhan vat (nuot ca Long Huyet Hoan 2641/2642/4018 va
	-- tasklink Da Tau). Truoc day chi can goi storm_clear() thieu tham so
	-- la no chay. Nay phai goi ro storm_clear(-1).
	if (gameid == nil) then
		Msg2Player("LÖnh nµy xo¸ s¹ch d÷ liÖu nhiÖm vô cña nh©n vËt. Ph¶i gäi râ storm_clear(-1) míi ch¹y.")
		return nil
	end
	if (gameid == -1) then
