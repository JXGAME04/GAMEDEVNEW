--Author: Fong KiÒu
--Date: 28/11/2020
--Function: Trap ®iÓm c¾m cê phe Kim
...
function main(nPlayerIndex)
	...
	if (nFlag == 2) then
		SetPMParam(MS_TONGKIM,nPlayerDataIdx,6,GetPMParam(MS_TONGKIM,nPlayerDataIdx,6)+FLAG_POINT) -- cong diem tich luy
		SetPMParam(MS_TONGKIM,nPlayerDataIdx,17,GetPMParam(MS_TONGKIM,nPlayerDataIdx,17) + FLAG_POINT)--tÝch luü nhÆt cê
		SetMission(M_TICHLUYB, GetMissionV(M_TICHLUYB)+FLAG_POINT) -- cong diem tich luy
		SetMission(M_SOCOPHEB, GetMissionV(M_SOCOPHEB)+1)
		local TAB_MSG = 
		{
			...
			"Ng­êi ch¬i <color=green>"..GetName().."<color> ®· c¾m cê thµnh c«ng, phe Kim ®· c¾m ®­îc <color=yellow>"..GetMissionV(M_SOCOPHEB).."<color> c©y cê.",
		}
		Msg2Player(TAB_MSG[1])
		SetPMParam(MS_TONGKIM,nPlayerDataIdx,8,0)	-- mat giu co
		SetPMParam(MS_TONGKIM,nPlayerDataIdx,9,GetPMParam(MS_TONGKIM,nPlayerDataIdx,9)+1)	-- so lan cam co
		ReSetMask()
		RestoreRunSpeed()
		Msg2MSGroup(MS_TONGKIM,TAB_MSG[2],2)

		checkqhamdoatco(2,nPlayerDataIdx)
