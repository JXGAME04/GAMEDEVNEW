...
function ActivityClass:OnMessage(tbParam, szType)
	...
	for varId, pData in pairs(tbMessage) do
...
function ActivityClass:GetTaskDaily999(nTaskIndex)
	local nTaskValue =  self.TaskGroup:GetTask(nTaskIndex)
	local nCurDate = tonumber(GetLocalDate("%y%m%d"))
...
function ActivityClass:CheckTaskDaily999(nTaskIndex, nValue, szFailMsg, szOption)
	nValue = lib:NumberParamTrans(nValue)
	
	local nTaskValue = self:GetTaskDaily999(nTaskIndex)
...
function ActivityClass:AddTaskDaily999(nTaskIndex, nValue)
	nValue = lib:NumberParamTrans(nValue)
	
	local nTaskValue = self:GetTaskDaily999(nTaskIndex) + nValue
	
	self:SetTaskDaily999(nTaskIndex, nTaskValue)
end
...
function ActivityClass:SetTaskDaily999(nTaskIndex, nValue)
	nValue = lib:NumberParamTrans(nValue)
	...
	local nTaskValue =  nCurDate * 1000 + nValue
...
function ActivityClass:GetItemExpiredTime(nItemIndex)
