--Author: Fong Ki襲 skill thi猲 nh蒼 gi竜
	...
	tianren120={ --天忍120级技能
		...
		skill_desc=
			function(level)
				...
				"Sau khi ch誸 c?x竎 su蕋 <color=orange>"..floor(Link(level,SKILLS.tianren120.autodeathskill[3])).."%<color> khi課 cho t鑙 產 "..
				floor(Link(level,SKILLS.quntisuijizoudong.missle_hitcount[1])).." m鬰 ti猽 g莕 b?ho秐g lo筺 trong <color=orange>"..
				floor(Link(level,SKILLS.quntisuijizoudong.randmove[2]) / 18).." gi﹜<color>\n"..
	...
	zhanren150={ --Giang Hai no Lan -150
		...
		skill_desc=
			function(level)
				...
				return "T╪g th猰 h譶h th鴆 th?hai <color=blue>Ma ﹎ k輈h<color>\nv韎 <color=orange>"..floor(100 -Link(level,SKILLS.zhanren150.missle_missrate[1])).."%<color> t?l?khi課 cho i phng khi誴 s?"..
