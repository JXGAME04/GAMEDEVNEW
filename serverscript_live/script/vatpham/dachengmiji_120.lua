-- [HOASON 01/09] chep nguyen ban Linux script\item\dachengmiji_120.lua
-- ====================== ÎÄ¼şĞÅÏ¢ ======================
...
function main(nItemIdx)
	...
	for i = 1, getn(tb_120skill[n_fac]) do
		...
		if (skill ~= -1 and skill ~= 20) then
			tinsert(tb_Desc, format("N©ng cÊp".."%s/#upgrade_skilllevel(%d)", GetSkillName(tb_120skill[n_fac][i]), tb_120skill[n_fac][i]));
		end
	end
...
function upgrade_skilllevel(n_skillid)
	...
	if (ConsumeItem(3, 1, 6, 1, 4964, -1) == 1) then
