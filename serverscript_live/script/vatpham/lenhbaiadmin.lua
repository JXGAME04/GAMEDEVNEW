...
function chucnangadmin(itemIdx)
	...
	if (nAdmin == 1) then
		...
		"Bé test hÖ thèng nh©n vËt: Lß rÌn, Kinh m¹ch, §ång hµnh, Phi Phong, Dung luyÖn/ADM_TestNhanVat",
...
ReLoadScript("\\script\\petsys\\bikip.lua")
ReLoadScript("\\script\\petsys\\petequip_def.lua")
ReLoadScript("\\script\\petsys\\petbox.lua")
ReLoadScript("\\script\\petsys\\petequip.lua")
...
function ADM_TestHoatDong()
	...
	"Ho¹t ®éng Linux: S¸t Thñ - Phong L¨ng §é - V­ît ¶i/HD3_AdminMenu",
	"Viªm §Õ/TTHD_Root",
	"Quay l¹i lÖnh bµi/ADM_Main",
	"KÕt thóc ®èi tho¹i/no"})
end

function ADM_TestNhanVat()
	SayEx({"<color=yellow>Bé test hÖ thèng nh©n vËt<color>: chän môc",
	"Lß rÌn/LR_Root",
	"Kinh M¹ch/KM_TestRoot",
	"§ång hµnh/BDH_Root",
	"Phi Phong/PP_Root",
	"Dung luyÖn/DL_TestRoot",
	"Quay l¹i lÖnh bµi/ADM_Main",
	"KÕt thóc ®èi tho¹i/no"})
end
