...
"""[SDL 08/09 2b-2] May chu GIA de kiem Rainbow.dll (lop mang client) ma khong can may chu game:
 ...
 - tuy chon --cham N: ngu N ms truoc khi tra loi (kiem doc ghep goi); --gop: gui 2 goi tra loi trong 1 lan send (kiem tach goi)
Dung: python may_chu_gia.py [--port 27015] [--cham 0] [--gop] [--thoi-gian 60]"""
...
def chay(port, cham, gop, thoi_gian, skey, ckey):
    srv = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    srv.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    srv.bind(("127.0.0.1", port)); srv.listen(4); srv.settimeout(1.0)
    print("[may chu gia] nghe 127.0.0.1:%d skey=%08x ckey=%08x cham=%d gop=%s" % (port, skey, ckey, cham, gop), flush=True)
    ...
    while time.time() < t_end:
        ...
        # bat tay: wLen(2) + ACCOUNT_BEGIN(32): type, mode, reserve1[6], serverkey, clientkey, reserve2[16]
        ...
        try:
            while True:
                ...
                while len(buf) >= 2:
                    ...
                    if payload == b"BYE":
                        if tra: c.sendall(tra)
                        print("[may chu gia] BYE -> dong ket noi phia may chu", flush=True)
                        raise ConnectionAbortedError
                    reply = xor_key(b"ECHO:" + payload, skey)
                    tra += struct.pack("<H", 2 + len(reply)) + reply
                    if not gop and tra:
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    chay(a.port, a.cham, a.gop, a.thoi_gian, a.skey, a.ckey)
