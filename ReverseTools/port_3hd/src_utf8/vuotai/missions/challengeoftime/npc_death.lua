...
function Mission_Complete(npc_index)
	...
	broadcast(GetMissionS(VARS_TEAM_NAME) .. "Đội ngũ đã thắng lợi hoàn thành nhiệm vụ đúng thời gian, đã dụng" .. floor(time / 60) .. " phút " .. mod(time, 60) .. "giây! Trước đó"..laddertime.." giây ");
	...
	if (time < 20 * 60) then
		...
		close_board_timer();
		Msg2MSAll(MISSION_MATCH, "<#>Nhiệm vụ bí mật đã khỏi động rồi");
...
function batch_finish(index)
	...
	if (GetMissionV(VARV_MISSION_RESULT) == 1) then
		...
		Msg2MSAll(MISSION_MATCH, GetMissionS(VARS_TEAM_NAME) .. "Đội ngũ đã tiêu diệt toàn bộ " .. batch .. "đám quái!");
