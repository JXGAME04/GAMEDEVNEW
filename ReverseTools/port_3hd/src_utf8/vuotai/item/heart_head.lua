...
function use_heart(bForever)
	...
	for i = 1, getn(tbBATTLEMAP) do 
		if ( nMapId == tbBATTLEMAP[i] ) then
			Msg2Player("Lúc này bạn không thể sử dụng vật phẩm này");
			return 1;
		end
	end	
	...
	if (nSubWorldID >= 375 and nSubWorldID <= 386) then
		Msg2Player("Xin lỗi! Bạn hiện ở khu vực đặc biệt, không thể sử dụngTâm Tâm Tương ánh Phù!");
		return 1
	end
	...
	if (IsDisabledUseHeart(PlayerIndex) == 1) then 
		Msg2Player("Xin lỗi! Hiện bạn không thể sử dụng Tâm Tâm Tương ánh Phù.");
		--AddItem(6,1,18,1,0,0,0);
		return 1
	end;
	...
	Lover = GetMateName();
	if (Lover == "") then 
		Msg2Player("Người chưa kết hôn không thể sử dụng!");
		--AddItem(6,1,18,1,0,0,0);
		return 1
	end;
	if (bForever == 0) then
...
function _LoverCallBack(TargetName, MoneyToPay, LifeMax, ManaMax, PKValue, PlayerLevel, MapName, nPosX, nPosY, nSex, nWorldRank, nSubWorldID, nFightMode, bTargetProtect,nMapTemplateID,  bForever)
	if (nSubWorldID > 70000) then
		Msg2Player("Xin lỗi! Phối ngẫu của bạn đang ở khu vực đặc biệt không thể dẫn vào.");
		if (bForever == 0) then
			AddItem(6,1,18,1,0,0,0);
		end
		return 0;
	...
	if checkActMaps(nSubWorldID) == 1 then
		Msg2Player("Xin lỗi! Phối ngẫu của bạn đang ở khu vực đặc biệt không thể sử dụng Tâm Tâm Tương ánh Phù.");
		if (bForever == 0) then
			AddItem(6,1,18,1,0,0,0);
		end
		return 1
...
function _UnlineCallBack(TargetName, MoneyToPay, bForever)
	Say("<#>Xin lỗi! "..TargetName.."<#>Tạm thời không Online ",0)
	if (bForever == 0) then
		AddItem(6,1,18,1,0,0,0)
	end;
	return 0
