...
function tbBirthday0905:IsPlayerEligible()
	...
	if self.nMinLevel and (GetLevel() < self.nMinLevel and ST_IsTransLife() ~= 1) then
		return nil, format("Cấp tích lũy không đủ %d.", self.nMinLevel)
	end
	
	if self.nMaxLevel and (GetLevel() > self.nMaxLevel and ST_IsTransLife() ~= 1) then
		return nil, format("Cấp tích lũy vượt quá %d.", self.nMaxLevel)
	end
	
	if self.bIsCharged and self.bIsCharged == 1 and IsCharged() ~= 1 then
		return nil, "Đại hiệp vẫn chưa nạp thẻ à!"
	end
	return 1
end
...
function tbBirthday0905:buy_tougu()
	...
	if (self:IsActDate()) then
		if GetCash() < self.ntonggu_cost then
			Say(format("Bán rong: Hàng trang không đủ tiền, ta cần %d lượng.", self.ntonggu_cost),0)
			return
		end
		...
		SyncItem(n_itemidx);
		local szstr = format("Chúc mừng đại hiệp nhận được %s", "Đồng Cổ")
		Msg2Player(szstr)
