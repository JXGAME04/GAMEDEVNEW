...
function storm_anotherday()
	...
	if (nowday <= lastday) then	--没有跨日（不允许时间倒退），直接返回
		...
		if (lastday + 1 == nowday) then	--只差一天，记录昨日积分
			...
			debug_out("Tích lũy hôm qua của bạn là:"..GetTask(STORM_TASKID_DAY_LASTPOINT))	--temp
		else
	...
	if (nowweek ~= lastweek) then	--有跨周
		if (lastweek + 1 == nowweek) then	--只差一周，记录上周积分
			...
			debug_out("Tích lũy tuần trước của bạn là:"..GetTask(STORM_TASKID_WEEK_LASTPOINT))	--temp
		else
	...
	if (nowmonth ~= lastmonth) then	--有跨月
		if (lastmonth + 1 == nowmonth) then	--只差一月，记录上月积分
			...
			debug_out("Tích lũy tháng trước của bạn là:"..GetTask(STORM_TASKID_MONTH_LASTPOINT))	--temp
		else
...
function storm_end(gameid, silent)
	...
	if (not silent) then
		Say(storm_gamename(gameid, "red").."Khiêu chiến Phong Ba đã kết thúc. Điểm tích lũy Phong Ba hiện có là:"..storm_point2str(GetTask(STORM_TASKID_DAY_POINT)), 0)
	else
		debug_out(storm_gamename(gameid).."Khiêu chiến Phong Ba đã kết thúc. Điểm tích lũy Phong Ba hiện có là:"..storm_point2str(GetTask(STORM_TASKID_DAY_POINT)))
	end
