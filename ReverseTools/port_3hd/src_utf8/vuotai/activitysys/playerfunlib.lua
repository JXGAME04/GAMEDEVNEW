...
function PlayerFunLib:IsCharged(szFailMsg)
	...
	if( GetExtPoint(0) >= 1 ) then
		...
		if szFailMsg == "default" then
			szFailMsg = "Yêu cầu cần phải nạp thẻ."
		end
		lib:ShowMessage(szFailMsg)
		return nil
	end;
...
function PlayerFunLib:CheckLevel(nLevel, szFailMsg, szOption)
	...
	if lib:OptionFunction(GetLevel(), nLevel,szOption) then
		...
		if szFailMsg == "default" then
			szFailMsg = format("Cấp phải đạt đến cấp <color=yellow>%d<color>.", nLevel)
		end
		lib:ShowMessage(szFailMsg)
	end
end
...
function PlayerFunLib:CheckTotalLevel(nLevel, szFailMsg, szOption)
	...
	if ST_IsTransLife() == 1 or lib:OptionFunction(GetLevel(), nLevel,szOption) then
		...
		if szFailMsg == "default" then
			szFailMsg = format("Cấp phải đạt đến<color=yellow>%d<color> hoặc đã chuyển sinh", nLevel)
		end
		lib:ShowMessage(szFailMsg)
	end
end
...
function PlayerFunLib:CheckFreeBagCell(nCount, szFailMsg)
	...
	if CalcFreeItemCellCount() >= nCount then
		...
		if szFailMsg == "default" then
			szFailMsg = format("Hành trang cần <color=yellow>%d<color> ô trống.", nCount)
		end
		lib:ShowMessage(szFailMsg)
	end
...
function PlayerFunLib:CheckFreeBagCellWH(nW, nH, nCount, szFailMsg)
	...
	if CountFreeRoomByWH(nW, nH) >= nCount then
		...
		if szFailMsg == "default" then
			szFailMsg = format("Để bảo đảm tài sản của đại hiệp, xin hãy để trống %d %dx%d hành trang", nCount, nW, nH)
		end
		lib:ShowMessage(szFailMsg)
	end
...
function PlayerFunLib:CheckCashLimit(nMaxCount, szFailMsg)
	...
	if GetCash() < nMaxCount then
		...
		if szFailMsg == "default" then
			szFailMsg = format("Vì sự an toàn vật dụng của bạn, xin đảm bảo tiền mang theo không vượt quá %d lượng", nMaxCount)
...
function PlayerFunLib:CheckCash(nCount, szFailMsg)
	...
	if GetCash() >= nCount then
		...
		if szFailMsg == "default" then
			szFailMsg = format("Ngân lượng không đủ, ít nhất cần <color=yellow>%d<color> lượng", nCount)
		end
		lib:ShowMessage(szFailMsg)
	end
end
...
function PlayerFunLib:CheckTaskWeekly(nTaskId, nValue, szFailMsg, szOption)
	...
	if lib:OptionFunction(mod(nTaskValue, WEEK_FACTOR), nValue, szOption) then
		return 1
	else
		lib:ShowMessage(szFailMsg)
	end
...
function PlayerFunLib:GetTaskDaily999(nTaskId)
	local nTaskValue =  self:GetActivityTask(nTaskId)
	local nCurDate = tonumber(GetLocalDate("%y%m%d"))
	if floor(nTaskValue/1000) ~= nCurDate then
		nTaskValue = nCurDate * 1000
		self:SetActivityTask(nTaskId, nTaskValue)
	end
	
	return mod(nTaskValue, 1000)
end
...
function PlayerFunLib:AddTaskDaily999(nTaskId, nValue)
	nValue = lib:NumberParamTrans(nValue)
	
	local nTaskValue =  self:GetTaskDaily999(nTaskId) + nValue
	
	self:SetTaskDaily999(nTaskId, nTaskValue)
end
...
function PlayerFunLib:SetTaskWeekly(nTaskId, nValue)
	...
	local nCurWeek = tonumber(GetLocalDate("%y%W"))
	local WEEK_FACTOR = 1024 --pow(2,10)
	local nTaskValue = nCurWeek * WEEK_FACTOR
	
	self:SetActivityTask(nTaskId, nTaskValue + nValue)
end

function PlayerFunLib:SetTaskDaily999(nTaskId, nValue)
	nValue = lib:NumberParamTrans(nValue)
	
	local nCurDate = tonumber(GetLocalDate("%y%m%d"))
	local nTaskValue =  nCurDate * 1000
	if nValue > 999 then
		nValue = 999
	end
...
function PlayerFunLib:PayExtPoint(nExtPointId, nConsumePoint)
	...
	if PayExtPoint(nExtPointId, nConsumePoint) == 1 then
		...
		Msg2Player(format("Đại hiệp đã nộp điểm số không đủ %d", nConsumePoint))
...
function PlayerFunLib:IsCityMap()
 	local nMapId =GetWorldPos()
	if (%tbMapList[nMapId] == 1 and GetFightState() == 0) then
		return 1
	else
		Talk(1,"","Xin hãy s?dụng tại khu vực phi chiến đấu")
		return
	end
end

function PlayerFunLib:CheckInCity(msg)
 	local nMapId =GetWorldPos()
	if (%tbMapList[nMapId] == 1 and GetFightState() == 0) then
		return 1
	else
		if msg == "default" then
			msg = "Có thể ở trong Thành thị và Thôn trang tiến hành thao tác này "
		end
		lib:ShowMessage(msg)
	end
end
...
-- VNG Lib - Modified by AnhHH - 20111110
function PlayerFunLib:CheckExtPointBit_VNG(nExtPointId, nBit, nValue, szFailMsg)
	szFailMsg = szFailMsg or ""
	
	if GetBit(GetExtPoint(nExtPointId), nBit) == nValue then
		return 1
	else
		lib:ShowMessage(szFailMsg)
		return
...
function PlayerFunLib:SetExtPointBit_VNG(nExtPointId, nBit, nValue)
	nValue = lib:NumberParamTrans(nValue)
	
	local nValuePoint	= GetExtPoint(nExtPointId)
	
	if (nValue == 1) then
		local nValueBit = SetBit(nValuePoint, nBit, 1)
		local nResult = nValueBit - nValuePoint
		if (AddExtPoint(nExtPointId, nResult) == 1) then
			return 1
		else
			return nil
		end
	else
		local nValueBit = SetBit(nValuePoint, nBit, 0);
		local nResult = nValuePoint - nValueBit;
		if (PayExtPoint(nExtPointId,nResult)	 == 1) then
			return 1
		else
			return nil
		end
