...
function RunMission()
	for i=1, 30 do
		...
		local npcindex	= AddNpc(724, 95, SubWorld, posx, posy, 0, "Thủy Tặc ");
	...
	for i = 1 , nCount do 
		...
		--此时为GM发布命令
		SetFightState(1)
		PutMessage("Thuyền đi rồi! 30 phút sau sẽ đến bờ Bắc Phong Lăng Độ.")
		if (idx == 0) then 
 			break
 		end 
 	end
...
function OnLeave(RoleIndex)
	...
	Msg2MSAll(MISSIONID, GetName().."Bạn không may tử vong trong lúc đi thuyền.")
	SetCreateTeam(1)
	ForbidEnmity(0);
--	SetTaskTemp(200,0);
	SetDeathScript("")
...
function Landing()
	Msg2MSAll(MISSIONID, "đã đến bờ Bắc Phong Lăng Độ.")
	...
	if (nDate >= jf0904_act_dateS and nDate < jf0904_act_dateE) then
		local szNews = "Quan phủ và các cao thủ đã hoàn tất chiến dịch tiêu diệt Thủy Tặc, hãy nhanh chóng giao [Truy Công Lệnh] cho Thuyền Phu để nhận thưởng!";
