...
function tbComposeClass:GetMaterialList(tbMaterial)
	...
	for i=1,getn(tbMaterial) do
		...
		local tbItem = tbMaterial[i]
		if tbItem.nJxb then
			szList = format("%s<enter><color=yellow>%-20s<color>  %s(%d/%d)<color>",szList,"Ngân lượng", szColor, tbCount[i], tbMaterial[i].nJxb)
...
function tbComposeClass:Compose(tbFormula, szLogTitle, nComposeCount, pFun, ...)
	...
	if CalcFreeItemCellCount() < nFreeItemCellLimit then
		Say(format("Để bảo đảm an toàn tài sản, xin hãy đảm bảo hành trang còn thừa %d ô trống.", nFreeItemCellLimit))
		return 0
	end
	
	
	if self:CheckMaterial(tbMaterial, nComposeCount) ~=1 then
		local szMsg = tbFormula.szFailMsg or "<color=red>Đại hiệp mang nguyên liệu không đủ rồi!<color>"
		Talk(1, "", szMsg)
		return 0;
	end
...
function tbComposeClass:ComposeDailog(tbFormula, szFunctionFormat, bIsAskNumber)
	...
	local tbMaterial = tbFormula.tbMaterial
	local tbProduct = tbFormula.tbProduct
	local szComposeTitle = tbFormula.szComposeTitle or format("Đổi %s", tbProduct.szName)
	local szMsg = format("<dec>%s cần: <enter>%s", szComposeTitle ,self:GetMaterialList(tbMaterial))
	local tbSay = 
	{
		szMsg,
		format("Xác nhận/%s", szFunctionFormat ),
		"Hủy bỏ /OnCancel"
	}
	CreateTaskSay(tbSay)
end
...
function tbComposeClass:AskNumber()	
	...
	if nMaxCount < 1 then
		...
		AskClientForNumber(format("%s__AskNumberCallBack", self.szClassName), 1, nMaxCount, "Xin mời nhập số");
	end
end
