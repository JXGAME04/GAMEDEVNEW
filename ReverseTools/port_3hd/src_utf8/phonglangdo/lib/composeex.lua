...
--
--tbFormula格式
--tbFormula = {tbMaterialÊơtbProduct}
--
...
function tbActivityCompose:_init(tbFormula, szLogTitle, nRoomType)
	...
	self.szLogTitle = szLogTitle or "Mặc đinh hợp thành"
	...
	for i=1,getn(tbMaterial) do
		...
		elseif tbItem.nJxb then
			...
			if Pay(nConsumeJxb) == 0 then
				...
				Msg2Player(format("Cần <color=yellow>%d<color> lượng", nConsumeJxb))
		...
		elseif tbItem.nTaskId then
			...
			if GetTask(tbItem.nTaskId) < nConsumeValue then
				...
				Msg2Player(format("Chi trả <color=yellow>%d <color>%s", nConsumeValue, tbItem.szName))
...
function tbActivityCompose:GetMaterialList(tbMaterial)
	local szList = format("%-20s  %s","vật phẩm ","Số lượng")
	...
	for i=1,getn(tbMaterial) do
		...
		if tbItem.nJxb then
			szList = format("%s<enter><color=yellow>%-20s<color>  %s(%d/%d)<color>",szList,"Ngân lượng", szColor, tbCount[i], tbMaterial[i].nJxb * tbMaterial[i].nCount)
...
function tbActivityCompose:Compose(nComposeCount)
	...
	if self.tbFormula.nWidth ~= 0 and self.tbFormula.nHeight ~= 0 and CountFreeRoomByWH(self.tbFormula.nWidth, self.tbFormula.nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("Để bảo đảm tài sản của đại hiệp, xin hãy để trống %d %dx%d hành trang", nFreeItemCellLimit, self.tbFormula.nWidth, self.tbFormula.nHeight))
		return 0
	end
	
	if self:CheckMaterial(tbMaterial, nComposeCount) ~=1 then
		local szMsg = self.tbFormula.szFailMsg or "<color=red>Đại hiệp mang nguyên liệu không đủ rồi!<color>"
		Talk(1, "", szMsg)
		return 0;
	end
	...
	if self:ConsumeMaterial(tbMaterial, nComposeCount, self.szLogTitle) ~= 1 then
		--Say("制作失败，部分物品丢失。",0)
		Msg2Player("Chế tạo thất bại, mất đi một số nguyên liệu.")
		return 0;
	end
...
function tbActivityCompose:ComposeGiveUI()
	...
	local szTitle = self.tbFormula.szComposeTitle or format("Đổi %s", tbProduct.szName)
...
function tbActivityCompose:GiveUIOk(nCount)
	if nCount < getn(self.tbFormula.tbMaterial) then
		return Talk(1, "", self.tbFormula.szFailMsg or "<color=red>Đại hiệp mang nguyên liệu không đủ rồi!<color>")
	end
...
function tbActivityCompose:ComposeDailog(bIsAskNumber,nMaxCount)
	...
	local szComposeTitle = self.tbFormula.szComposeTitle or format("Đổi %s", tbProduct.szName)
	
	local szMsg = format("%s yêu cầu: <enter>%s", szComposeTitle ,self:GetMaterialList(tbMaterial))
	local tbOpt = {}
	if bIsAskNumber == 1 then
		tbOpt[1] = {"Xác nhận", self.AskNumber, {self, nMaxCount}}
	else
		tbOpt[1] = {"Xác nhận", self.Compose, {self, 1}}
	end
	tbOpt[2] = {"Hủy bỏ "}
...
function tbActivityCompose:AskNumber(nMaxCount)	
	...
	if nMaxCount < 1 then
		...
		g_AskClientNumberEx(1, nMaxCount, "Xin mời nhập số", {self.ComposeCountComfirm, {self,nMaxCount}} )
	end
end
...
--	print("nMaxCount = " .. nMaxCount)
	if nCount > nMaxCount then
		Say(format("Ngươi nhập số lượng hợp thành đã vượt quá<color=red>%d<color>, xin hãy nhập lại", nMaxCount))
		return 0
	end
...
function tbActivityCompose:GetFormulaByString()
	...
	for i=1, getn(tbMaterial) do
		...
		if tbMaterial[i].nJxb then
			szName = transferDigit2CnNum(tbMaterial[i].nJxb).." lượng"
		else
