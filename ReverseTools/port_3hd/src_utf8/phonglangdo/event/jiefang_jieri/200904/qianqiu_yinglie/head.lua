	...
	if (game_level == 3) then
		...
		for i = 1, 3 do 
			if tbPlayer[i] and tbPlayer[i] > 0 then
				...
				Msg2Player(format("Bảng xếp hạng điểm: đứng thứ %d", i));
		...
		for i = 1, 3 do
			if tbPlayer[i] and tbPlayer[i] > 0 then
				PlayerIndex = tbPlayer[i];
				Msg2Player(format("Bảng xếp hạng liên trảm: đứng thứ %d", i));
				self:add_rank_award(i);
			end
		end
		...
		for i = 1, 3 do
			if tbPlayer[i] and tbPlayer[i] > 0 then
				PlayerIndex = tbPlayer[i];
				Msg2Player(format("Bảng xếp hạng PK: đứng thứ %d", i));
				self:add_rank_award(i);
			end
		end
		...
		if (MISSIONID == 16) then
			...
			for i = 1, 3 do
				if tbPlayer[i] and tbPlayer[i] > 0 then
					PlayerIndex = tbPlayer[i];
					Msg2Player(format("Bảng xếp hạng giết NPC: đứng thứ %d", i));
...
function TB_QIANQIU_YINGLIE0904:add_rank_award(nrank)
	...
	if (nrank == 1) then
		if CalcFreeItemCellCount()== 0 then
			Msg2Player("Khoảng trống hành trang không đủ, không thể nhận mặt nạ đại tướng quân");
		else
			...
			if (n_itemidx > 0) then
				...
				SyncItem(n_itemidx);
				Msg2Player(format("Nhận được %d %s", 1, "Mặt nạ Đại Tướng quân"));
			end
		end
	end
	...
	if nLiBaoCount == 3 then
		Msg2Player(format("Nhận được %d %s", nLiBaoCount, "Tống Kim lễ bao"));
	else
		Msg2Player(format("Do vì chỗ trống hành trang không đủ, chỉ nhận được %d %s", nLiBaoCount, "Tống Kim lễ bao"));
	end
end
...
function TB_QIANQIU_YINGLIE0904:add_end_award(tb_player, b_win)
	local game_level = BT_GetGameData(GAME_LEVEL);
	local nExpiredDate = FormatTime2Date(7 * 24 * 60 *60 + GetCurServerTime());
	local tb_award = {tbProp = {6,1,2005,1,1,0}, szName = "Tống Kim lễ bao",  nCount = 1+b_win, nExpiredTime = nExpiredDate};
	...
	if (game_level == 3) then
		...
		for i = 1, getn(tb_player) do
			...
			if (player_total_point >= 6000 and self:check_right() == 1) then
				...
				Msg2Player(format("Do vì điểm tống kim trên 6000, nên sẽ được thưởng %d Tống Kim lễ bao", tb_award.nCount))
			end
		end
		PlayerIndex = old_player;
	end
...
function TB_QIANQIU_YINGLIE0904:add_lucky_award(tb_player)
	...
	while (tb_player[i] and tb_player[i]>0 and nLuckyCount<3) do
		...
		if BT_GetData(PL_TOTALPOINT) >= 3000 then
			...
			if CalcFreeItemCellCount()== 0 then
				 Msg2Player("Hành trang không đủ chỗ trống, không thể nhận được mặt nạ nguyên soái");
			else
				nLuckyCount=nLuckyCount+1;
				local n_itemidx = AddItem(0,11,447,1,1,0);
				if (n_itemidx > 0) then
					SyncItem(n_itemidx);
					Msg2Player(format("Nhận được %d %s", 1, "Mặt nạ Nguyên soái"));
				end
			end
		end
	end 
