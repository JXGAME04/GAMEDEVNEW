...
function LoadTongSetting()	--载入配置
	...
	if b1~=1 then
		print("Đọc tệp config bang hội thất bại!")
		return
	end
...
LoadTongSetting()
	...
	TB_STUNTID_INFO = {
	[1] = {stuntid = 1,name = "Phụng Hoàng ấn", func = "stunt_fenghuang",},
	[2] = {stuntid = 2,name = "Luyện võ hồn", func = "stunt_duanwuhun",},
	[3] = {stuntid = 3,name = "Vô Song Sát Trận", func = "stunt_wushuang",},
	[4] = {stuntid = 4,name = "Đào Chu Tiền", func = "stunt_moneybox",},
	[5] = {stuntid = 5,name = "Võ thần tứ phúc", func = "stunt_wushenbless",},--暂定
	[6] = {stuntid = 6,name = "Linh Sâm Lực", func = "stunt_lingshenli",},
	[7] = {stuntid = 7,name = "Hoàng Kim Lệnh", func = "stunt_huangjinling",},
	}
	function LoadStuntData()
		local b1 = TabFile_Load("\\settings\\tong\\tongstunt_setting.txt", "stuntData")
		if b1~=1 then
			print("Đọc tệp config kỹ năng đặc biệt thất bại!")
			return
		end
...
function tongGetUpgradeWorkshopDemand(nTongID, nCurLevel) --获取升级所需作坊数
	...
	if (nRet == nil) then
		print("Gặp lỗi khi nâng cấp yêu cầu về số lượng tác phường!")
...
function tongGetUpgradeHighWorkshopsDemand(nTongID, nCurLevel) --获取升级所需高级作坊数
	...
	if (nRet == nil) then
		print("Gặp lỗi khi nâng cấp yêu cầu về số lượng tác phường cao cấp!")
		return INVALID_VALUE
	end
	return nRet
end
...
function tongGetUpgradeHighWorkshopsLevel(nTongID, nCurLevel) --获取升级所需高级作坊等级
	...
	if (nRet == nil) then
		print("Gặp lỗi khi nâng cấp yêu cầu về đẳng cấp tác phường!")
		return INVALID_VALUE
	end
	return nRet
end
...
function tongGetUpgradeCostFund(nTongID, nCurLevel)
	...
	if (nRet == nil or nRet <= 0)then
		print("Gặp lỗi khi nâng cấp yêu cầu về ngân sách kiến thết!")
		return INVALID_VALUE
	end
	return nRet
end
