...
function TongFundAdd(nTongID, nCurValue)
	...
	if (nTongWeekBuildFund < nWeekBuildUpper) then
		...
		if (nTongWeekBuildFund + nBuildFund >= nWeekBuildUpper) then
			...
			TONG_ApplyAddEventRecord(nTongID, "Ngân sách kiến thiết nhận được vượt quá giới hạn tuần, ngân sách kiến thiết dư ra sẽ chuyện sang ngân sách kiến thiết dự bị")
