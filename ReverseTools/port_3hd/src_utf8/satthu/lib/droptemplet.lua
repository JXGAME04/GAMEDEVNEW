...
function tbDropTemplet:GiveAwardByList(nNpcIndex, nBelongerIndex, tbItem, szLogTitle, nAwardCount)
	if not tbItem then --物品表为空
		self:Debug("Bảng vật phẩm trống.")
		return 0
	end
...
function tbDropTemplet:WriteLog(szLogTitle, szMsg)
	if szLogTitle then
		...
		if type(szLogTitle) == "table" then
			...
			tbLog:TabFormatLog(GetLocalDate("%Y-%m-%d %H:%M"), event_name, action, quantity, item_name, item_code, bug_desc, string_1, string_2, string_3)
		else
			tbLog:TabFormatLog(szLogTitle,GetLocalDate("%Y-%m-%d %H:%M"), szMsg)
		end
		
		
		
	else
		self:Debug("Hàm rơi vật phẩm không có log")
	end
end
