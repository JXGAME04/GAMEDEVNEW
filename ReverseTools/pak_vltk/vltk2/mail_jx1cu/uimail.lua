...
function UIMail:HeaderListArrival(nMinId, tbHeaderList, nComplete)
    ...
    if nComplete == 0 and nRepeatCount ~= getn(tbHeaderList) then -- ²»ÊÇÖØ¸´µÄÇëÇó£¬Ò»ÏÂ×Ó¸øÍæ¼Ò·¢¶à·âÓÊ¼þµÄÊ±ºò»áµ¼ÖÂ¿Í»§¶ËÖØ¸´ÇëÇó
...
function UIMail:GetAwardItemInfo(tbAward)
    ...
    for _, tbInfo in tbAward do
        ...
        elseif tbInfo.nBindingGold then
            tinsert(tbResult, {"\\spr\\item\\script\\jinding.spr", format("%d Ng©n §Ünh", tbInfo.nBindingGold), "", 0})
        elseif tbInfo.nBindingSilver then
            tinsert(tbResult, {"\\spr\\Ui4\\email\\ÒøÁ½Í¼±ê.spr", format("%d Ng©n §Ünh", tbInfo.nBindingSilver), "", 0})
            ...
            if nItemIndex > 0 then
                ...
                if tbInfo.nExpiredTime and tbInfo.nExpiredTime > 0 then
                    if tbInfo.nExpiredTime < 20190613 then
                        szDesc = format("%s <enter><color=red>hiÖu lùc %d ngµy<color>", szDesc, floor(tbInfo.nExpiredTime/1440))
                    else
                        ...
                        szDesc = format("%s <enter><color=red> hiÖu lùc tíi %d/%d/%d<color>", szDesc, nYear, nMonth, nDay)
                    end
                end
                ...
                tinsert(tbResult, {szIcon, szName, szDesc, tbInfo.nCount})
...
function UIMail:RequestDeleteMail()
    ...
    if not tbCurMail then
        Talk(1, "", "Cßn më th­ tïy ý")
        return
    end
    if tbCurMail.nAwardCount > 0 and tbCurMail.nState < MAILDEF.tbState.DRAWED then
        local szTitle ="Trong th­ cßn ®Ýnh kÌm ch­a nhËn, sau khi xãa kh«ng thÓ nhËn l¹i vËt phÈm ®Ýnh kÌm, x¸c nhËn xãa?"
        local tbOption =
        {
            "X¸c nhËn/g_ConfirmDeleteMail",
            "§Ó ta suy nghÜ/no",
        }
        ClientSay(szTitle, getn(tbOption), tbOption)
    else
...
function UIMail:RequestDeleteAllCheckedMail()
    if getn(self.tbMailCheckList) <= 0 then
        Talk(1, "", "Ch­a chän th­ tïy ý")
        return
    end
    ...
    if nUnDrawedCount > 0 then
        local szTitle ="Cßn th­ ch­a nhËn, x¸c nhËn xãa?"
        local tbOption =
        {
            "X¸c nhËn/g_ConfirmDeleteAllCheckedMail",
            "§Ó ta suy nghÜ/no",
        }
        ClientSay(szTitle, getn(tbOption), tbOption)
    else
...
function UIMail:OnMailIconClick()
    local szTitle = "§¹i hiÖp cßn bå c©u truyÒn tin ch­a ®äc, h·y ®Õn TÝn Sø ë c¸c thµnh chñ ®Ó xem th­"
    local tbOption =
    {
        "§i nhËn th­/g_ConfirmFindMessager",
        "§äc sau/no",
    }
    ClientSay(szTitle, getn(tbOption), tbOption)
end
...
function g_ConfirmFindMessager()
    ...
    AutoCrossMapFindPath(tbAutoFindPath[1], tbAutoFindPath[2], tbAutoFindPath[3])
end
