...
# ============================================================================ auction_npc.lua (Include vao dichquan.lua + lenhbaiadmin.lua)
def build_npc():
    P = "AUCTION_DEF"
    L = [
        "-- auction_npc.lua - " + MARK + " menu NPC dau gia (Include vao state cua NPC Dich Quan va lenh bai admin).",
        ...
        "    local szTong, nTong = GetTongName()",
        "    local tb = {}",
        "    tinsert(tb, \"" + V("Ký gửi vật phẩm (Ngân lượng)") + "/auc_npc_kygui_nl\")",
        "    tinsert(tb, \"" + V("Ký gửi vật phẩm (Xu)") + "/auc_npc_kygui_xu\")",
        "    if nTong and nTong > 0 and TONG_GetMaster(nTong) == GetName() then",
        "        tinsert(tb, \"" + V("Bang chủ: đưa vật phẩm vào phiên đấu giá bang") + "/auc_npc_tong\")",
        "    end",
        "    tinsert(tb, \"" + V("Mở cửa sổ đấu giá") + "/auc_npc_open\")",
        "    tinsert(tb, \"" + V("Rời khỏi") + "/no\")",
        "    Say(\"" + V("Chưởng quầy đấu giá: Ký gửi 24 giờ, phí ") + "\".." + P + ".nPersonalPutOnCost..\"" + V("% giá bán, thuế ") + "\".." + P + ".nAuctionTaxRate..\"" + V("% khi bán được. Tiền và vật phẩm trả qua hộp thư.") + "\", getn(tb), tb)",
        "end",
        "",
        "function auc_npc_open()",
        "    local h = OB_Create()",
        "    ObjBuffer:PushByType(h, OBJTYPE_NUMBER, " + P + ".tbAuctionTypeEnum.eType_PERSONAL)",
        "    AUC_SendTo(PlayerIndex, \"emSCRIPT_PROTOCOL_AUCTION_OPENWND\", h)",
        "    OB_Release(h)",
        "end",
        "",
        "function auc_npc_kygui_nl()",
        "    auc_npc_kygui(" + P + ".tbCurrency.MONEY)",
        "end",
        "",
        "function auc_npc_kygui_xu()",
        "    auc_npc_kygui(" + P + ".tbCurrency.XU)",
        "end",
        "",
        "function auc_npc_kygui(nCur)",
        "    if AUC_CountSeller(GetName()) >= " + P + ".nMaxItemPerSeller then",
        "        Say(\"" + V("Đại hiệp đang ký gửi tối đa ") + "\".." + P + ".nMaxItemPerSeller..\"" + V(" món, hãy chờ bán xong hoặc rút bớt.") + "\", 0)",
        "        return",
        "    end",
        "    AUC_TMP[PlayerIndex] = {nType = " + P + ".tbAuctionTypeEnum.eType_PERSONAL, szAct = \"\", nKind = " + P + ".tbItemTypeEnum.eType_DUTCH, nCur = nCur, nTong = 0}",
        "    OpenGetNumber(\"" + V("Nhập giá bán (") + "\"..AUC_CurName(nCur)..\")\", \"auc_npc_price\")",
        "end",
        "",
        ...
        "        Say(\"" + V("Giá không hợp lệ.") + "\", 0)",
        "        AUC_TMP[PlayerIndex] = nil",
        "        return",
        "    end",
        "    t.nPrice = nPrice",
        "    GiveItemUI(\"" + V("Đặt vật phẩm cần bán vào ô (1 món)") + "\", \"" + V("Giá ") + "\"..nPrice..\" \"..AUC_CurName(t.nCur), \"auc_npc_give_ok\", \"auc_npc_give_cancel\", 0, \"auc_npc_give_check\", 0)",
        "end",
        "",
        ...
        "    local nIdx = GetGiveItemUnit(1)",
        "    if nIdx == nil or nIdx <= 0 then",
        "        Msg2Player(\"" + V("Chưa đặt vật phẩm.") + "\")",
        "        return",
        "    end",
        ...
        "    Say(\"" + V("Phiên đấu giá bang: chọn kiểu đấu và loại tiền") + "\", 5,",
        "        \"" + V("Kiểu Anh (giá tăng dần) - Ngân lượng") + "/auc_npc_tong_a1\",",
        "        \"" + V("Kiểu Anh (giá tăng dần) - Xu") + "/auc_npc_tong_a2\",",
        "        \"" + V("Kiểu Hà Lan (giá giảm dần) - Ngân lượng") + "/auc_npc_tong_h1\",",
        "        \"" + V("Kiểu Hà Lan (giá giảm dần) - Xu") + "/auc_npc_tong_h2\",",
        "        \"" + V("Rời khỏi") + "/no\")",
        "end",
        "",
        ...
        "        Say(\"" + V("Chỉ Bang chủ mới mở được phiên bang.") + "\", 0)",
        "        return",
        "    end",
        "    AUC_TMP[PlayerIndex] = {nType = " + P + ".tbAuctionTypeEnum.eType_TONG, szAct = szTong, nKind = nKind, nCur = nCur, nTong = nTong}",
        "    OpenGetNumber(\"" + V("Giá khởi điểm (") + "\"..AUC_CurName(nCur)..\")\", \"auc_npc_price\")",
        "end",
        "",
        "function auc_npc_tong_a1() auc_npc_tong_start(" + P + ".tbItemTypeEnum.eType_ENGLISH, " + P + ".tbCurrency.MONEY) end",
        "function auc_npc_tong_a2() auc_npc_tong_start(" + P + ".tbItemTypeEnum.eType_ENGLISH, " + P + ".tbCurrency.XU) end",
        "function auc_npc_tong_h1() auc_npc_tong_start(" + P + ".tbItemTypeEnum.eType_DUTCH, " + P + ".tbCurrency.MONEY) end",
        "function auc_npc_tong_h2() auc_npc_tong_start(" + P + ".tbItemTypeEnum.eType_DUTCH, " + P + ".tbCurrency.XU) end",
        "",
        "-- GM (lenh bai admin): phien THE GIOI, ten phien tu dong theo gio mo",
        "function auc_admin_world()",
        "    Say(\"" + V("Phiên đấu giá thế giới (GM): chọn kiểu đấu và loại tiền") + "\", 5,",
        "        \"" + V("Kiểu Anh - Ngân lượng") + "/auc_admin_w_a1\",",
        "        \"" + V("Kiểu Anh - Xu") + "/auc_admin_w_a2\",",
        "        \"" + V("Kiểu Hà Lan - Ngân lượng") + "/auc_admin_w_h1\",",
        "        \"" + V("Kiểu Hà Lan - Xu") + "/auc_admin_w_h2\",",
        "        \"" + V("Rời khỏi") + "/no\")",
        "end",
        "",
        "function auc_admin_world_start(nKind, nCur)",
        "    local szAct = \"" + V("Phiên ") + "\"..GetLocalDate(\"%H:%M %d/%m\")",
        "    AUC_TMP[PlayerIndex] = {nType = " + P + ".tbAuctionTypeEnum.eType_WORLD, szAct = szAct, nKind = nKind, nCur = nCur, nTong = 0}",
        "    OpenGetNumber(\"" + V("Giá khởi điểm (") + "\"..AUC_CurName(nCur)..\")\", \"auc_npc_price\")",
        "end",
        "",
        "function auc_admin_w_a1() auc_admin_world_start(" + P + ".tbItemTypeEnum.eType_ENGLISH, " + P + ".tbCurrency.MONEY) end",
        "function auc_admin_w_a2() auc_admin_world_start(" + P + ".tbItemTypeEnum.eType_ENGLISH, " + P + ".tbCurrency.XU) end",
        "function auc_admin_w_h1() auc_admin_world_start(" + P + ".tbItemTypeEnum.eType_DUTCH, " + P + ".tbCurrency.MONEY) end",
        "function auc_admin_w_h2() auc_admin_world_start(" + P + ".tbItemTypeEnum.eType_DUTCH, " + P + ".tbCurrency.XU) end",
        "",
    ]
    return E.join(L)
