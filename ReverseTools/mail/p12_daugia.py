...
def build_manager():
    ...
    L = [
        ...
        "    end",
        "    if nS < 60 then",
        "        return \"" + V("còn ") + "\"..nS..\"" + V(" giây") + "\"",
        "    end",
        "    local nPhut = floor(nS / 60)",
        "    if nPhut < 60 then",
        "        return \"" + V("còn ") + "\"..nPhut..\"" + V(" phút") + "\"",
        "    end",
        "    if mod(nPhut, 60) == 0 then",
        "        return \"" + V("còn ") + "\"..floor(nPhut / 60)..\"" + V(" giờ") + "\"",
        "    end",
        "    return \"" + V("còn ") + "\"..floor(nPhut / 60)..\"" + V(" giờ ") + "\"..mod(nPhut, 60)..\"" + V(" phút") + "\"",
        "end",
        "",
        ...
        "    if nType == " + P + ".tbAuctionTypeEnum.eType_TONG then",
        "        return \"" + V(" Mở cửa sổ Đấu giá, thẻ Bang hội để trả giá.") + "\"",
        "    end",
        "    return \"" + V(" Mở cửa sổ Đấu giá, thẻ Thế giới để trả giá.") + "\"",
        "end",
        "",
        ...
        "    if nType == " + P + ".tbAuctionTypeEnum.eType_WORLD then",
        "        Msg2SubWorld(AUC_ChatCat(\"" + V("[Đấu giá thế giới] ") + "\"..sz))",
        "        return 1",
        "    end",
        "    if nType ~= " + P + ".tbAuctionTypeEnum.eType_TONG or (nTong or 0) <= 0 then",
        "        return 0",
        "    end",
        "    sz = AUC_ChatCat(\"" + V("[Đấu giá bang] ") + "\"..sz)",
        "    if AUC_MsgTong then",
        ...
        "    return nType == " + P + ".tbAuctionTypeEnum.eType_WORLD or nType == " + P + ".tbAuctionTypeEnum.eType_TONG",
        "end",
        "",
        ...
        "        return 1",
        "    end",
        "    local sz = szNguoi..\"" + V(" đưa ") + "\"..szTen..\"" + V(" lên sàn (") + "\"..szAct..\"): \"",
        "    if nKind == " + P + ".tbItemTypeEnum.eType_DUTCH then",
        "        sz = sz..\"" + V("giá mở ") + "\"..AUC_GiaTien(nCurP, nCur)..\"" + V(", hạ dần tới ") + "\"..AUC_GiaTien(nGuar, nCur)",
        "    else",
        "        sz = sz..\"" + V("khởi điểm ") + "\"..AUC_GiaTien(nGuar, nCur)",
        "        if (nBase or 0) > (nGuar or 0) then",
        "            sz = sz..\"" + V(", mua ngay ") + "\"..AUC_GiaTien(nBase, nCur)",
        "        end",
        "    end",
        "    sz = sz..\", \"..AUC_ConLai(nEnd)..\"" + V(" (kết thúc ") + "\"..AUC_GioDongHo(nEnd)..\").\"..AUC_ChatHuongDan(nType)",
        ...
        "        nStep = 1",
        "    end",
        "    local sz = r.buyer..\"" + V(" trả ") + "\"..AUC_GiaTien(r.cur, r.currency)..\"" + V(" cho ") + "\"..r.name..\"" + V(" - muốn vượt phải trả từ ") + "\"..AUC_GiaTien(r.cur + nStep, r.currency)",
        "    if (r.base or 0) > (r.guar or 0) then",
        "        sz = sz..\"" + V(", mua ngay ") + "\"..AUC_GiaTien(r.base, r.currency)",
        "    end",
        ...
        "    local sz",
        "    if bMuaNgay == 1 then",
        "        sz = szBuyer..\"" + V(" mua ngay ") + "\"..r.name..\"" + V(" với giá ") + "\"..AUC_GiaTien(nPrice, r.currency)..\"" + V(", vật phẩm gửi qua thư.") + "\"",
        "    else",
        "        sz = \"" + V("Hết giờ: ") + "\"..szBuyer..\"" + V(" thắng ") + "\"..r.name..\"" + V(" với giá ") + "\"..AUC_GiaTien(nPrice, r.currency)..\"" + V(", vật phẩm gửi qua thư.") + "\"",
        "    end",
        "    if r.atype == " + P + ".tbAuctionTypeEnum.eType_TONG then",
        "        if r.currency == " + P + ".tbCurrency.MONEY then",
        "            sz = sz..\"" + V(" Quỹ bang nhận ") + "\"..AUC_GiaTien(nNet, r.currency)..\".\"",
        "        else",
        "            sz = sz..\"" + V(" Người bán nhận ") + "\"..AUC_GiaTien(nNet, r.currency)..\".\"",
        "        end",
        "    end",
        "    AUC_ChatGui(r.atype, r.tong, sz)",
        "    return 1",
        ...
        "        return 1",
        "    end",
        "    local sz = \"" + V("Hết giờ: ") + "\"..r.name..\"" + V(" không ai trả giá, ") + "\"",
        "    if r.atype == " + P + ".tbAuctionTypeEnum.eType_WORLD then",
        "        sz = sz..\"" + V("đã thu hồi.") + "\"",
        "    else",
        "        sz = sz..\"" + V("trả về người bán qua thư.") + "\"",
        "    end",
        "    AUC_ChatGui(r.atype, r.tong, sz)",
        "    return 1",
        "end",
        ...
        "        return 1",
        "    end",
        "    AUC_ChatGui(r.atype, r.tong, r.name..\"" + V(" đã được rút khỏi sàn (chưa ai trả giá).") + "\")",
        "    return 1",
        "end",
        "",
        ...
        "        return 1",
        "    end",
        "    local sz = r.name..\"" + V(" hạ giá còn ") + "\"..AUC_GiaTien(r.cur, r.currency)",
        "    if (r.dropleft or 0) > 0 then",
        "        sz = sz..\"" + V(" (còn ") + "\"..r.dropleft..\"" + V(" lần hạ, thấp nhất ") + "\"..AUC_GiaTien(r.guar, r.currency)..\")\"",
        "    else",
        "        sz = sz..\"" + V(" (giá thấp nhất)") + "\"",
        "    end",
        "    sz = sz..\", \"..AUC_ConLai(r.endtime)..\".\"",
        "    AUC_ChatGui(r.atype, r.tong, sz)",
        "    return 1",
        ...
        "        return 1",
        "    end",
        "    local nType = " + P + ".tbAuctionTypeEnum.eType_WORLD",
        "    AUC_ChatGui(nType, 0, szAct..\"" + V(" mở: ") + "\"..n..\"" + V(" món mới, ") + "\"..AUC_ConLai(nEnd)..\"" + V(" (kết thúc ") + "\"..AUC_GioDongHo(nEnd)..\"" + V("). Ai trả cao nhất khi hết giờ được nhận qua thư.") + "\"..AUC_ChatHuongDan(nType))",
        "    for i = 1, n do",
        "        local m = tbBao[i]",
        "        local sz = i..\". \"..m.ten..\"" + V(": khởi điểm ") + "\"..AUC_GiaTien(m.kd, m.tien)",
        "        if (m.mn or 0) > 0 then",
        "            sz = sz..\"" + V(", mua ngay ") + "\"..AUC_GiaTien(m.mn, m.tien)",
        "        end",
        ...
        "    end",
        "    AUC_Settle(r, GetName(), nGiaMua, 1)",	# [DAUGIA-CHAT] 1 = mua ngay (cau bao kenh chat)
        ...
        "        Msg2Player(\"" + V("Đại hiệp đang giữ giá cao nhất, chờ người khác trả vượt đã.") + "\")",
        "        return",
        "    end",
        ...
        "        AUC_ChatThu(AUC_ChatTraGia, {r2})",	# [DAUGIA-CHAT] bao kenh chat: ai tra, bao nhieu, con bao lau
        "    end",
        "end",
        "",
        ...
        "        Msg2Player(\"" + V("Món của hệ thống, không rút được.") + "\")",
        "        return",
        "    end",
        ...
        "    AUC_ChatThu(AUC_ChatRut, {r})",	# [DAUGIA-CHAT]
        "    AUC_NotifyEndItem(nType, AUC_ActName(nType, r), nId, r.tong)",
        "end",
        "",
        "-- ---------------------------------------------------------------- chot giao dich",
        ...
        "    AUC_ChatThu(AUC_ChatBan, {r, szBuyer, nPrice, nNet, bMuaNgay or 0})",	# [DAUGIA-CHAT]
        "end",
        "",
        ...
        "        AUC_SetState(r.id, 3, 3)",
        "        AUC_ChatThu(AUC_ChatE, {r})",	# [DAUGIA-CHAT]
        "        return",
        "    end",
        ...
        "    AUC_ChatThu(AUC_ChatE, {r})",	# [DAUGIA-CHAT] chi phien bang len tieng (AUC_ChatLoai loc ky gui ca nhan)
        "end",
        "",
        ...
        "        return",
        "    end",
        "    AUC_Settle(r, r.buyer, r.cur, 0)",	# [DAUGIA-CHAT] 0 = het gio co nguoi thang
        "end",
        "",
        ...
        "    if nCur ~= " + P + ".tbCurrency.MONEY and nCur ~= " + P + ".tbCurrency.XU then",
        ...
        "        nBase = nStart",
        "    end",
        "    local nId = AUC_PutOn(" + P + ".tbAuctionTypeEnum.eType_WORLD, szAct, " + P + ".tbItemTypeEnum.eType_ENGLISH,",
        ...
        "    return \"\", nId, szName, nStart, nBuy, nCur",	# [DAUGIA-CHAT] tra them ten/gia de gom cau bao dot
        "end",
        "",
        ...
        "    local szAct = \"" + V("Đợt ") + "\"..nRound..\" \"..GetLocalDate(\"%H:%M %d/%m\")",
        ...
        "    local tbBao = {}",	# [DAUGIA-CHAT] {ten, kd, mn, tien} moi mon len san -> cau bao kenh chat
        ...
        "        -- [W3 notify] boc rieng: buoc nay quet ca bang + phat goi, loi o day khong duoc lam mat thong diep dot.",
        "        -- AUC_NotifyNewItem khong tra gi (call tra nil ca khi thanh cong) nen di qua AucWeb_Bao tra 1.",
        ...
        "end",
        "",
        "function AucWeb_Bao(szAct, nId)",
        "    AUC_NotifyNewItem(" + P + ".tbAuctionTypeEnum.eType_WORLD, szAct, nId, 0)",
        "    return 1",
        "end",
        "",
        ...
        "        if AUCWEB_DA_BAO_RONG ~= 1 then",	# [W8] chi ghi last_msg mot lan, khong UPDATE moi 30 giay
        ...
        "        return 1",	# tien trinh khac da gianh, hoac web vua tat
        "    end",
        ...
        "    else",
        "        Msg2Player(\"" + V("Phiên bang hội kéo dài 30 phút, tiền bán vào quỹ bang. Thuế ") + "\".." + P + ".nAuctionTaxRate..\"" + V("% khi bán được.") + "\")",
        "    end",
        ...
        "                    AUC_ChatThu(AUC_ChatHaGia, {r})",	# [DAUGIA-CHAT]
        "                end",
        "            end",
        "        end",
        ...
        "        end",
        "    end",
        "    return 1",	# [DAUGIA-WEB] de call(..., "x") phan biet duoc loi
        "end",
        "",
    ]
    return E.join(L)
