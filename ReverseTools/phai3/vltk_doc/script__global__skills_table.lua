...
function check_update()
	...
	if (GetTask(137) >= 20 or GetTripMode() == 2) then
		...
		rollback_prop()				-- 重置潜能点（2004-07-13Êâ
		skills_update()
		UpdateSkill()
		return 1
	end
...
--	Say("技能更新已经完成，请注意重新为各个技能分配技能点。",0)
	Msg2Player("Phục hồi kỹ năng đã hoàn tất!  Xin hãy phân phối lại điểm! ")
	xMagic = 0
	if HaveMagic(397) >= 0 then			-- 计算雨露重生技能，扣除相应点数（要防止返回-1Êâ
...
--	end
	...
	if (Ttask21 ~= MagicPoint_total + GetMagicPoint()) then		-- 不一致的要写log
		WriteLog(date("%Hgiờ%Mphút%Sgiây")..": tài khoản'"..GetAccount().."'nhân vật'"..GetName().."'Phục hồi kỹ năng đã lui về kỷ lục ban đầu: Đẳng cấp"..GetLevel()..", điểm kỹ năng ban đầu"..Ttask21..", được sửa lại"..MagicPoint_total+GetMagicPoint().."điểm (Bao gồm nhiệm vụ cộng"..GetByte(GetTask(86),2).."điểm, đạo cụ cộng"..GetTask(80).."điểm) .")
	end
...
function add_wh(lvl)			-- 根据任务等级加相应的武魂技能
	local nFactionNum = tbFacDef.tbIds.WUHUN
	local tbSkills = tbFacDef.tbSkills[nFactionNum]
		
	for nNeedLevel, tbSkillList in tbSkills do
		-- 给予所有传入等级以下的所有技能，但是逻辑上，此处应该只能给予70级以下的技能，以防万一在这里做了一层限制
		if (lvl >= nNeedLevel) and (nNeedLevel <= 70)then
			for _, nSkillId in tbSkillList do
...
function add_xy(lvl)			-- 根据任务等级加相应的武魂技能
	local nFactionNum = tbFacDef.tbIds.XIAOYAO
	local tbSkills = tbFacDef.tbSkills[nFactionNum]
...
--				AddProp(std_prop - player_prop)
				...
				WriteLog(date("%Hgiờ%Mphút%Sgiây")..": tài khoản'"..GetAccount().."'nhân vật'"..GetName().."'Phục hồi kỹ năng đã lui về kỷ lục ban đầu: Đẳng cấp"..GetLevel()..", điểm tiềmnăng ban đầu"..player_prop..", được tiềm năng sẵn có "..std_prop.."điểm (Nhiệm vụ cộng"..GetByte(GetTask(86),1).."điểm, đạo cụ cộng"..GetTask(81).."điểm) ."..x..".")
			end
--		end
		Msg2Player("Phục hồi tiềm năng đã hoàn tất! Xin hãy phân phối lại điểm! ")
	end
end
