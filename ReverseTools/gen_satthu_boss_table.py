...
L.append("\t-- dap xuong TOA DO MAC DINH cua map (waypoint) - dung y chu game: len map")
L.append("\t-- roi TU DI toi boss (client dan duong); khong tha canh boss.")
L.append("\tlocal nW, nX, nY = GetWayPointPos(t[1])")
L.append("\tif (nW == nil or nW ~= t[1]) then")
L.append("\t\tNewWorld(t[1], t[2] + 3, t[3] + 3)\t-- map khong co waypoint -> tha gan boss")
L.append("\telse")
L.append("\t\tNewWorld(nW, nX, nY)")
L.append("\t\tAddTermini(t[1])\t-- ghi diem da di (chuan Than Hanh Phu)")
L.append("\tend")
