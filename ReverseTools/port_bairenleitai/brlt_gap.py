#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Doi chieu API cho MOT DANH SACH TEP .lua cua ban LINUX (duong dan tuong doi script/).
Phan loai:  ENG = engine Linux co, du an KHONG dang ky, khong .lua nao dinh nghia
            ???  = khong ai dinh nghia, khong co trong ELF
            LNXLUA = .lua khac cua Linux dinh nghia (du an chua co) -> phai port kem
            PROJLUA = .lua cua du an dinh nghia
Dung: python brlt_gap.py <ten> <tep1> <tep2> ...
"""
import os, re, sys
...
def main():
    name = sys.argv[1]
    files = [os.path.join(G.LNX, l.replace("/", BS)) for l in sys.argv[2:]]
    ...
    print("NHOM %s: %d tep | %d ten duoc goi" % (name, len(files), len(calls)))
    for b in ("ENG", "???", "LNXLUA", "PROJLUA"):
        ...
        for n, sites in lst2:
            ...
            print("  %-28s x%-4d %s:%d%s" % (n, len(sites), os.path.relpath(p, G.LNX).replace(BS, "/"), l, extra))
    print("\n[OK] engine da co: %d | [TREE] tu dinh nghia: %d" % (len(buckets["OK"]), len(buckets["TREE"])))
    print("OK list:", ", ".join(sorted(n for n, _ in buckets["OK"])))
