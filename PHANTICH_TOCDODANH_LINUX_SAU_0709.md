...
**Cập nhật 10:05**: MATDO đã thay khe bằng build từ origin/main `c37a0b1b` (gộp SKEXP + TOCDO + MATDO b): `CoreServer.dll.moi` = **db5d064f**, `CoreClient.dll.moi` = **563c127e**. Đã kiểm: `c37a0b1b` chứa `ffffade4`, hai tệp `.moi` có chuỗi `CastFrame` (dấu hiệu gói TOCDO), live 1a33f617/5f86a7f7 không có. Bản `.moi.tocdo_e37ab486/ab99660c` vẫn giữ. Chờ chủ swap.

**LIVE 10:08 07/09**: chủ đã swap cặp db5d064f / 563c127e (MATDO b gộp SKEXP + TOCDO + DELTA e); kiểm live có chuỗi `CastFrame` ⇒ gói tốc độ đánh đang chạy thật cùng ini 18/18 và dữ liệu 75/175. Khe `.moi` tiếp theo là DELTA đợt f (bc6b2b80 / 9916d00f, gói vị trí 25 → 28 byte, phải swap cả server lẫn client).
