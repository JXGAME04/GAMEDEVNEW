...
function __saypage(n_from, n_total, n_perpage, str_function, str_parameter)
	...
	for _, option in pairs(tb_options) do
...
function strfill(str_org, n_len, n_style, str_space)
	...
	PushString(str_org)
	if (n_style == 1) then
		-- [LUA54 06/09] khoi duoi day von la chu thich kieu C (khong hop le trong Lua) - ma cu da bo,
		--while (strlen(str_org) < n_len) do
			--str_org = str_org..str_space
		--end
		-- da thay bang ham C. Doi sang chu thich tung dong: hop le ca Lua 4 lan 5.4.
