...
> **Kiểm chứng lần 2 (22/08, phiên sau):** đọc lại trọn 10 tệp cây + `config/9` + `basemission/lib.lua` + `msg2allworld.lua` bằng `gbktool`, chạy lại `brlt_closure.py` (31 tệp, 16 THIẾU) và `item_remap.py` (0 item), disasm lại `SetTmpCamp 0x0810BA50` / `KNpc::SetTmpCamp 0x0807B2D0` / `0x0807B260` / `Msg2Map 0x08105080` / `KNpcSet::GetRelation 0x0809EE50` / `SetPKFlag 0x0810F610→0x080C3740`, đối chiếu NPC template 3 bảng (`port_bairenleitai\npc_check.py`), đọc lại mọi `tệp:dòng` C++ dẫn trong mục C/G. Kết quả: **không có khẳng định nào sai**; 1 "CHƯA RÕ" (I.1.5) đã giải quyết; thêm G.11–G.12 và I.1.9–I.1.10; sửa `playerfunlib.lua:5` → `:4`.
...
| 3 | `missions\bairenleitai\npc_beauty.lua` | 35 | nguyên byte | Cổ Thủ: `G_ACTIVITY` có nhờ `playerfunlib.lua:4` Include `g_activity.lua` (`G_ACTIVITY:OnMessage` `:21` là no-op vì `FunSet` rỗng — đúng ý, không thêm mục) |
...
| `settings\maptraffic.ini [176] 38_Content=Quan nhắc nhở Hoàng Thành Tư` (điểm 1195,968) | Linux `:4081-4083` (`38_Type=-1`) | dự án `:3886-3888` đã có (`38_Type=1`) | không — lệch `Type` -1/1 là của bản đồ client, không thuộc phạm vi |
...
| C.4 | `RemoteExecute(szScript, szFun, hObj[, szCb, nCbId, dwGS])` | `0x08100740`: đóng gói proto `0x3D` ≤ 0x8000 byte gửi relay `g_Server+0x20030` | **1 GS → thực thi tại chỗ**: `hRes=OB_Create`; gọi `szFun(hObj, hRes, 0)` trong state `szScript` (có remap `\script\lib` → `scriptjx2\lib`); nếu có `szCb` gọi `szCb(nCbId, hRes)` trong state đang gọi; `OB_Release`. Đăng ký cạnh `GlobalExecute` `ScriptFuns.cpp:14588`. ObjBuffer dự án đủ 14 hàm `OB_*` | ~60 dòng. **ĐÃ RÕ (22/08 lần 2)**: bảng handle là `static std::map<int,KJx2ObjBuffer*> s_OBMap` + `s_nOBNextHandle` trong `KJx2SharedStore.cpp:34-35`; `LuaOB_Create :66-79` (new + `++s_nOBNextHandle`), `LuaOB_Release :81-96` (delete + erase); bộ đệm 4 KB (`JX2OB_BUF_SIZE 4096` `:25`, đủ cho {script, fun, {msg ~150 byte}}). ⇒ **Đặt `LuaJX2_RemoteExecute` ngay trong `KJx2SharedStore.cpp`** (dùng thẳng `s_OBMap`) hoặc thêm 2 hàm C không-static `int KJx2OB_CreateHandle()` / `void KJx2OB_ReleaseHandle(int h)` tách từ 2 hàm trên; khai `extern` ở `ScriptFuns.cpp:13313-13314` cạnh `LuaOB_Create/Release` |
...
| C.6 | **`CallPlayerFunction` không trả kết quả** (đã có, lỗi) | Linux trả mọi kết quả của hàm | `ScriptFuns.cpp:2196-2223` `lua_rawcall(L,nArgs,0); return 0;` (`:2218-2222`) ⇒ `hundred_arena.lua:177 szPlayerName = CallPlayerFunction(idx, GetName)` = **nil** → `tbPlayerList[nil]` lỗi ngay khi vào map; `:708-709 ST_GetDamageCounter` so sánh nil. Sửa `LUA_MULTRET` + trả `nRes`. Dự án hiện có **79 call site / 14 tệp** (`activitydetail.lua`, `header\shitu.lua`, `lib\awardtype\simple.lua`, `lib\worldlibrary.lua`, `eventsys\type\npc.lua`, `usetownportal.lua`, `leaguematch\npc\officer.lua`, `messenger_turerukou.lua`, `scriptjx2\lib\awardtype\*` 5 tệp, `scriptjx2\lib\player.lua`) — không tệp nào phụ thuộc "không trả gì" (Lua bỏ giá trị thừa) | 5 dòng |
...
| 11 | **`SetPKFlag` dự án = alias `SetFightState`** (`KJx2WarInfra.cpp:227-230`) còn Linux `SetPKFlag(n)` (`0x0810F610`) gọi `KPlayerPK::SetPKFlag(Player+0x5a50, n, 1)` (`0x080C3740`): ghi **cờ PK** `[pk+0]=n` + gửi gói `0x90` — **không đụng FightMode**. Hệ quả ở `ArenaField:Leave` `:541-543`: Linux = `SetFightState(0)` rồi bật cờ PK 1 (hoà bình nhưng cho phép PK); dự án = `SetFightState(0)` rồi **`SetFightState(1)`** ⇒ người bị đá khỏi đài đứng ở `OutPos` **vẫn ở trạng thái chiến đấu + `CurCamp` 4** (`Leave` không khôi phục camp, dòng `:549` comment — Linux cũng vậy) ⇒ có thể bắn chiêu xa vào 2 đấu sĩ trên đài (cả hai camp 4 + fight) và bị họ đánh lại. | Giữ alias (đổi sẽ ảnh hưởng `citywar_arena\head.lua:106`, `citywar_city\camper.lua:148`, 3 Hoạt động Phường dùng `SetPKFlag(1)` như "bật chiến đấu"). Ghi nhận là **lệch hành vi nhỏ có thể chấp nhận**; nếu chủ game thấy người ngoài phá trận thì sửa riêng: `LuaSetPKFlag` nhận giá trị 1 → không đổi FightMode (chỉ 0 và 2 mới map sang `SetFightState 0/1`) — phải kiểm lại 31 call site `SetPKFlag` trong cây trước khi đổi. |
| 12 | Ba hàm mới (`AddTimer`, `SetTmpCamp`, `Msg2Map`) **đã được 2 script có sẵn của dự án gọi** nhưng đang chết vì hàm nil: `script\activitysys\functionlib.lua:352-370` `lib:SetTimedTask` → `AddTimer(nTimeOut*18, szfnTimeOut, nCallbackParam)`; `script\skill\special\spider_web.lua:45,59` `SetTmpCamp(nTmpCamp, nNpcIndex)` + `Msg2Map` (chiêu "Ngũ Sắc Long Thổ", tham chiếu `settings\skills.txt:1487`). | Sau khi đăng ký hàm, hai đường này **sống lại**. `SetTimedTask` chỉ chạy khi có activity config gọi (dự án không có `activitysys\config`) — vô hại. `spider_web.lua` chỉ chạy nếu NPC nào có skill 1486 — kiểm `grep 1486 settings\npcs.txt` lúc thi công; nếu có, `SetTmpCamp(n, nNpcIdx)` sẽ đặt trại tạm cho NPC → luật C.2 áp cho NPC đó (đúng ngữ nghĩa Linux). Ghi nhận, không chặn. |
...
5. **`AddNpcEx` cho NPC thoại — ĐÃ RÕ (22/08 lần 2)**: `LuaAddNpcEx` (`ScriptFuns.cpp:6640-6704`) gọi `NpcSet.AddNpcSet2(MAKELONG(nLevel,nId), nSeries, nSubWorldIdx, nX, nY)` `:6676-6677` = đúng đường của `AddNpc` ⇒ `Kind` lấy từ template (`KNpc.cpp:7929` vùng nạp `pNpcTemp`), 5 NPC Kind 3 (1747/393/625/389/1571, `npc_check.py`) bấm được; `SetNpcScript` gắn script như boss bang hội đã chạy. **Lệch còn lại**: tham số 7 của `AddNpcEx` ở dự án = **`SetCurrentCamp`** (`:6684-6690`, ghi chú `:6632-6634`), còn Linux là cờ byte `+0x1824` (bNoRevive). `basemission_CallNpc:40` và `CallFightNpc:637` đều truyền `bNoRevive or 1` ⇒ mọi NPC của cây này có **camp 1 (`camp_justice`)** thay vì camp template (Kind-3: camp 6 `camp_event`; Cao thủ 1786-1795: camp 5 `camp_animal`). Với NPC thoại: `GenOneRelation` trả `relation_dialog` ngay dòng đầu khi Kind 3 (`KNpcSet.cpp:131-132`) ⇒ không ảnh hưởng. Với **Cao thủ** (Kind 0): vs Lôi chủ camp 4 → `Camp1≠Camp2` rồi `Kind==kind_normal` ⇒ **enemy** (`:151-155`) ✓ đánh được; vs khán giả **camp 1** (phái chính) → `Camp1==Camp2` ⇒ **ally** (`:148-149`) — khán giả chính phái không đánh được/không bị NPC đuổi (Linux camp 5: địch với tất cả). Lệch **có lợi** (bớt người ngoài phá) — giữ. Không cần đổi `bairen_boot.lua`.
...
8. `AddTimer` dùng `GetTickCount` (ms) thay khung 18/s — lệch < 1 khung. Rủi ro (4) trong JSON về chỉ số âm đã **giải quyết**: `Lua_ValueToNumber` là macro `lua_tonumber` (`Engine\Include\LuaLib.h:266`) — nhận chỉ số âm.
9. **Người bị đá khỏi đài giữ trạng thái chiến đấu** (G.11) — lệch nhỏ do alias `SetPKFlag`; theo dõi khi test: khán giả từng lên đài có bắn được vào đài không.
10. **Cao thủ NPC camp 1** thay vì 5 (I.1.5) — khán giả chính phái (camp 1) không thể hỗ trợ/phá; NPC có thể aggro sang khán giả tà/trung lập (camp 2/3) đứng sát đài như Linux aggro mọi người. Chấp nhận.
...
python port_bairenleitai\npc_check.py                           # 15 NPC template: Linux / server du an / client du an (Kind, Camp, Series, ten)
