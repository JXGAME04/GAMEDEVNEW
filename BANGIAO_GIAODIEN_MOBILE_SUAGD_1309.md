...
### 7.16 Lượt tkxh (02:45–03:20 14/09) — icon Tống Kim mở bảng xếp hạng, icon Kinh Mạch, ô dùng nhanh 1-4 bấm liên tục. Chủ: *"thêm icon tống kim ở kho vnku vào game bấm vào là hiện bản xếp hạng tống kim (ở client pc là bấm phím `)"*, *"kích sử dụng vào ô phím số 1 - 4 (ô bỏ item vào dùng nhanh) tôi kích liên tục mà không dùng liên tục mà rất chậm"*, *"thêm icon mở bản kinh mạch (client pc bấm F2)"*

- **Đích của hai phím PC** (đọc mã, không đoán): `ui/autoexec.lua` gán dấu huyền = `Open([[battlereport]])` và F2 = `Open([[meridian]])`; `ShortcutKey.cpp` case 22 "battlereport" bật/tắt `KUiRankData` (tệp nguồn ghi "Ui Tống Kim", chính là bảng có dòng "ấn phím ~ để bật/tắt"), case 12 "meridian" bật/tắt `KUiMeridian`. Tên "tongkim" chỉ số 20 trong bảng lệnh không có nhánh xử lý, bỏ qua.
- **Cơ chế thêm nút**: `KWndToolBar::Init` đọc `[Main] Button0..N` **liên tiếp, thiếu số là dừng**, mỗi mục có `ClassType=Player_X` thì `AddOneWindow` tạo lớp đã đăng ký (`RegisterSelfClass` ở `UiShell.cpp` cạnh `Player_WAuto`). Lần dựng đầu quên nối `Button8/9` nên nút không hiện và log bố cục thiếu dòng `ap KUiToolsControlBar|TongKimXH` — đó là dấu hiệu nhận biết; sửa dữ liệu, không cần dựng lại.
- **Sửa** `5745f4dc` (chỉ `#ifdef JX_ANDROID`): `Player_TongKimXH` / `Player_KinhMach` trong `UiShell.h/.cpp` (bấm = bật/tắt y hệt hai case trên, `CheckButton` sáng khi cửa sổ đang mở); overlay `uitoolscontrolbar.ini` thêm `[TongKimXH]` (54x48) và `[KinhMach]` (48x48) ở cột phải dưới nút Chạy, `Button8=TongKimXH`, `Button9=KinhMach`; danh sách trắng thêm `KUiToolsControlBar|TongKimXH=Xếp hạng Tống Kim`, `KUiToolsControlBar|KinhMach=Kinh mạch`; mặc định `862,297` và `862,349`, neo phải, người chơi dời được bằng trình chỉnh.
- **Ảnh**: `android/anh_tkxh_kinhmach_vnku.py` sinh `spr/uinew/uitoolscontrolbar/bxh_tk_m.spr` từ `UiPlayerBar/bxh_tk.spr` kho VNKU (187x165 → 54x48, khung 0 thường, khung 1 sáng vàng có sẵn) và `kinh_mach_m.spr` từ huy hiệu tròn bên trái nút `UiPlayerBar/luyencong.spr` (48x48, khung 1 làm sáng). Kho VNKU **không có** icon Kinh Mạch riêng (bộ 20 khung `icon_hoatdong.spr` là icon "Hoạt động" xoay sáng; `khung_kinh_mach.spr` của game giải mã sai vì khác định dạng) — ảnh Kinh Mạch là **tạm**, chủ chỉ tệp khác thì đổi trong kịch bản.
- **Ô dùng nhanh 1-4 "bấm liên tục rất chậm"** (gốc): `KSdlApp::ChamSuKien` đổi hai lần chạm cách nhau < 400 ms và < 24 điểm thành `WM_LBUTTONDBLCLK` (`CHAM_HAI_MS`/`CHAM_HAI_XA`, để nháy đúp dùng đồ trong túi); `KWndObjectBox::WndProc` chỉ bắt `WM_LBUTTONDOWN` (→ `WND_N_ITEM_PICKDROP` → `KUiPlayerBar::OnUseItem` theo `[ANDROID 11/09 OSO]`), không có nhánh DBLCLK → từ lần chạm nhanh thứ hai trở đi rơi mất, phải nghỉ > 400 ms mới dùng được món tiếp. **Sửa** `[OPHIM 14/09 DUP]`: đầu `WndProc`, với `UOC_IMMEDIA_ITEM` trên Android coi `WM_LBUTTONDBLCLK` = `WM_LBUTTONDOWN`. Bản PC không đổi.
- **Kiểm chứng giả lập** (bản a 109110259 + dữ liệu): hai icon hiện cột phải dưới "Chạy"; bấm Tống Kim → bảng "Thành tích cá nhân / Top 10 người đầu" mở, icon chuyển khung sáng; bấm Kinh Mạch → cửa sổ "Kinh Mạch – Bát Mạch Kỳ Kinh" mở; bấm ô 1 năm lần cách 150 ms: bình thuốc 7 → 4 (mã cũ chỉ được 1). Tiện thể trên đường vào game thấy hộp ĐĂNG NHẬP có tài khoản + 8 dấu sao + ô "Ghi nhớ" **tick sẵn**, và `uicommon.ini` được ghi 03:03:43 ngay sau bấm đăng nhập, trước khi chọn nhân vật — hai bằng chứng còn thiếu của §7.15.
- Bản b = 23cbde13 (5745f4dc + gộp b873bcc0 của DONHIP có [VE 11/09 b]) versionCode 109110312, md5 5c2e2ac3…, 20099023 byte; libmain có hai lớp mới, Represent3 có chuỗi [VE]. mobile-0809 FF = 23cbde13. Sáu tệp rời mới đã chép vào dt_v4; thay APK + restart 8765 sau khi DONHIP thử xong máy ảo (họ đang giữ máy ảo từ 03:11 để thử bản [VE b]; đã đề nghị đẩy một bản gộp thay vì hai lần).
- Ba dòng trên lượt chủ giao lúc 02:40 chưa nêu lại ở đây (F7-F8, Ctrl+Space) là phím PC của §7.15, không phải việc mới.
- **Đã đăng 03:18:08 14/09**: dt_v4 = bản gộp dựng từ 32c469d0 (= 499d7bc3 + 4692f5dc [VE 11/09 c] của DONHIP) versionCode 109110317, md5 139f7284…, 20099023 byte; libRepresent3 arm64 đổi so với bản b nên [VE c] có trong bản; máy chủ 8765 PID 301072, manifest liệt kê hai spr + bốn ini mới; config dt_v4 giữ nguyên (5 khoá NapKhung*/VeGiatMs có mặc định trong mã, DONHIP xác nhận không cần thêm). DONHIP không đẩy 109110305 nữa. mobile-0809 = nhánh tôi = 32c469d0.

### 7.17 Lượt bàn phím (03:25–10:00 14/09) — bàn phím ảo kẹt khi chat / nhập số; icon Kinh Mạch lấy từ kho jx-thiendieu; phân tích "phóng to spr cho dễ bấm". Chủ: *"phân tích tìm kiếm công nghệ có thể phóng to spr để chơi điện thoại có thể bấm dễ hơn"*, *"khi vào game nhập số hay chat dễ bị kẹt bàn phím máy điện thoại không ẩn lại được bàn phím"*, *"E:\SourceTuanLe\SourceVs22\jx-thiendieu\SwordOnline\bin\client\spr\ vào kho này kiếm icon kinh mạch"*

- **Gốc bàn phím kẹt** (đọc mã, loại trừ trước ba nghi vấn): bàn phím ảo chỉ bật ở `WND_M_SET_FOCUS` và chỉ tắt khi `KWndEdit` nhận `KILL_FOCUS` (hẹn 200 ms, `JxSdl_BanPhimNhip` trong vòng lặp chính, có chạy trong game). `Wnds.cpp:390` bỏ tiêu điểm khi chạm cửa sổ khác hoặc thế giới trống; Enter chat gọi `Wnd_SetFocusWnd(NULL)`; `Hide()` bỏ tiêu điểm cả ô con → hộp số đóng đúng. Kẽ hở thật: (1) chạm vào cần điều khiển / nút kỹ năng / icon NPC bị `KSdlApp::ChamSuKien` trả về sớm, không tới hệ cửa sổ → ô chat giữ tiêu điểm, bàn phím nằm mãi khi người chơi đi tiếp bằng cần; (2) Back đóng IME: `SDLDummyEdit.onKeyPreIme` → `onNativeKeyboardFocusLost` → `SDL_StopTextInput` nhưng game vẫn giữ tiêu điểm → chạm lại đúng ô không phát SET_FOCUS nên không mở lại; (3) nút ẩn của chính IME: SDL vẫn "active" mà bàn phím ẩn, chạm lại ô cũng không mở.
- **Sửa** `0bbf8ea5` (chỉ JX_ANDROID): `Wnds.cpp` thêm `JxUi_ChamKhiCoTieuDiem(x,y)` (0 không tiêu điểm / 1 chạm đúng ô / 2 chạm ngoài → đã bỏ) và `JxUi_BoTieuDiem()`; `KSdlApp.cpp` gọi lúc đặt ngón TRƯỚC các nhánh nuốt chạm (2 → tắt sau 200 ms theo cơ chế 12/09; 1 mà `!SDL_ScreenKeyboardShown` → tắt rồi mở lại `BanPhimMoLai`), và mỗi tick nếu `s_bBanPhimDangMo && !SDL_TextInputActive` → bỏ tiêu điểm cho đồng bộ. Lỗi dịch đầu do khai báo extern đặt sau chỗ dùng, đã chuyển lên trước `s_uBanPhimTat`. Cờ dịch Android có `-fsigned-char`, `char` có dấu như PC.
- **Kiểm chứng giả lập** (bản 109110954): chạm ô chat (406,571) rồi chạm `KyNang0` (957,575) → logcat `[BANPHIM] cham ngoai o nhap (957,575) -> bo tieu diem, tat ban phim`. LDPlayer không hiện bàn phím màn hình nên chỉ kiểm được bằng log; hành vi thật (Back, nút ẩn IME) nhờ chủ thử trên Fold 7.
- **Icon Kinh Mạch** `[KINHMACH 14/09 b]`: kho jx-thiendieu `spr\Ui3\UiBatMachChanKinh` có `level\tangthu1..9.spr` (9 huy hiệu phượng hoàng 35x35 theo cảnh giới), `skill\icon_*mach.spr`, `tab\0..7.spr`; lấy `tangthu2` (vàng) khung thường, `tangthu1` (đỏ) khung đang mở, 48x48, qua `android/anh_tkxh_kinhmach_vnku.py` (`KHO_TD`). Đã hiện dưới "Tống kim" trên máy ảo. Kho VNKU không có icon kinh mạch; spr `Ui3\Meridian` của game giải mã sai vì khác định dạng.
- **Phân tích "phóng to spr"** (sự thật đo được): Represent3 mặc định lọc `D3DTEXF_POINT`, lọc tuyến tính bảng màu chỉ có qua biến môi trường `REP3_PALLIN`; `PaintSmooth` là nhịp vẽ, không phải lọc ảnh. `TiLe` (300‰–3000‰) đổi cỡ cửa sổ và `KWndImage::PaintWindow` co giãn thật bằng `DrawSpritePartAlpha`, người chơi đã chụm được từng ô trong trình chỉnh, mặc định đã đặt 1,85 cho đấu giá, 1,75 thư, 1,5 nút bản đồ. Khung lô-gic cao 616 là gần sàn (hộp thoại họ 800x600). spr = 8-bit 256 màu RLE, bộ ghi tự lượng hoá. Kho VNKU là nguồn độ phân giải cao (icon Tống Kim gốc 187x165) nên "to mà nét" = sinh lại spr ở cỡ lớn từ nguồn, tăng `Width/Height` + mặc định `TiLe`, không cần AI; ảnh 8-bit cũ (`Ui3\UiPlayerBar\*.spr`: chạy, ngồi, lên ngựa, giao dịch, F9) không có nguồn lớn, phóng thì vỡ hạt → thay bằng ảnh VNKU/jx-thiendieu hoặc bật lọc. Chữ là font bitmap, mờ khi phóng, việc riêng. Đề xuất ba mức trong báo cáo cho chủ; chưa làm gì, chờ chủ chọn.
- Bản 109110954 (md5 9ed79f45…, 20100079 byte) lên dt_v4 09:57:53, PID 312200, manifest có `kinh_mach_m.spr` mới; gộp 74e64d98 (tài liệu DONHIP) = 7cf724e6, mobile-0809 FF = 7cf724e6.

### 7.18 Phân tích (10:00–10:30 14/09) — "phóng to ô vật phẩm trong hành trang / rương / Auto / các bảng tính năng, giữ nguyên hiện tại, chỉ phóng theo kích thước mỗi điện thoại" (CHƯA LÀM, chờ chủ chọn)

- **Tiền sử**: bản hành trang 44 px 12/09 (8daca0f9, ảnh VNKU, bố cục mới) đã bị chủ yêu cầu bỏ (dd19bcd4 "fix lại luôn cái hành trang như bản gốc"); pak dt_v4 vẫn còn ini 44 px nhưng tệp rời PC nạp trước nên không hiệu lực ở đâu. Kết luận: KHÔNG thiết kế lại từng bảng; chỉ phóng đồng bộ.
- **Sự thật trong mã** (đã đọc): ô lưới `KWndObjectMatrix` tính cỡ ô = Width/số cột, Height/số hàng (lúc Init); ô vật phẩm vẽ qua `DrawGameObj(..., m_Width, m_Height)` nên ô to là icon to; chữ mỗi ô có `m_nFontSize` riêng (`KWndText` Font=, `KWndLabeledButton` Font=), font chấm bitmap co giãn theo cỡ; `KWndImage::PaintWindow` giãn ảnh thật khi `m_nUiTiLe != 1000`; `UiDatTiLe` CHỈ phóng một cửa sổ, không phóng cây con, vì thế danh sách trắng đánh cờ 1 (không phóng) cho 22 bảng; `SetSize` cha chỉ xê dịch con có cờ `WND_S_*_WITH_R/B_EDGE`.
- **Phương án đề xuất — "PHONGBANG": phóng cả cây con của bảng theo màn hình** (chỉ JX_ANDROID): thêm `KWndWindow::UiPhongCay(k)` đệ quy: chụp số đo gốc mọi ô con một lần, rồi vị trí tương đối × k, cỡ qua `UiDatTiLe(k)` (ảnh/nút giãn sẵn), `m_nFontSize` × k (Text/Edit/LabeledButton/PureTextBtn), `KWndObjectMatrix` tính lại cỡ ô, danh sách/thanh cuộn tính lại cao dòng; đặt con SAU cha để tránh cộng dồn cờ R/B_EDGE. Kích hoạt lúc bảng nạp xong (LoadScheme) với k = min(khung_rộng/W, khung_cao/H) × 0,98, chặn trần `[Client] PhongBang` (mặc định 2000‰), và cho phép người chơi chỉnh riêng từng bảng bằng chụm hai ngón trong trình chỉnh (lưu TiLe trong hồ sơ như icon), tức bỏ cờ "không phóng" cho bảng. Vì k tính từ khung (điện thoại 616 cao, máy tính bảng 1040 rộng) nên tự theo từng máy.
- **Giới hạn thật** (tính từ ini PC, khung 1040x604): hành trang / rương / hành trang mở rộng 214x454 → k tối đa 1,33 (ô 28 → 37 px lô-gic ≈ 65 px vật lý ≈ 5,8 mm trên Fold 7, hiện 49 px ≈ 4,4 mm); cửa hàng 211x430 → 1,40; Auto 720x432 → 1,40; giao dịch 428x376 → 1,61 (ô 45 px); chọn máy chủ 281x421 → 1,43; ESC 214x250 → 2,42; Tuỳ chọn 205x252 → 2,40; Tống JX2 602x463 → 1,30; các màn 800x600 (Init/Login/ConnectInfo) → 1,0 (không phóng được). Muốn ô 44 px như bản 12/09 ở hành trang / rương thì buộc bố cục lại (bớt hàng + cuộn, hoặc lưới rộng hơn) — chính là cái chủ đã bỏ; chủ chọn.
- **Chất lượng ảnh**: bộ vẽ lọc POINT; phóng thêm 1,3–2,4 lên trên 1,75 của máy → ảnh khung/chữ vỡ hạt hơn hiện tại. Giảm nhẹ: bật lọc tuyến tính bảng màu cho lớp UI (hiện chỉ có biến môi trường REP3_PALLIN → thêm khoá config), hoặc chấp nhận (chữ chấm bitmap vốn đã phóng 1,75).
- **Công**: 1–2 phiên: lõi đệ quy + 5–6 lớp ô ghi đè (Matrix, Text, Edit, LabeledButton, List, ScrollBar) + trình chỉnh cho phép chụm bảng + thử 22 bảng trên máy ảo/Fold 7. Rủi ro: ô con đặc thù (tab `KWndPage`, `KWndImagePart` cắt ảnh, `KWndValueImage`) lệch, sửa từng lớp khi gặp.
- Không đụng bản PC (mọi thứ trong `#ifdef JX_ANDROID`).

### 7.19 Lượt PHONGBANG (10:30–11:50 14/09) — ĐÃ LÀM: bảng tính năng tự phóng cả cây con theo màn hình lúc mở. Chủ: *"Hãy làm - và chạm 2 ngón rất bất tiện cứ tính kích thước màng hình điện thoại lúc mở Ui lên phóng to vừa phải không ảnh hưởng đến hình ảnh là được"* (sau khi đã nói *"vẫn giữ nguyên hiện tại chỉ phóng to theo kích thước mỗi điện thoại"*)

- **Cách làm** (chỉ `JX_ANDROID`, PC giữ nguyên từng byte ở các nhánh `#else`): `KWndWindow::UiPhongCay(k)` (WndWindow.cpp) chụp gốc cả cây một lần (cỡ qua `m_nUiGocW/H`, vị trí tương đối cha, cỡ chữ) **trước** khi cha đổi cỡ (vì `SetSize` cha kéo con có cờ `WND_S_*_WITH_R/B_EDGE`), rồi phóng từng ô từ gốc: chữ (`UiLayFont/UiDatFont` ảo ở 7 lớp chữ) → cỡ (`UiDatTiLe`, ảnh giãn thật qua `m_nUiTiLe`) → `UiPhongRieng` (`KWndObjectMatrix` tính lại cỡ ô + viền từ gốc → ô vật phẩm to đúng bằng lưới, `DrawGameObj` vẽ giãn theo ô) → con đặt lại vị trí gốc × k. Kích hoạt: cờ mới `SUAGD_CO_PHONGBANG 16` trong `uitoado_danhsach.ini` (dòng `,23`), khoá `[Ui] PhongBang` (trần phần nghìn, mặc định 1400 trong mã, 0 = tắt), k = min(khung rộng / rộng gốc, khung cao / cao gốc) × 0,96, chặn trần, < 1,02 thì thôi; bảng dịch để giữ tâm rồi kẹp trong khung.
- **Bảng bố cục vẫn lưu vị trí ô cỡ GỐC**: lệch giữ tâm ghi vào `m_nUiPhongLech`, `GhiLaiO` cộng lại; ô con trong bảng đã phóng giữ toạ độ / TiLe thiết kế trong bảng, `UiToaDoM_KCayCha` hoá số khi áp (`ApMotO`, `SuaGd_ApKWnd`), `GhiLaiO` chia lại có làm tròn → kéo / kẹp vùng an toàn / đổi hồ sơ / mở lại không trôi, không nhân đôi. Bảng gốc ghi TiLe 1000 (tự tính lại); trình chỉnh không cho TO/NHỎ bảng (cờ 1 vẫn giữ).
- **Bốn bẫy đã gặp trên máy ảo** (mỗi bẫy một bản dựng, a→g): (1) `ApMotO` thoát sớm khi bảng không có mục bố cục (nhân vật nữ `KUiStatus|Female`) → phóng cả khi `nMuc < 0`; (2) `KWndWindow::Init` gọi `UiToaDo_ApChoO(this)` ngay khi đọc Left/Top → bảng gốc bị phóng lúc `Init(&Ini,"Main")` trong `LoadScheme`, **trước** khi ô con Init → gốc con chụp lúc chưa có (Auto: nút nhóm/tab mất, BẬT/TẮT nhảy lên góc; hành trang thoát vì mọi ô con có mục bố cục đè lên) → Init xoá gốc (`UiXoaGoc`) và đi qua `UiToaDo_ApChoOInit` (không phóng lúc Init), ô con Init khi bảng đang phóng (trang Auto đổi tab) được phóng ngay theo cây; (3) **cỡ chữ là MÃ font** trong `[FontList]` `uibasepublicsetting.ini` (10, 12, 13=12, 14, 16; `font.pak` chỉ có `gbk_fs12/14/16.fnt`) → `OutputText` với mã lạ (15, 18, 21) bỏ vẽ (`jx_rep3.log`: `[FONT] BO chu (font id=21 chua nap)`) → `UiPhongChu` chọn mã gần nhất trong {10,12,14,16}: chữ chỉ to tối đa 16 (12 → 16 ở k ≥ 1,25; 14/16 → 16); muốn chữ to hơn phải thêm .fnt + mã; (4) `KUiESCDlg::Initialize` gọi `Wnd_AddWindow` **trước** `LoadScheme` (lúc đó bảng chưa có số đo) → phóng ở `KWndWindow::Show()` của cửa sổ gốc (`m_pParentWnd == NULL`), khi cả cây đã nạp; (5) Auto tự căn giữa trong `OpenWindow` bằng cỡ ĐÃ phóng rồi mới `Show()` → PhongBang lần hai coi vị trí đó là vị trí ô cỡ gốc, trừ lệch → kẹp về (0,0) → chỉ dịch giữ tâm khi vị trí VỪA đặt từ bảng bố cục (`m_bUiViTriTuBang`), vị trí do chính PhongBang hay mã bảng đặt thì giữ, chỉ kẹp trong khung; áp lại bao nhiêu lần cũng một kết quả (log: hành trang 546,0 ba lần, Auto 37,13 rồi 37,12 ổn định).
- **Bảng nào phóng** (12 → 10 sau khi đọc mã 12 lớp, tác tử Explore 11:01): `KUiItem` (hành trang), `KUiItemEX`, `KUiStoreBox` (rương), `KUiShop`, `KUiTrade`, `KUiMailManager`, `KUiAuctionManager`, `KUiESCDlg`, `KUiOptions`, `KUiWAuto`. **Không phóng** (giữ `,7`): `KUiStatus` (Trang bị và thuộc tính) vẽ số liệu bằng toạ độ cứng trong `PaintWindow` → phóng cây thì chữ/khung lệch, khung vẽ hai lần (đã thấy trên máy ảo); `KUiTongJX2` (`RepositionRows` đặt lại lưới bằng hằng số mỗi lần đổi trang); `KUiSkillsNew` (số cấp kỹ năng vẽ tay, khoá gốc `Main=-26,-208` không mở được); `KUiTeamManager2|Main` gốc chỉ 27x23 (HUD tổ đội); 8 màn đăng nhập / bản đồ toàn màn hình. Muốn phóng các bảng này phải sửa lớp (nhân toạ độ với `UiLayPhongCay()`).
- **Auto (`KUiWAuto`)**: trang nội dung `KUiWAutoTrang::NapTab` Init lại chính nó + widget theo thiết kế mỗi lần đổi tab → được phóng ngay trong Init (bẫy 2) và phóng lại chữ ở cuối `NapTab` (`UiPhongChuCay`, vì Init đọc `Font=` sau `KWndWindow::Init`). Tiêu đề hộp nhóm (`KWndHopNhomWA::PaintWindow`: font 12 + 6 px/chữ cứng) vẫn cỡ cũ — nhỏ nhưng đọc được, để sau.
- **Số đo máy ảo 1040x604** (log `[PHONGBANG]`): hành trang 214x454 → k 1,276 → 273x579 tại 546,0 (ô 28 → 35 px); Auto 720x432 → 1,342 → 966x579; ESC 214x250 → 1,4 → 299x350; thư 624x445 → 1,302 → 812x579. Fold 7 (1436x616) sẽ ra ~1,3 cho bảng cao 454 (khung cao 616 là mức chặn), chữ 12 → 16.
- **Kiểm chứng**: 7 vòng máy ảo, bản cuối 109111137 (md5 d73bcdb8…, 20 179 407 B; ảnh `pi07_bag`, `pi12_auto`, `pi12e_auto2`, `pf12_auto_goc` = bản chưa phóng để so, `pi10_esc`): hành trang phóng đúng lưới 6 cột, tab/nhóm Auto bấm được (log `[WAUTO-UI] tab 2 → 5 → 1`), ESC 5 nút. Chưa thử rương / cửa hàng / giao dịch (cần NPC), mã 4 bảng này cùng kiểu hành trang (chỉ ô con ini).
- **Còn lại / lưu ý**: HUD (thanh công cụ phải, icon Ngồi…) vẽ đè lên bảng to (z-order cũ, bảng Auto 966 px chạm cột icon phải); chữ tối đa 16; rương / cửa hàng / giao dịch chưa nhìn tận mắt; `KUiStatus`, bang hội, võ công chưa phóng (cần sửa lớp). Mã: 769f3688 + 193672dc + ecf7806a, dựng từ mobile-0809 5a72a37a (có [VE f]); lên dt_v4 11:41 kèm `ui\uitoado_danhsach.ini` mới (10 dòng `,23`).

### 7.20 Lượt h (11:55–12:15 14/09) — ba ý mới của chủ: *"các icon tống kim - kinh mạch bạn mới thêm vào chưa chỉnh sửa giao diện được"*, *"phần đăng nhập tài khoản - mật khẩu có làm to lên theo kích thước màn hình được không?"*, *"Thêm một post item kênh chat chưa neo nên chơi màn hình rộng bị lệch"*

- **Icon không chỉnh được — gốc**: danh sách trắng `uitoado_danhsach.ini` đã 99 mục, trần `SUAGD_MAX_MUC` là 96 → `SuaGd_ThemMuc` bỏ ba mục cuối (dòng 98–99 là `KUiToolsControlBar|TongKimXH`, `KinhMach`) → trình chỉnh không vẽ khung, chạm không chọn (máy ảo: log `danh sach trang: 96 muc`, ảnh `pj03_pick_tk` không có khung quanh icon). Sửa: trần 256. Kiểm: log `99 muc`, chạm icon Tống Kim chọn được ("Xếp hạng Tống Kim | 871,311 | 54x48 | 100%", ảnh `pk06_pick_tk`). Bài học: mỗi lần thêm mục vào danh sách trắng phải nhìn trần này.
- **Khung đăng nhập phóng theo màn hình**: `KUiLogin|Main` là cửa sổ 800x600 (ảnh nền có hộp ĐĂNG NHẬP ở giữa, ô nhập / nút là con) → k theo khung < 1 (khung cao 616) nên PHONGBANG thường không phóng. Thêm cờ `SUAGD_CO_PHONGTRAN 32` (kèm 16): phóng đúng trần `[Ui] PhongBang` (1,4), căn giữa khung, cho phép toạ độ âm (phần ngoài khung là nền trong suốt), không kẹp. Danh sách trắng `KUiLogin|Main` ,7 → ,55. Máy ảo: hộp 280x184 → 392x258, ô nhập 156x22 → 218x31, nút 140x28 → 196x39, chữ giữ 16 (trần font); log `KUiLogin|Main goc 800x600 khung 1040x604 -> k=1400 -> 1120x840 tai -40,-118` (ảnh `pk04_login_zoom`). Nút Ghi (OpenRep, 20,560) rơi ra ngoài khung khi phóng — không dùng trên điện thoại. Trên Fold 7 cũng ra 1,4 (không phụ thuộc khung).
- **"Post item kênh chat" chưa neo**: cửa sổ vật phẩm đăng kênh chat `KUiChatItem` (UiChatItemInfo.ini, `Init "Main"` 220,100, cỡ theo ảnh) không có mục trong bảng bố cục mặc định → không neo, màn hình rộng nằm lệch trái, cũng không kéo được. Thêm `KUiChatItem|Main=220,100,1000,0,1,1` (neo giữa) vào `uitoado_macdinh.ini` + `_rong.ini` và `KUiChatItem|Main=Vật phẩm đăng kênh chat,7` vào danh sách trắng (100 mục) → màn hình rộng dịch theo tâm như các hộp thoại khác, kéo được trong trình chỉnh. Chưa nhìn tận mắt trên máy ảo (cần đăng vật phẩm lên kênh), suy từ cách các hộp `KUiMsgSel|Main=…,1,1` đã chạy.
- Mã cf44c530 (chỉ `UiToaDoMobile.inc` + 4 tệp dữ liệu lớp ghi đè); dt_v4 = 109111206 (md5 54d20809…, 20 179 503 B) 12:10, máy chủ 8765 PID 354212, tệp rời lên dt_v4: danh sách trắng + 2 mặc định; mobile-0809 = cf44c530.

### 7.21 Lượt i (12:20–12:50 14/09) — năm ý của chủ kèm hai ảnh Fold 7: *"post item kênh chat vẫn chưa được - post lên kênh chat nó bị lệch"* (thêm: *"lỗi post item lên kênh chat khi chơi ở điện thoại - giả lập không bị"*), *"đối thoại với npc hiển thị các dòng chọn khi lên điện thoại quá nhỏ để kích vào dễ nhầm"*, *"tới gần người chơi đang bày bán kích vào player đó sẽ mở sạp hàng người đó để xem"*, *"phóng to hành trang các item đó hơi bị lệch khung - shop cũng bị lệch 1 tí"*, *"khi kích vào item và đang mở shop thì không hiện chữ bán ở item"*

- **Lưới vật phẩm lệch khung (KHOPO)** — gốc: ô lưới `KWndObjectMatrix` = cỡ gốc × k / số ô làm tròn xuống (28 × 1,276 = 35,7 → 35) trong khi ảnh nền giãn liên tục (35,7) → lệch dồn 0,7 px mỗi ô, hàng 10 lệch 7 px, cột 6 lệch 4 px (đúng ảnh chủ gửi). Sửa: `UiKhopTiLe` ở lớp lưới đổi k sao cho ô = số nguyên (k' = ceil(n×1000/u): 1,276 → 1,25 ô 35; Fold 1,302 → 1,286 ô 36), `UiToaDoM_PhongBang` gọi `UiKhopTiLeCay` trước khi phóng. Máy ảo: hành trang 267x567 tại 549,0, hàng cuối / cột cuối khớp khung (ảnh `pl07_zoom_bot`). Cửa hàng / rương dùng cùng lưới 168x280 (ô 28) nên cùng sửa.
- **Chạm người bày bán mở sạp (SAP)** — PC chỉ xem sạp bằng Ctrl+chuột trái (`Mouse_Say` → `ProcessPeople ACTION_CHAT` → `GOI_FOLLOW_SOMEONE` + `GOI_VIEW_PLAYERSELLITEM`); chạm thường là `Mouse_Action` (đánh / nói chuyện NPC). Thêm ở `Mouse_Action` (chỉ Android): người chơi (`kind_player`) có `GetNPCBAITAN(uId)` → đi tới + xin xem sạp, log `[SAP]`. Chưa thử được trên máy ảo (cần người khác bày bán) — chủ thử: đứng gần rồi chạm vào người bán.
- **Nút "Bán" khi mở cửa hàng (BAN)** — trên PC phải bấm nút "Bán" của cửa hàng (trạng thái `UIS_S_TRADE_SALE`) rồi bấm món; trên điện thoại chạm món mở menu vật phẩm (`KUiVatPham`) không có "Bán". Thêm nút `Bán` (`nut_vp_ban.spr` sinh bởi `android/anh_vatpham_vnku.py`) khi `KUiShop` đang mở → `GOI_TRADE_NPC_SELL` y như `KUiItem`. Chưa nhìn tận mắt trên máy ảo (chưa mở được cửa hàng NPC trong lượt), mã dùng lại lệnh sẵn có.
- **Dòng chọn hội thoại NPC (NPC)** — hộp thoại `KUiMsgSel`/`KUiMsgSel2`/`KUiMsgSel4` dùng `KScrollMessageListBox` (KWndMessageListBox): bước dòng Android = font + 6 (15 → 20 px, Fold 1,75 ≈ 35 px, hơi nhỏ). Thêm khoá ini `DongThem=` (chỉ Android, `KWndMessageListBox::Init`, cộng vào `CaoDong`, phóng theo k trong `UiPhongRieng`) và lớp ghi đè `uimsgsel.ini`/`uimsgsel2.ini`/`uimsgsel4.ini` `[Select_List] DongThem=4`; ba bảng vào danh sách trắng cờ `,23` → phóng 1,4 (433x240 → 606x336), chữ 14 → 16, dòng = 16+6+5 ≈ 27 px lô-gic (Fold ≈ 48 px, đúng cỡ ngón tay), 4 dòng thấy một lúc (cuộn thêm). Chưa nhìn tận mắt trên máy ảo (không tìm được NPC gần; nút THOÁT không mở hộp xác nhận).
- **Bảng vật phẩm đăng kênh chat (CHATITEM)** — `KUiChatItem` (UiChatItemInfo.ini `Main` 220,100, cỡ tự tính theo chữ) nằm cố định theo thiết kế 800x600: máy ảo 1040 = thiết kế nên đúng, Fold 1436 thì xa dòng chat → "lệch" (đúng như chủ nói "giả lập không bị"). Sửa (Android): `OpenWindow` đặt bảng ngay **trên dòng vừa chạm** (`Wnd_GetCursorPos`, chú ý `KShortcutKeyCentre::ms_MouseX/Y` = 0 vì không đi qua phím tắt), kẹp trong khung → không phụ thuộc bề rộng màn hình; bỏ mục danh sách trắng (mã đặt lại mỗi lần mở; mục mặc định neo giữa vẫn còn, vô hại). Máy ảo: chạm dòng (150,230) → bảng 276x156 tại 12,62 (ảnh `pn12_chatitem`, log `[CHATITEM]`).
- Mã 68393817 (10 tệp nguồn + 5 tệp dữ liệu); dt_v4 = 109111244 (md5 6c6d2302…, 22 984 099 B — gradle lần này đóng gói nặng hơn ~2,8 MB cùng nội dung, chưa rõ vì sao) 12:47, máy chủ 8765 PID 358208; tệp rời mới lên dt_v4: `ui\ui3\uimsgsel*.ini` (3), `spr\ui3\uivatpham\*.spr`, danh sách trắng, 2 mặc định; mobile-0809 = 68393817.

### 7.22 Lượt l (12:55–13:05 14/09) — ảnh Fold 7 của chủ: *"lên điện thoại nó lệch ra ngoài"* (nút tên vật phẩm trong dòng chat) và *"các tính năng có nút cuộn ở chiến lệnh vuốt không cuộn được"*

- **Nút tên vật phẩm trong chat lệch ra ngoài câu** — gốc (đọc mã, không phải bảng thông tin như tôi tưởng ở 7.21): `KWndMessageListBox::PaintWindow` tính vị trí nút `<tên vật phẩm>` bằng toạ độ **tuyệt đối** (`Param.nX + cột × font/2`) rồi `KUiPlayerBar::SetItemBtnPos` gọi `SetPosition` — mà `SetPosition` là **tương đối cha** (`KUiPlayerBar`). Thanh dưới thiết kế ở (1,1) nên PC và máy ảo 1040 chỉ lệch 1 px; trên Fold 7 thanh neo giữa nằm ở (199,13) → nút lệch +198 px sang phải, +12 px xuống — đúng ảnh chủ (chữ "gia tot ne" ở chỗ chừa, nút `<Bạch Chủ ngọa>` văng sang phải). Sửa (chỉ Android): `SetItemBtnPos` trừ toạ độ tuyệt đối của thanh trước khi đặt; `y99` dùng bước dòng `CaoDong()` (Android = font + 6 + DongThem) thay vì `m_nFontSize` cho dòng thứ hai trở đi. Không kiểm được trên máy ảo (khung 1040 = thiết kế, lệch 1 px), suy từ mã; chủ kiểm trên Fold 7.
- **Vuốt không cuộn ở Chiến Lệnh** — vuốt dọc trên giao diện đã được dịch thành `WM_MOUSEWHEEL` từ 09/09 ([ANDROID 09/09 CUON], KSdlApp), nhưng chỉ `KWndList`/`KWndList2`/`KWndMessageListBox`/`KWndEdit` tự xử lý; bảng tự cuộn bằng `KWndScrollBar` (Chiến Lệnh `[DlyScr]`, cây con 3–4 tầng chuyển tiếp `WND_N_SCORLLBAR_POS_CHANGED`) không nhận. Sửa (Android, `Wnds.cpp` `Wnd_ProcessInput`): sau khi giao `WM_MOUSEWHEEL` cho cửa sổ dưới con trỏ, nếu cửa sổ đó không phải lớp danh sách thì tìm thanh cuộn **dọc** đang hiện (cao hơn rộng, có khoảng giá trị) trong con của nó rồi lên từng tổ tiên → `ScrollLine` (SetScrollPos báo cha y như bấm mũi tên). Áp cho mọi bảng dùng thanh cuộn (thư, đấu giá, bang hội…). Đã thử máy ảo 13:07 (bản 109111302): mở Chiến Lệnh, vuốt lên trong danh sách nhiệm vụ ngày → cuộn 3 dòng (từ "Tham gia 1 lần Tống Kim" sang "Diệt 500 quái… Online 10 phút"), vuốt xuống cuộn về (ảnh `po01_chienlenh`, `po02_vuot_len`, log `[CHAM] cuon tai 450,520`).
- Mã 1270a115; dt_v4 = 109111302 (md5 320a3c9b…, 20 206 383 B) 13:03, máy chủ 8765 PID 356816; mobile-0809 = 1270a115. Phiên mobile-144hz-optimization đã đồng bộ head này để dựng bản nhịp/FPS: 13:20 họ thay đè dt_v4 = 109111313 (md5 250bd14a…, 20 256 747 B; gộp 164e5889 + origin/main d8105a1a + [BKG/PALBUF/FPSNGOAI/MAU]), máy chủ 8765 PID 304848 chạy từ worktree của họ, tệp rời của tôi giữ nguyên; mobile-0809 = 1822835c. Điện thoại chủ đang chạy 109111302 (phiên 13:06), lần mở app tới nhận 109111313.
