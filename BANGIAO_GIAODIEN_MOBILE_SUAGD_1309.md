...
| Việc | Cách làm |
|---|---|
| Vào | nút **Chỉnh giao diện** (chỗ cũ `[Ui] SuaToaDoNutX/Y`), chỉ hiện khi ở trong thế giới; mobile mặc định `SuaToaDo=1` |
...
| Bảng mũi tên | nổi, kéo bằng tiêu đề: 4 mũi tên dịch **1 điểm** (giữ 400 ms thì chạy đều 60 ms/bước), `+`/`−` 5 %, số % |
| Thanh nút trên | `Dời 1 ô / Dời cả cụm`, `Giấu/hiện`, `Mặc định ô`, `Hoàn tác` (64 bước, gộp theo thao tác), `Xoá hết`, `Lưu mặc định` (chỉ khi `[Ui] SuaToaDoChu=1`), `Xong - lưu` |
...
1. Vào game, bấm **Chỉnh giao diện** (dưới bản đồ nhỏ).
