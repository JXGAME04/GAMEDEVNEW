...
### 6.7 Bổ sung 02/09 00:40 (chủ giao thêm)
- **Tẩy luyện phi phong "bấm tẩy báo thiếu nguyên liệu"**: box `KUiMantleWash` chỉ có **1 ô** `ObjBoxItem` (phi phong) nhưng `doWashRoll` (`script\global\mantlesystem\mantleupgrade_npc.lua`) đếm Lệnh Bài 4937 / Đại Bách Luyện 4933 **trong khay give-box** → khay không bao giờ chứa được nguyên liệu → luôn "thiếu". Vá `[WASH-PB 02/09]` (Lua, không cần build): đếm bằng `CalcEquiproomItemCount(6,1,4937|4933,-1)` và trừ bằng `ConsumeEquiproomItem` từ hành trang (ưu tiên 1 Lệnh Bài, không thì 2 Đại Bách Luyện + 20000 vạn), trừ TRƯỚC `Pay`; chuỗi thông báo/hướng dẫn nói rõ "trong hành trang". Backup `.truoc_pb_0209`. Lỗi "bỏ phi phong vào không hiện dòng ẩn" = lỗi #4 (`GDI_MANTLE_HIDDEN_DESC` index/dwID) đã vá trong `CoreClient.dll.moi 2C148668` — cần swap client mới thấy.
- **Bộ test dung luyện ở lệnh bài admin**: tệp mới `script\item	est_dungluyen_admin.lua` (`DL_TestRoot`), Include trong `lenhbaiadmin.lua`, mục "Test Dung luyện" trong `chucnangadmin` (menu gốc ~498/512 byte — ĐÃ SÁT TRẦN, thêm mục nữa phải rút nhãn cũ). Menu con: nhận 6 Văn Cương cấp 1 (6 loại thuộc tính: P 1/11/21/31/41/61) · Văn Cương cấp 5 và 8 (test trần phẩm chất) · 5 Tinh Thiết Khoáng + 5 Tinh Tinh Khoáng + 5 Huyền Hỏa Than · Văn Tinh Giáp cấp 1 x4 + cấp 2 x2 · 3 trang bị Bạch Hổ Phủ Tổ dung luyện được (áo 2x3, nhẫn 1x1, liên 2x1 — `AddItem2(2,0,rec,…)` rec 2289/2286/2284). Dung luyện thật vẫn làm tại Thợ Rèn (box cần script NPC).
- **Bộ `.moi` cuối** (phiên bot wauto-68 gộp đợt e vào server): `CoreServer.dll.moi` = `c769f6f8a3de` (00:45, = B0D3596B + KPlayerBot đợt e icon môn phái + đợt f bot vào bang im lặng; bản B0D3596B backup `_moi_backup9_b0d3596b\`), `CoreClient.dll.moi` = `2C14866841D3`, `Game.exe.moi` = `6AA1FEB9C55B`. Checklist mục 6.5 giữ nguyên, chỉ thay md5 server ở bước 3 thành `C769F6F8…`.

### 6.8 Phản biện đợt 2 (02/09 01:00, có chọn lọc: 4 lăng kính trên chính các sửa đổi 6.1/6.7) — 0 lỗi mới
40 điểm kiểm đều đạt (nạp DB genre 8/9, SetPrice/c2sTradeBuy/GetItemBillType, szJx2 + ExecuteItemScriptJX2, thứ tự gói view/sạp → item tạm tạo NGAY trong handler nên `s2c_syncfusion` tìm được, enum/g_nProtocolSize khớp, nút Hủy → `Destroy()` chỉ đặt cờ TOBEDESTROY nên không use-after-free, khay 14x4 cột 2 an toàn với trang bị tối đa 2x4, doWashRoll thứ tự kiểm/trừ/Pay, Say 512, magicscript 0 thẻ `<color` sau byte cao).
Ghi chú tiền tồn (KHÔNG do đợt này, chưa sửa — chủ quyết): (1) `SetPriceFromScript(idx,0)` (khoá bảo hiểm, ScriptFuns.cpp:3750) cũng không xoá `uPrice` → cùng gốc lỗi #5 ở đường khác, sửa 1 dòng khi tiện; (2) `c2sTradeBuy` không kiểm kết quả `AddKIL` (mua vào ô không hợp lệ = mất món đã trả tiền); (3) `GetItemBillType` coi "có giá trong danh sách sạp" là đang bày (kể cả sạp đóng) → phải gỡ giá mới dung luyện được, an toàn; (4) `doWashRoll` ghi đè `tbWashCu` mỗi lần quay → "Giữ nguyên" chỉ khôi phục kết quả lần quay trước, không phải dòng gốc (có từ PF13); (5) `RecoveryBox` lúc túi đầy có thể ném 1 món xuống đất (tiền lệ mọi box, đã có cảnh báo CoreShell.cpp:3735); (6) box dung luyện hiện viên/than ở cả 2 thẻ (chỉ hiển thị; bấm sai thẻ server trả mã 8/20).
