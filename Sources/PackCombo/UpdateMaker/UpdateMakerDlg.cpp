...
BOOL CUpdateMakerDlg::OnInitDialog()
{
	...
	m_UpFile.InsertColumn(0,"Name",LVCFMT_LEFT,80);
	m_UpFile.InsertColumn(1,"FullName",LVCFMT_LEFT,80);
...
{
	CFileDialog DlgOpenFile(true,NULL,NULL,OFN_HIDEREADONLY,"all file (*.*)|*.*",
		this->GetOwner());
...
void CUpdateMakerDlg::OnRun() 
{
	...
	if(FileOp.CopyFiles())
	{
		MessageBox("FileOp.CopyFiles","CopyFiles",MB_OK | MB_ICONHAND);
		return;
	}
...
*/
	...
	string aMsg;
	aMsg = "M1 "  + m_SaveFileDir + " M2 ";
	MessageBox(aMsg.data()," M3 ",MB_OK | MB_ICONASTERISK );
}
