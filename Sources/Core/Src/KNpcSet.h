...
public:
...
#ifdef _SERVER
	int				AddNpcSet3(int nSubWorld, void* pNpcInfo);	//H祄 add npc ?server
#endif
	...
	int				GetAroundNpcMonster(KUiPlayerItem *pList, int nCount, int nRange);//Add by phong ki襲 using fkauto
	int				GetAroundPlayerMonster(KUiPlayerItem* pList, int nCount, int nRange);//Add by phong ki襲 using fkauto
	int				GetAroundNpcBossMonster(KUiPlayerItem* pList, int nCount, int nRange);//Add by phong ki襲 using fkauto
