	...
	case defTONG_JX2_COP_TRANSFORM_MONEY:
		{
			...
			if (nMoney < nOffer)
			{
				static const char szNo[] = "Ng©n s¸ch bang héi kh«ng ®ñ";
			...
			{
				static const char szUp[] = "Kh«ng thÓ chuyÓn ng©n, v× sÏ lµm ng©n s¸ch kiÕn thiÕt bang v­ît qu¸ giíi h¹n tuÇn!";
			...
			{
				char szLog[200];
				sprintf(szLog, "%s lµm cho %.0f Ng©n s¸ch bang chuyÓn %u v¹n l­îng vµo ng©n s¸ch kiÕn thiÕt",
	...
	case defTONG_JX2_COP_TRANSFORM_BUILD:
		{
			...
			{
				static const char szNo[] = "Ng©n s¸ch kiÕn thiÕt bang kh«ng ®ñ!";
				KPlayerChat::SendSystemInfo(1, nPlayerIdx, MESSAGE_SYSTEM_ANNOUCE_HEAD, (char*)szNo, (int)strlen(szNo));
				return 20;
			}
			...
			{
				char szLog[200];
				sprintf(szLog, "%s lµm cho %d v¹n ng©n s¸ch kiÕn thiÕt chuyÓn sang ng©n s¸ch chiÕn bÞ.",
	...
	case defTONG_JX2_COP_FORCE_RETIRE:
		{
			...
			if (nRetire)
			{
				if (nFig != 2 && nFig != 3)
				{
					sprintf(szMsg, "%s ®¶m nhiÖm chøc vô quan träng, kh«ng thÓ tho¸i Èn, chØ cã ®éi tr­ëng vµ bang chóng míi cã thÓ tho¸i Èn!",
						bSelf ? "B¹n " : pTarget->szName);
				...
				if (nRet >= (nAct + nRet) / 2)
				{
					sprintf(szMsg, "Tr­íc m¾t sè ng­êi tho¸i Èn cña bang ®· ®¹t giíi h¹n, kh«ng thÓ tiÕp tôc sö dông thao t¸c tho¸i Èn %d", nRet);
					KPlayerChat::SendSystemInfo(1, nPlayerIdx, MESSAGE_SYSTEM_ANNOUCE_HEAD, (char*)szMsg, (int)strlen(szMsg));
					return 20;
				}
				if (!bSelf)
				{
					...
					{
						static const char szD[] = "ChØ cã thÓ cho tho¸i Èn ®èi víi thµnh viªn kh«ng lªn m¹ng tõ 7 ngµy trë lªn!";
						KPlayerChat::SendSystemInfo(1, nPlayerIdx, MESSAGE_SYSTEM_ANNOUCE_HEAD, (char*)szD, (int)strlen(szD));
						return 20;
					}
					sprintf(szMsg, "%s vs %s TiÕn hµnh thao t¸c tho¸i Èn", Player[nPlayerIdx].m_PlayerName, pTarget->szName);
				}
				else
					sprintf(szMsg, "%s Tho¸i Èn khái bang héi", pTarget->szName);
				sJX2_Msg2Tong(pTong, szMsg);
			...
			if (nFig != 4)
			{
				static const char szN[] = "Kh«ng ë tr¹ng th¸i tho¸i Èn.";
			...
			{
				static const char szW[] = "KÓ tõ ngµy tho¸i Èn 7 ngµy sau ngµy tho¸i Èn míi cã thÓ hñy bá tho¸i Èn!";
			...
			{
				static const char szOK[] = " Hñy bá tho¸i Èn thµnh c«ng!";
	...
	case defTONG_JX2_COP_DEMISE:
		{
			...
			{
				char szLog[200];
				sprintf(szLog, "%s chuyÓn nh­îng vÞ trÝ Bang Chñ cho %s.", Player[nPlayerIdx].m_PlayerName, pTarget->szName);
	...
	case defTONG_JX2_COP_SET_TITLE:
		{
			...
			{
				static const char szF[] = "ChØ bang chñ, tr­ëng l·o hoÆc ®éi tr­ëng míi cã danh hiÖu riªng!";
