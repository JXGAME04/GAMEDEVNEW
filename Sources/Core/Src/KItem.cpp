...
BOOL KItem::SetAttrib_Base(const KEQCP_BASIC* pBasic)
{
		 ...
		 i < sizeof(m_aryBaseAttrib)/sizeof(m_aryBaseAttrib[0]); i++)
	{
		...
		if (pDst->nAttribType == magic_durability_v) //#kh«ng thÓ ph¸ huû
...
void KItem::operator = (const KBASICPROP_MAGICSCRIPT& sData)
{
	...
	pCA->bShortKey			= sData.m_bShortKey;	//bá xuèng « phÝm t¾t
	...
	::strcpy(pCA->szItemName,  sData.m_szName);
	::strcpy(pCA->szScript,  sData.m_szScript);
	pCA->nPickExecute = sData.m_nPickExecute;//add by phong kiÒu item sö dông ngay
	pCA->nParam				= -1;
#ifndef _SERVER
	::strcpy(pCA->szImageName, sData.m_szImageName);
	::strcpy(pCA->szIntro,	   sData.m_szIntro);
...
void KItem::PaintItem(int nX, int nY, bool bResize/* = false*/, bool bPaintStack/* = false*/, unsigned int sidx /* = 0*/)
{
	...
	if(sidx > 0)
	{
		...
		if(nPos == pos_immediacy && !IsFkItemSkill()) //VÏ l¹i vÞ trÝ Icon cho nh÷ng item kh«ng ph¶i lµ skill
		{
...
void KItem::GetDesc(char* pszMsg, bool bShowPrice, int nPriceScale, int nActiveAttrib)
{
	...
	if (m_CommonAttrib.nItemGenre == item_magicscript && (m_CommonAttrib.nParticularType == 1083 || m_CommonAttrib.nParticularType == 1084)) //Håi thµnh phï
	{
		strcat(pszMsg, "  \n  ");
		char NumUse[128];
		sprintf(NumUse, " Cßn l¹i <color=red> %d <color> lÇn sö dông ", m_CommonAttrib.nParam);
	...
	if (bShowPrice && nPriceScale > 0)
	{
		...
		else
		{
			if (m_CommonAttrib.nPrice / nPriceScale%10000 == 0)
				sprintf(szPrice, "<color=White>Gi¸: %d v¹n l­îng<color>",  m_CommonAttrib.nPrice / nPriceScale /10000);
			else
				sprintf(szPrice, "<color=White>Gi¸: %d v¹n %d l­îng<color>",  m_CommonAttrib.nPrice / nPriceScale/10000,  m_CommonAttrib.nPrice / nPriceScale%10000);
		}
	...
	if (m_CommonAttrib.nItemGenre == item_magicscript &&  m_CommonAttrib.nParticularType >= 199 &&  m_CommonAttrib.nParticularType <= 204)
	{
		...
		strcat(pszMsg, " <color=White>Yªu cÇu trang bÞ:<color> <color=Green>");
		strcat(pszMsg, szIntor);
		strcat(pszMsg, "  \n  ");
		strcat(pszMsg, "  \n  ");
	}
		...
		m_CommonAttrib.nParticularType >= 238 && m_CommonAttrib.nParticularType <= 390)
	{
		...
		if (AtlasTab.Load("\\Settings\\Item\\atlas_compound.txt"))
		{
			...
			if (nHang >= 2)
			{
				...
				strcat(pszMsg, " <color=Fire>Nguyªn liÖu cÇn:  \n  ");
				for (int k = 1; k <= 6; k++)
				{
					char szCot[32];
					char szTenNl[64];
					...
					if (nCap > 0 && szDescNl[0])
						_snprintf(szDongNl, sizeof(szDongNl) - 1, " <color=Green>%s (cÊp %d) - %s  \n  ", szTenNl, nCap, szDescNl);
					else if (nCap > 0)
						_snprintf(szDongNl, sizeof(szDongNl) - 1, " <color=Green>%s (cÊp %d)  \n  ", szTenNl, nCap);
					else
						_snprintf(szDongNl, sizeof(szDongNl) - 1, " <color=Green>%s  \n  ", szTenNl);
					szDongNl[sizeof(szDongNl) - 1] = 0;
					strcat(pszMsg, szDongNl);
				}
				strcat(pszMsg, " <color=White>KÌm: HuyÒn Tinh Kho¸ng Th¹ch + 100000 l­îng  \n  ");
			}
		}
	}
	...
	for (i = 0; i < 7; i++)
	{
		...
		if (m_aryBaseAttrib[i].nAttribType == magic_durability_v) //#kh«ng thÓ ph¸ huû
		{
			char	szDurInfo[32];
			if (m_nCurrentDur == -1)
			{
				sprintf(szDurInfo, "<color=Yellow>Kh«ng thÓ ph¸ huû");
			}
			else if(m_nCurrentDur == 0)
			{
				sprintf(szDurInfo, "<color=Red>Trang bÞ háng");
			}
			else
			{
...
/*
			...
			case equip_meleeweapon:
				{
					...
					{
					case series_metal:
						strcat(pszMsg, "\n<color=Yellow> CÇn hÖ <color=Earth>Thæ <color>");
						break;
					case series_wood:
						strcat(pszMsg, "\n<color=Yellow> CÇn hÖ <color=Water>Thñy <color>");
						break;
					case series_water:
						strcat(pszMsg, "\n<color=Yellow> CÇn hÖ <color=Metal>Kim <color>");
						break;
					case series_fire:
						strcat(pszMsg, "\n<color=Yellow> CÇn hÖ <color=Wood>Méc <color>");
						break;
					case series_earth:
						strcat(pszMsg, "\n<color=Yellow> CÇn hÖ <color=Fire>Háa <color>");
						break;
					}
					strcat(pszMsg, "cña d©y chuyÒn vµ y phôc ®Ó kÝch tÝnh ©m<color> \n");
					break;
			...
			case  equip_armor:
				{
					...
					strcat(pszMsg, "cña nhÉn (d­íi) vµ th¾t l­ng ®Ó kÝch tÝnh ©m<color>\n");
					break;
			...
			case equip_ring:
				{
					...
					case series_earth:
                        ...
                        strcat(pszMsg, "cña nãn vµ vò khÝ ®Ó kÝch tÝnh ©m<color> \n");
					break;
			...
			case equip_belt:
				{
					...
					strcat(pszMsg, "cña ngäc béi vµ bao tay ®Ó kÝch tÝnh ©m<color> \n");
					break;
			...
			case equip_cuff:
				{
					...
					strcat(pszMsg, "cña giÇy vµ nhÉn (trªn) ®Ó kÝch ho¹t ©m<color> \n");
					break;
				}
			case equip_pendant:
				{
					...
					strcat(pszMsg, "cña nhÉn (trªn) vµ giµy ®Ó kÝch tÝnh ©m<color>\n");
					break;
				}
				strcat(pszMsg, "\n");
				break;		
	...
	if (m_CommonAttrib.LimitTime.bYear)
	{
		...
		if(thoigianhet > 0)
		{
			...
			{
                snprintf(sTmp, sizeof(sTmp), "<color=red>HÕt h¹n sö dông vËt phÈm sÏ bÞ hñy");
			}
	...
	if(GetExpirePoint() > 0)
	{
		...
		sprintf(sTmp, "<color=fire>Thêi h¹n sö dông: %d Ngµy %d Giê . <color>", days, hours);
		strcat(pszMsg, "  \n  ");
		strcat(pszMsg,sTmp);
	}
