...
void KItem::PF_AppendDesc(char* pszMsg) const
{
	...
	for (i = 1; i <= nHole && i <= PF_MAX_STONE; i++)
	{
		...
		if (p && p->szName[0])
			...
			sprintf(szLine, "<color=Green>%d sao LÁ kh∂m trËng <color>", nLv);	// G_STR_COLOR_EMPTY_XING
		strcat(pszMsg, szLine);
		strcat(pszMsg, "  \n  ");
	}
...
void KItem::GetDesc(char* pszMsg, bool bShowPrice, int nPriceScale, int nActiveAttrib)
{
	...
	else
	{
		...
		if (m_CommonAttrib.nItemGenre == 0)
		{
			...
			{
				sprintf(TextLevel, "%s (C p %d)", m_CommonAttrib.szItemName, GetMaxStoneNum());
			}
	...
	if (m_CommonAttrib.nItemGenre == item_starstone)
	{
		...
		if (pDa && pDa->nAttribType > 0)
		{
			...
			else
			{
				...
				char szTho[96];
				sprintf(szTho, "  \n  <color=200,120,255>ThuÈc t›nh khi kh∂m (lÁ 10 sao): +%d <color>  \n  <color=255,255,255>", pDa->nValue[9]);
				strcat(pszMsg, szTho);
			}
		}
#endif
...
void KItem::GetDesc(char* pszMsg, bool bShowPrice, bool bPriceScale, int nActiveAttrib, int nGoldActiveAttrib)
{
	...
	if (sPF_LaPhiPhong(&m_CommonAttrib) && GetMaxStoneNum() > 0)
	{
		sprintf(pszTemp, " (C p %d)", GetMaxStoneNum());
		strcat(pszMsg, pszTemp);
		pszTemp[0] = 0;
	}
