...
int LuaSetMask(Lua_State * L)
{
	...
	if (Lua_IsNumber(L,1))
	{
		...
		Npc[Player[nPlayerIndex].m_nIndex].m_MaskType = nValue;
	}
	return 0;
}
...
int LuaAddEventItem(Lua_State *L)/*AddEventItem(id)*/
{
	...
	int nIndex = ItemSet.AddItemSet2(item_task, 0, 0, 0, nEventId, 0,  0, g_SubWorldSet.GetGameVersion(),0,nStackNum,nEnChance,nPoint, nYear,nMonth,nDay,nHour);
	if (nIndex <= 0) 
	{
		Lua_PushNumber(L,0);
		return 1;
...
int LuaAddGoldItem(Lua_State *L)
{
	...
	int nSerise = 0;
	if(!g_GoldItemTab.GetInteger(nEventId + 2, "Series", 0, &nSerise))
	{
		nSerise = 0;
	}

	int nIndex = ItemSet.AddGoldItem(nEventId,NULL,nSerise,0,nYear,nMonth,nDay,nHour);
	if (nIndex <= 0) 
	{
		Lua_PushNumber(L,0);
		return 1;
...
int LuaAddItem(Lua_State * L)/*AddItem(nItemClass, nDetailType, nParticualrType, nLevel, nSeries, nLuck, nItemLevel..6)*/
{
	...
	int nIndex = ItemSet.AddItemSet2(nItemClass, nSeries, nLevel, nLuck, nDetailType, nParticularType, nItemLevel, g_SubWorldSet.GetGameVersion(),0,nStackNum,nEnChance,nPoint, nYear,nMonth,nDay,nHour);
	if (nIndex <= 0)
	{
		Lua_PushNumber(L, 0);
		return 1;
...
int LuaAddMineItem(Lua_State *L)
{
	...
	int nIndex = ItemSet.AddItemSet2(item_mine, nSerise, nLevel, 1, 1, nEventId, 0, 0, 0, 1, 0, 0, nYear, nMonth, nDay, nHour);
...
int LuaRemoveRankBattle(Lua_State * L)
{
	...
	if(btRank > 0)
	{
		Npc[Player[nPlayerIndex].m_nIndex].SetRankBattle(btRank, 1, 1);
		return 1;
	}
	return 0;
}
...
int LuaSetDeathRevivalPos(Lua_State * L)//SetTempRev(worldid, x, y) khi chÕt sÏ quay vÒ
{
	int nPlayerIndex = GetPlayerIndex(L);
...
int LuaUpdateBattleBox(Lua_State * L)// UpdateBattleBox
{
	...
	if (nParamCount == 3)
	{
		...
		if (nType == 3)
		{
			...
			BYTE m_btRobFlag = pMission->m_MissionPlayer.GetParam(nValue[0], 9);//nhÆt cê
			int m_nTPPoint	 = pMission->m_MissionPlayer.GetParam(nValue[0], 6);//tÝch luü
			int m_nTPDeath	 = pMission->m_MissionPlayer.GetParam(nValue[0], 4);//sè lÇn chÕt
...
int LuaIgnoreState(Lua_State * L) //Lo¹i bá c¸c miniskill trªn ng­êi, lÖnh bµi, chiÕn cæ
{
	int nPlayerIndex = GetPlayerIndex(L);
	if (nPlayerIndex <= 0)
		return 0;
...
int LuaSetProtectTime(Lua_State * L) //vong tron bat tu, vßng trßn bÊt tö
{
	int nPlayerIndex = GetPlayerIndex(L);
	if (nPlayerIndex <= 0)
		return 0;
...
{	
	...
	{"SetProtectTime",	LuaSetProtectTime}, //vong tron bat tu, vßng trßn bÊt tö
...
//	{"GetNpcBoss",	LuaGetNpcBoss},
	...
	{"GetWordRank",		LuaGetWordRank},//GetRankInW XÕp h¹ng thÕ giíi
