...
int LuaModifyAttrib(Lua_State* L)//add by phong ki�u v�t ph�m m�u l�n t�ng kim
{
	int nPlayerIndex = GetPlayerIndex(L);
	if (nPlayerIndex <= 0)
	{
...
int LuaST_IsRideHorse(Lua_State* L)//add by phong ki�u check player tr�n ng�a
{
	int nPlayerIndex = GetPlayerIndex(L);
	if (nPlayerIndex < 0)
	{
...
int LuaAddObstacle(Lua_State* L) //#Set V�t C�n
{
	if (Lua_GetTopIndex(L) < 4)
	{
		Lua_PushNumber(L, 1);
...
int LuaDropItem(Lua_State* L)//#lua drop item
{
	...
	//-------------------------------Fix by Fong Ki�u th�m MagicLevel khi drop item
	int nMagicLevel = 1;
	if (nParamNum > 7)
...
int LuaSetNpcActionScript(Lua_State* L)
{
	...
	{
		Npc[nNpcIndex].ExecuteScript(szScript, "OnRevive", nNpcIndex);		// monster #idx c�a npc khi h�i sinh
	}
	//
	return 0;
}
...
int LuaSetNpcHitRecover(Lua_State *L) //Set th�i gian ph�c h�i
{
	int nParamNum = Lua_GetTopIndex(L);
	if (nParamNum < 2)
		return 0;
...
int LuaGetSubWorldPlayerCount(Lua_State* L)
{
	...
	if (nParamNum <= 0)
	{
		nSubWorldIndex = GetSubWorldIndex(L);    //��ͼ 
		if (nSubWorldIndex >= 0)
...
int LuaSetLevel(Lua_State* L)
{
	...
	Npc[Player[nPlayerIndex].m_nIndex].m_Level = 1;	           //�ȼ�Ϊ1��

	Lua_PushNumber(L, Player[nPlayerIndex].m_nSkillPoint);	   //����ԭ����ʣ��ļ��ܵ�
	Lua_PushNumber(L, Player[nPlayerIndex].m_nAttributePoint); //����ԭ�������Ե� 

	Player[nPlayerIndex].m_nSkillPoint = 0;//Player[nPlayerIndex].m_cReBorn.GetReBornKeepJpiont();		//���ܵ����
	Player[nPlayerIndex].m_nAttributePoint = 0;//Player[nPlayerIndex].m_cReBorn.GetReBornKeepQpiont();	    //���Ե����
...
int LuaInitMission(Lua_State* L)
{
	...
	if (pMission)
	{
		...
		if (Lua_GetTopIndex(L) > 4) //add by phong ki�u using t�ng kim
		{
...
int LuaSetDeathRevivalPos(Lua_State* L)//SetTempRev(worldid, x, y) khi ch�t s?quay v?
{
	int nPlayerIndex = GetPlayerIndex(L);
...
{
//	int nSubWorldIndex = GetSubWorldIndex(L); //��ͼ
	int nParamNum = Lua_GetTopIndex(L);
    int ulCount=0;
	int nSubWorldIndex;
	if (nParamNum <=0)
...
int LuaUpdateBattleBox(Lua_State* L)// UpdateBattleBox
{
	...
	if (nParamCount == 3)
	{
		...
		if (nType == 3)
		{
			...
			BYTE m_btRobFlag = pMission->m_MissionPlayer.GetParam(nValue[0], 9);//nh�t c?
			int m_nTPPoint = pMission->m_MissionPlayer.GetParam(nValue[0], 6);//t�ch lu?
			int m_nTPDeath = pMission->m_MissionPlayer.GetParam(nValue[0], 4);//s?l�n ch�t
			int m_nTPKill = pMission->m_MissionPlayer.GetParam(nValue[0], 5);//max li�n tr�m
	...
	if (nParamCount == 4)
	{
		...
		if (nType == 5)
		{
			...
			int m_nKPlayer = pMission->m_MissionPlayer.GetParam(nValue[0], 2);//kill player
			int m_nKNpc = pMission->m_MissionPlayer.GetParam(nValue[0], 3);//kill npc
			int m_nCurKill = pMission->m_MissionPlayer.GetParam(nValue[0], 7);//li�n tr�m hi�n t�i
...
int LuaSetMissionPlayerParam(Lua_State* L)//SetMPParam(missionid, nDidx, vid, v)
{
	...
	if (nSubWorldIndex >= 0)
	{
		...
		if (pMission)
		{
			...
			pMission->SetPlayerParam(nDataIndex, nParamId, nValue); //add by phong ki�u using t�ng kim
		}
	}
	return 0;
}
...
int LuaGetMSLadder(Lua_State* L)//add by phong ki�u using t�ng kim
{
	int nParamCount = Lua_GetTopIndex(L);
	if (nParamCount < 2)
		return 0;
...
int LuaIgnoreState(Lua_State* L) //Lo�i b?c�c miniskill tr�n ng��i, l�nh b�i, chi�n c?
{
	int nPlayerIndex = GetPlayerIndex(L);
	if (nPlayerIndex <= 0)
		return 0;
...
int LuaSetProtectTime(Lua_State* L) //vong tron bat tu, v�ng tr�n b�t t?
{
	int nPlayerIndex = GetPlayerIndex(L);
	if (nPlayerIndex <= 0)
		return 0;
...
int LuaAddSkillState(Lua_State* L)
{
	...
	//
	if (nIfMagic)//kh�ng ��a thu�c t�nh skill v�o
	{
...
TLua_Funcs GameScriptFuns[] =
{
...
#ifdef _SERVER
	{"ST_IsRideHorse", LuaST_IsRideHorse},//add by phong ki�u check player tr�n ng�a
	{"ModifyAttrib",	LuaModifyAttrib}, //add by phong ki�u v�t ph�m m�u l�n t�ng kim
	...
	{"SetProtectTime",	LuaSetProtectTime}, //vong tron bat tu, v�ng tr�n b�t t?
	...
	{"AddObj",			LuaAddObj},
	{"AddObstacle",			LuaAddObstacle}, //#Set V�t C�n
	...
	{"AddMineItem",		LuaAddMineItem	},	//AddMineItem(�¼���ƷID)	
	{"DelItem",			LuaDelItem},
	...
	{ "DelAllMagic",		LuaDelAllMagic },    //ɾ��ȫ������	
	...
	{"SetNpcRevTime",	LuaSetNpcRevTime},	//Set th�i gian h�i sinh
	...
	{"SetNpcPos",		LuaSetNpcPos},			//SetNpcPos(x,y)����ĳ��	
	...
	{"GetFactionValueName",		LuaGetPlayerFactionValueName,	},//GetFaction()�����ҵ������� 
	...
	{"GetWordRank",		LuaGetWordRank},//GetRankInW X�p h�ng th?gi�i
	...
	//	{"GetPramaItemIdx",LuaGetGetPramaItemIdx},
		...
		{"GetMSLadder",LuaGetMSLadder}, //add by phong ki�u using t�ng kim
		...
		{"DelAllNpc",	      LuaDelAllNpcInWro},    //ɾ��NPC
		{"DelAllNpcName",	      LuaDelAllNpcInWroName },
		{"SetPMParam", LuaSetMissionPlayerParam }, //add by phong ki�u using t�ng kim
	...
	{ "TONG_GetApplyCount",	LuaTONG_GetApplyCount },
		...
		{"RANDOM",				LuaRANDOM},  // th�m h�m m�i load npc theo tuy�t th?
		{"RANDOMC",				LuaRANDOMC}, // th�m h�m m�i load npc theo tuy�t th?
		{"SetNpcKind",		LuaSetNpcKind}, // th�m h�m m�i load npc theo tuy�t th?
