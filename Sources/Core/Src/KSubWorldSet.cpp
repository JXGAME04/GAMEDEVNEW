...
static void WorldInDong()
{
	...
	{	// [WORLD 09/09 b] dong trung binh theo NPC / pha
		...
		{	// [TRANGTRI 11/09] NPC + vat the nen nap tu du lieu vung
			...
			extern unsigned g_uTTVe;	// [TRANGTRI 11/09 f]
			...
			{	// [TRANGTRI 11/09 b] in ke ca khi bang 0: phan biet "map khong co du lieu" voi "duong dan sai"
				...
				if (pT)
				{
					fprintf(pT, "[TRANGTRI] 10s: npc them %u hong %u thieu_anh %u xoa %u | obj them %u ve_than %u | khoi ve %u bo qua %u | map %s | ten: %s %s %s\n",
						g_uTTNpcThem, g_uTTNpcHong, g_uTTNpcThieuAnh, g_uTTNpcXoa, g_uTTObjThem, g_uTTVe, g_uKhoiVe, g_uKhoiBoQua,
						g_szTTMap, g_szTTTen[0], g_szTTTen[1], g_szTTTen[2]);
					fclose(pT);
				}
			}
			g_uTTNpcThem = g_uTTNpcHong = g_uTTNpcXoa = g_uTTObjThem = 0;
			g_uTTNpcThieuAnh = 0; g_uKhoiVe = 0; g_uKhoiBoQua = 0; g_uTTVe = 0;	// [KHOI 11/09 c]
