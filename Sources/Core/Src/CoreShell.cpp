...
int	KCoreShell::GetGameData(unsigned int uDataId, unsigned int uParam, int nParam)
{
			...
			case CGOG_ITEM:
				{
					...
					else
					{
						...
						if(money_unit == moneyunit_money && item_place == pos_equiproom)//#ch� hi�n th� gi� b�n b�ng ti�n v�n
						{
							Item[pObj->Obj.uId].GetDesc(pszDescript, true, true, 0);
						}
						else
	...
	case GDI_PLAYER_BASE_INFO:
		if (uParam)
		{
			...
			if (nIndex)
			{
				...
				pInfo->nRankInWorld = Player[CLIENT_PLAYER_INDEX].m_nWorldStat; //x�p h�ng th� gi�i
	...
	case GDI_PLAYER_HOLD_FKCOIN:	//add by phong ki�u s� xu ng��i tr�n player
			...
			if(uParam)
			{
				...
				{	
					...
					else
					{
							...
							m_sInfo->m_bStack,
							m_sInfo->m_bEnChance,
							m_sInfo->m_bPoint //�� t�m s� d�ng �i�m n�y t�i �a 6 d�ng
							);						
					}
					if (nItemIdx > 0)
					{
	...
	case GDI_IS_TONG_MASTER:
		...
		break;
	case GDI_MISSION_SELFDATA: //add by phong ki�u T�ng Kim Battle
		if (uParam)
		{
...
static void DT_Msg(int nPlayerIdx, const char* szMsg)
{
	...
	try
	{
		l_pDataChangedNotifyFunc->ChannelMessageArrival(0, "[D� T�u]", (char*)szMsg, strlen(szMsg), TRUE);
	}
	catch (...) {}
}
...
static int DT_Hold(int nPlayerIdx, const char* szWhy, UINT uCurTime, UINT uMs)
{
	...
	{
		sprintf(szNghi, "%s <color=Gray>(t�m ngh� %u ph�t)", szWhy, uMs / 60000u);
...
static int DT_FindNpcTpl(int nPlayerIdx, int nTpl, int nRadius)
{
	...
	while (nIdx = NpcSet.GetNextIdx(nIdx))
	{
		...
		if (nRadius > 0 && g_GetDistance(nX, nY, dX, dY) > nRadius)
			continue;
		if (strcmp(Npc[nIdx].Name, "D� T�u") == 0)
...
static int TG_XaFuStart()
{
	...
	if (g_nTGXaFuOn)
	{
		TG_XaFuStop("<color=Cyan>[Ch� nam] �� h�y d�n ���ng.");
		return 0;
	}
	...
	{
		DT_Msg(nPlayerIdx, "<color=Yellow>[Ch� nam] Auto D� T�u �ang ch�y - �� auto t� lo vi�c di chuy�n.");
		return 0;
	}
	...
	{
		DT_Msg(nPlayerIdx, "<color=Yellow>[Ch� nam] Kh�ng t�m th�y Xa Phu � th�nh n�y.");
		return 0;
	}
	g_nTGXaFuOn = 1;
	g_nTGXaFuMap = nMap;
	...
	DT_Msg(nPlayerIdx, "<color=Cyan>[Ch� nam] �ang t� ch�y ��n Xa Phu - b�m l�i v�o d�ng nhi�m v� �� h�y.");
	return 1;
}
...
static void TG_XaFuTick()
{
	...
	{
		TG_XaFuStop("<color=Yellow>[Ch� nam] �i qu� l�u - d�ng d�n ���ng.");
		return;
	}
	MapStation::iterator it = g_MoveStation.find(g_nTGXaFuMap);
	if (it == g_MoveStation.end() || it->second.empty())
	{
		TG_XaFuStop("<color=Yellow>[Ch� nam] Kh�ng t�m th�y Xa Phu � th�nh n�y.");
		return;
	}
	...
	if (nIdx)
	{
		...
		if (g_GetDistance(nX, nY, dX, dY) <= 128)
		{
			if (g_nTGXaFuPhase == 1)
			{
				...
				DT_Msg(nPlayerIdx, "<color=Cyan>[Ch� nam] �� g�p Xa Phu - �ang t� ch�n m�c �i map nhi�m v�...");
				return;
			}
			...
			if (g_sDTCap.uDlgSeq != g_uTGXaFuDlgSeen)
			{
				...
				if (nOpt >= 0)
				{
					...
					TG_XaFuStop("<color=Cyan>[Ch� nam] �� ch�n m�c �i t�i map nhi�m v� - ch� chuy�n map.");
					return;
				}
				TG_XaFuStop("<color=Yellow>[Ch� nam] Tho�i kh�ng c� m�c �i map - h�y t� ch�n trong khung tho�i.");
				return;
			}
			if ((g_nTGXaFuTry % 8) == 0)
...
static int TG_SatThuStart()
{
	...
	if (g_nTGSTOn)
	{
		TG_SatThuStop("<color=Cyan>[Ch� nam] �� h�y d�n ���ng t�i boss.");
		return 0;
	}
	if (Player[nPlayerIdx].m_nIndex <= 0)
		return 0;
	...
	if (nBoss < 1 || nBoss > ST3_POS_MAX)
	{
		...
		g_nTGSTOn = 1;
		DT_Msg(nPlayerIdx, "<color=Cyan>[Ch� nam] Ch�a nh�n nhi�m v� - �ang d�ng ph� v� th�nh g�p Nhi�p Th� Tr�n...");
		return 1;
	}
	g_nTGSTBoss = nBoss;
	...
	else
	{
		...
		if (it == g_MoveStation.end() || it->second.empty())
		{
			DT_Msg(nPlayerIdx, "<color=Yellow>[Ch� nam] Kh�ng th�y Xa Phu � map n�y - h�y v� th�nh r�i b�m l�i.");
			return 0;
		}
		g_nTGSTPhase = 1;
	}
	g_nTGSTOn = 1;
	DT_Msg(nPlayerIdx, "<color=Cyan>[Ch� nam] �ang d�n ���ng t�i boss - b�m l�i v�o d�ng nhi�m v� �� h�y.");
	return 1;
}
...
static void TG_SatThuTick()
{
	...
	{
		TG_SatThuStop("<color=Yellow>[Ch� nam] �i qu� l�u - d�ng d�n ���ng.");
		return;
	}
	...
	if (g_nTGSTPhase == 10)
	{
		...
		if ((g_nTGSTTry % 12) == 1)	// ~5s thu dung phu mot lan
		{
			...
			{
				TG_SatThuStop("<color=Yellow>[Ch� nam] Kh�ng c� ph� v� th�nh trong t�i - h�y t� v� th�nh.");
				return;
			}
		}
		return;
	...
	if (g_nTGSTPhase == 11)
	{
		...
		if (nNpc)
		{
			...
			if (g_GetDistance(nX, nY, dX, dY) <= 160)
			{
				...
				TG_SatThuStop("<color=Cyan>[Ch� nam] �� t�i Nhi�p Th� Tr�n - h�y ch�n nhi�m v� s�t th�.");
				return;
			}
	...
	if (g_nTGSTPhase == 1)
	{
		...
		if (it == g_MoveStation.end() || it->second.empty())
		{
			TG_SatThuStop("<color=Yellow>[Ch� nam] Kh�ng th�y Xa Phu � map n�y - h�y v� th�nh r�i b�m l�i.");
			return;
		}
		sStation& s = DT_TramGan(it->second, nPlayerIdx);
		int nIdx = DT_FindNpcName(nPlayerIdx, "xa phu", s.x, s.y, 400);
		if (nIdx)
		{
			...
			if (g_GetDistance(nX, nY, dX, dY) <= 160)
			{
				...
				{
					DT_Msg(nPlayerIdx, "<color=Yellow>[Ch� nam] B�n �� n�y Xa Phu kh�ng ch� t�i - thu� xe ri�ng (t�n ti�n).");
					SendUiCmdScript(6, (char*)"st3_goboss");
					g_nTGSTPhase = 5;
					return;
				}
				DT_Msg(nPlayerIdx, "<color=Cyan>[Ch� nam] �� g�p Xa Phu - �ang ch�n b�n �� nhi�m v�...");
				g_nTGSTPhase = 2;
				return;
			}
			DT_WalkTo(nPlayerIdx, dX, dY, 128, uCur);
	...
	if (g_nTGSTPhase >= 2 && g_nTGSTPhase <= 4)
	{
		...
		{
			TG_SatThuStop("<color=Yellow>[Ch� nam] Tho�i Xa Phu kh�ng c� m�c c�n ch�n - h�y t� ch�n trong khung tho�i.");
		}
		return;
	}
	...
	if (g_nTGSTPhase == 5)
	{
		if (SubWorld[0].m_SubWorldID == g_nTGSTMap)
		{
			g_nTGSTPhase = 6;
			g_nTGSTTry = 0;
			DT_Msg(nPlayerIdx, "<color=Cyan>[Ch� nam] �� l�n b�n �� nhi�m v� - �ang ch�y t�i boss...");
			return;
		}
		if (g_nTGSTTry > 25)
			TG_SatThuStop("<color=Yellow>[Ch� nam] Ch�a l�n ���c b�n �� - d�ng d�n ���ng.");
		return;
	}
	...
	if (DT_WalkTo(nPlayerIdx, g_nTGSTX * 32, g_nTGSTY * 32, 250, uCur))
		TG_SatThuStop("<color=Cyan>[Ch� nam] �� t�i khu v�c boss - c�n th�n!");
}
...
static int DT_CatKhoe(int nPlayerIdx, const autoData* pAp, UINT uCurTime)
{
	 ...
	 || nPlace == pos_exbox2room || nPlace == pos_exbox3room)
	{
		...
		if (nIdx && g_nDTKhoeTry > 0)
			DT_Msg(nPlayerIdx, "<color=Cyan>�� c�t m�n khoe v�o r��ng - l�n sau g�p nhi�m v� khoe s� l�y ra d�ng l�i.");
		g_dwDTKhoeId = 0;
		g_nDTKhoeTry = 0;
		return 0;
	}
	...
	if (++g_nDTKhoeTry > 12)
	{
		DT_Msg(nPlayerIdx, "<color=Yellow>Kh�ng c�t ���c m�n khoe v�o r��ng (r��ng kh�a/��y?) - �� l�i trong t�i.");
		g_dwDTKhoeId = 0;
		g_nDTKhoeTry = 0;
		return 0;
	}
	...
	{
		DT_Msg(nPlayerIdx, "<color=Yellow>R��ng �ang kh�a m� ch�a c� m�t kh�u � tab H�u c�n - kh�ng c�t ���c m�n khoe, �� l�i trong t�i.");
		g_dwDTKhoeId = 0;
		g_nDTKhoeTry = 0;
		return 0;
	}
	...
	if (!nDst)
	{
		DT_Msg(nPlayerIdx, "<color=Yellow>R��ng ��y - kh�ng c�t ���c m�n khoe, �� l�i trong t�i.");
		g_dwDTKhoeId = 0;
		g_nDTKhoeTry = 0;
		return 0;
	}
	if (g_nDTKhoeTry == 1)
		DT_Msg(nPlayerIdx, "<color=Cyan>Khoe xong - c�t m�n khoe v�o r��ng �� l�n sau d�ng l�i...");
...
{
	{ 1,   "Ph��ng T��ng" },
	{ 11,  "Th�nh ��" },	// (r5b) bo " Phu": menu khu cua THP chi ghi "Thanh Do Trung Tam/Dong/..." -> co " Phu" la strstr truot, ket 12s
	{ 162, "��i L�" },
	{ 37,  "Bi�n Kinh" },
	{ 78,  "T��ng D��ng" },
	{ 80,  "D��ng Ch�u" },
...
#define DTM_SAP_THANHTHI "th�nh th� �� �i qua"
...
static int DT_Skip(int nPlayerIdx, const autoData* pAp, UINT uCurTime, const char* szWhy)
{
	...
	if (pAp->nDTSkipMode == 1 && bLoaiBiTat)
	{
		...
		if (strlen(szWhy) < 440)
		{
			sprintf(szHuy, "%s <color=Gray>(h�y nhi�m v� - lo�i n�y �ang t�t)", szWhy);
...
static int DT_BagRelease(int nPlayerIdx, const autoData* pAp, UINT uCurTime, const char* szWhy)
{
	...
	DT_Msg(nPlayerIdx, szWhy);
	ea.uDTStatusTime = 0;
	DT_Msg(nPlayerIdx, "<color=Yellow>T�i ��y - auto t� b�n b�t trang b� tr�ng/xanh ngay t�i ch�...");
...
static int DT_Process(int nPlayerIdx, const autoData* pAp, UINT uCurTime)
{
	...
	if (ea.nDTPhase == DTP_HOLD)
	{
		...
		else
		{
			 ...
			 && !ea.nDTHoldFreeze && ea.uDTPath < uCurTime)
			{
				...
				{
					DT_Msg(nPlayerIdx, "<color=Green>�� c� �� ��t y�u c�u nhi�m v� - quay v� g�p D� T�u tr� ngay!");
					ea.uDTHoldUntil = 0;
					ea.uDTPath = 0;
					ea.nDTStep = DTI_NONE;
	...
	if (ea.nDTPhase == DTP_SELLJUNK)
	{
		...
		{
			DT_Msg(nPlayerIdx, "<color=Cyan>T�i �� c� ch� tr�ng - quay l�i l�m D� T�u.");
			DT_SellResume(ea);
			ea.uDTHoldUntil = 0;
		}
		else if (!pAp->bSellItem)
		{
			...
			ea.uDTHoldUntil = 0;
			return DT_Hold(nPlayerIdx, "<color=Yellow>T�i ��y - h�y d�n t�i, ho�c b�t � 'B�n v�t ph�m' � tab H�u c�n �� auto t� b�n trang b� tr�ng/xanh.", uCurTime, 15 * 60 * 1000);
		}
		else if (uCurTime > ea.uDTHoldUntil)
		{
			...
			return DT_Hold(nPlayerIdx, "<color=Yellow>B�n r�c xong v�n ch�a �� � tr�ng - h�y d�n b�t t�i gi�p auto.", uCurTime, 15 * 60 * 1000);
		}
		else if (ea.uDTNext <= uCurTime)
		{
			...
			return DT_Hold(nPlayerIdx, "<color=Yellow>B�n h�t r�c m� v�n ch�a �� � tr�ng - h�y d�n b�t t�i gi�p auto.", uCurTime, 15 * 60 * 1000);
		}
		return ea.nDTEngaged;
	}
	...
	if (ea.nDTPhase == DTP_FARM && ea.nDTQType == 5 && ea.nDTStatType == 2)
	{
		...
		if (ea.uDTFarmStall && uCurTime - ea.uDTFarmStall > 20u * 60u * 1000u)
			return DT_Hold(nPlayerIdx, "<color=Red>C�y kinh nghi�m qu� l�u kh�ng ti�n tri�n.", uCurTime, 15 * 60 * 1000);
		if (g_dDTExpGain >= (double)ea.nDTReqNum)
		{
			DT_Msg(nPlayerIdx, "<color=Green>�� �� kinh nghi�m - quay v� tr� nhi�m v�.");
	...
	case DTP_IDLE:
	{
		...
		if (i >= g_nDTNpcCount)
		{
			DT_Msg(nPlayerIdx, "<color=Cyan>Kh�ng � th�nh c� D� T�u - d�ng Th� ��a Ph� v� th�nh.");
			ea.nDTPhase = DTP_RETURN;
			ea.nDTStep = DTI_NONE;
			ea.nDTEngaged = 1;
			return 1;
		}
		DT_Msg(nPlayerIdx, "<color=Cyan>�ang ��n ch� NPC D� T�u �� nh�n / tr� nhi�m v�.");
		ea.nDTPhase = DTP_GOTONPC;
		ea.nDTStep = DTI_NONE;
		ea.nDTRetry = 0;
		ea.nDTEngaged = 1;
	...
	case DTP_RETURN:
	{
		...
		for (i = 0; i < g_nDTNpcCount; ++i)
		{
			if (g_DTNpc[i].nMapId == nMap)
			{
				...
				if (pAp->bSellItem && DT_CoRac(nPlayerIdx, pAp))
				{
					...
					DT_Msg(nPlayerIdx, "<color=Cyan>V� th�nh - b�n h�t r�c tr��c r�i l�m ti�p...");
					return 1;
				}
				if (ea.nDTBackXaFu)
				{
					...
					ea.nDTPhase = DTP_GOXAFU;
					ea.nDTRetry = 0;
					DT_Msg(nPlayerIdx, "<color=Cyan>�� v� th�nh - ra Xa Phu �i ti�p t�i map nhi�m v�.");
					return 1;
				}
				ea.nDTPhase = DTP_GOTONPC;
				ea.nDTRetry = 0;
		...
		if (!Npc[Player[nPlayerIdx].m_nIndex].m_FightMode)
			return DT_Hold(nPlayerIdx, "<color=Red>K�t: kh�ng � th�nh m� kh�ng d�ng ���c Th� ��a Ph�.", uCurTime, 10 * 60 * 1000);
		if (++ea.nDTRetry > 8)
			return DT_Hold(nPlayerIdx, "<color=Red>K�t: kh�ng v� ���c th�nh (h�t ph� v�?).", uCurTime, 10 * 60 * 1000);
		if (!DT_UsePortal(nPlayerIdx))
		{
			if (!DT_PortalPull(nPlayerIdx, pAp, uCurTime))
				return DT_Hold(nPlayerIdx, "<color=Yellow>H�t ph� v� trong t�i (ph� � r��ng kh�ng l�y ���c khi ngo�i th�nh) - b� Th� ��a Ph� / ph� v� h�n v�o t�i gi�p auto.", uCurTime, 10 * 60 * 1000);
		}
		ea.uDTNext = uCurTime + 3000;
		return 1;
	}
	...
	case DTP_GOTONPC:
	{
		...
		{
			if (++ea.nDTRetry > 20)
				return DT_Hold(nPlayerIdx, "<color=Red>Kh�ng th�y NPC D� T�u � t�a �� �� ��nh.", uCurTime, 5 * 60 * 1000);
		}
		return 1;
	}
	...
	case DTP_WAITDLG:
	{
		...
		if (cap.uTalkSeq != ea.uDTTalkSeen)
		{
			...
			if (DT_Has(cap.szTalk, DTM_MSG_PUNISH))
				return DT_Hold(nPlayerIdx, "<color=Orange>B� D� T�u ph�t v� h�y nhi�u - ch� h�t ph�t s� l�m ti�p.", uCurTime, 11 * 60 * 1000);
		}
		if (cap.uDlgSeq == ea.uDTDlgSeen)
		{
			// moi vong ~250ms; 16 vong (~4s) khong co phan hoi -> go lai; 60 vong -> treo
			if (++ea.nDTRetry > 60)
				return DT_Hold(nPlayerIdx, "<color=Red>NPC kh�ng tr� l�i h�i tho�i.", uCurTime, 5 * 60 * 1000);
			if ((ea.nDTRetry % 16) == 0)
			{
				ea.nDTPhase = DTP_GOTONPC;
		...
		if (DT_Has(szQ, DTM_MSG_LIMIT))
		{
			...
			DT_Hold(nPlayerIdx, "<color=AYellow>Tuy�t! �� �� 40 nhi�m v� D� T�u h�m nay - ngh�, qua ng�y auto t� ch�y l�i.", uCurTime, 60 * 60 * 1000);
		...
		if (DT_Has(szQ, DTM_MSG_BAGFULL))
		{
			...
			sprintf(szDay, "<color=Yellow>T�i ��y (c�n �t nh�t %d � tr�ng) - auto d�n b�t r�i ch�y ti�p.", nCanOk);
		...
		if (DT_Has(szQ, DTM_MSG_FAILREQ) || DT_Has(szQ, DTM_MSG_FAILSHXT))
		{
			...
			if (nThu5 < 0)
				DT_Msg(nPlayerIdx, "<color=Yellow>�� trong h�p giao ch�a l�y ra ���c (t�i thi�u ch�) - s� t� l�y sau khi d�n t�i.");
			else if (nThu5 > 0)
				DT_Msg(nPlayerIdx, "<color=Cyan>Tr� tr��t - thu l�i �� trong h�p giao v� t�i.");
			...
			{
				DT_Msg(nPlayerIdx, "<color=Cyan>Ch�a �� m�nh - quay l�i map ��nh ti�p...");
				ea.nDTStep = DTI_NONE;
			...
			if (ea.nDTQType == 5 && ea.nDTStatType == 2 && ea.nDTStep == DTI_TURNIN)
			{
				...
				ea.nDTPhase = DTP_FARM;
				ea.nDTEngaged = 0;
				DT_Msg(nPlayerIdx, "<color=Cyan>Ch�a �� kinh nghi�m - c�y ti�p...");
				return 0;
			}
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Kh�ng ��p �ng ���c nhi�m v� n�y.");
		}
		...
		if ((idx = DT_FindAns(apAns, nAns, DTM_OPT_CANCEL2)) >= 0)
		{
			...
			return DT_Hold(nPlayerIdx, "<color=Orange>H�t l��t h�y nhi�m v�.", uCurTime, 15 * 60 * 1000);
		}
		...
		if ((idx = DT_FindAns(apAns, nAns, DTM_OPT_ACCEPT)) >= 0)
		{
			...
			if (ea.nDTQType <= 0)
				return DT_Hold(nPlayerIdx, "<color=Red>Kh�ng hi�u n�i dung nhi�m v� (l�) - h�y b�o l�i l�i n�y.", uCurTime, 10 * 60 * 1000);
			...
			if (!pAp->bDTType[ea.nDTQType - 1])
			{
				...
				if (pAp->bDTLenhBai == 1 && ea.nDTLBTry < 2)
				{
					...
					if (nLB && nLBPos == pos_equiproom)
					{
						...
						if (Player[nPlayerIdx].m_ItemList.CalcFreeItemCellCount(1, 1, room_equipment) < 5)
							return DT_BagRelease(nPlayerIdx, pAp, uCurTime, "<color=Yellow>C�n �t nh�t 5 � tr�ng �� nh�n th��ng l�nh b�i - d�n t�i tr��c.");
						CoreDataChanged(GDCNI_UI_ACT, 1, 0);
						++ea.nDTLBTry;
						Player[nPlayerIdx].m_ItemList.AutoUseItem(6, 1, 4818, nPlayerIdx);
						DT_Msg(nPlayerIdx, "<color=AYellow>D�ng L�nh b�i ho�n th�nh D� T�u - ch� nh�n th��ng...");
						ea.nDTStep = DTI_NONE;
						ea.nDTPhase = DTP_WAITDLG;
					...
					DT_Msg(nPlayerIdx, "<color=Red>L�nh b�i ho�n th�nh kh�ng c� t�c d�ng (server ch�a c�p nh�t script?) - l�m theo c�u h�nh.");
				...
				return DT_Hold(nPlayerIdx, "<color=Yellow>Lo�i nhi�m v� n�y �ang <color=Red>t�t<color> trong tab D� T�u - treo ch�.", uCurTime, 15 * 60 * 1000);
			}
			// den de tra?
			if (ea.nDTStep == DTI_TURNIN)
			{
				if (Player[nPlayerIdx].m_ItemList.CalcFreeItemCellCount(1, 1, room_equipment) < 5)
					return DT_BagRelease(nPlayerIdx, pAp, uCurTime, "<color=Yellow>C�n �t nh�t 5 � tr�ng �� tr� nhi�m v� - h�y d�n t�i.");
				DT_Answer(nPlayerIdx, idx);
			...
			if (ea.nDTStep == DTI_CANCEL)
			{
				...
				return DT_Hold(nPlayerIdx, "<color=Red>Kh�ng th�y n�t h�y trong h�i tho�i.", uCurTime, 10 * 60 * 1000);
			}
		...
		if (ea.nDTUnknown > 6)
			return DT_Hold(nPlayerIdx, "<color=Red>H�i tho�i kh�ng nh�n d�ng ���c (server ��i l�i tho�i?).", uCurTime, 5 * 60 * 1000);
		CoreDataChanged(GDCNI_UI_ACT, 1, 0);
		ea.nDTPhase = DTP_GOTONPC;
		ea.uDTNext = uCurTime + 900;
		return 1;
	...
	case DTP_EXEC:
	{
		...
		{
			if (ea.nDTCandNum <= 0)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Kh�ng kh�p ���c d�ng b�ng d� li�u nhi�m v�.");
			int nPos = 0;
			...
			if (nItem)
			{
				if (nPos != pos_equiproom)
				{
					// keo tu ruong ve tui
					if (!pAp->bDTUseBox)
						return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Yellow>�� c�n tr� �ang n�m trong r��ng - h�y b�t � <color=White>L�y ��/ti�n t� r��ng<color> � tab D� T�u.");
					if (!DT_EnsureUnlock(nPlayerIdx, pAp, uCurTime))
					{
						if (++ea.nDTRetry > 8)
							return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Yellow>Kh�ng m� kh�a ���c r��ng - ki�m tra m�t kh�u r��ng � tab H�u c�n.");
						return 1;
					}
					if (Player[nPlayerIdx].m_ItemList.CalcFreeItemCellCount(
						Item[nItem].GetWidth(), Item[nItem].GetHeight(), room_equipment) < 1)
						return DT_BagRelease(nPlayerIdx, pAp, uCurTime, "<color=Yellow>T�i kh�ng �� ch� �� l�y �� t� r��ng - h�y d�n t�i.");
			...
			if (ea.nDTQType == 1)
			{
				...
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Orange>Nhi�m v� c�n v� kh�/ng�a - h�y b� s�n m�n �� v�o r��ng (xem t�n trong nhi�m v�).");
			}
			if (ea.nDTQType == 2)
			{
				...
				if (pAp->bDTMuaSap == 1)
				{
					...
					DT_Msg(nPlayerIdx, "<color=Cyan>Kh�ng c� �� c�n t�m trong t�i/r��ng - �i xem s�p ng��i b�n �� mua...");
					return 1;
				}
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Orange>Kh�ng c� �� c�n t�m trong t�i/r��ng - n�n t�ch tr� s�n trang s�c.");
			}
			...
			if (ea.nDTQType == 3 && pAp->bDTMuaSap == 1)
			{
				...
				DT_Msg(nPlayerIdx, "<color=Cyan>Kh�ng c� trang b� c�n khoe trong t�i/r��ng - �i xem s�p ng��i b�n �� mua...");
				return 1;
			}
			return DT_Skip(nPlayerIdx, pAp, uCurTime,
				"<color=Orange>Kh�ng c� trang b� c�n t�m (khoe) ph� h�p trong t�i/r��ng.");
		}
		case 4:
		{
			if (ea.nDTMapId <= 0 || ea.nDTReqNum <= 0)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Kh�ng ��c ���c t�n map / s� l��ng c�a nhi�m v� lo�i 4 (��nh qu�i).");
			if (!pAp->bFight)
				return DT_Hold(nPlayerIdx, "<color=Yellow>Nhi�m v� ��nh qu�i: h�y b�t � <color=White>��nh qu�i<color> � tab Chi�n ��u.", uCurTime, 10 * 60 * 1000);
			...
			if (!DT_HasPortalInBag(nPlayerIdx))
			{
				...
				DT_Msg(nPlayerIdx, "<color=Yellow>Ch�a c� ph� v� th�nh trong t�i - n�n b� s�n Th� ��a Ph� ho�c ph� v� h�n v�o t�i/r��ng.");
			}
			ea.uDTFarmStall = uCurTime;
			if (nMap == ea.nDTMapId)
			{
		...
		case 5:
		{
			if (ea.nDTStatType == 2)
			{
				...
				g_dDTExpGain = 0;
				ea.uDTFarmStall = uCurTime;
				DT_Msg(nPlayerIdx, "<color=Cyan>Nhi�m v� kinh nghi�m: th� cho auto th��ng c�y, �� s� t� v� tr�.");
				ea.nDTPhase = DTP_FARM;
				ea.nDTEngaged = 0;
				return 0;
			}
		...
		return DT_Hold(nPlayerIdx, "<color=Red>Lo�i nhi�m v� l� - auto ch�a h� tr�.", uCurTime, 10 * 60 * 1000);
	}
	...
	case DTP_GOSHOP:
	{
		...
		if (it == g_ShopStation.end() || it->second.empty())
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Th�nh n�y ch�a c� t�a �� ti�m t�p h�a.");
	...
	case DTP_SHOPTALK:
	{
		...
		if (cap.uDlgSeq != ea.uDTDlgSeen)
		{
			...
			if (nPick >= nAns)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Ti�m kh�ng c� m�c giao d�ch ph� h�p.");
		...
		std::map<int, StationVector>::iterator it = g_ShopStation.find(nMap);
		if (it == g_ShopStation.end() || it->second.empty())
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>M�t t�a �� ti�m t�p h�a.");
		sStation& s = DT_TramGan(it->second, nPlayerIdx);
		...
		if (nIdx)
		{
			...
			if (g_GetDistance(nX, nY, dX, dY) <= 128)
			{
				...
				ea.uDTNext = uCurTime + 800;
				if (++ea.nDTRetry > 10)
					return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Kh�ng m� ���c c�a s� ti�m t�p h�a.");
				return 1;
			}
			DT_WalkTo(nPlayerIdx, dX, dY, 96, uCurTime);
			return 1;
		...
		if (++ea.nDTRetry > 10)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Kh�ng th�y ch� ti�m t�p h�a.");
		return 1;
	}
	...
	case DTP_BUY:
	{
		...
		for (int b = 0; b < BuySell.GetWidth(); ++b)
		{
			...
			&& pItem->GetSeries() == r.nFive)
			{
				if (Player[nPlayerIdx].m_ItemList.GetEquipmentMoney() < pItem->GetPrice())
				{
					...
					return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Yellow>Kh�ng �� ti�n mua �� nhi�m v�.");
				}
				int x, y;
				if (!Player[nPlayerIdx].m_ItemList.CheckCanPlaceInEquipment(
				...
				{
					CoreDataChanged(GDCNI_UI_ACT, 3, 0);
					return DT_BagRelease(nPlayerIdx, pAp, uCurTime, "<color=Yellow>T�i ��y, kh�ng mua ���c ��.");
				}
		...
		CoreDataChanged(GDCNI_UI_ACT, 3, 0);
		if (ea.nDTShopTry >= 4)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Ti�m kh�ng b�n m�n c�n mua.");
		ea.nDTPhase = DTP_SHOPTALK;
		ea.uDTNext = uCurTime + 700;
		return 1;
	}
	...
	case DTP_GOXAFU:
	{
		...
		if (it == g_MoveStation.end() || it->second.empty())
		{
			...
			{
				DT_Msg(nPlayerIdx, "<color=Cyan>Kh�ng ph�i map nhi�m v� - d�ng Th� ��a Ph� v� th�nh �i l�i.");
				ea.nDTBackXaFu = 1;
				ea.nDTPhase = DTP_RETURN;
				ea.nDTRetry = 0;
				return 1;
			}
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Th�nh n�y ch�a c� t�a �� Xa Phu.");
		}
		sStation& s = DT_TramGan(it->second, nPlayerIdx);
		int nIdx = DT_FindNpcName(nPlayerIdx, "xa phu", s.x, s.y, 400);
		if (nIdx)
		{
			...
			if (g_GetDistance(nX, nY, dX, dY) <= 128)
			{
				...
				{
					g_nDTXaFuCam = 0;
					return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Xa Phu kh�ng tr� l�i tho�i sau nhi�u l�n g� - b� nhi�m v� n�y.");
				}
			...
			{
				g_nDTXaFuDi = 0;
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Kh�ng t�i ���c ch� Xa Phu (k�t ���ng?) - b� nhi�m v� n�y.");
			}
			DT_WalkTo(nPlayerIdx, dX, dY, 96, uCurTime);
			return 1;
		}
		...
		{
			if (++ea.nDTRetry > 15)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Kh�ng th�y Xa Phu.");
		}
		return 1;
	}
	...
	case DTP_XAFUTALK_DONE:
	{
		...
		{
			DT_Msg(nPlayerIdx, "<color=Green>�� t�i map nhi�m v� - b�t ��u ��nh qu�i nh�t cu�n.");
			ea.nDTMapId = nMap;
			ea.nDTPhase = DTP_FARM;
			ea.nDTEngaged = 2;
			ea.nDTRetry = 0;
		...
		if (++ea.nDTRetry > 12)
		{
			...
			if (++ea.nDTXaFuTry > 5)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Xa Phu kh�ng ch� �i map nhi�m v� (ki�m tra l�i nhi�m v�).");
			ea.nDTPhase = DTP_GOXAFU;
			ea.nDTRetry = 0;
		}
		ea.uDTNext = uCurTime + 700;
	...
	case DTP_FARM:
	{
		...
		if (nMap != ea.nDTMapId)
		{
			...
			if (bThanhF && (bCoRac || bDayTP))
			{
				if (pAp->bReturn && uCurTime - g_uDTYieldT > DT_YIELD_GAP)
					return DT_Yield(nPlayerIdx, uCurTime, "<color=Cyan>Ph� v� th�nh c� r�c - �� H�u c�n b�n r�c/d�n t�i xong s� ch�y D� T�u ti�p.");
				if (pAp->bSellItem && bCoRac)
				{
					...
					DT_Msg(nPlayerIdx, "<color=Cyan>Ph� v� th�nh c� r�c (ch�a b�t 'V� th�nh') - b�n r�c theo b� l�c r�i ra Xa Phu �i ti�p.");
					return 1;
				}
				...
				if (bDayTP)
					return DT_Hold(nPlayerIdx, "<color=Yellow>H�nh trang ��y theo m�c � tab C� b�n m� kh�ng d�n ���c (t�t 'V� th�nh'/'B�n r�c', ho�c v�a d�n xong v�n ��y) - h�y d�n t�i gi�p auto.", uCurTime, 15 * 60 * 1000);
			}
			ea.nDTPhase = DTP_EXEC;
			ea.nDTEngaged = 1;
			return 1;
		...
		{
			ea.nDTHaveBase = 1;
			DT_Msg(nPlayerIdx, "<color=Cyan>C� th� �� �� m�nh (tin nh�n tr�i) - v� th� tr� nhi�m v�...");
			ea.nDTStep = DTI_TURNIN;
			ea.nDTPhase = DTP_RETURN;
			ea.nDTRetry = 0;
			ea.nDTEngaged = 1;
		...
		if (ea.uDTFarmStall && uCurTime - ea.uDTFarmStall > 20u * 60u * 1000u)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>��nh qu�i qu� l�u kh�ng ti�n tri�n.");
		if (ea.nDTProg >= ea.nDTReqNum && ea.nDTReqNum > 0)
		{
			DT_Msg(nPlayerIdx, "<color=Green>�� nh�t �� s� cu�n - quay v� tr� nhi�m v�.");
			ea.nDTStep = DTI_TURNIN;
			ea.nDTPhase = DTP_RETURN;
			ea.nDTRetry = 0;
			ea.nDTEngaged = 1;
	...
	case DTP_REWARD:
	{
		...
		if (cap.uFinSeq != ea.uDTFinSeen)
		{
			...
			if (nThu < 6)
			{
				...
				if (nThu == 0)
					DT_Msg(nPlayerIdx, "<color=AYellow>R��ng th��ng �� m� - auto b�m ch�n ph�n th��ng...");
			...
			DT_Msg(nPlayerIdx, "<color=Red>Kh�ng ch�n ���c r��ng th��ng (�� th� �� 6 n�t) - b� qua.");
			ea.nDTRwTry = 0;
			ea.nDTStep = DTI_NONE;
		...
		DT_Msg(nPlayerIdx, "<color=AYellow>�� nh�n th��ng xong - �i nh�n nhi�m v� k� ti�p!");
		ea.nDTRwTry = 0;
		ea.nDTStep = DTI_NONE;
		// (r5d) bi cuop pha giua luc di cho -> quay lai pha dang do (GIU nhiem vu)
		if (ea.nDTPhaseBack == DTP_MUASAP || ea.nDTPhaseBack == DTP_CITYHOP)
	...
	case DTP_YIELD:
	{
		...
		{
			DT_Msg(nPlayerIdx, nXongHC ? "<color=Cyan>H�u c�n d�n xong - D� T�u l�m ti�p."
			                           : "<color=Cyan>Kh�ng ch� H�u c�n n�a - D� T�u l�m ti�p.");
			ea.uDTHoldUntil = 0;
	...
	case DTP_MUASAP:
	{
		...
		if (uCurTime > ea.uDTHoldUntil)
		{
			...
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>�i ch� qu� l�u m� ch�a mua ���c m�n c�n t�m.");
		}
		// mua xong chua? (item ve tui la xong viec di cho)
		{
			...
			{
				DT_SapDong(nPlayerIdx);
				DT_Msg(nPlayerIdx, "<color=Green>�� mua ���c �� c�n t�m � s�p - quay v� tr� nhi�m v�!");
				ea.nDTItemIdx = nCo;
				ea.nDTStep = DTI_TURNIN;
				ea.nDTPhase = DTP_GOTONPC;
				ea.nDTRetry = 0;
		...
		if (g_dwDTSapCur)
		{
			if (g_cSellItem.m_nId > 0)
			{
				...
				{
					DT_Msg(nPlayerIdx, "<color=Yellow>S�p c� m�n c�n nh�ng gi� v��t tr�n cho ph�p - b� qua s�p n�y.");
					nBuy = 0;
				}
				if (nBuy && Player[nPlayerIdx].m_ItemList.GetEquipmentMoney() < nGia)
				{
					...
					DT_Msg(nPlayerIdx, "<color=Yellow>Th�y m�n c�n � s�p m� kh�ng �� ti�n mua.");
					nBuy = 0;
				}
				if (nBuy)
				{
					 ...
					 || sVi.nPlace != pos_equiproom)
					{
						...
						return DT_BagRelease(nPlayerIdx, pAp, uCurTime,
							"<color=Yellow>T�i kh�ng c�n � tr�ng �� nh�n �� mua � s�p.");
					}
		...
		// het thanh nay -> danh dau roi qua thanh ke
		{
			...
			if (nKe < 0)
			{
				DT_SapDong(nPlayerIdx);
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Orange>�� xem s�p kh�p 10 th�nh/th�n m� kh�ng c� m�n c�n t�m.");
			}
			...
			g_uDTSapHopT = 0;
			{
				...
				char szTKSap[220];
				sprintf(szTKSap, "<color=Cyan>Th�nh n�y: xem %d s�p (%d kh�ng ph�n h�i) - kh�ng c� m�n c�n, nh� Xa Phu qua th�nh k�...",
	...
	case DTP_CITYHOP:
	{
		...
		if (uCurTime > g_uDTSapHopT)
		{
			...
			if (nKe2 < 0)
			{
				DT_SapDong(nPlayerIdx);
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Orange>Kh�ng nh� Xa Phu qua th�nh kh�c ���c (thi�u ti�n?).");
			}
			ea.nDTShopTry = nKe2;
			return 1;
		}
		...
		if (itXa == g_MoveStation.end() || itXa->second.empty())
		{
			 ...
			 || (!DT_UsePortal(nPlayerIdx) && !DT_PortalPull(nPlayerIdx, pAp, uCurTime)))
			{
				DT_SapDong(nPlayerIdx);
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Red>L�c map kh�ng c� Xa Phu m� kh�ng d�ng ���c ph� v� th�nh.");
			}
			ea.uDTNext = uCurTime + 3000;
			return 1;
		}
		...
		if (cap.uDlgSeq != ea.uDTDlgSeen)
		{
			...
			if ((idx2 = DT_FindAns(apAns2, nAns2, "thu�t th�n h�nh")) >= 0)
			{
				DT_Answer(nPlayerIdx, idx2);
				ea.uDTNext = uCurTime + 900;
				return 1;
			...
			if (g_uDTThpT > 1)
			{
				...
				idx2 = DT_FindAns(apAns2, nAns2, bThon5 ? "Th�n trang" : "Th�nh th�");
				if (idx2 >= 0)
				{
					DT_Answer(nPlayerIdx, idx2);
					ea.uDTNext = uCurTime + 900;
		...
		if (g_uDTThpT == 0)
		{
			if (Player[nPlayerIdx].m_ItemList.AutoUseItem(6, 1, 1271, nPlayerIdx))
			{
				...
				DT_Msg(nPlayerIdx, "<color=Cyan>D�ng Th�n H�nh Ph� d�ch chuy�n t�i th�nh k� ti�p...");
				ea.uDTNext = uCurTime + 1200;
				return 1;
			}
		...
		else if (g_uDTThpT > 1)
		{
			...
			DT_Msg(nPlayerIdx, "<color=Yellow>Th�n H�nh Ph� kh�ng ��a �i ���c - ch�y t�i Xa Phu v�y.");
		}
	...
	case DTP_USEPD:
	{
		ea.nDTEngaged = 1;
		if (ea.nDTUsedPD > 40)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>D�ng qu� nhi�u Ph�c Duy�n L� m� v�n ch�a �� �i�m.");
		...
		if (ea.nDTUsedPD >= 2 && nFY <= ea.nDTFuYuanPrev)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Ph�c Duy�n L� kh�ng c� t�c d�ng (server thi�u script?).");
			...
			for (i = 0; i < 3; ++i)
			{
				...
				if (nItem && nPos != pos_equiproom)
				{
					if (!DT_EnsureUnlock(nPlayerIdx, pAp, uCurTime))
					{
						...
						if (++ea.nDTRetry > 12)
						{
							DT_Msg(nPlayerIdx, "<color=Yellow>Kh�ng m� kh�a ���c r��ng (ki�m tra m�t kh�u � tab H�u c�n).");
		...
		else
		{
			...
			for (i = 0; i < 3; ++i)
			{
				...
				{
					DT_Msg(nPlayerIdx, "<color=Yellow>R��ng c� Ph�c Duy�n L� nh�ng �ang kh�a - nh�p m�t kh�u r��ng � tab H�u c�n.");
					break;
				}
			}
		}
		...
		return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>H�t Ph�c Duy�n L� m� v�n ch�a �� �i�m.");
	}
...
static void TK_Msg(int nPlayerIdx, const char* szMsg)
{
	...
	try
	{
		l_pDataChangedNotifyFunc->ChannelMessageArrival(0, "[T�ng Kim]", (char*)szMsg, strlen(szMsg), TRUE);
	}
	catch (...) {}
}
...
static int TK_Process(int nPlayerIdx, const autoData* pAp, UINT uCurTime)
{
	...
	if (ea.nTKPhase == TKP_OFF || ea.nTKPhase == TKP_DONE)
	{
		...
		if (Npc[nSelf].m_Level < TK_LEVEL_MIN)
		{
			...
			TK_Msg(nPlayerIdx, "<color=Yellow>Ch�a �� c�p 80 - b� qua khung gi� T�ng Kim n�y.");
			return 0;
		}
		...
		{
			TK_Msg(nPlayerIdx, "<color=Yellow>Kh�ng c� T�ng Kim Chi�u th� l�n Th�n H�nh Ph� trong t�i - th� l�i sau 1 ph�t.");
			ea.nTKPhase = TKP_OFF;
			ea.uTKNext = uCurTime + 60000;
			return 0;
		}
		if (Player[nPlayerIdx].GetFactionNo() < 0)
			TK_Msg(nPlayerIdx, "<color=Yellow>H�nh nh� ch�a v�o m�n ph�i - m�y ch� c� th� t� ch�i b�o danh.");
		ea.nTKKey = nKhoa;
		...
		TK_Msg(nPlayerIdx, "<color=Cyan>T�i gi� T�ng Kim - t�m d�ng vi�c �ang l�m �� �i b�o danh.");
	}
	...
	{
		TK_Msg(nPlayerIdx, "<color=Yellow>M�t b��c c�a auto T�ng Kim k�t qu� 3 ph�t - b� khung gi� n�y.");
	...
	case TKP_GO:
	{
		...
		if (ea.nTKStep == 0)
		{
			...
			{
				TK_Msg(nPlayerIdx, "<color=Cyan>H�t T�ng Kim Chi�u th� - �i b�o danh b�ng Th�n H�nh Ph�.");
				ea.nTKStep = 10;
				ea.uTKNext = uCurTime + 900;
				return 1;
			}
			TK_Msg(nPlayerIdx, "<color=Yellow>Kh�ng d�ng ���c c� Chi�u th� l�n Th�n H�nh Ph� - th� l�i sau 1 ph�t.");
			ea.nTKPhase = TKP_OFF;
			ea.nTKHold = 0;
			ea.uTKNext = uCurTime + 60000;
			return 0;
	...
	case TKP_SIGNUP:
	{
		...
		if (nMap == TK_MAP_TRAN)		// bao danh thanh cong = bi chuyen sang map tran
		{
			...
			TK_Msg(nPlayerIdx, "<color=Green>B�o danh xong - �ang � h�u doanh.");
			TK_Pha(nPlayerIdx, TKP_CAMP, uCurTime);
			return 1;
		}
		if (nMap != TK_MAP_BAODANH)
		...
		if (cap.uDlgSeq != ea.uTKDlgSeen)
		{
			...
			if (pAp->nTKPhe == 2 && ea.nTKStep == 0)
			{
				...
				if (nT >= 0 && nK >= 0)
				{
					...
					{
						ea.nTKPhe = nMuon;
						TK_Msg(nPlayerIdx, "<color=Cyan>Qu�n s� l�ch - qua b�o danh phe �t ng��i h�n.");
			...
			if (ea.nTKThe)
			{
				TK_Msg(nPlayerIdx, "<color=Cyan>Tr�n �� k�t th�c - d�n t�i r�i nh� Xa Phu v� th�nh �� ch�n.");
			...
			TK_Msg(nPlayerIdx, "<color=Yellow>Tr�n ch�a m� - ch� t�i gi� r�i b�o danh l�i.");
			return 1;
		}
		if (ea.nTKStep == 1)
		{
			...
			if (++ea.nTKTry >= 3 && pAp->nTKPhe == 2)
			{
				...
				TK_Msg(nPlayerIdx, "<color=Yellow>Phe n�y kh�ng nh�n th�m - qua �i�m b�o danh phe kia.");
				TK_Pha(nPlayerIdx, TKP_SWAP, uCurTime);
				return 1;
			}
			if (ea.nTKTry >= 10)
			{
				TK_Msg(nPlayerIdx, "<color=Yellow>B�o danh b� t� ch�i nhi�u l�n - b� khung gi� n�y.");
				TK_Pha(nPlayerIdx, TKP_END, uCurTime);
				return 1;
			}
		...
		}
		{
			...
			if (nR == 1)
				...
				TK_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y NPC b�o danh quanh ��y.");
		}
		return 1;
	}
	...
	case TKP_SWAP:
	{
		...
		}
		{
			...
			{
				TK_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y Xa Phu - t� �i b� qua NPC phe kia.");
				TK_Pha(nPlayerIdx, TKP_SIGNUP, uCurTime);
			}
		}
		return 1;
	...
	case TKP_CAMP:
	{
		...
		if (uCurTime - ea.uTKPhaseT < TK_CHO_NPC)
		{
			...
			if (uCurTime - ea.uTKPhaseT < 500u)
				TK_Msg(nPlayerIdx, "<color=Cyan>V� doanh tr�i - ��ng ch� NPC hi�n ra r�i m�i �i mua thu�c.");
			ea.uTKNext = uCurTime + 400;
			return 1;
		}
		if (pAp->nTKMuaMau == 1)
		...
		if (cap.uDlgSeq != ea.uTKDlgSeen)
		{
			...
			if (nOpt >= 0)
			{
				...
				else
				{
					ea.nTKMua = 1;
					ea.uTKNext = uCurTime + 1500;
					TK_Msg(nPlayerIdx, "<color=Cyan>�� mua nhanh thu�c � Qu�n Y.");
				}
				return 1;
			}
			ea.uTKNext = uCurTime + 1200;
		...
		}
		{
			...
			else if (nR < 0)
			{
				...
				TK_Msg(nPlayerIdx, "<color=Yellow>Ch� 8 gi�y kh�ng th�y Qu�n Y hi�n ra - ra tr�n lu�n.");
				ea.nTKMua = 1;
				TK_Pha(nPlayerIdx, TKP_TRAP, uCurTime);
			}
		}
	...
	case TKP_TRAP:
	{
		...
		{
			TK_Msg(nPlayerIdx, "<color=Green>�� v�o chi�n tr��ng - ��nh theo c�u h�nh tab PK.");
			TK_Pha(nPlayerIdx, TKP_FIGHT, uCurTime);
			return 2;
		}
		if (TK_AnThuoc(nPlayerIdx, pAp, uCurTime))
	...
	case TKP_FIGHT:
	{
		...
		if (nMap == TK_MAP_BAODANH)
		{
			...
			if (nTrongGio)
			{
				TK_Msg(nPlayerIdx, "<color=Yellow>B� ��a v� �i�m b�o danh - b�o danh l�i �� v�o ti�p.");
				TK_Pha(nPlayerIdx, TKP_SIGNUP, uCurTime);
				return 1;
			}
			TK_Msg(nPlayerIdx, "<color=Cyan>H�t tr�n T�ng Kim - �ang r�i �i�m b�o danh.");
			TK_Pha(nPlayerIdx, TKP_END, uCurTime);
			return 1;
		}
		if (nMap != TK_MAP_TRAN)
	...
	case TKP_END:
	{
		...
		if (nMap != TK_MAP_BAODANH)
		{
			TK_Msg(nPlayerIdx, "<color=Cyan>�� r�i �i�m b�o danh - �i v� th�nh �� ch�n.");
		...
		if (cap.uDlgSeq != ea.uTKDlgSeen)
		{
			...
			{
				DT_Answer(nPlayerIdx, nOpt);
				TK_Msg(nPlayerIdx, "<color=Cyan>Xong T�ng Kim - nh� Xa Phu ��a v� th�nh �� ch�n.");
			...
			{
				ea.nTKStep = 4;
				TK_Msg(nPlayerIdx, "<color=Yellow>Xa Phu kh�ng li�t k� th�nh �� ch�n (ch�a t�ng �i qua?) - r�i map r�i t� �i ti�p.");
				CoreDataChanged(GDCNI_UI_ACT, 1, 0);
				ea.uTKNext = uCurTime + 1200;
				return 1;
			}
		...
		}
		{
			...
			else if (nR < 0)
			{
				...
				TK_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y Xa Phu �� r�i �i�m b�o danh.");
				ea.nTKPhase = TKP_DONE;
				ea.nTKHold = 0;
				return 0;
			}
	...
	case TKP_VETHANH:
	{
		...
		}
		{
			...
			{
				ea.uLDHopT = 0;
				TK_Msg(nPlayerIdx, "<color=Cyan>�� v� t�i th�nh �� ch�n - tr� m�y l�i cho auto c�.");
				ea.nTKPhase = TKP_DONE;
				ea.nTKHold = 0;
				return 0;
			}
			...
			{
				ea.uLDHopT = 0;
				TK_Msg(nPlayerIdx, "<color=Yellow>Kh�ng �i t�i ���c th�nh �� ch�n (h�t Th�n H�nh Ph� / kh�ng th�y Xa Phu) - tr� m�y t�i ch�.");
				ea.nTKPhase = TKP_DONE;
				ea.nTKHold = 0;
				return 0;
			}
...
static void LD_Msg(int nPlayerIdx, const char* szMsg)
{
	...
	try
	{
		l_pDataChangedNotifyFunc->ChannelMessageArrival(0, "[Li�n ��u]", (char*)szMsg, strlen(szMsg), TRUE);
	}
	catch (...) {}
}
...
static int LD_DiThanh(int nPlayerIdx, const autoData* pAp, int nDestMap, UINT uCurTime)
{
	...
	if (cap.uDlgSeq != ea.uLDDlgSeen)
	{
		...
		if ((nOpt = DT_FindAns(apAns, nAns, "thu�t th�n h�nh")) >= 0)
		{
			DT_Answer(nPlayerIdx, nOpt);
			ea.uLDNext = uCurTime + 900;
			return 1;
		}
		if (ea.uLDThpT > 1 && (nOpt = DT_FindAns(apAns, nAns, "Th�nh th�")) >= 0)
		{
	...
	if (ea.uLDThpT == 0)
	{
		if (Player[nPlayerIdx].m_ItemList.AutoUseItem(6, 1, 1271, nPlayerIdx))
		{
			...
			LD_Msg(nPlayerIdx, "<color=Cyan>D�ng Th�n H�nh Ph� �i t�i th�nh �� ch�n...");
			ea.uLDNext = uCurTime + 1200;
			return 1;
		}
...
static int LD_Process(int nPlayerIdx, const autoData* pAp, UINT uCurTime)
{
	...
	if (ea.nLDPhase == LDP_OFF || ea.nLDPhase == LDP_DONE)
	{
		...
		if (!nHang)
		{
			...
			LD_Msg(nPlayerIdx, "<color=Yellow>Ch�a �� c�p 80 - b� qua khung gi� Li�n ��u n�y.");
			return 0;
		}
		ea.nLDKey = nKhoa;
		ea.nLDNeed = 0;
		...
		LD_Msg(nPlayerIdx, "<color=Cyan>T�i gi� Li�n ��u - t�m d�ng vi�c �ang l�m �� �i b�o danh.");
	}
	...
	{
		LD_Msg(nPlayerIdx, "<color=Yellow>M�t b��c c�a auto Li�n ��u k�t qu� 3 ph�t - b� khung gi� n�y.");
		if (nLoai == 1)
	...
	case LDP_GO:
	{
		...
		}
		{
			...
			if (pAp->nLDThanhBD >= 0 && pAp->nLDThanhBD < LD_VE_COUNT)
			{
				...
				{
					LD_Msg(nPlayerIdx, "<color=Yellow>Th�nh b�o danh �� ch�n kh�ng ��ng h�ng c�a b�n - d�ng th�nh �ang ��ng.");
					nDest = -1;
				}
			}
			if (nDest > 0 && nMap != nDest)
			{
				...
				if (nDi < 0)
				{
					LD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng �i t�i ���c th�nh b�o danh - b� khung gi� Li�n ��u n�y.");
					ea.nLDPhase = LDP_DONE;
					ea.nLDHold = 0;
					return 0;
				}
			...
			{
				LD_Msg(nPlayerIdx, "<color=Yellow>�ang kh�ng � th�nh c� S� gi� ��ng h�ng - ch�n th�nh � � B�o danh � r�i b�t l�i.");
				ea.nLDPhase = LDP_DONE;
				ea.nLDHold = 0;
				return 0;
			}
			...
			{
				LD_Msg(nPlayerIdx, "<color=Cyan>�ang t�m b�n di�n quanh S� gi� �� t� ��i Song ��u.");
			...
			{
				LD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y S� gi� li�n ��u quanh ��y.");
	...
	case LDP_TEAM:
	{
		...
		if (nLoai == 1)
		{
			LD_Msg(nPlayerIdx, "<color=Green>�� v�o h�i tr��ng li�n ��u.");
			LD_Pha(nPlayerIdx, LDP_STASH, uCurTime);
			return 1;
		}
		if (nLoai)
		...
		if (cap.uInpSeq != ea.uLDInpSeen)
		{
			...
			LD_Msg(nPlayerIdx, "<color=Cyan>�ang l�p chi�n ��i li�n ��u.");
			return 1;
		}
		if (cap.uDlgSeq != ea.uLDDlgSeen)
		{
			...
			if (DT_Has(szBuf, LDM_SAY_DONGCUA))
			{
				...
				LD_Msg(nPlayerIdx, "<color=Yellow>H�i tr��ng ch�a m� - ch� t�i gi� r�i v�o l�i.");
			...
			if (nCanThem)
			{
				...
				if (nOpt >= 0)
				{
					...
					ea.nLDPtDone = 1;
					ea.nLDStep = 0;
					LD_Msg(nPlayerIdx, "<color=Green>�� ��ng k� b�n di�n v�o chi�n ��i.");
					ea.uLDNext = uCurTime + 1500;
					return 1;
				}
			...
			if (++ea.nLDTry >= 8)
			{
				LD_Msg(nPlayerIdx, "<color=Yellow>Chi�n ��i ch�a h�p l� - h�y t� v�o S� gi� x� l� r�i b�t l�i.");
				ea.nLDPhase = LDP_DONE;
				ea.nLDHold = 0;
				return 0;
			}
	...
	case LDP_STASH:
	{
		...
		}
		{
			...
			else if (nR == 1)
			{
				...
				LD_Msg(nPlayerIdx, "<color=Yellow>C�n �� c�m m� r��ng kh�ng c�t ���c - h�y d�n tay r�i b�t l�i.");
		}
		LD_Pha(nPlayerIdx, LDP_SIGNUP, uCurTime);
		return 1;
	}
	...
	case LDP_SIGNUP:
	{
		...
		if (nLoai == 2)
		{
			LD_Msg(nPlayerIdx, "<color=Green>�� v�o khu chu�n b� - ch� gh�p c�p.");
			LD_Pha(nPlayerIdx, LDP_PREP, uCurTime);
			return 1;
		}
		if (nLoai == 3)
		...
		if (cap.uDlgSeq != ea.uLDDlgSeen)
		{
			...
			if (DT_Has(szBuf, LDM_SAY_DOCAM))
			{
				...
				{
					++ea.nLDTry;
					LD_Msg(nPlayerIdx, "<color=Yellow>B� ch�n v� �� c�m - �ang c�t v�o r��ng r�i b�o danh l�i.");
					LD_Pha(nPlayerIdx, LDP_STASH, uCurTime);
					return 1;
				}
				LD_Msg(nPlayerIdx, "<color=Yellow>Trong ng��i c�n d��c ph�m c�m - h�y c�t v�o r��ng r�i b�t l�i.");
		...
		}
		{
			...
			{
				LD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y Quan vi�n h�i tr��ng quanh ��y.");
				ea.uLDNext = uCurTime + 5000;
			}
		}
		return 1;
	...
	case LDP_PREP:
	{
		...
		if (nLoai == 3)
		{
			LD_Msg(nPlayerIdx, "<color=Green>�� v�o ��u tr��ng - ��nh theo c�u h�nh tab PK.");
			LD_Pha(nPlayerIdx, LDP_FIGHT, uCurTime);
			return 2;
		}
		if (nLoai == 1)
	...
	case LDP_FIGHT:
	{
		...
		if (nLoai == 1)
		{
			LD_Msg(nPlayerIdx, "<color=Cyan>H�t tr�n - v� h�i tr��ng ch� l��t sau.");
			LD_Pha(nPlayerIdx, LDP_WAIT, uCurTime);
			return 1;
		}
		if (nLoai == 2)
	...
	case LDP_AWARD:
	{
		...
		if (cap.uDlgSeq != ea.uLDDlgSeen)
		{
			...
			{
				LD_Msg(nPlayerIdx, "<color=Cyan>Xong vi�c nh�n th��ng li�n ��u cu�i m�a.");
				ea.nLDPhase = LDP_DONE;
				ea.nLDHold = 0;
				return 0;
			}
	...
	case LDP_LEAVE:
	{
		...
		if (nLoai != 1)
		{
			LD_Msg(nPlayerIdx, "<color=Cyan>Xong Li�n ��u - tr� m�y l�i cho auto c�.");
			ea.nLDPhase = LDP_DONE;
			ea.nLDHold = 0;
			return 0;
		}
		{
			...
			{
				LD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y Xa phu �� r�i h�i tr��ng.");
				ea.nLDPhase = LDP_DONE;
				ea.nLDHold = 0;
				return 0;
			}
	...
	case LDP_PARTY:
	{
		...
		if (LD_TeamSize(nPlayerIdx) >= 2)
		{
			LD_Msg(nPlayerIdx, "<color=Green>�� �� t� ��i 2 ng��i - v�o S� gi� ��ng k� chi�n ��i.");
			ea.nLDPtDone = 0;
			LD_Pha(nPlayerIdx, LDP_TEAM, uCurTime);
			return 1;
		}
		...
		{
			LD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng g�p b�n di�n quanh S� gi� - �i b�o danh m�t m�nh.");
			ea.nLDPtDone = 1;
			LD_Pha(nPlayerIdx, LDP_TEAM, uCurTime);
			return 1;
		}
...
static void HD_Msg(int nPlayerIdx, const char* szMsg)
{
	...
	try
	{
		l_pDataChangedNotifyFunc->ChannelMessageArrival(0, "[Ho�t ��ng]", (char*)szMsg, strlen(szMsg), TRUE);
	}
	catch (...) {}
}
...
static int HD_Process(int nPlayerIdx, const autoData* pAp, UINT uCurTime)
{
	...
	if (ea.nHDPhase == HDP_OFF || ea.nHDPhase == HDP_DONE)
	{
		 ...
		 && HD_TrongCua(pAp, pAp->nHDBCGio, pAp->nHDBCPhut, HD_BC_CUA))
		{
			...
			HD_Msg(nPlayerIdx, "<color=Cyan>T�i gi� Bang Chi�n - �i Ba L�ng Huy�n b�o danh.");
			ea.nHDHold = 1;
			return 1;
		}
		...
		if (pAp->bHDTinSu)
		{
			 ...
			 && (nDoDang || HD_TSLuotNay(nPlayerIdx, nNgay) < HD_TSMucTieu(pAp)))
			{
				HD_Pha(nPlayerIdx, HDP_TS_GO, uCurTime);
				{
					...
					sprintf(szTB, "%s (gi� m�y ch� %02d:%02d)", nDoDang
						? "<color=Cyan>L�m ti�p nhi�m v� T�n S� �ang d�"
						: "<color=Cyan>T�i gi� T�n S� - �i D�ch Quan nh�n nhi�m v�",
						nPh / 60, nPh % 60);
		 ...
		 && HD_TrongCua(pAp, pAp->nHDBNGio, pAp->nHDBNPhut, 600))
		{
			...
			HD_Msg(nPlayerIdx, "<color=Cyan>T�i gi� B�ch Nh�n L�i ��i - �i L�m An c�y kinh nghi�m.");
			ea.nHDHold = 1;
			return 1;
		}
		ea.nHDPhase = HDP_OFF;
	...
	{
		HD_Msg(nPlayerIdx, "<color=Yellow>M�t b��c c�a auto Ho�t ��ng k�t qu� 4 ph�t - b� l��t n�y.");
	...
	case HDP_BN_GO:
	{
		...
		if (nMap == HD_BN_MAP)
		{
			HD_Msg(nPlayerIdx, "<color=Green>�� v�o L�i ��i Ho�ng Th�nh T�.");
			ea.uHDVaoT = uCurTime;
			ea.uHDIdleT = uCurTime;
			ea.uHDLuotT = uCurTime;
			HD_Pha(nPlayerIdx, HDP_BN_IN, uCurTime);
		...
		if (nMap != HD_BN_MAP_NPC)
		{
			...
			if (nDi < 0)
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng �i t�i ���c L�m An - b� l��t B�ch Nh�n h�m nay.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
		...
		if (cap.uDlgSeq != ea.uHDDlgSeen)
		{
			...
			{
				LD_Huy(nPlayerIdx, nAns);
				HD_Msg(nPlayerIdx, "<color=Yellow>Ch�a t�i gi� m� L�i ��i - ch� 5 ph�t th� l�i.");
				ea.uHDPhaseT = uCurTime;
			...
			{
				LD_Huy(nPlayerIdx, nAns);
				HD_Msg(nPlayerIdx, "<color=Yellow>Ch�a �� c�p tham gia B�ch Nh�n - b�.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
		...
		}
		{
			...
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y NPC l�i v�o B�ch Nh�n quanh ��y.");
	...
	case HDP_BN_IN:
	{
		...
		if (nMap != HD_BN_MAP)
		{
			...
			{
				HD_Msg(nPlayerIdx, "<color=Cyan>B� m�i ra v� ��ng y�n 90 ph�t - quay l�i L�i ��i �n ti�p.");
				HD_Pha(nPlayerIdx, HDP_BN_GO, uCurTime);
				return 1;
			}
			HD_Msg(nPlayerIdx, "<color=Cyan>�� r�i L�i ��i - tr� m�y l�i cho auto c�.");
			ea.nHDPhase = HDP_DONE;
			ea.nHDHold = 0;
			return 0;
		}
		...
		// dieu kien ra
		{
			...
			if (nVe)
			{
				HD_Msg(nPlayerIdx, (nVe == 2)
					? "<color=Cyan>�� �n �� 50 l��t kinh nghi�m h�m nay - r�i L�i ��i."
					: "<color=Cyan>H�t gi� c�y B�ch Nh�n - r�i L�i ��i.");
		...
		if (pAp->bHDBNBuff && HD_CoTin(nPlayerIdx, HDM_MSG_COTHU))
		{
			...
			ea.nHDCothu = 0;
			HD_Msg(nPlayerIdx, "<color=Cyan>C� Th� xu�t hi�n - �i t�m �n buff x2.");
	...
	case HDP_BN_BUFF:
	{
		...
		if (cap.uDlgSeq != ea.uHDDlgSeen)
		{
			...
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>C� Th� h�t su�t ��t n�y - nghe loa ��t sau th� ti�p.");
				HD_Pha(nPlayerIdx, HDP_BN_IN, uCurTime);
				return 1;
			}
			if (DT_Has(szBuf, HDM_MSG_NGHERO))
			{
				...
				ea.uHDIdleT = uCurTime;
				HD_Msg(nPlayerIdx, "<color=Green>�� nh�n buff C� Th� x2 kinh nghi�m.");
				HD_Pha(nPlayerIdx, HDP_BN_IN, uCurTime);
				return 1;
			}
	...
	case HDP_BN_OUT:
	{
		...
		if (nMap != HD_BN_MAP)
		{
			HD_Msg(nPlayerIdx, "<color=Cyan>Xong B�ch Nh�n - tr� m�y l�i cho auto c�.");
			ea.nHDPhase = HDP_DONE;
			ea.nHDHold = 0;
			return 0;
		}
	...
	case HDP_BC_GO:
	{
		...
		if (nBCSign)
		{
			HD_Msg(nPlayerIdx, "<color=Green>�� v�o khu b�o danh Bang Chi�n.");
			HD_Pha(nPlayerIdx, HDP_BC_SIGN, uCurTime);
			return 1;
		}
		if (nBCFight)
		...
		{
			HD_Msg(nPlayerIdx, "<color=Yellow>H�t c�a s� Bang Chi�n h�m nay - tr� m�y.");
			ea.nHDPhase = HDP_DONE;
			ea.nHDHold = 0;
			return 0;
		}
		if (nMap != HD_BC_MAP_NPC)
		{
			...
			if (nDi < 0)
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng �i t�i ���c Ba L�ng Huy�n - b� Bang Chi�n h�m nay.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
		...
		if (cap.uDlgSeq != ea.uHDDlgSeen)
		{
			...
			{
				LD_Huy(nPlayerIdx, nAns);
				HD_Msg(nPlayerIdx, "<color=Yellow>NPC t� ch�i (ch�a c� bang / bang kh�ng li�n minh / thi�u c�p / �eo �� c�m) - tr� m�y.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
			...
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>10 ph�t kh�ng th�y m�c tham gia - h�m nay kh�ng c� tr�n Bang Chi�n, tr� m�y.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
			HD_Msg(nPlayerIdx, "<color=Yellow>Bang Chi�n ch�a t�i pha thi ��u - 60 gi�y th� l�i.");
			ea.uHDPhaseT = uCurTime;
		...
		}
		{
			...
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y NPC V� L�m Truy�n Nh�n quanh ��y.");
				ea.uHDNext = uCurTime + 5000;
			}
		}
		return 1;
	...
	case HDP_BC_SIGN:
	{
		...
		if (nBCFight)
		{
			HD_Msg(nPlayerIdx, "<color=Green>�� v�o chi�n tr��ng Bang Chi�n - ��nh theo c�u h�nh tab PK.");
			HD_Pha(nPlayerIdx, HDP_BC_FIGHT, uCurTime);
			return 2;
		}
		if (!nBCSign)
		...
		if (uCurTime - ea.uHDPhaseT > 40u * 60000u)
		{
			...
			HD_Msg(nPlayerIdx, "<color=Yellow>Ch� m�i kh�ng v�o ���c tr�n Bang Chi�n - ra Xa phu v� th�nh.");
		...
		if (cap.uDlgSeq != ea.uHDDlgSeen)
		{
			...
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>Tr�n ch�a m� c�a - 30 gi�y ��p l�i trap.");
				ea.uHDNext = uCurTime + 30000u;
				return 1;
			}
			if (DT_Has(szBuf, HDM_SAY_HETMANG))
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>�� ch�t �� tr�n m�ng Bang Chi�n - ra Xa phu v� th�nh.");
				HD_Pha(nPlayerIdx, HDP_BC_OUT, uCurTime);
				return 1;
			}
			if (DT_Has(szBuf, HDM_SAY_SAIKHU))
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>Nh�m khu chu�n b� c�a li�n minh kh�c - ra Xa phu v� th�nh.");
				HD_Pha(nPlayerIdx, HDP_BC_OUT, uCurTime);
				return 1;
			}
			if (DT_Has(szBuf, HDM_SAY_BCTHIEUCAP))
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>Ch�a �� c�p v�o tr�n Bang Chi�n - ra Xa phu v� th�nh.");
				HD_Pha(nPlayerIdx, HDP_BC_OUT, uCurTime);
				return 1;
			}
			if (DT_Has(szBuf, HDM_SAY_DAYNGUOI))
			{
				...
				HD_Msg(nPlayerIdx, "<color=Yellow>Phe �� �� 150 ng��i - 90 gi�y th� l�i.");
		...
		}
		{
			...
			if (g_GetDistance(nX, nY, TK_O((int)g_HDBCRej.x), TK_O((int)g_HDBCRej.y)) < 64)
			{
				if (++ea.nHDTry >= 8)
				{
					HD_Msg(nPlayerIdx, "<color=Yellow>Trap t� ch�i nhi�u l�n - ra Xa phu v� th�nh.");
					HD_Pha(nPlayerIdx, HDP_BC_OUT, uCurTime);
					return 1;
				}
	...
	case HDP_BC_FIGHT:
	{
		...
		if (nBCSign)
		{
			...
			if (nTran > 0 && nChet >= nTran)
				HD_Msg(nPlayerIdx, "<color=Yellow>�� ch�t �� tr�n m�ng - ngh� tr�n n�y.");
			else
				HD_Msg(nPlayerIdx, "<color=Cyan>R�i chi�n tr��ng Bang Chi�n.");
		...
		if (!nBCFight)
		{
			HD_Msg(nPlayerIdx, "<color=Cyan>Xong Bang Chi�n - tr� m�y l�i cho auto c�.");
			ea.nHDPhase = HDP_DONE;
			ea.nHDHold = 0;
			return 0;
		}
	...
	case HDP_TS_GO:
	{
		...
		if (nSt == 10 || nSt == 20 || nSt == 21)
		{
			...
			if (nDiVe < 0)
			{
				...
				HD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng v� ���c th�nh �� �i ti�p T�n S� - th� l�i sau 10 ph�t.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
		...
		if (nMap != nDest)
		{
			...
			if (nDi < 0)
			{
				ea.uLDHopT = 0;
				ea.uHDTSNghi = uCurTime + 600000;
				HD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng �i t�i ���c th�nh D�ch Quan - th� l�i sau 10 ph�t.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
		...
		if (cap.uDlgSeq != ea.uHDDlgSeen)
		{
			 ...
			 || DT_Has(szBuf, HDM_SAY_TSQUAMET))
			{
				...
				HD_Msg(nPlayerIdx, "<color=Yellow>H�t l��t T�n S� h�m nay - tr� m�y.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
			if (DT_Has(szBuf, HDM_SAY_TSTHIEUCAP))
			{
				...
				HD_Msg(nPlayerIdx, "<color=Yellow>Ch�a �� c�p 90 l�m T�n S� - tr� m�y.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
			if (DT_Has(szBuf, HDM_SAY_TSTHIEUO))
			{
				...
				HD_Msg(nPlayerIdx, "<color=Yellow>H�nh trang thi�u 5 � tr�ng �� nh�n th��ng T�n S� - h�y d�n t�i, 10 ph�t n�a auto th� l�i.");
				ea.nHDPhase = HDP_DONE;
				ea.nHDHold = 0;
				return 0;
			}
			...
			if (nSt == 25 || nSt == 30)
			{
				...
				{
					DT_Answer(nPlayerIdx, nOpt);
					HD_Msg(nPlayerIdx, "<color=Green>�� giao nhi�m v� T�n S� - nh�n th��ng.");
		...
		if (nSt == 0 && HD_TSLuotNay(nPlayerIdx, nNgay) >= HD_TSMucTieu(pAp))
		{
			...
			HD_Msg(nPlayerIdx, "<color=Cyan>�� �� l��t T�n S� h�m nay - tr� m�y l�i cho auto c�.");
			ea.nHDPhase = HDP_DONE;
			ea.nHDHold = 0;
			return 0;
		}
		{
			...
			{
				HD_Msg(nPlayerIdx, "<color=Yellow>Kh�ng th�y D�ch Quan quanh ��y.");
				ea.uHDNext = uCurTime + 5000;
			}
		}
		return 1;
	...
	case HDP_TS_XAPHU:
	{
		...
		if (nMap == HD_TS_MAP_AI)
		{
			HD_Msg(nPlayerIdx, "<color=Green>�� v�o �i Thi�n B�o Kh�.");
			HD_Pha(nPlayerIdx, HDP_TS_AI, uCurTime);
			ea.uHDVaoT = uCurTime;
			return 1;
		}
	...
	case HDP_TS_AI:
	{
		...
		{
			ea.nHDKeyTS = nNgay;
			HD_Msg(nPlayerIdx, "<color=Yellow>K�t trong �i Thi�n B�o Kh� qu� l�u - auto T�n S� ngh� h�t h�m nay, c�n x� l� tay.");
			ea.nHDPhase = HDP_DONE;
			ea.nHDHold = 0;
			return 0;
		}
		...
		if (nSt == 20)
		{
			...
			if (cap.uDlgSeq != ea.uHDDlgSeen)
			{
				...
				if (nOpt >= 0)
				{
					DT_Answer(nPlayerIdx, nOpt);
					if (nXong)
						HD_Msg(nPlayerIdx, "<color=Green>�� m� �� 5 B�o R��ng - ra c�ng mang th� �i giao.");
					else
					{
						ea.nHDKeyTS = nNgay;	// bo cuoc - nghi het hom nay
						HD_Msg(nPlayerIdx, "<color=Yellow>Qu� 25 ph�t ch�a xong 5 r��ng - b� l��t, ngh� h�t h�m nay.");
					}
					ea.uHDNext = uCurTime + 2000;
					return 1;
				}
		...
		if (cap.uDlgSeq != ea.uHDDlgSeen)
		{
			...
			if (DT_Has(szBuf, HDM_SAY_TSDOITRUONG))
			{
				...
				HD_Msg(nPlayerIdx, "<color=Yellow>�ang � t� ��i m� kh�ng ph�i ��i tr��ng - kh�ng b�t ��u T�n S� ���c. R�i t� ��i r�i b�t l�i auto.");
				ea.uHDNext = uCurTime + 1500;
				return 1;
			}
			int nOpt = -1;
			...
			if (nOpt >= 0)
			{
				...
				if ((nSt == 10 || nSt == 21) && !nBoCuoc)
				{
					...
					HD_Msg(nPlayerIdx, "<color=Green>B�t ��u m� 5 B�o R��ng theo ��ng th� t�.");
				}
				ea.uHDNext = uCurTime + 2000;
				if (nSt == 0)
				{
...
#define STM_OPT_NV90		"Nhi�m v� c�p 90"
#define STM_OPT_GHEP		"H�p th�nh s�t th� gi�n"
#define STM_OPT_HUY			"H�y nhi�m v�"
...
#define STM_OPT_DONG		"��ng"
#define STM_SAY_HETLUOT		"khinh k� b�i tr�n"
#define STM_SAY_SAICAP		"c�p c�a ng��i kh�ng ph� h�p"
#define STM_SAY_DANGCO		"v�n c�n s�ng"
#define STM_SAY_GHEPOK		"h�p th�nh m�t"
#define STM_SAY_GHEPSAI		"�� kh�ng ��ng"
#define STM_SAY_GHEPNHIEU	"�� qu� nhi�u"
...
// Nguyen van: "thu ph�c ���c t�n s�t th�" (cat tu chinh script may chu, khong go tay).
#define STM_TIN_XONG		"thu ph�c ���c t�n s�t th�"
#define STM_SAY_GHEPIT		"�� qu� �t"
...
static void ST_Msg(int nPlayerIdx, const char* szMsg)
{
	...
	try
	{
		l_pDataChangedNotifyFunc->ChannelMessageArrival(0, "[S�t Th�]", (char*)szMsg, strlen(szMsg), TRUE);
	}
	catch (...) {}
}
...
static int ST_Process(int nPlayerIdx, const autoData* pAp, UINT uCurTime)
{
	...
	{
		ea.uNpcID = 0;
		ST_Msg(nPlayerIdx, "<color=Green>M�y ch� b�o ho�n th�nh nhi�m v� s�t th� - nh�t �� r�i d�ng ph� v� th�nh nh�n l��t k�.");
	...
	if (ea.nSTPhase == STP_OFF || ea.nSTPhase == STP_DONE)
	{
		...
		{
			ea.nSTNgay = nNgay;
			ST_Msg(nPlayerIdx, "<color=Cyan>�� �� l��t s�n boss s�t th� h�m nay - ngh� t�i ng�y mai.");
			return 0;
		}
		...
		if (nTask >= 1 && nTask <= ST3_POS_MAX)
		{
			...
			ST_Msg(nPlayerIdx, "<color=Cyan>�ang c� nhi�m v� s�t th� d� - �i gi�t boss tr��c.");
		}
		else
		{
			ea.nSTMucBoss = 0;
			ST_Pha(nPlayerIdx, STP_NPC, uCurTime);
			ST_Msg(nPlayerIdx, "<color=Cyan>B�t ��u v�ng s�n boss s�t th� - v� th�nh g�p Nhi�p Th� Tr�n.");
		}
		ea.nSTHold = 1;
		return 1;
	}
	...
	if (ea.nSTPhase == STP_TOIBOSS && uCurTime - ea.uSTPhaseT > ST_HANDIBO)
		return ST_Nghi(nPlayerIdx, "<color=Yellow>�i b� t�i ch� boss qu� 8 ph�t (k�t ��a h�nh?) - ngh� 5 ph�t r�i l�m l�i.", uCurTime, 300000u);
	if (ea.nSTPhase == STP_DANH && uCurTime - ea.uSTPhaseT > ST_HANDANH)
	{
		ST_Msg(nPlayerIdx, "<color=Yellow>��nh boss qu� 15 ph�t m� nhi�m v� v�n ch�a xong - h�y nhi�m v� �� ��i con kh�c.");
	...
	if (ea.nSTPhase != STP_DANH && ea.nSTPhase != STP_NHAT && ea.nSTPhase != STP_TOIBOSS
	 && uCurTime - ea.uSTPhaseT > ST_HANPHA)
		return ST_Nghi(nPlayerIdx, "<color=Yellow>M�t b��c c�a auto S�t Th� k�t qu� 4 ph�t - ngh� 5 ph�t r�i l�m l�i.", uCurTime, 300000u);
	...
	if (uCurTime - ea.uSTVongT > ST_HANVONG)
		return ST_Nghi(nPlayerIdx, "<color=Yellow>M�t v�ng s�n boss ch�y qu� 45 ph�t - ngh� 5 ph�t r�i l�m l�i.", uCurTime, 300000u);
	...
	case STP_NPC:
	{
		 ...
		 && ea.uSTBanT <= uCurTime)
		{
			...
			{
				if (ea.nSTBan == 0)
					ST_Msg(nPlayerIdx, "<color=Cyan>V� th�nh - b�n r�c theo b� l�c tab H�u c�n tr��c khi nh�n nhi�m v� k�.");
			...
			else
			{
				...
				{
					char szBan[160];
					sprintf(szBan, "<color=Cyan>�� b�n %d m�n r�c - �i nh�n nhi�m v� k�.", ea.nSTBan);
		...
		{
			if ((ea.nSTTry % 25) == 1)
				ST_Msg(nPlayerIdx, "<color=Yellow>T�i ��y - nh��ng cho H�u c�n b�n r�c theo b� l�c r�i m�i nh�n nhi�m v�.");
			ea.nSTHold = 4;
			ea.uSTNext = uCurTime + 400;
			return 4;
		}
		...
		if (nR < 0)
			return ST_Nghi(nPlayerIdx, "<color=Yellow>Kh�ng v� ���c th�nh c� Nhi�p Th� Tr�n (h�t ph� v�?) - ngh� 5 ph�t.", uCurTime, 300000u);
		if (nR == 0)
			return 1;
		...
		if (pAp->bSTGhep && ea.nSTGhepTry < 3 && ST_DemLenh(nPlayerIdx, &nCap) >= 5)
		{
			...
			ST_Msg(nPlayerIdx, "<color=Yellow>T�i ��y - ch�a gh�p ���c S�t Th� Gi�n, c� �i nh�n nhi�m v�.");
		}
	...
	case STP_MENU:
	{
		...
		if (nTask >= 1 && nTask <= ST3_POS_MAX)
		{
			...
			{
				char szTB[256];
				sprintf(szTB, "<color=Green>�� nh�n nhi�m v� gi�t <color=Yellow>%s <color>- ra Xa Phu �i b�n �� nhi�m v�.", s_szST3BossTen[nTask]);
		...
		ea.uSTDlgSeen = cap.uDlgSeq;
		{
			...
			if (DT_Has(szBuf, STM_SAY_HETLUOT))
			{
				...
				ST_Msg(nPlayerIdx, "<color=Cyan>H�m nay �� �� l��t s�n s�t th� - ngh� t�i ng�y mai.");
				ea.nSTPhase = STP_DONE;
				ea.nSTHold = 0;
				return 0;
			}
			if (DT_Has(szBuf, STM_SAY_SAICAP))
			{
				ea.nSTNgay = nNgay;
				ST_BamMuc(nPlayerIdx, apAns, nAns, STM_OPT_DONG, uCurTime, 600);
				ST_Msg(nPlayerIdx, "<color=Yellow>C�p nh�n v�t ch�a �� 90 - auto S�t Th� ngh� h�t h�m nay.");
				ea.nSTPhase = STP_DONE;
				ea.nSTHold = 0;
				return 0;
			}
			...
			if (ST_BamMuc(nPlayerIdx, apAns, nAns, STM_OPT_NV90, uCurTime, 1500))
				return 1;
			if (++ea.nSTStep > 6)
				return ST_Nghi(nPlayerIdx, "<color=Yellow>Tho�i Nhi�p Th� Tr�n kh�ng c� m�c c�n ch�n - ngh� 5 ph�t r�i th� l�i.", uCurTime, 300000u);
	...
	case STP_GHEP:
	{
		...
		if (ea.nSTStep >= 100)
		{
			if (cap.uDlgSeq != ea.uSTDlgSeen)
			{
				...
				if (DT_Has(szBuf, STM_SAY_GHEPOK))
				{
					...
					ST_Msg(nPlayerIdx, "<color=Green>�� h�p th�nh 1 S�t Th� Gi�n t� 5 S�t Th� l�nh.");
				}
					  ...
					  || DT_Has(szBuf, STM_SAY_GHEPIT))
				{
					...
					ST_Msg(nPlayerIdx, "<color=Yellow>Gh�p S�t Th� Gi�n kh�ng th�nh (sai s� l��ng ho�c kh�c c�p) - b� qua.");
				}
				else
				{
					ea.nSTGhepTry = 3;
		...
		if (cap.uDlgSeq != ea.uSTDlgSeen)
		{
			...
			{
				ea.nSTGhepTry = 3;
				ST_Msg(nPlayerIdx, "<color=Yellow>Kh�ng m� ���c giao di�n h�p th�nh - b� gh�p v�ng n�y.");
				ST_Pha(nPlayerIdx, STP_NPC, uCurTime);
				return 1;
			}
			CoreDataChanged(GDCNI_UI_ACT, 1, 0);
	...
	case STP_XAPHU:
	{
		...
		if (ea.nSTStep == 0)
		{
			...
			{
				SendUiCmdScript(6, (char*)"st3_goboss");
				ST_Msg(nPlayerIdx, "<color=Yellow>Xa Phu kh�ng ch� t�i b�n �� n�y - thu� xe ri�ng (t�n ti�n).");
		...
		ea.uSTDlgSeen = cap.uDlgSeq;
		{
			...
			if (ST_BamMuc(nPlayerIdx, apAns, nAns, szMark, uCurTime, 1200))
			{
				...
				{
					ST_Pha(nPlayerIdx, STP_LENMAP, uCurTime);
					ST_Msg(nPlayerIdx, "<color=Cyan>�� ch�n b�n �� nhi�m v� - �ang l�n ���ng.");
				}
				else
					++ea.nSTStep;
				return 1;
	...
	case STP_LENMAP:
	{
		...
		{
			ST_Pha(nPlayerIdx, STP_TOIBOSS, uCurTime);
			ST_Msg(nPlayerIdx, "<color=Cyan>�� l�n b�n �� nhi�m v� - �ang ch�y t�i ch� boss...");
			return 1;
		}
		if (uCurTime - ea.uSTPhaseT > 30000u)
	...
	case STP_TOIBOSS:
	{
		...
		if (DT_WalkTo(nPlayerIdx, nBX, nBY, 200, uCurTime))
		{
			...
			ea.uSTThayT = uCurTime;
			ST_Msg(nPlayerIdx, "<color=Cyan>�� t�i ch� boss s�t th� - b�t ��u ��nh.");
		}
		return 1;
	}
	...
	case STP_DANH:
	{
		...
		{
			ea.uNpcID = 0;
			ST_Msg(nPlayerIdx, "<color=Green>�� h� boss s�t th� - nh�t �� theo c�i ��t tab Nh�t ��.");
			ST_Pha(nPlayerIdx, STP_NHAT, uCurTime);
			ea.nSTHold = 3;
		...
		if (!pAp->bSTChoHS)
		{
			ST_Msg(nPlayerIdx, "<color=Yellow>Boss ch�a h�i sinh - h�y nhi�m v� �� ��i con kh�c.");
			ST_Pha(nPlayerIdx, STP_HUY, uCurTime);
			return 1;
		}
		if (uCurTime - ea.uSTThayT > ST_CHO_HOISINH)
		{
			ST_Msg(nPlayerIdx, "<color=Yellow>Ch� 10 ph�t kh�ng th�y boss h�i sinh - ��i nhi�m v� kh�c.");
			ST_Pha(nPlayerIdx, STP_HUY, uCurTime);
			return 1;
		}
		if ((ea.nSTTry % 25) == 1)
			ST_Msg(nPlayerIdx, "<color=Gray>Boss ch�a h�i sinh - ��ng ch� t�i ch� (nh�p kho�ng 7,5 ph�t).");
	...
	case STP_NHAT:
	{
		...
		{
			char szLuot[200];
			sprintf(szLuot, "<color=Cyan>Xong nhi�m v� s�t th� th� %d/%d h�m nay - �i nh�n nhi�m v� k�.",
				(int)Player[nPlayerIdx].m_cTask.GetSaveVal(ST_TSK_DEM), ST_MucLuot(pAp));
		...
		{
			char szTB[200];
			sprintf(szTB, "<color=Cyan>Xong 1 l��t s�n boss - ngh� %d ph�t r�i l�m l��t k�.", pAp->nSTNghi);
			ST_Msg(nPlayerIdx, szTB);
		...
		ST_Msg(nPlayerIdx, "<color=Cyan>Nh�t xong - d�ng ph� v� th�nh nh�n nhi�m v� k�.");
		ea.uSTVongT = uCurTime;
		ST_Pha(nPlayerIdx, STP_NPC, uCurTime);
		ea.nSTHold = 1;
		return 1;
...
void WA_HoatDong(int nPlayerIdx, char* szOut, int nMax)
{
	...
	if (ea.nSTPhase)
	{
		switch (ea.nSTPhase)
		{
		case STP_NPC:       sz = "S�t Th�: t�i Nhi�p Th� Tr�n"; break;
		case STP_MENU:      sz = "S�t Th�: nh�n nhi�m v�"; break;
		case STP_GHEP:      sz = "S�t Th�: gh�p S�t Th� Gi�n"; break;
		case STP_HUY:       sz = "S�t Th�: h�y nhi�m v�"; break;
		case STP_XAPHU:     sz = "S�t Th�: nh� Xa Phu �i b�n ��"; break;
		case STP_LENMAP:    sz = "S�t Th�: ch� l�n b�n ��"; break;
		case STP_TOIBOSS:   sz = "S�t Th�: ch�y t�i ch� boss"; break;
		case STP_DANH:      sz = "S�t Th�: �ang ��nh boss"; break;
		case STP_NHAT:      sz = "S�t Th�: nh�t ��"; break;
		default: break;
		}
	}
	else if (ea.nLDPhase)
	{
		switch (ea.nLDPhase)
		{
		case LDP_GO:     sz = "Li�n ��u: �i t�i S� gi�"; break;
		case LDP_TEAM:   sz = "Li�n ��u: l�m vi�c v�i S� gi�"; break;
		case LDP_PARTY:  sz = "Li�n ��u: t�m b�n di�n t� ��i"; break;
		case LDP_NAME:   sz = "Li�n ��u: ��t t�n chi�n ��i"; break;
		case LDP_STASH:  sz = "Li�n ��u: c�t �� c�m"; break;
		case LDP_SIGNUP: sz = "Li�n ��u: b�o danh"; break;
		case LDP_PREP:   sz = "Li�n ��u: ch� gh�p c�p"; break;
		case LDP_FIGHT:  sz = "Li�n ��u: �ang thi ��u"; break;
		case LDP_WAIT:   sz = "Li�n ��u: ch� l��t sau"; break;
		case LDP_AWARD:  sz = "Li�n ��u: nh�n th��ng cu�i m�a"; break;
		case LDP_LEAVE:  sz = "Li�n ��u: r�i h�i tr��ng"; break;
		default: break;
		}
	}
	else if (ea.nHDPhase)
	{
		switch (ea.nHDPhase)
		{
		case HDP_BN_GO:   sz = "B�ch Nh�n: �i t�i L�i ��i"; break;
		case HDP_BN_IN:   sz = "B�ch Nh�n: �ang c�y kinh nghi�m"; break;
		case HDP_BN_BUFF: sz = "B�ch Nh�n: �i �n buff C� Th�"; break;
		case HDP_BN_OUT:  sz = "B�ch Nh�n: r�i L�i ��i"; break;
		case HDP_BC_GO:   sz = "Bang Chi�n: �i b�o danh"; break;
		case HDP_BC_SIGN: sz = "Bang Chi�n: ch� v�o tr�n"; break;
		case HDP_BC_FIGHT: sz = "Bang Chi�n: �ang ��nh tr�n"; break;
		case HDP_BC_OUT:  sz = "Bang Chi�n: v� th�nh"; break;
		case HDP_TS_GO:   sz = "T�n S�: l�m vi�c v�i D�ch Quan"; break;
		case HDP_TS_XAPHU: sz = "T�n S�: nh� Xa Phu v�o �i"; break;
		case HDP_TS_AI:   sz = "T�n S�: m� B�o R��ng trong �i"; break;
		default: break;
		}
	}
	else if (ea.nTKPhase)
	{
		...
		{
		case TKP_GO:
		case TKP_BOOK:   sz = "T�ng Kim: d�ng Chi�u th�"; break;
		case TKP_SIGNUP: sz = "T�ng Kim: b�o danh"; break;
		case TKP_SWAP:   sz = "T�ng Kim: ��i �i�m b�o danh"; break;
		case TKP_CAMP:   sz = "T�ng Kim: mua thu�c h�u doanh"; break;
		case TKP_TRAP:   sz = "T�ng Kim: ra tr�n"; break;
		case TKP_FIGHT:  sz = "T�ng Kim: �ang ��nh tr�n"; break;
		case TKP_END:    sz = "T�ng Kim: r�i �i�m b�o danh"; break;
		case TKP_VETHANH: sz = "T�ng Kim: v� th�nh �� ch�n"; break;
		default: break;
		}
	}
	else if (ea.nDTEngaged || ea.nDTPhase)
	{
		switch (ea.nDTPhase)
		{
		case DTP_FARM:   sz = "D� T�u: ��nh qu�i nhi�m v�"; break;
		case DTP_MUASAP:
		case DTP_CITYHOP: sz = "D� T�u: �i mua �� � s�p"; break;
		case DTP_HOLD:   sz = "D� T�u: t�m ngh�"; break;
		default:         sz = "D� T�u: l�m vi�c trong th�nh"; break;
		}
	}
	if (sz)
...
int	KCoreShell::OperationRequest(unsigned int uOper, unsigned int uParam, int nParam)
{
	...
	case GDI_SET_TRADE_ITEM:
		{
			...
			if (nIdx > 0 && nIdx < MAX_ITEM)
			{
				...
				{
					KSystemMessage	sMsg;
					sprintf(sMsg.szMessage, "B�n h�y m� kh�a b�o v� m�i c� th� th�c hi�n ���c !");
					sMsg.eType = SMT_NORMAL;
					sMsg.byConfirmType = SMCT_NONE;
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
	...
	case GDI_PLAYER_TRADE:
		{
			...
			{
				KSystemMessage	sMsg;
				sprintf(sMsg.szMessage, "�ang c��i ng�a kh�ng th� b�y b�n!");
				sMsg.eType = SMT_NORMAL;
				sMsg.byConfirmType = SMCT_NONE;
				sMsg.byPriority = 0;
				sMsg.byParamSize = 0;
			...
			{
				KSystemMessage	sMsg;
				sprintf(sMsg.szMessage, "Kh�ng th� b�y b�n � khu v�c chi�n ��u!");
				sMsg.eType = SMT_NORMAL;
				sMsg.byConfirmType = SMCT_NONE;
				sMsg.byPriority = 0;
				sMsg.byParamSize = 0;
			...
			if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_BaiTan == 0)
			{
				...
				else
				{
					...
					KSystemMessage	sMsg;
					sprintf(sMsg.szMessage, "Ch�a ��nh gi� v�t ph�m !!! ");
					sMsg.eType = SMT_NORMAL;
					sMsg.byConfirmType = SMCT_NONE;
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
	...
	case GOI_GAMBLE_DESIRE_ITEM:
		...
		break;
	case GOI_TRADE_WILLING: //giao dich giao d�ch
		...
		break;
	case GOI_GAMBLE_WILLING: //giao dich giao d�ch
		//if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].GetMenuState() == PLAYER_MENU_STATE_TRADEOPEN)
		//{
		//	Player[CLIENT_PLAYER_INDEX].TradeApplyClose();
		//}
	...
	case GOI_AUTOPLAY_ACTION:
		{
			...
			switch(uParam)
			{
					...
					while(i)
					{
						...
						|| Object[i].m_nKind == Obj_Kind_Money))
						{
							...
							if(Object[i].m_nKind == Obj_Kind_Item)
							{
								...
								else if(pApData->nPickType == 2) //�� m�u
								{
									if(Object[i].m_nGenre == item_equip
									&& !Object[i].m_nColorID)
									{
								...
								else if(pApData->nPickType == 3) //1 � m�u
								{
								...
								else if(pApData->nPickType == 4) //1-4 � m�u
								{
								...
								else if(pApData->nPickType == 5) //ti�n
								{
									i = ObjSet.GetNext(i);
									continue;
								}
					...
					{	//t�m trong list �� m�i
						...
						while (nIdx = NpcSet.GetNextIdx(nIdx))
						{
							...
							if (Npc[nIdx].m_RegionIndex < 0)
								continue;
							if(pApData->bJoinPtByList && pApData->nIJPtCount)
							{	//n�u nh�n theo list th� tra c� t�n
								bool bNamefound = false;
								for(int i=0;i<pApData->nIJPtCount;++i)
								{
									if(!strcmp(Npc[nIdx].Name, pApData->szIJPtName[i]))
				...
				case ATYPE_RETURN:
				{
					...
					{	//c�t ho�c qu�ng m�n tr�n tay n�u c�
						g_ScenePlace.RemoveFlag();
							...
							if(nDist < 300)
							{
								...
								while (nIdx = NpcSet.GetNextIdx(nIdx))
								{
									...
									if(g_GetDistance(nX, nY, dX, dY) < 128)
									{
										...
										g_StrLower(szBuff);
										if(strstr(szBuff, "d��c") || strstr(szBuff, "thu�c")
										|| strstr(szBuff, "th�n y"))
										{
											Player[nPlayerIdx].DialogNpc(nIdx);
											return 1;
										}
					...
					{	//�i xa phu
						...
						{	//l�a ch�n khung dialog
							...
							for(int i=0;i < CoreDataChanged(GDCNI_UI_ACT, 4, 0);++i)
							{
								...
								|| (pApData->nSelStation == 4 && strstr(szBuff, "n�i l�m")))
								{
									CoreDataChanged(GDCNI_UI_ACT, 1, 0);
	...
	case GOI_OPTION_SETTING:			
		...
		else if(uParam == OPTION_QUALITY_GIAMPLAYER)//add by phong ki�u
		{
	...
	case GOI_AUTO_PLAY: 
		{ 
			...
			case 14:
				...
				break;
			case 15:	
				Player[CLIENT_PLAYER_INDEX].m_AutoLifeReplenishP = nParam; //buff theo ph�n tr�m m�u
				break;
			case 16:
			...
			case 36:
				...
				break;
			case 37:
				Player[CLIENT_PLAYER_INDEX].m_bActiveAutoParty = nParam;	//k�ch ho�t t�nh n�ng t� ��i
				break;
			case 38:
				Player[CLIENT_PLAYER_INDEX].m_bAutoParty = nParam;	//t� ��ng t� ��i
				break;
			case 39:
				Player[CLIENT_PLAYER_INDEX].m_cTeam.m_bAutoAccecpt = nParam;	//t� ��ng nh�n l�i m�i
				break;
			case 40:
	...
	case GOI_PLAYER_ACTION:
		{
			...
			case PA_RIDE:// len xuong ngua
				...
				if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_Doing != do_sit)
				{
				   if(GetTickCount() - Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_TimeHorse >= TIME_RIDE)
					...
					else
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						sprintf(Msg.szMessage, "B�n qu� m�t m�i, kh�ng th� ti�p t�c l�n xu�ng ng�a!");
				...
				else
				{
					...
					Msg.byPriority = 1;
					Msg.byParamSize = 0;
					sprintf(Msg.szMessage, "B�n �ang ng�i thi�n kh�ng th� l�n ng�a");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
				}
				break;
			case PW_NOT_SAME: // mat khau ko trung nhau
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "M�t kh�u x�c th�c kh�ng gi�ng nhau");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			case PW_NOT_LONG: // mat khau ko du do dai
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "M�t kh�u ph�i �� 6 ch� s�");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			case PW_ACCEPTED: // mat khau dc chap nhan
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "M�t kh�u ���c ch�p nh�n");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			case HT_CN:
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "Ch�c n�ng n�y �ang ���c ho�n thi�n");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			...
			case CN_GH:
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "Ch�a m� kh�a r��ng. Kh�ng th� th�c hi�n thao t�c n�y !");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			case EX_BOX:
				{	
					...
					if(nExbox == 0)
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "B�n ch�a m� r�ng r��ng ��n g�p Th�m C�u t�i Ba L�ng Huy�n 188/198");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
					else 
			...
			case EX_BOX2:
				{	
					...
					if(nExbox == 1)
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "B�n ch�a m� r�ng r��ng 2 ��n g�p Th�m C�u t�i Ba L�ng Huy�n 188/198");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
			...
			case EX_BOX3:
				{	
					...
					if(nExbox == 2 || nExbox == 1)
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "B�n ch�a m� r�ng r��ng 3 ��n g�p Th�m C�u t�i Ba L�ng Huy�n 188/198");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
					else if(nExbox == 1 || nExbox == 2 || nExbox == 3)
			...
			case ITEMEX:
				{	
					...
					if(nItemEX == 0)
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "B�n ch�a mua m� r�ng h�nh trang � Th�m C�u");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
					else if(nItemEX >= 1)
			...
			case PA_SIT:
				...
				else
				{
					...
					Msg.byPriority = 1;
					Msg.byParamSize = 0;
					strcpy(Msg.szMessage, "Trong l�c �i ng�a kh�ng th� th�c hi�n");//edit by phong kieu dang tren ngua khong the ngoi
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
				}
				break;
			}
	...
	case GOI_PK_SETTING:		
		{
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							Msg.byPriority = 1;
							Msg.byParamSize = 0;
							sprintf(Msg.szMessage, "PK luy�n c�ng chuy�n sang phi luy�n c�ng m�t 3 ph�t !");
							CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
						}
					}
				}
			...
			else
			{
				...
				Msg.byPriority = 1;
				Msg.byParamSize = 0;
				strcpy(Msg.szMessage, "Khu v�c n�y kh�ng cho ph�p ��i PK!");
				CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
			}
		}
		break;
...
void KCoreShell::SetMoveMap(int nType, int nPos, int nValue)
{
	switch(nType)
	{
		case GAUTO_AUTO_MOVEMPSID: //to� �� train
		...
		case GAUTO_AUTO_MOVEMPSY:
			...
			break;
		case GAUTO_AUTO_PT_PLAYERTEAM://qu�n l� t� ��i
			{
		...
		case GAUTO_AUTO_MOVETPSID: //to� �� ra b�i
			if (nPos >= 0 && nPos < defMAX_AUTO_MOVEMPSL)
...
int KCoreShell::TeamOperation(unsigned int uOper, unsigned int uParam, int nParam)
{
	...
	case TEAM_OI_REFUSE_INVITE:		
		Player[CLIENT_PLAYER_INDEX].m_cTeam.SetAutoRefuseInvite(nParam);//0 t�t t� ��ng t� ch�i l�i m�i//1 b�t t� ��ng t� ch�i l�i m�i
		break;
...
int KCoreShell::AutoPlayOperation(unsigned int uOper, unsigned int uParam, int nParam)//fkauto
{
	...
	switch(uOper)
	{	
		case AUTOPLAY_OI_ACTIVE:	//-----------chi�n ��u--------------
			{
		...
		case AUTOPLAY_OI_PAUSE:
			...
			break;
		case AUTOPLAY_OI_FIGHT_B: //chi�n ��u
			{
		...
		case AUTOPLAY_OI_FIGHT_S:
			...
			break;
		case AUTOPLAY_OI_DISTANCE_B: //ti�p c�n
			{
		...
		case AUTOPLAY_OI_DISTANCE_S:
			...
			break;
		case AUTOPLAY_OI_SELFDEF_B://t� v�
			{
		...
		case AUTOPLAY_OI_SUPPORT_SKILL:
			{
				switch(uParam)
				{
					...
					case 4:
						...
						Player[CLIENT_PLAYER_INDEX].SetLeftSkill(nParam);//set chi�u tay tr�i
						break;
					case 5:
						Player[CLIENT_PLAYER_INDEX].m_cAuto.m_RightSkillID = nParam;
						Player[CLIENT_PLAYER_INDEX].SetRightSkill(nParam);//set chi�u tay ph�i
						break;
					case 6:
		...
		case AUTOPLAY_OI_NR_DO_SKILL_B:
			...
			break;
		case AUTOPLAY_OI_HP_B://b�m sl //-----------ph�c h�i---------------
			{
		...
		case AUTOPLAY_OI_TP_6:
			{
				...
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_nTownMoneyEditBox = nParam * 10000;//��n v� t�nh v�n l��ng
			}
			break;
		...
		case AUTOPLAY_OI_OBJ_8:
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bRiAmPeCheckBox= uParam;//gi� trang s�c
			}
			break;
		...
		case AUTOPLAY_OI_OBJ_12:
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bFillterItemCheckBox = uParam; //l�c ��
			}
		case AUTOPLAY_OI_OBJ_13:
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bKeepPurpleCheckBox= uParam; //nh�t �� t�m
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_nKeepPurpleEditBox= nParam; // s� d�ng
			}
			break;
		case AUTOPLAY_OI_MOVE_1: //-- di chuy�n
			{
		...
		case AUTOPLAY_OI_MOVE_2:
			...
			break;
		case AUTOPLAY_OI_MOVE_3://quanh �i�m
			{
		...
		case AUTOPLAY_OI_MOVE_4:
			...
			break;
		case AUTOPLAY_OI_MOVE_5: //theo to� ��
			{
		...
		case AUTOPLAY_OI_MOVE_7:
			...
			break;
		case AUTOPLAY_OI_MOVE_8://��nh qu�i tr�n ���ng �i
			{
		...
		case AUTOPLAY_OI_MAP_7:
			...
			break;
		case AUTOPLAY_OI_MAP_8: //t�n m�u
			{
				if(uParam)
		...
		case AUTOPLAY_OI_MAP_9:
			...
			break;
		case AUTOPLAY_OI_MAP_10://t�n mana
			{
				if(uParam)
		...
		case AUTOPLAY_OI_MAP_17:
			...
			break;
		case AUTOPLAY_OI_TEAM_1://---t� ��i---
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bAutoCTeamCheckBox= uParam;
			}
			break;
		case AUTOPLAY_OI_TEAM_2: //lu�n l�m nh�m tr��ng
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bAlwayLeaderCheckBox= uParam;
			}
			break;
		case AUTOPLAY_OI_TEAM_3: //t� ��ng PT t�t c�
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bPTAllCheckBox= uParam;
			}
			break;
		case AUTOPLAY_OI_TEAM_4: //t� ch�i m�i l�i m�i
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bDecAllInviCheckBox= uParam;
				Player[CLIENT_PLAYER_INDEX].m_cTeam.SetAutoRefuseInvite(uParam);//0 t�t t� ��ng t� ch�i l�i m�i//1 b�t t� ��ng t� ch�i l�i m�i
			}
			break;
		...
		case AUTOPLAY_OI_TEAM_8:
			...
			break;
		case AUTOPLAY_OI_TEAM_9://PT c�ng bang
			{
