...
int	KCoreShell::GetGameData(unsigned int uDataId, unsigned int uParam, int nParam)
{
			...
			case CGOG_ITEM:
				{
					...
					else
					{
						...
						if(money_unit == moneyunit_money && item_place == pos_equiproom)//#chÿ hi÷n thÿ gi∏ b∏n bªng ti“n vπn
						{
							Item[pObj->Obj.uId].GetDesc(pszDescript, true, true, 0);
						}
						else
	...
	case GDI_PLAYER_BASE_INFO:
		if (uParam)
		{
			...
			if (nIndex)
			{
				...
				pInfo->nRankInWorld = Player[CLIENT_PLAYER_INDEX].m_nWorldStat; //x’p hπng th’ giÌi
	...
	case GDI_PLAYER_RT_ATTRIBUTE:
		if (uParam)
		{
			...
			{
			case series_water:
				strcpy(pInfo->StatusDesc, "H÷ ThÒy");
				break;
			case series_wood:
				strcpy(pInfo->StatusDesc, "H÷ MÈc");
				break;
			case series_metal:
	...
	case GDI_PLAYER_HOLD_FKCOIN:	//add by phong ki“u sË xu ng≠Íi tr™n player
			...
			if(uParam)
			{
				...
				{	
					...
					else
					{
							...
							m_sInfo->m_bStack,
							m_sInfo->m_bEnChance,
							m_sInfo->m_bPoint //ÆÂ t›m sˆ dÙng Æi”m nµy tËi Æa 6 dﬂng
							);						
					}
					if (nItemIdx > 0)
					{
	...
	case GDI_IS_TONG_MASTER:
		...
		break;
	case GDI_MISSION_SELFDATA: //add by phong ki“u TËng Kim Battle
		if (uParam)
		{
...
static void DT_Msg(int nPlayerIdx, const char* szMsg)
{
	...
	try
	{
		l_pDataChangedNotifyFunc->ChannelMessageArrival(0, "[D\267 T\310u]", (char*)szMsg, strlen(szMsg), TRUE);
	}
	catch (...) {}
}
...
static int DT_Hold(int nPlayerIdx, const char* szWhy, UINT uCurTime, UINT uMs)
{
	...
	{
		sprintf(szNghi, "%s <color=Gray>(t\271m ngh\330 %u ph\363t)", szWhy, uMs / 60000u);
...
static void DT_ParseQuest(int nPlayerIdx, const char* szQ)
{
	...
	ea.nDTBook = 0;
	ea.nDTStatType = 0;
	ea.nDTHaveBase = 0;	// (r3) cho phep "ve thu tra som" lai cho nhiem vu moi
	int i, k;
...
static int DT_Skip(int nPlayerIdx, const autoData* pAp, UINT uCurTime, const char* szWhy)
{
	...
	if (pAp->nDTSkipMode == 1 && bLoaiBiTat)
	{
		...
		if (strlen(szWhy) < 440)
		{
			sprintf(szHuy, "%s <color=Gray>(h\361y nhi\326m v\364 - lo\271i n\265y \256ang t\276t)", szWhy);
...
static int DT_BagRelease(int nPlayerIdx, const autoData* pAp, UINT uCurTime, const char* szWhy)
{
	...
	DT_Msg(nPlayerIdx, szWhy);
	ea.uDTStatusTime = 0;
	DT_Msg(nPlayerIdx, "<color=Yellow>T\363i \256\307y - auto t\371 b\270n b\355t trang b\336 tr\276ng/xanh ngay t\271i ch\347...");
...
static int DT_Process(int nPlayerIdx, const autoData* pAp, UINT uCurTime)
{
	...
	if (ea.nDTPhase == DTP_HOLD)
	{
		if (ea.uDTHoldUntil && uCurTime > ea.uDTHoldUntil)
		{
			...
			if (ea.nDTHoldFreeze)
			{
				...
				if (!nOThanhH)
				{
					ea.uDTHoldUntil = uCurTime + 60u * 60u * 1000u;
					ea.nDTEngaged = 0;
					return 0;
				}
			}
	...
	if (ea.nDTPhase == DTP_SELLJUNK)
	{
		...
		else if (uCurTime > ea.uDTHoldUntil)
		{
			...
			if (nTrong >= 5)
			{
				DT_SellResume(ea);	// du 5 o de tra nhiem vu - lam tiep dung mach
				return 1;
			}
			return DT_Hold(nPlayerIdx, "<color=Yellow>B\270n r\270c xong v\311n ch\255a \256\361 \253 tr\350ng - h\267y d\344n b\355t t\363i gi\363p auto.", uCurTime, 15 * 60 * 1000);
		}
		else if (ea.uDTNext <= uCurTime)
		{
			...
			return DT_Hold(nPlayerIdx, "<color=Yellow>B\270n h\325t r\270c m\265 v\311n ch\255a \256\361 \253 tr\350ng - h\267y d\344n b\355t t\363i gi\363p auto.", uCurTime, 15 * 60 * 1000);
		}
		return ea.nDTEngaged;
	}
	...
	if (ea.nDTPhase == DTP_FARM && ea.nDTQType == 5 && ea.nDTStatType == 2)
	{
		...
		if (ea.uDTFarmStall && uCurTime - ea.uDTFarmStall > 20u * 60u * 1000u)
			return DT_Hold(nPlayerIdx, "<color=Red>C\265y kinh nghi\326m qu\270 l\251u kh\253ng ti\325n tri\323n.", uCurTime, 15 * 60 * 1000);
		if (g_dDTExpGain >= (double)ea.nDTReqNum)
		{
			DT_Msg(nPlayerIdx, "<color=Green>\247\267 \256\361 kinh nghi\326m - quay v\322 tr\266 nhi\326m v\364.");
	...
	case DTP_IDLE:
	{
		...
		if (i >= g_nDTNpcCount)
		{
			DT_Msg(nPlayerIdx, "<color=Cyan>Kh\253ng \353 th\265nh c\343 D\267 T\310u - d\357ng Th\346 \247\336a Ph\357 v\322 th\265nh.");
			ea.nDTPhase = DTP_RETURN;
			ea.nDTStep = DTI_NONE;
			ea.nDTEngaged = 1;
			return 1;
		}
		DT_Msg(nPlayerIdx, "<color=Cyan>\247ang \256\325n ch\347 NPC D\267 T\310u \256\323 nh\313n / tr\266 nhi\326m v\364.");
		ea.nDTPhase = DTP_GOTONPC;
		ea.nDTStep = DTI_NONE;
		ea.nDTRetry = 0;
		ea.nDTEngaged = 1;
	...
	case DTP_RETURN:
	{
		...
		for (i = 0; i < g_nDTNpcCount; ++i)
		{
			if (g_DTNpc[i].nMapId == nMap)
			{
				 ...
				 && Player[nPlayerIdx].m_ItemList.CalcFreeItemCellCount(1, 1, room_equipment) < 10)
				{
					...
					ea.nDTRetry = 0;
					ea.nDTShopTry = Player[nPlayerIdx].m_ItemList.CalcFreeItemCellCount(1, 1, room_equipment);
					ea.uDTHoldUntil = uCurTime + 4u * 60u * 1000u;
					DT_Msg(nPlayerIdx, "<color=Cyan>V\322 th\265nh - b\270n b\355t r\270c tr\255\355c r\345i l\265m ti\325p...");
					return 1;
				}
				if (ea.nDTBackXaFu)
				{
					...
					ea.nDTPhase = DTP_GOXAFU;
					ea.nDTRetry = 0;
					DT_Msg(nPlayerIdx, "<color=Cyan>\247\267 v\322 th\265nh - ra Xa Phu \256i ti\325p t\355i map nhi\326m v\364.");
					return 1;
				}
				ea.nDTPhase = DTP_GOTONPC;
				ea.nDTRetry = 0;
		...
		if (!Npc[Player[nPlayerIdx].m_nIndex].m_FightMode)
			return DT_Hold(nPlayerIdx, "<color=Red>K\321t: kh\253ng \353 th\265nh m\265 kh\253ng d\357ng \256\255\356c Th\346 \247\336a Ph\357.", uCurTime, 10 * 60 * 1000);
		if (++ea.nDTRetry > 8)
			return DT_Hold(nPlayerIdx, "<color=Red>K\321t: kh\253ng v\322 \256\255\356c th\265nh (h\325t ph\357 v\322?).", uCurTime, 10 * 60 * 1000);
	...
	case DTP_GOTONPC:
	{
		...
		{
			if (++ea.nDTRetry > 20)
				return DT_Hold(nPlayerIdx, "<color=Red>Kh\253ng th\312y NPC D\267 T\310u \353 t\344a \256\351 \256\267 \256\336nh.", uCurTime, 5 * 60 * 1000);
		}
		return 1;
	}
	...
	case DTP_WAITDLG:
	{
		...
		if (cap.uTalkSeq != ea.uDTTalkSeen)
		{
			...
			if (DT_Has(cap.szTalk, DTM_MSG_PUNISH))
				return DT_Hold(nPlayerIdx, "<color=Orange>B\336 D\267 T\310u ph\271t v\327 h\361y nhi\322u - ch\352 h\325t ph\271t s\317 l\265m ti\325p.", uCurTime, 11 * 60 * 1000);
		}
		if (cap.uDlgSeq == ea.uDTDlgSeen)
		{
			// moi vong ~250ms; 16 vong (~4s) khong co phan hoi -> go lai; 60 vong -> treo
			if (++ea.nDTRetry > 60)
				return DT_Hold(nPlayerIdx, "<color=Red>NPC kh\253ng tr\266 l\352i h\351i tho\271i.", uCurTime, 5 * 60 * 1000);
			if ((ea.nDTRetry % 16) == 0)
			{
				ea.nDTPhase = DTP_GOTONPC;
		...
		if (DT_Has(szQ, DTM_MSG_LIMIT))
		{
			...
			// (20/08) treo co han 60 phut: neu ngay cua SERVER lech ngay may (mui gio)
			// thi moi gio tu ra hoi lai NPC mot lan, server reset la chay tiep ngay.
			DT_Hold(nPlayerIdx, "<color=AYellow>Tuy\326t! \247\267 \256\361 40 nhi\326m v\364 D\267 T\310u h\253m nay - ngh\330, qua ng\265y auto t\371 ch\271y l\271i.", uCurTime, 60 * 60 * 1000);
			ea.nDTHoldFreeze = 1;	// danh dau hold "du 40" (han che quet/thu lai)
			// (r3 - "dung yen khong len lai map treo") NHA MAY HOAN TOAN va danh thuc
			// may Hau can tu dau: nHomeStep cu co the dang ket o trang thai "da xong"
			// tu chuyen truoc (chi duoc reset khi fight-mode lat 1->0) nen khong ai
			// dua nhan vat di ban rac / mua do / len map treo ca.
			ea.nHomeStep = 0;
			ea.nSubStep = 0;
			ea.nDTEngaged = 0;
			return 0;
		...
		if (DT_Has(szQ, DTM_MSG_BAGFULL))
			return DT_BagRelease(nPlayerIdx, pAp, uCurTime, "<color=Yellow>T\363i \256\307y (d\255\355i 5 \253 tr\350ng) - h\267y d\344n t\363i r\345i auto ch\271y ti\325p.");
		...
		if (DT_Has(szQ, DTM_MSG_FAILREQ) || DT_Has(szQ, DTM_MSG_FAILSHXT))
		{
			...
			{
				DT_Msg(nPlayerIdx, "<color=Cyan>Ch\255a \256\361 m\266nh - quay l\271i map \256\270nh ti\325p...");
				ea.nDTStep = DTI_NONE;
			...
			if (ea.nDTQType == 5 && ea.nDTStatType == 2 && ea.nDTStep == DTI_TURNIN)
			{
				...
				ea.nDTPhase = DTP_FARM;
				ea.nDTEngaged = 0;
				DT_Msg(nPlayerIdx, "<color=Cyan>Ch\255a \256\361 kinh nghi\326m - c\265y ti\325p...");
				return 0;
			}
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Kh\253ng \256\270p \370ng \256\255\356c nhi\326m v\364 n\265y.");
		}
		...
		if ((idx = DT_FindAns(apAns, nAns, DTM_OPT_CANCEL2)) >= 0)
		{
			...
			return DT_Hold(nPlayerIdx, "<color=Orange>H\325t l\255\356t h\361y nhi\326m v\364.", uCurTime, 15 * 60 * 1000);
		}
		...
		if ((idx = DT_FindAns(apAns, nAns, DTM_OPT_ACCEPT)) >= 0)
		{
			...
			if (ea.nDTQType <= 0)
				return DT_Hold(nPlayerIdx, "<color=Red>Kh\253ng hi\323u n\351i dung nhi\326m v\364 (l\271) - h\267y b\270o l\271i l\347i n\265y.", uCurTime, 10 * 60 * 1000);
			...
			if (!pAp->bDTType[ea.nDTQType - 1])
			{
				...
				if (pAp->bDTLenhBai == 1 && ea.nDTLBTry < 2)
				{
					...
					if (nLB && nLBPos == pos_equiproom)
					{
						...
						DT_Msg(nPlayerIdx, "<color=AYellow>D\357ng L\326nh b\265i ho\265n th\265nh D\267 T\310u - ch\352 nh\313n th\255\353ng...");
						ea.nDTStep = DTI_NONE;
						ea.nDTPhase = DTP_WAITDLG;
					...
					DT_Msg(nPlayerIdx, "<color=Red>L\326nh b\265i ho\265n th\265nh kh\253ng c\343 t\270c d\364ng (server ch\255a c\313p nh\313t script?) - l\265m theo c\312u h\327nh.");
				...
				return DT_Hold(nPlayerIdx, "<color=Yellow>Lo\271i nhi\326m v\364 n\265y \256ang <color=Red>t\276t<color> trong tab D\267 T\310u - treo ch\352.", uCurTime, 15 * 60 * 1000);
			}
			// den de tra?
			if (ea.nDTStep == DTI_TURNIN)
			{
				if (Player[nPlayerIdx].m_ItemList.CalcFreeItemCellCount(1, 1, room_equipment) < 5)
					return DT_BagRelease(nPlayerIdx, pAp, uCurTime, "<color=Yellow>C\307n \335t nh\312t 5 \253 tr\350ng \256\323 tr\266 nhi\326m v\364 - h\267y d\344n t\363i.");
				DT_Answer(nPlayerIdx, idx);
			...
			if (ea.nDTStep == DTI_CANCEL)
			{
				...
				return DT_Hold(nPlayerIdx, "<color=Red>Kh\253ng th\312y n\363t h\361y trong h\351i tho\271i.", uCurTime, 10 * 60 * 1000);
			}
		...
		if (ea.nDTUnknown > 6)
			return DT_Hold(nPlayerIdx, "<color=Red>H\351i tho\271i kh\253ng nh\313n d\271ng \256\255\356c (server \256\346i l\352i tho\271i?).", uCurTime, 5 * 60 * 1000);
		CoreDataChanged(GDCNI_UI_ACT, 1, 0);
		ea.nDTPhase = DTP_GOTONPC;
		ea.uDTNext = uCurTime + 900;
		return 1;
	...
	case DTP_EXEC:
	{
		...
		{
			if (ea.nDTCandNum <= 0)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Kh\253ng kh\355p \256\255\356c d\337ng b\266ng d\367 li\326u nhi\326m v\364.");
			int nPos = 0;
			...
			if (nItem)
			{
				if (nPos != pos_equiproom)
				{
					// keo tu ruong ve tui
					if (!pAp->bDTUseBox)
						return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Yellow>\247\345 c\307n tr\266 \256ang n\273m trong r\255\254ng - h\267y b\313t \253 <color=White>L\312y \256\345/ti\322n t\365 r\255\254ng<color> \353 tab D\267 T\310u.");
					if (!DT_EnsureUnlock(nPlayerIdx, pAp, uCurTime))
					{
						if (++ea.nDTRetry > 8)
							return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Yellow>Kh\253ng m\353 kh\343a \256\255\356c r\255\254ng - ki\323m tra m\313t kh\310u r\255\254ng \353 tab H\313u c\307n.");
						return 1;
					}
					if (Player[nPlayerIdx].m_ItemList.CalcFreeItemCellCount(
						Item[nItem].GetWidth(), Item[nItem].GetHeight(), room_equipment) < 1)
						return DT_BagRelease(nPlayerIdx, pAp, uCurTime, "<color=Yellow>T\363i kh\253ng \256\361 ch\347 \256\323 l\312y \256\345 t\365 r\255\254ng - h\267y d\344n t\363i.");
			...
			if (ea.nDTQType == 1)
			{
				...
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Orange>Nhi\326m v\364 c\307n v\362 kh\335/ng\371a - h\267y b\341 s\275n m\343n \256\343 v\265o r\255\254ng (xem t\252n trong nhi\326m v\364).");
			}
			if (ea.nDTQType == 2)
			{
				...
				if (pAp->bDTMuaSap == 1)
				{
					...
					DT_Msg(nPlayerIdx, "<color=Cyan>Kh\253ng c\343 \256\345 c\307n t\327m trong t\363i/r\255\254ng - \256i xem s\271p ng\255\352i b\270n \256\323 mua...");
					return 1;
				}
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
		...
		case 4:
		{
			if (ea.nDTMapId <= 0 || ea.nDTReqNum <= 0)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Kh\253ng \256\344c \256\255\356c t\252n map / s\350 l\255\356ng c\361a nhi\326m v\364 lo\271i 4 (\256\270nh qu\270i).");
			if (!pAp->bFight)
				return DT_Hold(nPlayerIdx, "<color=Yellow>Nhi\326m v\364 \256\270nh qu\270i: h\267y b\313t \253 <color=White>\247\270nh qu\270i<color> \353 tab Chi\325n \256\312u.", uCurTime, 10 * 60 * 1000);
			ea.nDTProg = -1;
			...
			if (!DT_HasPortalInBag(nPlayerIdx))
			{
				...
				DT_Msg(nPlayerIdx, "<color=Yellow>Ch\255a c\343 ph\357 v\322 th\265nh trong t\363i - n\252n b\341 s\275n Th\346 \247\336a Ph\357 ho\306c ph\357 v\253 h\271n v\265o t\363i/r\255\254ng.");
			}
			ea.uDTFarmStall = uCurTime;
			if (nMap == ea.nDTMapId)
			{
		...
		case 5:
		{
			if (ea.nDTStatType == 2)
			{
				...
				g_dDTExpGain = 0;
				ea.uDTFarmStall = uCurTime;
				DT_Msg(nPlayerIdx, "<color=Cyan>Nhi\326m v\364 kinh nghi\326m: th\266 cho auto th\255\352ng c\265y, \256\361 s\317 t\371 v\322 tr\266.");
				ea.nDTPhase = DTP_FARM;
				ea.nDTEngaged = 0;
				return 0;
			}
		...
		return DT_Hold(nPlayerIdx, "<color=Red>Lo\271i nhi\326m v\364 l\271 - auto ch\255a h\347 tr\356.", uCurTime, 10 * 60 * 1000);
	}
	...
	case DTP_GOSHOP:
	{
		...
		if (it == g_ShopStation.end() || it->second.empty())
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Th\265nh n\265y ch\255a c\343 t\344a \256\351 ti\326m t\271p h\343a.");
	...
	case DTP_SHOPTALK:
	{
		...
		if (cap.uDlgSeq != ea.uDTDlgSeen)
		{
			...
			if (nPick >= nAns)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Ti\326m kh\253ng c\343 m\364c giao d\336ch ph\357 h\356p.");
		...
		std::map<int, StationVector>::iterator it = g_ShopStation.find(nMap);
		if (it == g_ShopStation.end() || it->second.empty())
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>M\312t t\344a \256\351 ti\326m t\271p h\343a.");
		sStation& s = it->second[0];
		...
		if (nIdx)
		{
			...
			if (g_GetDistance(nX, nY, dX, dY) <= 128)
			{
				...
				ea.uDTNext = uCurTime + 800;
				if (++ea.nDTRetry > 10)
					return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Kh\253ng m\353 \256\255\356c c\366a s\346 ti\326m t\271p h\343a.");
				return 1;
			}
			DT_WalkTo(nPlayerIdx, dX, dY, 96, uCurTime);
			return 1;
		...
		if (++ea.nDTRetry > 10)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Kh\253ng th\312y ch\361 ti\326m t\271p h\343a.");
		return 1;
	}
	...
	case DTP_BUY:
	{
		...
		for (int b = 0; b < BuySell.GetWidth(); ++b)
		{
			...
			&& pItem->GetSeries() == r.nFive)
			{
				if (Player[nPlayerIdx].m_ItemList.GetEquipmentMoney() < pItem->GetPrice())
				{
					...
					return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Yellow>Kh\253ng \256\361 ti\322n mua \256\345 nhi\326m v\364.");
				}
				int x, y;
				...
				{
					CoreDataChanged(GDCNI_UI_ACT, 3, 0);
					return DT_BagRelease(nPlayerIdx, pAp, uCurTime, "<color=Yellow>T\363i \256\307y, kh\253ng mua \256\255\356c \256\345.");
				}
		...
		CoreDataChanged(GDCNI_UI_ACT, 3, 0);
		if (ea.nDTShopTry >= 4)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Ti\326m kh\253ng b\270n m\343n c\307n mua.");
		ea.nDTPhase = DTP_SHOPTALK;
		ea.uDTNext = uCurTime + 700;
		return 1;
	}
	...
	case DTP_GOXAFU:
	{
		...
		if (it == g_MoveStation.end() || it->second.empty())
		{
			...
			{
				DT_Msg(nPlayerIdx, "<color=Cyan>Kh\253ng ph\266i map nhi\326m v\364 - d\357ng Th\346 \247\336a Ph\357 v\322 th\265nh \256i l\271i.");
				ea.nDTBackXaFu = 1;
				ea.nDTPhase = DTP_RETURN;
				ea.nDTRetry = 0;
				return 1;
			}
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Th\265nh n\265y ch\255a c\343 t\344a \256\351 Xa Phu.");
		}
		sStation& s = it->second[0];
		int nIdx = DT_FindNpcName(nPlayerIdx, "xa phu", s.x, s.y, 400);
		if (nIdx)
		...
		{
			if (++ea.nDTRetry > 15)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>Kh\253ng th\312y Xa Phu.");
		}
		return 1;
	}
	...
	case DTP_XAFUTALK_DONE:
	{
		...
		{
			DT_Msg(nPlayerIdx, "<color=Green>\247\267 t\355i map nhi\326m v\364 - b\276t \256\307u \256\270nh qu\270i nh\306t cu\351n.");
			ea.nDTMapId = nMap;
			ea.nDTPhase = DTP_FARM;
			ea.nDTEngaged = 2;
			ea.nDTRetry = 0;
		...
		if (++ea.nDTRetry > 12)
		{
			...
			if (++ea.nDTXaFuTry > 5)
				return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Xa Phu kh\253ng ch\353 \256i map nhi\326m v\364 (ki\323m tra l\271i nhi\326m v\364).");
			ea.nDTPhase = DTP_GOXAFU;
			ea.nDTRetry = 0;
		}
		ea.uDTNext = uCurTime + 700;
	...
	case DTP_FARM:
	{
		...
		ea.nDTEngaged = 2;
		// doc thong diep tien do - (r3) doc TUNG tin theo vong 4 khe, khong sot tin chot
		while (cap.uMsgSeq != ea.uDTMsgSeen)
		{
			if (cap.uMsgSeq - ea.uDTMsgSeen > 3)
				ea.uDTMsgSeen = cap.uMsgSeq - 3;	// qua cu, khe da bi de - nhay toi
			++ea.uDTMsgSeen;
		...
		{
			ea.nDTHaveBase = 1;
			DT_Msg(nPlayerIdx, "<color=Cyan>C\343 th\323 \256\267 \256\361 m\266nh (tin nh\276n tr\253i) - v\322 th\366 tr\266 nhi\326m v\364...");
			ea.nDTStep = DTI_TURNIN;
			ea.nDTPhase = DTP_RETURN;
			ea.nDTRetry = 0;
			ea.nDTEngaged = 1;
		...
		if (ea.uDTFarmStall && uCurTime - ea.uDTFarmStall > 20u * 60u * 1000u)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>\247\270nh qu\270i qu\270 l\251u kh\253ng ti\325n tri\323n.");
		if (ea.nDTProg >= ea.nDTReqNum && ea.nDTReqNum > 0)
		{
			DT_Msg(nPlayerIdx, "<color=Green>\247\267 nh\306t \256\361 s\350 cu\351n - quay v\322 tr\266 nhi\326m v\364.");
			ea.nDTStep = DTI_TURNIN;
			ea.nDTPhase = DTP_RETURN;
			ea.nDTRetry = 0;
			ea.nDTEngaged = 1;
	...
	case DTP_REWARD:
	{
		...
		if (cap.uFinSeq != ea.uDTFinSeen)
		{
			...
			if (nThu < 6)
			{
				...
				if (nThu == 0)
					DT_Msg(nPlayerIdx, "<color=AYellow>R\255\254ng th\255\353ng \256\267 m\353 - auto b\312m ch\344n ph\307n th\255\353ng...");
			...
			DT_Msg(nPlayerIdx, "<color=Red>Kh\253ng ch\344n \256\255\356c r\255\254ng th\255\353ng (\256\267 th\366 \256\361 6 n\363t) - b\341 qua.");
			ea.nDTRwTry = 0;
			ea.nDTStep = DTI_NONE;
			ea.nDTQType = 0;
			ea.nDTItemIdx = 0;
		...
		DT_Msg(nPlayerIdx, "<color=AYellow>\247\267 nh\313n th\255\353ng xong - \256i nh\313n nhi\326m v\364 k\325 ti\325p!");
		ea.nDTRwTry = 0;
		ea.nDTStep = DTI_NONE;
		ea.nDTQType = 0;
		ea.nDTItemIdx = 0;
	...
	case DTP_MUASAP:
	{
		...
		if (uCurTime > ea.uDTHoldUntil)
		{
			...
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>\247i ch\356 qu\270 l\251u m\265 ch\255a mua \256\255\356c m\343n c\307n t\327m.");
		}
		// mua xong chua? (item ve tui la xong viec di cho)
		{
			...
			{
				DT_SapDong(nPlayerIdx);
				DT_Msg(nPlayerIdx, "<color=Green>\247\267 mua \256\255\356c \256\345 c\307n t\327m \353 s\271p - quay v\322 tr\266 nhi\326m v\364!");
				ea.nDTItemIdx = nCo;
				ea.nDTStep = DTI_TURNIN;
				ea.nDTPhase = DTP_GOTONPC;
				ea.nDTRetry = 0;
		...
		// tim sap gan nhat CHUA xem trong tam dong bo
		{
			...
			if (nGan)
			{
				...
				{
					char szXemSap[160];
					sprintf(szXemSap, "<color=Cyan>Xem s\271p \"%s\" ...",
		...
		// het thanh nay -> danh dau roi qua thanh ke
		{
			...
			if (nKe < 0)
			{
				DT_SapDong(nPlayerIdx);
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Orange>\247\267 xem s\271p kh\276p 10 th\265nh/th\253n m\265 kh\253ng c\343 m\343n c\307n t\327m.");
			}
			...
			g_uDTSapHopT = 0;
			{
				...
				char szTKSap[220];
				sprintf(szTKSap, "<color=Cyan>Th\265nh n\265y: xem %d s\271p (%d kh\253ng ph\266n h\345i) - kh\253ng c\343 m\343n c\307n, nh\352 Xa Phu qua th\265nh k\325...",
	...
	case DTP_CITYHOP:
	{
		...
		if (uCurTime > g_uDTSapHopT)
		{
			...
			if (nKe2 < 0)
			{
				DT_SapDong(nPlayerIdx);
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Orange>Kh\253ng nh\352 Xa Phu qua th\265nh kh\270c \256\255\356c (thi\325u ti\322n?).");
			}
			ea.nDTShopTry = nKe2;
			return 1;
		}
		...
		if (itXa == g_MoveStation.end() || itXa->second.empty())
		{
			 ...
			 || (!DT_UsePortal(nPlayerIdx) && !DT_PortalPull(nPlayerIdx, pAp, uCurTime)))
			{
				DT_SapDong(nPlayerIdx);
				return DT_Skip(nPlayerIdx, pAp, uCurTime,
					"<color=Red>L\271c map kh\253ng c\343 Xa Phu m\265 kh\253ng d\357ng \256\255\356c ph\357 v\322 th\265nh.");
			}
			ea.uDTNext = uCurTime + 3000;
			return 1;
		}
	...
	case DTP_USEPD:
	{
		ea.nDTEngaged = 1;
		if (ea.nDTUsedPD > 40)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>D\357ng qu\270 nhi\322u Ph\363c Duy\252n L\351 m\265 v\311n ch\255a \256\361 \256i\323m.");
		...
		if (ea.nDTUsedPD >= 2 && nFY <= ea.nDTFuYuanPrev)
			return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Red>Ph\363c Duy\252n L\351 kh\253ng c\343 t\270c d\364ng (server thi\325u script?).");
		...
		else
		{
			...
			for (i = 0; i < 3; ++i)
			{
				...
				{
					DT_Msg(nPlayerIdx, "<color=Yellow>R\255\254ng c\343 Ph\363c Duy\252n L\351 nh\255ng \256ang kh\343a - nh\313p m\313t kh\310u r\255\254ng \353 tab H\313u c\307n.");
					break;
				}
			}
		}
		...
		return DT_Skip(nPlayerIdx, pAp, uCurTime, "<color=Orange>H\325t Ph\363c Duy\252n L\351 m\265 v\311n ch\255a \256\361 \256i\323m.");
	}
...
int	KCoreShell::OperationRequest(unsigned int uOper, unsigned int uParam, int nParam)
{
	...
	case GDI_SET_TRADE_ITEM:
		{
			...
			if (nIdx > 0 && nIdx < MAX_ITEM)
			{
				...
				{
					KSystemMessage	sMsg;
					sprintf(sMsg.szMessage, "Bπn h∑y mÎ kh„a b∂o v÷ mÌi c„ th” th˘c hi÷n Æ≠Óc !");
					sMsg.eType = SMT_NORMAL;
					sMsg.byConfirmType = SMCT_NONE;
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
	...
	case GDI_PLAYER_TRADE:
		{
			...
			{
				KSystemMessage	sMsg;
				sprintf(sMsg.szMessage, "ßang c≠Îi ng˘a kh´ng th” bµy b∏n!");
				sMsg.eType = SMT_NORMAL;
				sMsg.byConfirmType = SMCT_NONE;
				sMsg.byPriority = 0;
				sMsg.byParamSize = 0;
			...
			{
				KSystemMessage	sMsg;
				sprintf(sMsg.szMessage, "Kh´ng th” bµy b∏n Î khu v˘c chi’n Æ u!");
				sMsg.eType = SMT_NORMAL;
				sMsg.byConfirmType = SMCT_NONE;
				sMsg.byPriority = 0;
				sMsg.byParamSize = 0;
			...
			if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_BaiTan == 0)
			{
				...
				else
				{
					...
					KSystemMessage	sMsg;
					sprintf(sMsg.szMessage, "Ch≠a Æﬁnh gi∏ vÀt ph»m !!! ");
					sMsg.eType = SMT_NORMAL;
					sMsg.byConfirmType = SMCT_NONE;
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
	...
	case GOI_GAMBLE_DESIRE_ITEM:
		...
		break;
	case GOI_TRADE_WILLING: //giao dich giao dﬁch
		...
		break;
	case GOI_GAMBLE_WILLING: //giao dich giao dﬁch
		//if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].GetMenuState() == PLAYER_MENU_STATE_TRADEOPEN)
		//{
		//	Player[CLIENT_PLAYER_INDEX].TradeApplyClose();
		//}
					...
					while(i)
					{
						...
						|| Object[i].m_nKind == Obj_Kind_Money))
						{
							...
							if(Object[i].m_nKind == Obj_Kind_Item)
							{
								...
								else if(pApData->nPickType == 2) //ÆÂ mµu
								{
									if(Object[i].m_nGenre == item_equip
									&& !Object[i].m_nColorID)
									{
					...
					{	//t◊m trong list Æ∑ mÍi
						...
						while (nIdx = NpcSet.GetNextIdx(nIdx))
						{
							...
							if (Npc[nIdx].m_RegionIndex < 0)
								continue;
							if(pApData->bJoinPtByList && pApData->nIJPtCount)
							{	//n’u nhÀn theo list th◊ tra c„ t™n
								bool bNamefound = false;
								for(int i=0;i<pApData->nIJPtCount;++i)
								{
									if(!strcmp(Npc[nIdx].Name, pApData->szIJPtName[i]))
							...
							if(nDist < 300)
							{
								...
								while (nIdx = NpcSet.GetNextIdx(nIdx))
								{
									...
									if(g_GetDistance(nX, nY, dX, dY) < 128)
									{
										...
										g_StrLower(szBuff);
										if(strstr(szBuff, "d≠Óc") || strstr(szBuff, "thuËc")
					...
					{	//Æi xa phu
						...
						{	//l˘a ch‰n khung dialog
							...
							for(int i=0;i < CoreDataChanged(GDCNI_UI_ACT, 4, 0);++i)
							{
								...
								g_StrLower(szBuff);
								if((pApData->nSelStation == 0 && strstr(szBuff, "lπi"))
	...
	case GOI_AUTO_PLAY: 
		{ 
			...
			case 36:
				...
				break;
			case 37:
				Player[CLIENT_PLAYER_INDEX].m_bActiveAutoParty = nParam;	//k›ch hoπt t›nh n®ng tÊ ÆÈi
				break;
			case 38:
				Player[CLIENT_PLAYER_INDEX].m_bAutoParty = nParam;	//t˘ ÆÈng tÊ ÆÈi
				break;
			case 39:
				Player[CLIENT_PLAYER_INDEX].m_cTeam.m_bAutoAccecpt = nParam;	//t˘ ÆÈng nhÀn lÍi mÍi
				break;
			case 40:
			...
			/*case ePickInFightState://nh∆t ÆÂ trong thµnh
				{
					Player[CLIENT_PLAYER_INDEX].m_Auto_PickInFightState = nParam;
					break;
				}*/
			case eAutoRightSkill: //sˆ dÙng chi™u b™n ph∂i
				{
	...
	case GOI_PLAYER_ACTION:
		{
			...
			case PA_RIDE:// len xuong ngua
				...
				if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_Doing != do_sit)
				{
				   if(GetTickCount() - Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_TimeHorse >= TIME_RIDE)
					...
					else
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						sprintf(Msg.szMessage, "Bπn qu∏ m÷t m·i, kh´ng th” ti’p tÙc l™n xuËng ng˘a!");
				...
				else
				{
					...
					Msg.byPriority = 1;
					Msg.byParamSize = 0;
					sprintf(Msg.szMessage, "Bπn Æang ngÂi thi“n kh´ng th” l™n ng˘a");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
				}
				break;
			case PW_NOT_SAME: // mat khau ko trung nhau
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "MÀt kh»u x∏c th˘c kh´ng giËng nhau");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			case PW_NOT_LONG: // mat khau ko du do dai
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "MÀt kh»u ph∂i ÆÒ 6 ch˜ sË");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			case PW_ACCEPTED: // mat khau dc chap nhan
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "MÀt kh»u Æ≠Óc ch p nhÀn");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			case HT_CN:
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "Ch¯c n®ng nµy Æang Æ≠Óc hoµn thi÷n");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			...
			case CN_GH:
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "Ch≠a mÎ kh„a r≠¨ng. Kh´ng th” th˘c hi÷n thao t∏c nµy !");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
				break;
			case EX_BOX:
				{	
					...
					if(nExbox == 0)
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "Bπn ch≠a mÎ rÈng r≠¨ng Æ’n g∆p Th»m Cˆu tπi Ba L®ng Huy÷n 188/198");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
					else 
			...
			case EX_BOX2:
				{	
					...
					if(nExbox == 1)
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "Bπn ch≠a mÎ rÈng r≠¨ng 2 Æ’n g∆p Th»m Cˆu tπi Ba L®ng Huy÷n 188/198");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
			...
			case EX_BOX3:
				{	
					...
					if(nExbox == 2 || nExbox == 1)
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "Bπn ch≠a mÎ rÈng r≠¨ng 3 Æ’n g∆p Th»m Cˆu tπi Ba L®ng Huy÷n 188/198");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
					else if(nExbox == 1 || nExbox == 2 || nExbox == 3)
			...
			case ITEMEX:
				{	
					...
					if(nItemEX == 0)
					{
						...
						Msg.byPriority = 1;
						Msg.byParamSize = 0;
						strcpy(Msg.szMessage, "Bπn ch≠a mua mÎ rÈng hµnh trang Î Th»m Cˆu");
						CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
					}
					else if(nItemEX >= 1)
			...
			case PA_SIT:
				...
				else
				{
					...
					Msg.byPriority = 1;
					Msg.byParamSize = 0;
					strcpy(Msg.szMessage, "Trong lÛc Æi ng˘a kh´ng th” th˘c hi÷n");//edit by phong kieu dang tren ngua khong the ngoi
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
				}
				break;
			}
	...
	case GOI_PK_SETTING:		
		{
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							Msg.byPriority = 1;
							Msg.byParamSize = 0;
							sprintf(Msg.szMessage, "PK luy÷n c´ng chuy”n sang phi luy÷n c´ng m t 3 phÛt !");
							CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
						}
					}
				}
			...
			else
			{
				...
				Msg.byPriority = 1;
				Msg.byParamSize = 0;
				strcpy(Msg.szMessage, "Khu v˘c nµy kh´ng cho ph–p ÆÊi PK!");
				CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&Msg, 0);
			}
		}
		break;
...
void KCoreShell::SetMoveMap(int nType, int nPos, int nValue)
{
	switch(nType)
	{
		case GAUTO_AUTO_MOVEMPSID: //toπ ÆÈ train
		...
		case GAUTO_AUTO_MOVEMPSY:
			...
			break;
		case GAUTO_AUTO_PT_PLAYERTEAM://qu∂n l˝ tÊ ÆÈi
			{
		...
		case GAUTO_AUTO_MOVETPSID: //toπ ÆÈ ra b∑i
			if (nPos >= 0 && nPos < defMAX_AUTO_MOVEMPSL)
...
int KCoreShell::TeamOperation(unsigned int uOper, unsigned int uParam, int nParam)
{
	...
	case TEAM_OI_REFUSE_INVITE:		
		Player[CLIENT_PLAYER_INDEX].m_cTeam.SetAutoRefuseInvite(nParam);//0 tæt t˘ ÆÈng tı chËi lÍi mÍi//1 bÀt t˘ ÆÈng tı chËi lÍi mÍi
		break;
...
int KCoreShell::GetOwnValue(int nMoneyUnit)
{
	...
	switch (nMoneyUnit)
	{
		...
		case moneyunit_honor: //lien dau
			...
			break;
		case moneyunit_respect: //c´ng trπng uy danh
...
int KCoreShell::AutoPlayOperation(unsigned int uOper, unsigned int uParam, int nParam)//fkauto
{
	...
	switch(uOper)
	{	
		case AUTOPLAY_OI_ACTIVE:	//-----------chi’n Æ u--------------
			{
		...
		case AUTOPLAY_OI_PAUSE:
			...
			break;
		case AUTOPLAY_OI_FIGHT_B: //chi’n Æ u
			{
		...
		case AUTOPLAY_OI_FIGHT_S:
			...
			break;
		case AUTOPLAY_OI_DISTANCE_B: //ti’p cÀn
			{
		...
		case AUTOPLAY_OI_NR_DO_SKILL_B:
			...
			break;
		case AUTOPLAY_OI_HP_B://b¨m sl //-----------phÙc hÂi---------------
			{
		...
		case AUTOPLAY_OI_TP_6:
			{
				...
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_nTownMoneyEditBox = nParam * 10000;//Æ¨n vﬁ t›nh vπn l≠Óng
			}
			break;
		...
		case AUTOPLAY_OI_RING_TDP:
			...
			break;
		case AUTOPLAY_OI_OBJ_1:	//-----------nh∆t ÆÂ---------------
			{
		...
		case AUTOPLAY_OI_OBJ_12:
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bFillterItemCheckBox = uParam; //l‰c ÆÂ
			}
		case AUTOPLAY_OI_OBJ_13:
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bKeepPurpleCheckBox= uParam; //nh∆t ÆÂ t›m
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_nKeepPurpleEditBox= nParam; // sË dﬂng
			}
			break;
		...
		case AUTOPLAY_OI_MOVE_2:
			...
			break;
		case AUTOPLAY_OI_MOVE_3://quanh Æi”m
			{
		...
		case AUTOPLAY_OI_MOVE_4:
			...
			break;
		case AUTOPLAY_OI_MOVE_5: //theo toπ ÆÈ
			{
		...
		case AUTOPLAY_OI_MOVE_7:
			...
			break;
		case AUTOPLAY_OI_MOVE_8://Æ∏nh qu∏i tr™n Æ≠Íng Æi
			{
		...
		case AUTOPLAY_OI_MAP_17:
			...
			break;
		case AUTOPLAY_OI_TEAM_1://---tÊ ÆÈi---
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bAutoCTeamCheckBox= uParam;
			}
			break;
		case AUTOPLAY_OI_TEAM_2: //lu´n lµm nh„m tr≠Îng
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bAlwayLeaderCheckBox= uParam;
			}
			break;
		case AUTOPLAY_OI_TEAM_3: //t˘ ÆÈng PT t t c∂
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bPTAllCheckBox= uParam;
			}
			break;
		case AUTOPLAY_OI_TEAM_4: //tı chËi m‰i lÍi mÍi
			{
				Player[CLIENT_PLAYER_INDEX].m_cAuto.m_bDecAllInviCheckBox= uParam;
				Player[CLIENT_PLAYER_INDEX].m_cTeam.SetAutoRefuseInvite(uParam);//0 tæt t˘ ÆÈng tı chËi lÍi mÍi//1 bÀt t˘ ÆÈng tı chËi lÍi mÍi
			}
			break;
		...
		case AUTOPLAY_OI_TEAM_8:
			...
			break;
		case AUTOPLAY_OI_TEAM_9://PT cÔng bang
			{
