...
void KSkillList::RemoveAllSkill()
{
	...
	for (int i = 0; i < MAX_NPCSKILL; ++i)
	{
		if (m_Skills[i].SkillId > 0 && m_Skills[i].SkillId != 1 && m_Skills[i].SkillId != 2 && m_Skills[i].SkillId != 53 && m_Skills[i].SkillId != 400 && m_Skills[i].SkillId != 210)  //!CheckNoSkill("NoCanXiSui",m_Skills[i].SkillId)
		{
			...
			//m_Skills[i].EnChance = 0;   //潞贸脌麓虁铆录脫碌脛
...
int KSkillList::Add(int nSkillID, int nSkillLevel, int nSkillExp, BOOL bTempSkill, int nMaxTimes, int RemainTimes)
{
	...
	if (i)
	{
		...
		if (nSkillLevel > 0)
		{
			...
			if (!pSkill) 
				return 0 ;
			if (pSkill->GetSkillStyle() == SKILL_SS_PassivityNpcState)//#can kiem tra
			{
				((KSkill*)pSkill)->Cast(m_nNpcIndex, -1, m_nNpcIndex);
			}
		}
...
int KSkillList::GetCurrentLevel(int nSkillID)
{
	...
	{
		//_ASSERT(m_Skills[i].SkillLevel < MAX_SKILLLEVEL);
		AUTOLOG_EVERY(5000, "[E3_LIST_CURLEVEL] skill=%d slot=%d level=%d cur_level=%d", nSkillID, i, m_Skills[i].SkillLevel, m_Skills[i].CurrentSkillLevel);
		if (m_Skills[i].SkillLevel > 0)
...
BOOL KSkillList::CanCast(int nSkillID, DWORD dwTime)
{
	...
	AUTOLOG_EVERY(1000, "[E3_LIST_COOLDOWN] skill=%d slot=%d next=%u now=%u remain=%u wait=%d level=%d", nSkillID, i, (unsigned int)m_Skills[i].NextCastTime, (unsigned int)dwTime, (unsigned int)(m_Skills[i].NextCastTime - dwTime), m_Skills[i].WaitCastTime, m_Skills[i].SkillLevel);
	if (m_Skills[i].NextCastTime > dwTime)
		return FALSE;

	AUTOLOG_EVERY(1000, "[E3_LIST_LEVEL0] skill=%d slot=%d level=%d cur_level=%d exp=%d temp=%d", nSkillID, i, m_Skills[i].SkillLevel, m_Skills[i].CurrentSkillLevel, m_Skills[i].SkillExp, (int)m_Skills[i].TempSkill);
	if (m_Skills[i].SkillLevel <= 0)
		return FALSE;
...
#ifndef _SERVER
	m_Skills[i].TotalTime = dwNextCastTime - dwCurrentTime;  // them moi tiktok
#endif
}	
...
int KSkillList::GetNextAllSkill(int nIndex)
{
	if (nIndex > 0)
	{
		int i = 0, j = 0;
		for (i = 1; i < MAX_NPCSKILL; i++)
		{
			...
			{	
				...
				if(nCurLevel > 0)
				{
					...
					if (pOrdinSkill->IsAura() || (pOrdinSkill->GetSkillStyle() == SKILL_SS_Missles || 
						pOrdinSkill->GetSkillStyle() == SKILL_SS_InitiativeNpcState) || (pOrdinSkill->GetSkillStyle() == SKILL_SS_Missles || 
		...
		{	
			...
			if(nCurLevel > 0)
			{
				...
				if (pOrdinSkill->IsAura() || (pOrdinSkill->GetSkillStyle() == SKILL_SS_Missles 
					...
					|| (pOrdinSkill->GetSkillStyle() == SKILL_SS_Missles || pOrdinSkill->GetSkillStyle() == SKILL_SS_Melee)
					)
					_nCount++;
			}
			else
