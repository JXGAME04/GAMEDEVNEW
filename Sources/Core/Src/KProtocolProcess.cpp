...
KProtocolProcess::KProtocolProcess()
{
	...
	ProcessFunc[s2c_playeritemlocksync] = s2cPlayerItemLockSync;
	ProcessFunc[s2c_opendataubox] = OpenDaTauBox;
	ProcessFunc[s2c_opendatau1box] = OpenDaTau1Box;
	ProcessFunc[s2c_battlebox] = s2cBattleBox;
	ProcessFunc[s2c_syncsupershop] = s2cSyncSuperShop;
	ProcessFunc[s2c_extpointsync] = s2cExtPointValueSync; // protocol xu
	...
	ProcessFunc[s2c_ridesync] = s2cRideSync; // protocol len xuong ngua
	ProcessFunc[s2c_ignorestate] = IgnoreState;
	ProcessFunc[s2c_openaffairbox] = OpenAffairBox;
	ProcessFunc[s2c_playersync_magic_attr] = s2cPlayerSync_MA;
	ProcessFunc[s2c_playeritemtimesync] = s2cPlayerItemTimeSync;
	ProcessFunc[s2c_timebox] = s2cTimeBox;
	ProcessFunc[s2c_talkex] = s2cTalkEx;
	ProcessFunc[s2c_syncmasklock] = SyncMaskLock;
	ProcessFunc[s2c_taskvalue] = s2cTaskValueSync; // protocol task
	ProcessFunc[s2c_playermissiondata] = s2cSetMissionData;
	ProcessFunc[s2c_inputbox] = s2cInPutBox;
	ProcessFunc[s2c_setobstacle] = s2cSetObstacle; //#Set vÀt c∂n
	ProcessFunc[s2c_returncityowntong] = s2cReturnCityOwnTong;
	ProcessFunc[s2c_playerloginreplay] = s2cPlayerLoginReplay; //fix by phong ki“u chuy”n gs bﬁ m t skill
	ProcessFunc[s2c_openquestfinishdlg] = s2cOpenQuestFinishDlg;// protocol quest finish
	ProcessFunc[s2c_imagenpc] = s2cImageNpcSync; 
	...
	ProcessFunc[s2c_syncrankdata2] = s2cSyncRankData2;
