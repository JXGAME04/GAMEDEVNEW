...
KProtocolProcess::KProtocolProcess()
{
	...
	ProcessFunc[s2c_setobstacle] = &KProtocolProcess::s2cSetObstacle; //#Set v�t c�n
	ProcessFunc[s2c_returncityowntong] = &KProtocolProcess::s2cReturnCityOwnTong;
	ProcessFunc[s2c_playerloginreplay] = &KProtocolProcess::s2cPlayerLoginReplay; //fix by phong ki�u chuy�n gs b� m�t skill
...
void	KProtocolProcess::IgnoreState(BYTE* pMsg) //B� qua tr�ng th�i
{
...
void	KProtocolProcess::s2cAutoPlaySync(BYTE* pMsg)
{
	...
	if (btAuto == 1)
	{
		if (bActive)
		{
			...
			sMsg.byPriority = 0;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage, "�� b�t auto!");
...
void KProtocolProcess::s2cPlayerItemLockSync(BYTE* pMsg)
{
	...
	if(pSync && pSync->m_ItemIdx > 0)
	{
		...
		Item[nIdx].SetPrice(pSync->sPrice);//#Fix ban vat pham khoa bao hiem // v�t ph�m kho� kh�ng th� b�n set gi� b�ng 0

		KSystemMessage	sMsg;
		sprintf(sMsg.szMessage, "Thao t�c b�o hi�m trang b� ho�n t�t.");
		sMsg.eType = SMT_NORMAL;
		sMsg.byConfirmType = SMCT_NONE;
		sMsg.byPriority = 0;
		sMsg.byParamSize = 0;
...
void KProtocolProcess::s2cSyncItem(BYTE* pMsg)
{
	...
	//	Item[nIndex].SetParam(pItemSync->m_Param); //#s?l�n s?d�ng item
...
void KProtocolProcess::SyncNpc(BYTE* pMsg)	//Sync 1 l�n khi npc trong �� c� player
{
	...
	Npc[nIdx].m_CurrentHitRecover   = NpcSync->m_CurrentHitRecover;		//th�i gian ph�c h�i
	Npc[nIdx].m_HitRecover		    = NpcSync->m_HitRecover;		//th�i gian ph�c h�i
...
void KProtocolProcess::SyncNpcMin(BYTE* pMsg)	//Sync li�n t�c npc trong �� c� player v� npc
{
	...
	else
	{
		...
		if (nIdx != Player[CLIENT_PLAYER_INDEX].m_nIndex)	// ����?
		{
			...
			//{
			//	Npc[nIdx].SetBlood(nOldLife - Npc[nIdx].m_CurrentLife); // M�u qu�i b?gi�m hi�n th?l�n ��u
			//}
			Npc[nIdx].m_Series				= NpcSync->m_bySeries;
		}
		...
		//
		Npc[nIdx].m_nProtectedTime = NpcSync->m_nProtectedTime;			//vong tron bat tu, v�ng tr�n b�t t?
		Npc[nIdx].m_CurrentLife			= NpcSync->m_CurrentLife;
		Npc[nIdx].m_CurrentLifeMax	= NpcSync->m_CurrentLifeMax;
		Npc[nIdx].m_LifeMax				= NpcSync->m_LifeMax;
		Npc[nIdx].m_WalkSpeed			= NpcSync->m_WalkSpeed;
...
void KProtocolProcess::SyncNpcMinPlayer(BYTE* pMsg) //Sync li�n t�c ch?player x?l?khi v�o c�c region < 0
{	
...
void KProtocolProcess::SyncPlayer(BYTE* pMsg) //sync player 1 l�n ��u ti�n
{
	...
	Npc[nIdx].nRankInWorld			= pPlaySync->RankInWorld; //x�p h�ng th� gi�i
...
void KProtocolProcess::SyncPlayerMin(BYTE* pMsg) //Sync Player li�n t�c
{
	...
	case enumMSG_TONG_RECRUIT_CLOSE:
		{
			...
			sMsg.byPriority = 0;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage, "Bang h�i c�a %s ��ng gia nh�p th�nh vi�n", szName);
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
		}
		break;
...
void KProtocolProcess::s2cPlayerLoginReplay(BYTE* pMsg) //fix by phong ki�u chuy�n gs b� m�t skill
{
...
void KProtocolProcess::s2cSetObstacle(BYTE* pMsg) //#Set V�t C�n
{
...
void KProtocolProcess::s2cExtendChat(BYTE* pMsg)
{
	...
	if (protocol == chat_someonechat)//Chat m�t
	{
	...
	else if (protocol == chat_channelchat)//Chat k�nh
	{
		...
		if (!strcmp(pCccSync->someone, "H� Th�ng"))
		{
	...
	case enumTONG_SYNC_ID_TRANSFER_ADD_APPLY:
		{
			...
			// �����淢��Ϣ���յ����룬�Ƿ�ͬ��
			KSystemMessage	sMsg;
			//sprintf(sMsg.szMessage, "%s��������ᣡ enumTONG_SYNC_ID_TRANSFER_ADD_APPLY", szName);
			sprintf(sMsg.szMessage, "%s xin gia nh�p bang", szName);
			...
			case 0:		// �Է�������
				{
					...
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
					sprintf(sMsg.szMessage, "Phong ch�c th�t b�i, ��i ph��ng kh�ng online!");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
				}
				break;
			case 1:		// �Է�����������
				{
					...
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
					sprintf(sMsg.szMessage, "Phong ch�c th�t b�i, ��i ph��ng kh�ng �� t�i l�nh ��o!");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
				}
				break;
			case 2:		// ���ֲ��ԣ�
				{
					...
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
					sprintf(sMsg.szMessage, "Phong ch�c th�t b�i, b�n bang kh�ng c� ng��i n�y!");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
				}
				break;
			...
			case 0:		
				{
					...
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
					sprintf(sMsg.szMessage, "��i t�n th�t b�i, ��i ph��ng kh�ng tr�n m�ng.");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
				}
				break;
			case 1:		
				{
					...
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
					sprintf(sMsg.szMessage, "��i t�n th�t b�i, ��i ph��ng kh�ng �� t�i l�nh ��o.");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
				}
				break;
			case 2:		
				{
					...
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
					sprintf(sMsg.szMessage, "��i t�n th�t b�i, b�n bang kh�ng c� ng��i n�y.");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
				}
				break;
			default:
			...
			case enumTONG_SYNC_ID_CHANGE_CAMP_FAIL:
				{
					...
					sprintf(sMsg.szMessage, "Kh�ng th� thay ��i m�u bang h�i ");
					CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
...
void KProtocolProcess::StartTrade(int nIndex, BYTE* pProtocol)
{
	...
	if (nIndex > 0 && nIndex < MAX_PLAYER)
	{
		...
		if (pST->m_bSet != 0 && Player[nIndex].GetTradeCount())
		{
			...
			{
				char szTemp[64];
				sprintf(szTemp, "Thu� su�t khu v�c n�y %d %s. ", s_CityTax, "%");
...
void KProtocolProcess::c2sTradeBuy(int nIndex, BYTE* pProtocol)
{
	...
	//nPlayerIdx ng��i b�n //nIndex ng��i mua //add by phong ki?u log giao d~ch
