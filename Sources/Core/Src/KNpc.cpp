...
void	KNpc::ReSetRes(int nMark)
{
	...
	if (nMark == 1)
	{
		...
		if (m_NpcSettingIdx == PLAYER_MALE_NPCTEMPLATEID)
		{
			...
			//strcpy(szNpcTypeName, "������");		kiÓu npc kieu npc bao gåm nam vµ n÷ man and lady
		...
		else
		{
			...
			//strcpy(szNpcTypeName, "Ů����");		kiÓu npc kieu npc bao gåm nam vµ n÷ man and lady
		...
		if (m_SkillList.CanCast(m_ActiveSkillID, SubWorld[m_SubWorldIndex].m_dwCurrentTime))
		{
			...
			case 1: //!IsPlayer() -> chuan cho ca server va client
				...
				if (IsPlayer() && eStyle != SKILL_SS_Thief && ((KSkill*)pSkill)->GetCostSpKey() > 0 && HS_SpGet(((KSkill*)pSkill)->GetCostSpKey()) < ((KSkill*)pSkill)->GetCostSp())
				{
					...
					sprintf(sMsgSp.szMessage, "Kh�ng �� %d t�ng %s", ((KSkill*)pSkill)->GetCostSp(), (((KSkill*)pSkill)->GetCostSpKey() == 2116) ? "�m Lu�t" : "N�");
...
int KNpc::PaintInfo(int nHeightOffset, bool bSelect, int nFontSize, DWORD dwBorderColor)
{
	...
	/*	if (m_szTeamMem[0])//VÏ th«ng tin tõ auto lªn ®Çu nh©n vËt
		...
		if (m_szTongName[0])
		{
			...
			//	{
					...
					case enumTONG_FIGURE_DIRECTOR:
						strcat(szTong, "[Tr��ng L�o]");
						break;
					case enumTONG_FIGURE_MASTER:
						strcat(szTong, "[Bang Ch�]");
						break;
					}
				}
			else
		...
		if (m_BaiTan) //#bµy b¸n
		{
			...
			char*sOutm = "\\Spr\\Ui3\\��̯\\��̯ͷ��������.spr"; //Head
			...
			char* sOutR = "\\Spr\\Ui3\\��̯\\��̯ͷ��������.spr";	//Head
			RUIconImageR.nType = ISI_T_SPR;
			RUIconImageR.Color.Color_b.a = 255;
			RUIconImageR.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
			RUIconImageR.uImage = 0;
			...
			char* sOutL = "\\Spr\\Ui3\\��̯\\��̯ͷ��������.spr";	//Head
			RUIconImageR.nType = ISI_T_SPR;
			RUIconImageR.Color.Color_b.a = 255;
			RUIconImageR.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
			RUIconImageR.uImage = 0;
