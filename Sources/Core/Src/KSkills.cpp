...
int KSkill::CanCastSkill(int nLauncher, int &nParam1, int &nParam2)  const 
{
	...
	{
		AUTOLOG_EVERY(1000, "[E3_REJECT_TARGETONLY] skill=%d lv=%d launcher=%d p1=%d p2=%d need_npc_target", (int)m_nId, (int)m_ulLevel, nLauncher, nParam1, nParam2);
...
relationisvalid:
	...
	if (Npc[nLauncher].IsPlayer())
	{
		...
		if (-2 != m_nEquiptLimited)
		{
			...
			AUTOLOG_EVERY(1000, "[SKILL-REFUSE-WEAPON] sk=%d lv=%d launcher=%d wpn_detail=%d wpn_particular=%d yeucau=%d -> tra ve 0 (IM LANG)", (int)m_nId, (int)m_ulLevel, nLauncher, nDetailType, nParticularType, m_nEquiptLimited);
		...
		if (nParam1 == -1)
		{
			...
			{
				AUTOLOG_EVERY(1000, "[SKILL-REFUSE-FIGHTMODE] sk=%d launcher=%d fm=%d tgt=%d fm=%d -> tra ve 0", (int)m_nId, nLauncher, Npc[nLauncher].m_FightMode, nParam2, Npc[nParam2].m_FightMode);
		...
		if(m_bTargetEnemy || m_bTargetAlly)
		{
			...
			else
			{
					...
					m_eMisslesForm == SKILL_MF_AtFirer*/)
				{
					...
					AUTOLOG_EVERY(1000, "[SKILL-DIST-XY] sk=%d form=%d launcher=%d mps(%d,%d) des(%d,%d) d=%d radius=%d", (int)m_nId, (int)m_eMisslesForm, nLauncher, nLauncherX, nLauncherY, nParam1, nParam2, distance, GetAttackRadius());
#ifndef _SERVER				
...
BOOL	KSkill::Cast(int nLauncher, int nParam1, int nParam2, int nWaitTime, eSkillLauncherType eLauncherType)  const 
{
	...
	AUTOLOG_EVERY(1000, "[E3_REJECT_NOPARAM] skill=%d launcher=%d ltype=%d p1=%d p2=%d", (int)m_nId, nLauncher, (int)eLauncherType, nParam1, nParam2);
...
int		KSkill::CastSpread(TOrdinSkillParam * pSkillParam, int nDir, int nRefPX, int nRefPY)  const 
{
	...
	if (pSkillParam->nTargetId > 0)
	{
		...
		if(nDistance == 0) //#can kiem tra dang bi loi cai bang rong
			return 0;
...
void	KSkill::CreateMissle(int nLauncher, int nChildSkillId, int nMissleIndex)  const 
{
	...
	pMissle->m_bIsPhysical		= m_bIsPhysical;
	...
	AUTOLOG_EVERY(1000, "[MISSLE-PARAM] msl=%d sk=%d lv=%d launcher=%d move=%d speed=%d life=%d start=%d dmgrange=%d colrange=%d missrate=%d hitcount=%d useAR=%d melee=%d h=%d", nMissleIndex, (int)m_nId, (int)m_ulLevel, nLauncher, (int)pMissle->m_eMoveKind, pMissle->m_nSpeed, pMissle->m_nLifeTime, pMissle->m_nStartLifeTime, pMissle->m_nDamageRange, pMissle->m_nCollideRange, pMissle->m_nMissRate, pMissle->m_nHitCount, (int)pMissle->m_bUseAttackRating, (int)m_bIsMelee, pMissle->m_nHeight);
	if (m_bIsMelee)
...
BOOL	KSkill::GetInfoFromTabFile(KITabFile *pSkillsSettingFile, int nRow)
{
	...
	if (!m_szSkillIcon[0])	strcpy(m_szSkillIcon, "\\spr\\skill\\Í¼±ê\\Í¨ÓÃ.spr");
		KImageParam sImage;
	if (g_pRepresent && g_pRepresent->GetImageParam(m_szSkillIcon, &sImage, ISI_T_SPR) == false) 
	{
...
#ifdef _SERVER
KMissleMagicAttribsData* KSkill::CreateMissleMagicAttribsData(int nLauncher)  const //When nLauncher == 0 , means neednt  AppendSkillEffect;
{
...
BOOL KSkill::CastInitiativeSkill(int nLauncher, int nParam1, int nParam2, int nWaitTime)  const 
{
	...
	{
		AUTOLOG_EVERY(1000, "[E3_INIT_BADTARGET] skill=%d lv=%d launcher=%d p1=%d p2=%d self_flag=%d", (int)m_nId, (int)m_ulLevel, nLauncher, nParam1, nParam2, (int)m_bTargetSelf);
...
lab_processdamage:			
	...
	AUTOLOG_EVERY(1000, "[E3_INIT_NOATTRIB] skill=%d lv=%d launcher=%d target=%d create_attrib_failed", (int)m_nId, (int)m_ulLevel, nLauncher, nParam2);
	if (!pAttribsData)
    	return FALSE;
...
BOOL	KSkill::ParseString2MagicAttrib(unsigned long ulLevel, char * szMagicAttribName, char * szValue)  
{
	...
	for (int i  = 0 ; i <= magic_normal_end; i ++)
	{
		if (!strcmp(szMagicAttribName, MagicAttrib2String(i)))
		{
			...
			if ((i > magic_missle_begin && i < magic_missle_end) || (i > magic_missle_exp_begin && i < magic_missle_exp_end))
			{
			...
			if (i > magic_damage_begin && i < magic_damage_end)
			{
				switch(i)
				{
				case magic_attackrating_v:						//attackrating[0]
				case magic_attackrating_p:
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_ignoredefense_p:					//ignoredefense[1]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_magicdamage_v:		//magic damage[2]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_seriesdamage_p: //ngò hµnh t­¬ng kh¾c //seriesdamage[3]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_deadlystrike_p: //tÊn c«ng chÝ m¹ng //deadlystrike[4]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_fatallystrike_p: //tÊn c«ng chÝ tö //fatallystrike[5]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_steallife_p:			//steallife[6]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_stealmana_p:		//stealmana[7]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_stealstamina_p:			//stealstamina[8]	
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_physicsdamage_v:	//physics damage[9]
				case magic_physicsenhance_p:
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_colddamage_v:	//cold damage[10]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_firedamage_v:	//fire damage[11]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_lightingdamage_v:	//lighting damage[12]
					m_DamageAttribs[12].nAttribType = i;
					m_DamageAttribs[12].nValue[0] = nValue1;
					m_DamageAttribs[12].nValue[1] = nValue2;
					m_DamageAttribs[12].nValue[2] = nValue3;
					m_nDamageAttribsNum ++;
					break;
				case magic_poisondamage_v:		//poison damage[13]
				...
				case magic_stun_p:		//stun[14]
					...
					m_nDamageAttribsNum ++;
					break;
				case magic_randmove:	//randmove[16]
...
void	KSkill::GetDesc(unsigned long ulSkillId, unsigned long ulCurLevel, char * pszMsg, int nOwnerIndex,  bool bNextLevelDesc)
{
    ...
    //if(strlen(szTemp) > 255) szTemp[255] = 0;
	...
	TEnterTextFromCharArray(pTempSkill->m_szSkillDesc, szIntroEnter, 40); //add by phong kiÒu 50 ký tù enter xuèng
	strcat(pszMsg, szIntroEnter);
	strcat(pszMsg, "\n\n");
...
static void VhtdAutoSkillDesc(const KMagicAttrib* pA, char* pszMsg)
{
	...
	// Chuoi duoi ghep tu byte co san cua MagicDesc.ini (muc autoreplyskill, ban VLTK) - khong tu go chu Viet.
		...
		sprintf(szLine, "Khi bÞ c«ng kÝch cã <color=orange>%d%%<color> tû lÖ xuÊt <color=blue>%s<color>\n", nRate, pEv->GetSkillName());
	else
	...
	if (nNum > 1)
	{
		if (nSkillId == 1368)	// [VHTD 02/09y] chi 1368 moi THAT SU len 4..9 kiem (huashan.lua:340 skill_misslenum_v)
			...
			sprintf(szLine, "Sè l­îng kiÕm xuÊt ra: <color=orange>%d<color> kiÕm\n", nNum);
		else
