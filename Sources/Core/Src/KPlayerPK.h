...
public:
	...
	void	SetNormalPKState(BYTE bFlag, BOOL bLock);	// 设定正常PK状态
...
public:
	void	Init();							// 初始化
	void	ApplySetNormalPKState(BYTE bFlag, BOOL bLock);// 向服务器申请打开、关闭正常PK状态
	void	SetNormalPKState(BYTE bFlag, BOOL bLock, BOOL bShowMsg = TRUE);	// 设定正常PK状态
	BOOL	GetNormalPKState();				// 获得正常PK状态
	BOOL	GetLockPKState();	
