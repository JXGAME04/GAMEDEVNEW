...
public:
	...
	int				m_nType;					// 类型：头顶、脚底、身上	//Head	Foot
...
public:
	...
	BOOL			SetRideHorse(BOOL bRideHorse);						
	void			SetPos(int nNpcIdx, int x, int y, int z = 0, BOOL bFocus = FALSE, BOOL bMenu = FALSE);// 设定 npc 位置
	void			SetState(BYTE *pNpcStateList, KNpcResList *pNpcResList);	
	...
	// do chinh xac theo tick la du - tranh PluckRto/AddLeafPoint + cap phat den.
	void			SetDrawPos(int x, int y, int z){ m_nXpos = x; m_nYpos = y; m_nZpos = z; };	// current DRAWN position (interpolated when PaintInterp is on)
