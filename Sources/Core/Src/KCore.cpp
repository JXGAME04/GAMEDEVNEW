...
extern KTabFile g_TabFileLib;//Add by phong ki“u
extern KTabFile g_TabFileCtrlLib;//Add by phong ki“u
#else
IClient* g_pClient;
BOOL	 g_bPingReply;
#endif
...
unsigned int	InitAdjustColorTab()
{
	...
	{
		_ASSERT(0);
		g_DebugLog("Œﬁ∑®¥Úø™%s", ADJUSTCOLOR_TABFILE);
		return 0;
	}
...
BOOL InitGameSetting()
{
	...
	if (g_GameSetting.Load(GAME_SETTING_FILE_INI))
	{
		...
		g_GameSetting.GetInteger("ServerConfig", "NotAddNpcNormal", 1, &g_NotAddNpcNormal); //#c u h◊nh kh´ng add npc normal 1 lµ kh´ng add 0 lµ cho ph–p add
#endif
		g_GameSetting.GetInteger("ServerConfig", "WriteScriptNpcLog", 0, &g_WriteScriptNpcLog);	//c u h◊nh ghi log Æ≠Íng d…n script khi ÆËi thoπi vÌi npc
	}
	else
	{
