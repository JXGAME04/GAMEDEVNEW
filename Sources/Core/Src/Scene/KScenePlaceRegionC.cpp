...
void KScenePlaceRegionC::Clear()
{
	...
	{
		//”√KRUImage::GROUND_IMG_OCCUPY_FLAG(bMatchReferenceSpot)¿¥±Ì æKRUImage∂‘œÛ «∑Ò±ª’º”√
		//”√KRUImage::GROUND_IMG_OK_FLAG(bFrameDraw)¿¥±Ì æKRUImage∂‘œÛ «∑Ò“—æ≠‘§‰÷»æ∫√¡À
...
void KScenePlaceRegionC::SetObstacle(long value, int nMapX, int nMapY) //#Set VÀt C∂n
{
	if(m_ObstacleInfo[nMapX][nMapY] == 0)
		m_ObstacleInfo[nMapX][nMapY] = value;
}
void KScenePlaceRegionC::SetObstacleMin(long value, int nMapX, int nMapY, int nOffX, int nOffY) //#Set VÀt C∂n
{
	if(m_ObstacleInfo[nMapX][nMapY] == 0)
		m_ObstacleInfo[nMapX][nMapY] = value;
}
...
void KScenePlaceRegionC::PaintGroundDirect()
{
	...
	for (nIndex = 0; nIndex < m_GroundLayerData.uNumObject; nIndex++, pObj++)
	{
		...
		else
		{
			...
			ImgPart.nFrame = pObj->nFrame;				//Æang lÁi chÁ nµy
			ImgPart.uImage = 0;
...
void KScenePlaceRegionC::PaintGround(BOOL bPrerenderGroundImg)//add by phong ki“u h◊nh n“n hoa s¨n
{
...
void KScenePlaceRegionC::PaintObstacle()
{
	...
	for (i = 0; i < 16; i++, nX += RWP_OBSTACLE_WIDTH)
	{
		...
		{		
			...
			{
			case Obstacle_Full:
				Line[0].oEndPos.nX += 32;	//…œ
				Line[1].oEndPos.nY += 32;	//◊Û
				Line[2].oPosition.nX    += 32;	//”“
				Line[2].oEndPos.nX += 32;
				Line[2].oEndPos.nY += 32;
				Line[3].oPosition.nX	  = Line[0].oPosition.nX;	//œ¬
			...
			case Obstacle_RT:
				...
				break;
			case Obstacle_LB:
				Line[0].oEndPos.nY += 32;	//◊Û
				Line[1].oPosition.nY    += 32;	//œ¬
				Line[1].oEndPos.nX += 32;
				Line[1].oEndPos.nY += 32;
				Line[2].oEndPos.nX += 32;	//”“–±
				Line[2].oEndPos.nY += 32;
			...
			case Obstacle_RB:
				...
				Line[1].oPosition.nY	  += 32;	//◊Û–±
				Line[1].oEndPos.nX += 32;
				Line[2].oPosition.nY    += 32;	//œ¬
				Line[2].oEndPos.nX += 32;
				Line[2].oEndPos.nY += 32;
				nNumLine = 3;
				break;
			...
			if (nObs > 0)
			{
				...
				if(fkDebug1 == 1)
				{
					int nRegionx = m_RegionIndex.x * 1;
					int nRegiony = m_RegionIndex.y * 1;

					sprintf(szDebugObs, "%d/%d", nRegionx, nRegiony);
					g_pRepresent->OutputText(12, szDebugObs, KRF_ZERO_END, nX, nY, 0xffffffff, 0, 0);
				}

				if(fkDebug2 == 1)
				{
					int nRegionx = m_RegionIndex.x * 8 *32;
					int nRegiony = m_RegionIndex.y * 16 *32;
