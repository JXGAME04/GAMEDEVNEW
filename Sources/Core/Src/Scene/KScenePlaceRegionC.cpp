...
void KScenePlaceRegionC::SetObstacle(long value, int nMapX, int nMapY) //#Set V藅 C秐
{
	if(m_ObstacleInfo[nMapX][nMapY] == 0)
		m_ObstacleInfo[nMapX][nMapY] = value;
}
void KScenePlaceRegionC::SetObstacleMin(long value, int nMapX, int nMapY, int nOffX, int nOffY) //#Set V藅 C秐
{
	if(m_ObstacleInfo[nMapX][nMapY] == 0)
		m_ObstacleInfo[nMapX][nMapY] = value;
}
...
void KScenePlaceRegionC::PaintGroundDirect()
{
	...
	for (nIndex = 0; nIndex < m_GroundLayerData.uNumObject; nIndex++, pObj++)
	{
		...
		else
		{
			...
			ImgPart.nFrame = pObj->nFrame;				//產ng l鏸 ch?n祔
			ImgPart.uImage = 0;
...
void KScenePlaceRegionC::PaintGround(BOOL bPrerenderGroundImg)//add by phong ki襲 h譶h n襫 hoa s琻
{
