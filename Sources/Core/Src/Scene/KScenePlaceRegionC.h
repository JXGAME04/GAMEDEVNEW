...
class KScenePlaceRegionC
{
  ...
  public:
	...
	//##Documentation
	//## 绘制场景区域地表层
	void PaintGround(BOOL bPrerenderGroundImg);//add by phong ki襲 h譶h n襫 hoa s琻
	...
	void SetObstacle(long value, int nMapX, int nMapY); //#Set V藅 C秐
	void SetObstacleMin(long value, int nMapX, int nMapY, int nOffX, int nOffY); //#Set V藅 C秐
