	...
	if (m_bHavePicMap && g_pRepresent)
	{
		...
		if (nNpcIdx >= 0 && Npc[nNpcIdx].m_RegionIndex >= 0)
		{
			...
			PaintFindPos(nX + m_DirectPos.x - m_MapCoverArea.left - 1, 
				...
				nY + nNpcY - m_MapCoverArea.top  - 1);//绘画旗帜
			//----------------------------开始寻路
			//if (g_ScenePlace.bPaintMode)  	//是否插旗,寻路状态
			//{
			//	if (!(g_SubWorldSet.GetGameTime() % 10))
			//		AutoRunTo(m_DirectPos.x,m_DirectPos.y);
			//}

			PaintSymbol(nX);               // 绘画NPC 说明
			//-----------------------------
...
void KScenePlaceMapC::PaintAutoPath(int nX, int nY) {
	for (int i = 0; i < Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_PathFind.size(); ++i) {
		if (i % 3 == 0) { // Check if the index is a multiple of 3
			FindPathNode node = Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_PathFind[i];
			...
			FootSpot.oPosition.nX = nX + node.x / 16 - m_MapCoverArea.left - 1;
			FootSpot.oPosition.nY = nY + node.y / 32 - m_MapCoverArea.top - 1;
			FootSpot.oEndPos.nX = FootSpot.oPosition.nX + 3;
			FootSpot.oEndPos.nY = FootSpot.oPosition.nY + 3;
...
#define	MAX_NUM_CHARACTER	40
	...
	while (nNpcIdx = NpcSet.GetNextIdx(nNpcIdx))
	{
		...
		bool			bIsInMsgr = ((Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_nMissionGroup != -1) && (Npc[nNpcIdx].m_nMissionGroup != -1)); 
...
void KScenePlaceMapC::DirectFindPos(int nX, int nY, BOOL bSync, BOOL bPaintLine)
{
	...
	Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetChatInfo("T?ng t譵 阯g", sMsg.szMessage, strlen(sMsg.szMessage));
...
BYTE** KScenePlaceMapC::GetbtBarrier()
{
	return m_cLittleMap.GetBarrierBuf();
}
