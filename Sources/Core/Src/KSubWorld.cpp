...
BOOL KSubWorld::LoadMap(int nId, int nRegion)
{
	...
	if (nId != m_SubWorldID)
	{
		...
		if (m_bBgPic)
		{
			...
			g_ScenePlace.LoadGround(&IniFile); //add by phong ki襲 h譶h n襫 hoa s琻
		}
...
void KSubWorld::GetRandomObjPos(POINT& pos)// add by phong ki襲 r琲 item v?tr?ng蓇 nhi猲 quanh to?
{
	int i = g_Random(3);
	POINT	posLocal = pos;
	POINT	posTemp;
...
void KSubWorld::SetObstacle(long value, int nMpsX, int nMpsY, int nRange) //#Set V藅 C秐
{
	int nRegion, nMapX, nMapY, nOffX, nOffY;
	Mps2Map(nMpsX, nMpsY, &nRegion, &nMapX, &nMapY, &nOffX, &nOffY);
	if (nRegion == -1)
