...
#ifdef _SERVER
BOOL  KItemList::CheckItemInAll(int nIdx)//add by phong kiÒu antihack
{
...
int KItemList::FindNumberInAll(int nIdx)
{
	...
	/*for (int j=0;j<MAX_PGBOX_ITEM;j++)//fix by phong kiÒu ch­a hoµn thiÖn antihack
	{
...
void KItemList::ExchangeItem(ItemPos* SrcPos, ItemPos* DesPos)
{
	...
	{
	case pos_hand:	//--------------------------- Trªn tay ------------------------------------
		g_DebugLog("%s exchange item error", Npc[Player[m_PlayerIdx].m_nIndex].Name);
		return;
		break;

	case pos_skilltakewith:	//--------------------------- Trªn b¶ng skill lÊy ra ------------------------------------
		{
			uGenre = SrcPos->nX; //uGenre cña skill
			nEquipIdx1 = SrcPos->nY; //uId cña skill
			...
			if(pSkill) //chØ cho phÐp lÊy ra nh÷ng lo¹i skill chiÕn ®Êu vµ buff	//attr 1008 hç trî bÞ ®éng 1009 hç trî chñ ®éng
			{
				if(pSkill->IsAura() || pSkill->GetSkillId() == 709 || pSkill->GetSkillId() == 252 //skill kh«ng ®­îc phÐp
					...
					return;
				//
				if(!(pSkill->IsTargetEnemy() || pSkill->IsTargetSelf() || pSkill->GetSkillId() == 210 || pSkill->GetAttribType() == 1009)) //skill ®­îc phÐp
					return;
			}
	...
	case pos_repositoryroom:	//--------------------------- R­¬ng chøa ®å ------------------------------------
		if (Player[m_PlayerIdx].CheckTrading())	
			return;
	...
	case pos_equiproom:	//--------------------------- hµnh trang ------------------------------------
		...
		if (m_Hand)
		{
			if(Item[m_Hand].IsFkItemSkill()) //item skill shorcut lÊy ra mÊt lu«n
			{
				EatMecidine(m_Hand);
				return;
			}
...
/*		if (m_Hand && Item[m_Hand].GetGenre() == item_task)
	...
	case pos_affairitem:	//--------------------------- b¶ng giao 2 ------------------------------------
