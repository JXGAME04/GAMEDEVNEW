...
int		KObjSet::AddMoneyObj(KMapPos MapPos, int nMoneyNum)
{
	...
	sInfo.m_dwNpcId1 = 0;	//add by Fong KiÒu fix lçi auto kh«ng nhÆt tiÒn
...
int		KObjSet::Add(int nDataID, KMapPos MapPos, KObjItemInfo sItemInfo)
{
	...
	{
		sItemInfo.m_nColorID = Item[sItemInfo.m_nItemID].GetColorItem();
	}
	else if(Object[nAddNo].m_nKind == Obj_Kind_Box || Object[nAddNo].m_nKind == Obj_Kind_Money)
	{
		sItemInfo.m_nColorID = gold_item;
	}

	Object[nAddNo].SetWorldID(GetID());
	Object[nAddNo].m_dwNpcId2 = sItemInfo.m_dwNpcId1;
	if (Object[nAddNo].m_nKind == Obj_Kind_Item) // This is an item
    {
		...
		Object[nAddNo].m_nParticularType = sItemInfo.m_nParticularType;
		Object[nAddNo].m_bPickExecute = Item[sItemInfo.m_nItemID].GetPickExecute();//add by phong kiÒu item sö dông ngay
    }
	...
	cObjAdd.m_nParticularType = sItemInfo.m_nParticularType;
	cObjAdd.m_dwNpcId = sItemInfo.m_dwNpcId1;
	cObjAdd.m_btFlag = 0;
...
//		if (strcmp(szBuffer, g_szObjKind[i]) == 0)
	...
	m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LifeTime, 0, &Object[nFreeNo].m_nLifeTime);//load thoi gian ton tai object khi roi ra dat
	
	if(NUM_LOW_LIFETIME && Object[nFreeNo].m_nLifeTime > NUM_LOW_LIFETIME)			//#thoi gian song item giam bot
		Object[nFreeNo].m_nLifeTime = Object[nFreeNo].m_nLifeTime / NUM_LOW_LIFETIME;
	...
	Object[nFreeNo].m_bPickExecute = FALSE;//add by phong kiÒu item sö dông ngay
...
int		KObjSet::ClientAdd(int nID, int nDataID, int nState, int nDir, int nCurFrame, int nXpos, int nYpos, KObjItemInfo sInfo)
{
	...
	Object[nAddIndex].m_nParticularType = sInfo.m_nParticularType;
	Object[nAddIndex].m_dwNameColor = this->GetNameColor(sInfo.m_nColorID);
	Object[nAddIndex].m_dwNpcId2 = sInfo.m_dwNpcId1;
	if (sInfo.m_szName[0])
...
#ifndef _SERVER
int KObjSet::GetDistanceMps(int nRx1, int nRy1, int nRx2, int nRy2)
{
	return (int)sqrt((float)((nRx1 - nRx2) * (nRx1 - nRx2) + (nRy1 - nRy2) * (nRy1 - nRy2)));
}

//fkauto by Fong KiÒu
int	KObjSet::AutoGetObjNear(int nX1, int nY1, int nDistance, int nPickOption /*= 0*/, BOOL bPickMoney /*= FALSE*/)
{
	...
	while(1)
	{
		...
		if(Object[nIdx].m_nKind == Obj_Kind_Item)
		{
			...
			if(Object[nIdx].m_nGenre == item_equip) //item lµ trang bÞ th× míi läc
			{
				switch(nPickOption)
				{
				case 1:						//vËt phÈm 1 x 1 «
					if(nWidth != 1)
						continue;
					if(nHeight != 1)
						continue;
					break;
				case 2:						//vËt phÈm 1 x 2 «
					if(nWidth != 1)
						continue;
					if(nHeight != 2)
						continue;
					break;
				case 3:						//vËt phÈm 1 x 3 «
					if(nWidth != 1)
						continue;
					if(nHeight != 3)
						continue;
					break;
				case 4:					//vËt phÈm 1 x 4 «
					if(nWidth != 1)
						continue;
					if(nHeight != 4)
						continue;
					break;
				case 5:					//vËt phÈm 2 x 1 «
					if(nWidth != 2)
						continue;
					if(nHeight != 1)
						continue;
					break;
				case 6:					//vËt phÈm 2 x 2 «
					if(nWidth != 2)
						continue;
					if(nHeight != 2)
						continue;
					break;
				case 7:				//vËt phÈm 2 x 3 «
					if(nWidth != 2)
						continue;
					if(nHeight != 3)
						continue;
					break;
				case 8:				//vËt phÈm 2 x 4 «
					if(nWidth != 2)
					continue;
					if(nHeight != 4)
					continue;
		...
		nDistancePlayer = GetDistanceMps(nX2, nY2, nX3, nY3);
