...
#include "KItemDice.h"	// 26/08: he xuc xac chia do (DICEITEM) - Viem De Bao Tang
...
int CoreServerShell::Breathe()
{
...

	{
		...
		KItemDice_Breathe();	// 26/08: he xuc xac chia do (DICEITEM) - tu do nhip 1 giay ben trong
	}
...
void CoreServerShell::SetCharacterLixianCompleted(int nIndex)//#uy thac
{
	...
	Player[nIndex].m_nLixian = 2;	//�� th�ng b�o cho s3relay tho�t ra �� u� th�c kh�ng t�nh limit n�a
...
BOOL CoreServerShell::PayForSpeech(int nIndex, int nType)
{
	...
	if (nNpcIdx <= 0)
		return FALSE;
	if (Player[nIndex].m_nForbiddenFlag & KPlayer::FF_CHAT)	//c�m chat, cam chat
		return FALSE;
