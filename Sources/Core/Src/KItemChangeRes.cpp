...
BOOL KItemChangeRes::Init()
{
	...
	if (!m_Horse.Load(CHANGERES_HORSE_FILE))
		return FALSE;
	if (!m_Gold.Load(CHANCERES_GOLD_FILE_O))
		return FALSE;
	if (!m_Gold2.Load(CHANCERES_GOLD_FILE_N))
		return FALSE;
	if (!m_Platina.Load(CHANCERES_PLATINA_FILE))
		return FALSE;
...
int	KItemChangeRes::GetWeaponRes(int nDetail, int nParti, int nLevel)
{
	...
	case equip_rangeweapon:
		m_RangeWeapon.GetInteger(nRow - 1, 2, 2, &nRet);	
		break;
	}
	return nRet - 2;
}
...
int	KItemChangeRes::GetGoldItemRes(int nDetail, int nRow, unsigned uFlash/* = 0*/)
{
	...
	m_Gold2.GetString(szRow, "≤øº˛±‡∫≈", "19", pszInfo, sizeof(pszInfo));
	nRet = atoi(pszInfo);
	nRet -= 2;
...
int	KItemChangeRes::GetPlatinaItemRes(int nDetail, int nRow, unsigned uFlash/* = 0*/)
{
	int nRet;
	char szRow[16];
	char pszInfo[6];
