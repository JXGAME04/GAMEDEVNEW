...
BOOL	KTrade::StartGamble(int nDest, int nEquipMoney, int nRepositoryMoney)
{
	if (m_nIsGambling)
		return FALSE;
	if (nDest < 0)
...
#ifndef _SERVER
void	KTrade::SetGambleState(int nSelfLock, int nDestLock, int nSelfStateOk, int nDestStateOk)
{
	if (nSelfLock)
	{
		if (m_nTradeLock == 0)
		{
			KSystemMessage	sMsg;
			sprintf(sMsg.szMessage, MSG_GAMBLE_SELF_LOCK);
			sMsg.eType = SMT_NORMAL;
			sMsg.byConfirmType = SMCT_NONE;
			sMsg.byPriority = 0;
			sMsg.byParamSize = 0;
	...
	else
	{
		...
		{
			KSystemMessage	sMsg;
			sprintf(sMsg.szMessage, MSG_GAMBLE_SELF_UNLOCK);
			sMsg.eType = SMT_NORMAL;
			sMsg.byConfirmType = SMCT_NONE;
			sMsg.byPriority = 0;
			sMsg.byParamSize = 0;
	...
	if (nDestLock)
	{
		...
		{
			KSystemMessage	sMsg;
			sprintf(sMsg.szMessage, MSG_GAMBLE_DEST_LOCK, m_szDestName);
			sMsg.eType = SMT_NORMAL;
			sMsg.byConfirmType = SMCT_NONE;
			sMsg.byPriority = 0;
			sMsg.byParamSize = 0;
	...
	else
	{
		...
		{
			KSystemMessage	sMsg;
			sprintf(sMsg.szMessage, MSG_GAMBLE_DEST_UNLOCK, m_szDestName);
			sMsg.eType = SMT_NORMAL;
			sMsg.byConfirmType = SMCT_NONE;
			sMsg.byPriority = 0;
			sMsg.byParamSize = 0;
...
#ifndef _SERVER
void	KTrade::ReplyGambleInvite(int nDestIdx, int nReply)
{
	if (nDestIdx < 0)
		return;
	GAMBLE_REPLY_START_COMMAND	sReply;
	sReply.ProtocolType = c2s_gamblereplystart;
	sReply.m_bDecision = (nReply != 0);
	sReply.m_nDestIdx = nDestIdx;
	if (g_pClient)
		g_pClient->SendPackToServer(&sReply, sizeof(GAMBLE_REPLY_START_COMMAND));
}
#endif
