...
static char szScrRootPath[MAX_PATH] = "";
...
void SaveScreenToFile(ScreenFileType eType, unsigned int nQuality)
{
	...
	if(GetAPrintScreenFileName(&szFile[nLen], sizeof(szFile) - nLen))
	{
		...
		GetCurrentDirectory(MAX_PATH, szScrRootPath);
		...
		//sprintf(Msg.szMessage, "%s%s", szScrRootPath,szFile);
		sprintf(Msg.szMessage, "H譶h  l璾 %s", szFile);
