...
int KWndMessageListBox::Init(KIniFile* pIniFile, const char* pSection)
{
	...
	{
		int	nValue;
		pIniFile->GetInteger(pSection, "MaxMsgCount", 120, &nValue); //m∆c Æﬁnh 16 fix by phong ki“u
		SetCapability(nValue);	//Set sË l≠Óng dﬂng tËi Æa cho t«n sË chat li™n quan Æ’n item chat
...
int KScrollMessageListBox::Init(KIniFile* pIniFile, const char* pSection)
{
	if (KWndWindow::Init(pIniFile, pSection))
	{
		char Buffer[128];
