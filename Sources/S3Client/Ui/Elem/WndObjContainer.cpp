...
#include "../Elem/vongsang.h"

unsigned int l_BgColors[] =
{
	0x0a001e13,		//IIEP_NORMAL 
	0x0a2c0000,		//IIEP_NOT_USEABLE 
	0x0a654915,		//IIEP_SPECIAL
...
void KWndObjectBox::PaintWindow()
{
	...
	if (m_Object.uGenre != CGOG_NOTHING && g_pRepresentShell)
	{
		...
		//
		if(/*g_pCoreShell->GetGenreItem2(m_Object.uId) == item_equip &&*/ m_nContainerId == UOC_EQUIPTMENT) //edit by Fong KiÒu chØ vÏ mµu nÒn víi item lµ trang bÞ, trªn ng­êi
		{
		...
		Obj.Obj.uGenre = m_Object.uGenre;
		Obj.Obj.uId = m_Object.uId;
		Obj.Region.h = m_Object.DataX;
		Obj.Region.v = m_Object.DataY;
		Obj.Region.Width  = m_Object.DataW;
		Obj.Region.Height = m_Object.DataH;
		//#vong sang ¸p dông cho c¸c item lµ ¸o, mò, giµy, ®ai l­ng, vò khÝ, bao tay
		if (m_Object.uGenre == CGOG_ITEM || m_Object.uGenre == CGOG_PLAYERSELLITEM || m_Object.uGenre == CGOG_NPCSELLITEM)
		{
			if (g_pCoreShell->GetNatureItem(m_Object.uId, m_Object.uGenre) == gold_item)//vong sang F3 mang trªn ng­êi, xem th«ng tin ng­êi kh¸c
			{
				DrawBorder2(m_bCountFrame, m_nFrame, m_ulTimeDelay, m_nAbsoluteLeft, m_nAbsoluteTop, m_Width, m_Height, ehoangkim);
				DrawBorder(m_Object.m_posvs,m_Object.m_dirvs,m_Object.m_vs_lastframe,m_nAbsoluteLeft-1,m_nAbsoluteTop-1,m_Width+1,m_Height+1,((m_Width-12)+(m_Height-12))/2,ehoangkim);
			}
			else if (g_pCoreShell->GetNatureItem(m_Object.uId, m_Object.uGenre) == purple_item)
			{
...
int KWndObjectBox::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WM_LBUTTONDOWN:
		if (m_pParentWnd)
		{
			...
			//------------------------------------------------
				if (Wnd_GetDragObj(NULL)) //®Æt object xuèng «
				{
					DropObject(false);
				}
				else if (m_Object.uGenre != CGOG_NOTHING) //NhÊc object lªn khái «
				{
...
int KWndObjectBox::DropObject(bool bTestOnly)
{
	...
	if(DragObj.uGenre == CGOG_SKILL_FIGHT) //skill phÝm t¾t dev by Fong KiÒu
	{
	...
	else if (m_uAcceptableGenre != CGOG_NOTHING)
	{
...
void KWndObjectMatrix::PaintWindow() // edit by phong kieu vong sang item 
{
	++m_time;
        if (m_time % 4 == 0)
    {    
	...
	for (int i = 0; i < m_nNumObjects; i++)
	{
		...
		if (Shadow.Color.Color_dw)
		{
			...
			//vËt phÈm bµy b¸n vÏ mµu vµng s¸ng
			if(g_pCoreShell->GetPriceSell2(pObj->uId) && (g_pCoreShell->GetNPCBAITAN2() || g_UiBase.GetStatus() == UIS_S_TRADE_SETPRICE))
			{
				Shadow.Color.Color_dw = l_BgColors[4];
				g_pRepresentShell->DrawPrimitives(1, &Shadow, RU_T_SHADOW, true);
			}
			//
			if(pObj->uGenre == CGOG_ITEM && g_pCoreShell->GetGenreItem2(pObj->uId) == item_equip) //edit by Fong KiÒu chØ vÏ mµu nÒn víi item lµ trang bÞ trªn hµnh trang
			{
				g_pRepresentShell->DrawPrimitives(1, &Shadow, RU_T_SHADOW, true);
			}
			//
			else if(pObj->uGenre == CGOG_PLAYERSELLITEM && g_pCoreShell->GetGenreItem2(pObj->uId) == item_equip) //edit by Fong KiÒu chØ vÏ mµu nÒn víi item lµ trang bÞ trªn shop player
			{
				g_pRepresentShell->DrawPrimitives(1, &Shadow, RU_T_SHADOW, true);
			}
			//
			else if(pObj->uGenre == CGOG_NPCSELLITEM && g_pCoreShell->GetGenreItem(pObj->uId, CGOG_NPCSELLITEM) == item_equip) //edit by Fong KiÒu chØ vÏ mµu nÒn víi item lµ trang bÞ trªn shop npc
			{
				g_pRepresentShell->DrawPrimitives(1, &Shadow, RU_T_SHADOW, true);
			}
		}
		...
		KUiObjAtContRegion Obj; //edit by phong kieu code vong sang item
		Obj.Obj.uGenre = pObj->uGenre;
		Obj.Obj.uId = pObj->uId;
		...
		//#vong sang ¸p dông cho c¸c item lµ ¸o, mò, giµy, ®ai l­ng, vò khÝ, bao tay
		if (pObj->uGenre == CGOG_ITEM || pObj->uGenre == CGOG_PLAYERSELLITEM || pObj->uGenre == CGOG_NPCSELLITEM)
		{
			if (g_pCoreShell->GetNatureItem(pObj->uId, pObj->uGenre) == gold_item)//#vong sang khi giao dich, hµnh trang F4, khi xem ng­êi kh¸c bµy b¸n, xem trong shop liªn ®Êu
			{
				DrawBorder2(m_bCountFrame, m_nFrame, m_ulTimeDelay, Shadow.oPosition.nX, Shadow.oPosition.nY, width, height, ehoangkim);
				DrawBorder(pObj->m_posvs,pObj->m_dirvs,pObj->m_vs_lastframe,Shadow.oPosition.nX,Shadow.oPosition.nY,width,height,((width-12)+(height-12))/2,ehoangkim);
			}
			else if (g_pCoreShell->GetNatureItem(pObj->uId, pObj->uGenre) == purple_item)
			{
...
int KWndObjectMatrix::AddObject(KUiDraggedObject* pObject, int nCount)
{
	if (pObject && nCount > 0)
	{
		...
		if (nValidCount)
		{
			KUiDraggedObject* pNewList = (KUiDraggedObject*)realloc(m_pObjects, sizeof(KUiDraggedObject) * (m_nNumObjects + nValidCount)); // cÊp ph¸t thªm vïng nhí
			if (pNewList)
			{
				m_pObjects = pNewList;
				for (i = 0; i < nCount; i++)
