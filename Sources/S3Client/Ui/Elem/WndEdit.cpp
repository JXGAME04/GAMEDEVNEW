...
static KIme	s_Ime;
#ifdef JX_ANDROID
extern "C" void JxSdl_BanPhimAo(int bBat, int nMatKhau);	// KSdlApp.cpp; [DANGNHAP 12/09] them kieu o (mat khau)
#endif
...
#ifdef JX_ANDROID
		JxSdl_BanPhimAo(1, (m_Flag & WNDEDIT_ES_MASK_CHARACTER) ? 1 : 0);		// [ANDROID 09/09 CHAM] o nhap co tieu diem -> day ban phim ao len; [DANGNHAP 12/09] kieu o
#endif
		if (m_pText)
		{
			if (m_Flag & WNDEDIT_ES_IME_AVAILABLE)
			...
			m_nCaretPos = m_nTextLen;	//把光标位置设在文本末尾 mo cho nay ra thu nghiem
...
#ifdef JX_ANDROID
		JxSdl_BanPhimAo(0, 0);		// [ANDROID 09/09 CHAM] khong con o nhap nao -> cat ban phim ao di
#endif
//        DestroyCaret();
...
void KWndEdit::PaintWindow()
{
	...
	if (MULTI_LINE == 0)
	{
		...
		{   
			...
			//edit by phong kieu xoa dau nhay go chu
	...
	else
	{
		...
		do
		{
			...
			if (bShowCaret && m_nCaretPos >= nLineAhead && m_nCaretPos <= nLineEnd)
			{
				...
				//edit by phong kieu xoa dau nhay
