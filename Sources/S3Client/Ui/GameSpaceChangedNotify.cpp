...
#include "UiCase/UiInformation.h" //add by phong ki�u using fkauto autoclick v� th�nh d��ng s�c
...
int CoreDataChangedCallback(unsigned int uDataId, unsigned int uParam, int nParam)
{
	...
	case GDCNI_OPEN_STORE_BOX:
		//KUiStatus::OpenWindow(); 	//open F3 khi m� r��ng edit by Fong Ki�u
	...
	{
		if (nParam == 0) //t� ��ng b�m ch�n c�c � ��i tho�i mua thu�c, mua th� ��a ph�, �i xa phu
		{
			if (uParam >= 0) KUiMsgSel::OnClickAutoMsg(uParam);
		}
		else if (nParam == 1) // t� ��ng b�m v� th�nh d��ng s�c
		{
	...
	case GDCNI_OPEN_INPUT2:
		...
		break;
	case GDCNI_PLAYER_LOGIN_REPLAY: //fix by phong ki�u chuy�n gs b� m�t skill
		if (uParam)
		{
	...
	case GDCNI_OPEN_TIME_BOX:
	{
		...
		if (pInfo->nTime == -1)
		{
			if (KUiTimeBox::GetIfVisible())
			{
				...
				char szInfo[256];
				int n = sprintf(szInfo, "B� gi�n �o�n !!!");
	...
	{
		if (nParam) //#type shop = 1 shop dynamic 0 l� k� tr�n c�c chu�n VNG
		{
	...
	case GDCNI_PLAYER_BAUCUA_RESULT_SYNC:
		if (nParam) {
			...
			if (pBauCuaResultSync->nResultType == BAUCUA_RESULT_DEPOSIT) {
				...
				{
					char szInfo[256];
					int n = sprintf(szInfo, "N�p xu kh�ng th�nh c�ng.");
					KUiMsgCentrePad::SystemMessageArrival(szInfo, n);
				}
				else {
					char szInfo[256];
					int n = sprintf(szInfo, "N�p th�nh c�ng %d xu.", pBauCuaResultSync->nResultValue);
					KUiMsgCentrePad::SystemMessageArrival(szInfo, n);
				}
			}
...
void KClientCallback::ChannelMessageArrival(DWORD nChannelID, char* szSendName, const char* pMsgBuff, unsigned short nMsgLength, bool bSucc, bool bIsNpcChat, bool bIsShowMsgPad)
{
	...
	{
		char szInfo[256];
		int n = sprintf(szInfo, "L�i kh�ng g�i ���c th�ng tin!");
		KUiMsgCentrePad::SystemMessageArrival(szInfo, n);
		return;
	}
...
void KClientCallback::MSNMessageArrival(char* szSourceName, char* szSendName, const char* pMsgBuff, unsigned short nMsgLength, bool bSucc)
{
	...
	{
		char szInfo[256];
		int n = sprintf(szInfo, "Ng��i ch�i, %s kh�ng c� tr�n m�ng!", szSendName);
		KUiMsgCentrePad::SystemMessageArrival(szInfo, n);
		return;
	}
