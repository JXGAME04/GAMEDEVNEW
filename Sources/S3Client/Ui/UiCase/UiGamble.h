...
class KUiGamble : protected KWndShowAnimate
{
public:
	//----界面面板统一的接口函数----
	static KUiGamble*	OpenWindow(KUiPlayerItem* pOther);//打开窗口，返回唯一的一个类对象实例
	static KUiGamble*	GambleResetWindow(KUiPlayerItem* pOther);
	static KUiGamble*	GetIfVisible();					//如果窗口正被显示，则返回实例指针
	static void			CloseWindow();					//关闭窗口，同时可以选则是否删除对象实例
	static void			LoadScheme(const char* pScheme);//载入界面方案

	void				OnChangedTakewithItem(KUiObjAtRegion* pObj, int bAdd);	//对方变更随身物品
	void				setResult(BYTE choose);
...
private:
	...
	KWndButton			m_Scissors;
	KWndButton			m_Rock;
	KWndButton			m_Paper;
	KWndButton			m_Result;
	KWndButton			m_SelfPick;
	int					m_nSelfTradeMoney;
	BYTE				m_bPick; //0 rock 1 paper 2 sissors
