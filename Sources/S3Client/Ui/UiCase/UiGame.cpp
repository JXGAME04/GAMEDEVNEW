...
int KUiGameSpace::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WM_MOUSEHOVER:
		if (uParam & MK_LBUTTON)
		{
			if (g_pCoreShell)
			{
				g_pCoreShell->AutoPlayOperation(AUTOPLAY_OI_PAUSE, TRUE, 0);//fkauto
		...
		{
			if (g_pCoreShell)
				g_pCoreShell->AutoPlayOperation(AUTOPLAY_OI_PAUSE, FALSE, 0);
		}
...
void KUiGameSpace::OnMouseMoveCursor(int x, int y)
{
	...
	if (g_pCoreShell->FindSelectNPC(x, y, relation_all, true, &SelectPlayer, nKind))
	{
		...
		else if (g_pCoreShell->GetNPCBAITAN(SelectPlayer.uId)) //Set chuot khi Player ngoi rao ban
			Wnd_SwitchCursor(CURSOR_BUY);
