...
void KUiChatCentre::LoadScheme(class KIniFile* pIni)
{
	...
	//m_UnitBtnOther.SetText("ÆäËü");
	m_UnitBtnOther.SetText("Kh¸c");//edit by phong kieu lien lac viet hoa chu khac
...
void KUiChatCentre::OnSelGroupMenu(int nGroupIndex, int nCmd)
{
	...
	case GROUP_MENU_ITEM_RENAME:
		{
			...
			KUiGetString::OpenWindow("NhËp tªn nhãm míi", Name, (KWndWindow*)this, ISP_RENAME_GROUP + nGroupIndex);
		}
		break;
	...
	case GROUP_MENU_ITEM_NEW:
		KUiGetString::OpenWindow("NhËp tªn nhãm", "", (KWndWindow*)this, ISP_NEW_GROUP);
		break;
	}
}
...
void KUiChatCentre::UpdateData(UICHATCENTRE_UPDATE eFlag, KUPARAM uParam, int nGroupIndex)
{
	if (m_pSelf)
	{
		switch(eFlag)
