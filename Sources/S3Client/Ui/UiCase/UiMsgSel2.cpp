...
KUiMsgSel2* KUiMsgSel2::OpenWindow(KUiQuestionAndAnswer* pContent, KUiNpcSpr *pImage)
{
    ...
    if (m_pSelf == NULL)
    {
        ...
        if (m_pSelf)
        {
			...
			//m_pSelf->m_NpcSpr.SetImage(ISI_T_SPR, "\\Spr\\skill150\\skills150-32.spr",true);
            int nWidth, nHeight;
...
//	功能：窗口消息函数
//--------------------------------------------------------------------------
int KUiMsgSel2::WndProc(unsigned int uMsg, KUPARAM uParam, KNPARAM nParam)
{
	int nRet = 0;
	switch(uMsg)
	{
		...
		else
		{
			for (i = 0; i < pContent->AnswerCount ; i++)
			{
				if (!m_MsgScrollList.GetMessageListBox()->AddOneMessage(pContent->Answer[i].AnswerText, pContent->Answer[i].AnswerLen))
					m_MsgScrollList.GetMessageListBox()->AddOneMessage("[--сng--]", -1);
			}				
		}
	}
