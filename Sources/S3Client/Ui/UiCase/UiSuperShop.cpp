...
KUiShoppingCart* KUiShoppingCart::OpenWindow(KUiObjAtContRegion* pObj, KUiItemBuySelInfo* pPriceInfo)
{
	if (m_pSelf == NULL)
	{
...
void KUiShoppingCart::Breathe()
{
	...
	for(int i = 0; i < MAX_ITEM_SPC; i++)
	{
		...
		{
			m_nBuyCheck = m_nBuyCheck + m_GioHangNew.m_ListCount[i];
			m_nTotalCostValue = m_nTotalCostValue + m_GioHangNew.m_ListPriceInfo[i].nCurPrice * m_GioHangNew.m_ListCount[i];
		}
	...
	if (m_nOwnValue >= m_nTotalCostValue && m_nBuyCheck > 0)
		m_ConfirmBuy.Enable(true);
	else
		m_ConfirmBuy.Enable(false);
		
	m_OwnValueText.SetTextColor((m_nOwnValue - m_nTotalCostValue < 0)?0x008785:0xffd94f);
}

void KUiShoppingCart::AddCount(int nIndex)
{	
	...
	m_GoodsInfo_CountEdit[nIndex].GetText(szBuff2, sizeof(szBuff2), true);
	if (strcmp(szBuff1, szBuff2))
		m_GoodsInfo_CountEdit[nIndex].SetIntText(m_nBuyValue);
	Wnd_SetFocusWnd(&m_pSelf->m_GoodsInfo_CountEdit[nIndex]);
...
void KUiShoppingCart::DelCount(int nIndex)
{	
	int nNumber = m_GoodsInfo_CountEdit[nIndex].GetIntNumber();
	nNumber --;
	...
	KUiSuperShop::GetCurGioHang(&m_GioHangNew);
	m_GioHangNew.m_ListCount[nIndex] = m_GioHangNew.m_ListCount[nIndex] - 1;
	if (m_GioHangNew.m_ListCount[nIndex] <= 0)
...
{	
	...
	char	szBuff1[16], szBuff2[16];
	itoa(m_nBuyValue, szBuff1, 10);
	m_GoodsInfo_CountEdit[0].GetText(szBuff2, sizeof(szBuff2), true);
	if (strcmp(szBuff1, szBuff2))
		m_GoodsInfo_CountEdit[0].SetIntText(m_nBuyValue);
}

void KUiShoppingCart::Show()
{
	//Wnd_SetFocusWnd(&m_pSelf->m_GoodsInfo_CountEdit); //focus vµo control count edit
	KWndShowAnimate::Show();
	//Wnd_SetExclusive((KWndWindow*)this);	//ChØ hiÖn thÞ UI trªn cïng c¸c ui d­íi kho¸ hÕt
}
...
void KUiShoppingCart::UpdateData()
{
	if (g_pCoreShell)
	{
		...
		for(int i = 0; i < MAX_ITEM_SPC; i++)
		{
			//m_GoodsInfo_ItemBox[i].Celar();
			m_pSelf->m_GoodsInfo_ItemImg[i].SetImage(ISI_T_SPR, "", true);
			if(m_GioHangNew.m_ListCount[i] > 0)
			{
		...
		m_nOwnValue = g_pCoreShell->GetOwnValue(m_PriceInfo.nMoneyUnit);	//xu hiÖn cã	
...
{
	AddChild(&m_GH_ListGoodsWnd);
	AddChild(&m_GH_ScrollBar);
	m_GH_ListGoodsWnd.SetScrollbar(&m_GH_ScrollBar);
	...
	for(int i = 0; i < MAX_ITEM_SPC; i++)
	{
		AddChild(&m_GoodsInfo[i]);
		//AddChild(&m_GoodsInfo_ItemBox[i]);
		//m_GoodsInfo_ItemBox[i].SetContainerId((int)UOC_NPC_SHOP);
		AddChild(&m_GoodsInfo_ItemImg[i]);
		AddChild(&m_GoodsInfo_GoodsName[i]);
		AddChild(&m_GoodsInfo_TotalPrice[i]);
		AddChild(&m_GoodsInfo_DelItem[i]);
		AddChild(&m_GoodsInfo_AddCount[i]);
		AddChild(&m_GoodsInfo_DelCount[i]);
		AddChild(&m_GoodsInfo_CountEdit[i]);
	}
...
{	
	...
	if (Ini.Load(Buff))
	{
		...
		int nX = 14, nY = 65, nOffsetY = 56, i = 0;
		Ini.GetInteger("GoodsInfo","Left", 14, &nX);
		Ini.GetInteger("GoodsInfo","Top", 65, &nY);
		Ini.GetInteger("GoodsInfo","OffsetY", 56, &nOffsetY);
		for(i = 0; i < MAX_ITEM_SPC; i++)
		{
			m_GoodsInfo[i].Init(&Ini, "GoodsInfo");
			m_GoodsInfo[i].SetPosition(nX, nY);
			nY = nY + nOffsetY;
		}
		//
		Ini.GetInteger("GoodsInfo_ItemBox","Left", 14, &nX);
		Ini.GetInteger("GoodsInfo_ItemBox","Top", 65, &nY);
		Ini.GetInteger("GoodsInfo_ItemBox","OffsetY", 56, &nOffsetY);
		for(i = 0; i < MAX_ITEM_SPC; i++)
		{
			//m_GoodsInfo_ItemBox[i].Init(&Ini, "GoodsInfo_ItemBox");
			//m_GoodsInfo_ItemBox[i].SetPosition(nX, nY);
			m_GoodsInfo_ItemImg[i].Init(&Ini, "GoodsInfo_ItemBox");
			m_GoodsInfo_ItemImg[i].SetPosition(nX, nY);
			nY = nY + nOffsetY;
		}
		//
		Ini.GetInteger("GoodsInfo_GoodsName","Left", 14, &nX);
		Ini.GetInteger("GoodsInfo_GoodsName","Top", 65, &nY);
		Ini.GetInteger("GoodsInfo_GoodsName","OffsetY", 56, &nOffsetY);
		for(i = 0; i < MAX_ITEM_SPC; i++)
		{
			m_GoodsInfo_GoodsName[i].Init(&Ini, "GoodsInfo_GoodsName");
			m_GoodsInfo_GoodsName[i].SetPosition(nX, nY);
			nY = nY + nOffsetY;
		}
		//
		Ini.GetInteger("GoodsInfo_TotalPrice","Left", 14, &nX);
		Ini.GetInteger("GoodsInfo_TotalPrice","Top", 65, &nY);
		Ini.GetInteger("GoodsInfo_TotalPrice","OffsetY", 56, &nOffsetY);
		for(i = 0; i < MAX_ITEM_SPC; i++)
		{
			m_GoodsInfo_TotalPrice[i].Init(&Ini, "GoodsInfo_TotalPrice");
			m_GoodsInfo_TotalPrice[i].SetPosition(nX, nY);
			nY = nY + nOffsetY;
		}
		//
		Ini.GetInteger("GoodsInfo_DelItem","Left", 14, &nX);
		Ini.GetInteger("GoodsInfo_DelItem","Top", 65, &nY);
		Ini.GetInteger("GoodsInfo_DelItem","OffsetY", 56, &nOffsetY);
		for(i = 0; i < MAX_ITEM_SPC; i++)
		{
			m_GoodsInfo_DelItem[i].Init(&Ini, "GoodsInfo_DelItem");
			m_GoodsInfo_DelItem[i].SetPosition(nX, nY);
			nY = nY + nOffsetY;
		}
		//
		Ini.GetInteger("GoodsInfo_AddCount","Left", 14, &nX);
		Ini.GetInteger("GoodsInfo_AddCount","Top", 65, &nY);
		Ini.GetInteger("GoodsInfo_AddCount","OffsetY", 56, &nOffsetY);
		for(i = 0; i < MAX_ITEM_SPC; i++)
		{
			m_GoodsInfo_AddCount[i].Init(&Ini, "GoodsInfo_AddCount");
			m_GoodsInfo_AddCount[i].SetPosition(nX, nY);
			nY = nY + nOffsetY;
		}
		//
		Ini.GetInteger("GoodsInfo_DelCount","Left", 14, &nX);
		Ini.GetInteger("GoodsInfo_DelCount","Top", 65, &nY);
		Ini.GetInteger("GoodsInfo_DelCount","OffsetY", 56, &nOffsetY);
		for(i = 0; i < MAX_ITEM_SPC; i++)
		{
			m_GoodsInfo_DelCount[i].Init(&Ini, "GoodsInfo_DelCount");
			m_GoodsInfo_DelCount[i].SetPosition(nX, nY);
			nY = nY + nOffsetY;
		}
		//
		Ini.GetInteger("GoodsInfo_CountEdit","Left", 14, &nX);
		Ini.GetInteger("GoodsInfo_CountEdit","Top", 65, &nY);
		Ini.GetInteger("GoodsInfo_CountEdit","OffsetY", 56, &nOffsetY);
		for(i = 0; i < MAX_ITEM_SPC; i++)
		{
			m_GoodsInfo_CountEdit[i].Init(&Ini, "GoodsInfo_CountEdit");
			m_GoodsInfo_CountEdit[i].SetPosition(nX, nY);
			nY = nY + nOffsetY;
		}
		//
		m_GH_ListGoodsWnd.Init(&Ini, "DynamicsGoodsWnd");
		m_GH_ScrollBar.Init(&Ini, "Scroll");
		m_ConfirmBuy.Init(&Ini, "ConfirmBuy");
		m_PrePaidBtn.Init(&Ini, "PrePaid");	// n¹p thÎ
		m_CloseBtn.Init(&Ini, "CloseBtn");
	}
}

void KUiShoppingCart::CartRelease(int nIndex)
{
	m_nBuyValue = 1;
	m_nCostValue = 0;
	m_nOldValue = 0;
	...
	for(int i = 0; i < MAX_ITEM_SPC; i++)
	{
		nFKBuyCount = nFKBuyCount + m_GioHangNew.m_ListCount[i];
	}
	if(!nFKBuyCount)
	{
		CloseWindow(true);
...
int	KUiShoppingCart::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	if (uMsg == WND_N_BUTTON_CLICK )
	{
		for(int i = 0; i < MAX_ITEM_SPC; i++)
		{
			if ((KWndWindow*)uParam == (KWndWindow*)&m_GoodsInfo_DelItem[i])
			{
				CartRelease(i);
			}			
			else if ((KWndWindow*)uParam == (KWndWindow*)&m_GoodsInfo_AddCount[i])
				AddCount(i);
			else if ((KWndWindow*)uParam == (KWndWindow*)&m_GoodsInfo_DelCount[i])
				DelCount(i);
		}
		...
		{
			KUiSuperShop::BuyItem();
			CartRelease(-1);
		}
		else if ((KWndWindow*)uParam == (KWndWindow*)&m_PrePaidBtn) // n¹p thÎ
		{
			KIniFile Ini;
...
void KWndSellItem::LoadScheme(const char* pScheme)
{
	...
	if ( Ini.Load(Buff))
	{
		...
		m_OriginalPrice_NumberText.Init(&Ini,"MarketGoodsOPrice_Number" );//
...
KUiSuperShop::KUiSuperShop() // khi më lªn
{
	m_pBSinfo.Clear();
	m_nStoreActive = 0;
	m_nSellTypeCount = 0;
...
KUiSuperShop::~KUiSuperShop() // khi ®ãng l¹i
{
	m_GioHang.Clear();
}
...
void KUiSuperShop::CloseWindow(bool bDestroy)
{
	if (m_pSelf)
	{
		...
		for(int i = 0; i < MAX_ITEM_SPC; i++)
		{
			if (m_pSelf->m_GioHang.m_ListItemInfo[i].Obj.uGenre == CGOG_NPCSELLITEM)
			{
				nCheckGH = nCheckGH + 1;
			}
		}
...
void KUiSuperShop::LoadScheme(const char* pScheme)
{
	...
	{		
		...
		if (Ini.Load(Buff))
		{
			...
			m_pSelf->m_PrePaidBtn.Init(&Ini, "PrePaid");		// n¹p thÎ
			...
			int nLeft, nTop, nWoffset, nStartX, nStartY, nDiffW, nDiffH, nWidth, nHeight, i;
			Ini.GetInteger("SellType", "Left", 30, &nLeft);
			Ini.GetInteger("SellType", "Top", 52, &nTop);
			Ini.GetInteger("SellType", "Width", 60, &nWoffset);
			int nCount = 0;
			for (i = 0; i < MAX_SUPERSHOP_PAGETAB; i ++)
			{
				m_pSelf->m_SellType[i].Init(&Ini, "SellType");
				m_pSelf->m_SellType[i].SetPosition(nLeft, nTop);
				nLeft = nLeft + nWoffset + 2;
				m_pSelf->m_SellType[i].GetSize(&nWidth, &nHeight);
				if (i > 0)
				{
					//m_pSelf->m_SellType[i - 1].GetAbsolutePos(&nLeft, &nTop);
					//nLeft = 91;
					//m_pSelf->m_SellType[i - 1].GetSize(&nWidth, &nHeight);
					//m_pSelf->m_SellType[i].SetPosition(nLeft + nWidth* (i - 1) , nTop);
				}
			}
			for (i = 0; i < 15; i ++)
			{
...
void KUiSuperShop::SetSellTypeStart(int nStart)
{
	...
	for (i = m_nSellTypeStart; i < m_nStoreActive + m_nSellTypeStart; i ++)
	{
		...
		{
			cTabFile.GetString((m_pBSinfo.m_nShopIdx[i]+2), "ShopName", "", szName, sizeof(szName));
		} 
		else 
		{
	...
	if (pItem)
	{
		if (pItem->Obj.uGenre != CGOG_MONEY)
		{
			...
			if (g_pCoreShell->GetGameData(GDI_TRADE_ITEM_PRICE, (unsigned int)(&Obj), (int)(&Price)))
			{
				...
				m_WndSellItem[nAdd].m_PriceText.SetText("Gi¸ b¸n:");
...
void KUiSuperShop::SellSortChange(int i)
{
	if (g_pCoreShell)
	{
		...
		for (int j = 0; j < m_nStoreActive; j ++)
		{
			...
			m_SellType[j].SetLabelColor(i == j ?m_uInvalidTextColor : m_uEnableTextColor);
		}
	}
}
...
int	KUiSuperShop::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	{	
		...
		else if ((KWndWindow*)uParam == (KWndWindow*)&m_PrePaidBtn)
		{
			...
			{
				char szFilePath[MAX_PATH];
				Ini.GetString("URL", "PrePaid", "", szFilePath, sizeof(szFilePath));		// n¹p thÎ
				ShellExecute(NULL,"open",szFilePath,NULL,NULL,SW_SHOWNORMAL); 
			}
		}	
...
void KUiSuperShop::GetCurGioHang(FKGioHang* pGH)
{
	if(m_pSelf)
	{
		*pGH = m_pSelf->m_GioHang;
	}
}

void KUiSuperShop::UpdateGioHang(FKGioHang* pGH)
{
	if(m_pSelf)
	{
		m_pSelf->m_GioHang = *pGH;
	}
}

bool KUiSuperShop::PutItem(KUiObjAtContRegion* pObj, KUiItemBuySelInfo* pPriceInfo)
{
	if (m_pSelf && pObj != NULL)
	{
		...
		for(int i = 0; i < MAX_ITEM_SPC; i++)
		{
			if(m_pSelf->m_GioHang.m_ListPriceInfo[i].szItemName[0] && strcmp(m_pSelf->m_GioHang.m_ListPriceInfo[i].szItemName,  pPriceInfo->szItemName) == 0)
			{
				checkExists = true;
			...
			if(!m_pSelf->m_GioHang.m_ListPriceInfo[i].szItemName[0] && icardnull == -1)
			{
				icardnull = i;
			}
		}
		// nÕu ch­a cã trong giá vµ t×m thÊy vÞ trÝ
		if(!checkExists && icardnull >= 0)
		{
			m_pSelf->m_GioHang.m_ListItemInfo[icardnull] = *pObj;
			m_pSelf->m_GioHang.m_ListPriceInfo[icardnull] = *pPriceInfo;
			m_pSelf->m_GioHang.m_ListCount[icardnull] = 1;
			m_pSelf->m_GioHang.m_ListShopId[icardnull] = m_pSelf->m_nCurrentShopId;
			return true;
		}
		if(!checkExists && icardnull < 0) // kh«ng t×m ®­îc vÞ trÝ vµ kh«ng tån t¹i trong giá
		{
			char tmpsz[64];
			sprintf(tmpsz, "Giá hµng ®· ®Çy tèi ®a %d vËt phÈm", MAX_ITEM_SPC);
			UIMessageBox(tmpsz);
			return false;
		}
		return true;
	...
	else
	{
		...
		m_pSelf->m_ShoppingCartBtn.CheckButton(false);
...
// ================= KUiDynamicShop ============================
#define SCHEME_INI_VN		"UiDynamicShop.ini"
...
void KUiDynamicShop::UpdateShop(BuySellInfo *pBSinfo)
{
	...
	for (int i = 0; i < pBSinfo->m_nShopNum; i ++)
	{
		...
		if (cTabFile.Load(BUYSELL_FILE)) 
		{
			cTabFile.GetString((pBSinfo->m_nShopIdx[i]+2), 1 , "", szName, sizeof(szName));
		} 
		else 
		{
			itoa(pBSinfo->m_nShopIdx[i], szName, 10);
