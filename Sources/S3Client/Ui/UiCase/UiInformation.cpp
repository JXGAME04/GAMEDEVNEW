...
void KUiInformation::FkAutoHideClickBtn(int nBtnIndex)
{
	Hide(nBtnIndex);
}
...
void KUiInformation::Hide(int nBtnIndex)
{
	if (m_pCallerWnd)
	{
		m_pCallerWnd->WndProc(WND_M_OTHER_WORK_RESULT, m_uCallerParam, WND_OPER_RESULT(nBtnIndex));
		m_pCallerWnd = NULL;
	}
...
int KUiInformation::WndProc(unsigned int uMsg, KUPARAM uParam, KNPARAM nParam)
{
	int nRet = 0;
	switch(uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (KUPARAM)(KWndWindow*)&m_FirstBtn)
			Hide(0);
		else if (uParam == (KUPARAM)(KWndWindow*)&m_SecondBtn)
			Hide(1);
		break;
	case WM_KEYDOWN:
