...
KUiSelServer::KUiSelServer()
{
	...
	if (pSetting)
	{
		pSetting->GetInteger($LOGIN, "SelServerRegion", 0, &m_nRegionIndex);
		g_UiBase.CloseCommSettingFile(false);
	}
	/*
	KIniFile* pConfigFile = NULL;
	pConfigFile = g_UiBase.GetCommConfigFile();
	if (pConfigFile)
	{
		pConfigFile->GetInteger("Main", "SelRegion", 0 , (int*)(&m_nRegionIndex));
...
void KUiSelServer::OnLogin()
{
	...
	if (m_pServList && nSelServer >= 0)
	{
		if (m_bSelRegion)
		{
			...
			if (pConfigFile)
			{
				pConfigFile->WriteInteger("Main", "SelRegion", nSelServer);
				g_UiBase.CloseCommConfigFile();
			}*/
		}
		else
