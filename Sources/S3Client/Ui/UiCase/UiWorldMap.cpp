...
void KUiWorldmap::SetCityOwnTong(const char* szTongName)
{
	char	szBuffer[128];
	char szTongInfo[32] = "Bang hÈi chi’m l‹nh: %s";
	sprintf(szBuffer, szTongInfo, szTongName);
...
void KUiWorldmap::RefreshCityLabels()
{
	...
	for (int c = 1; c <= 7; c++)
	{
		...
		if (!g_pCoreShell->GetGameData(GDI_CITY_INFO, c, (int)&sV))
			continue;
		char szBuffer[160];
		_snprintf(szBuffer, sizeof(szBuffer) - 1, "Bang hÈi chi’m l‹nh: %s - Thu’ %d%%", sV.szOwner[0] ? sV.szOwner : "V´ chÒ", sV.nTax);
...
{	
	...
	AddChild(&m_yangzhou);	
	AddChild(&m_linan);	
	AddChild(&m_xiangyang);	//#tuong duong
	AddChild(&m_chengdu);	
	AddChild(&m_dali);		
	...
	if (Ini.Load(szBuffer))
	{
		...
		m_xiangyang.Init(&Ini, "PureTextBtn"); //#tuong duong
	...
	if (g_pCoreShell)
	{
		...
		if (Ini.Load(WORLD_MAP_INFO_FILE))
		{
			...
			if (Ini.GetString("List", "WorldMapImage", "", szBuffer, sizeof(szBuffer)))	
			{
				...
				//sprintf(szBuffer, szTongInfo, "V´ h÷ ");
...
void KWorldMapLocs::PaintHover(int nAbsLeft, int nAbsTop, int nWndWidth)
{
	...
	int nLen;
	if (szLoai[0])
		nLen = _snprintf(szText, sizeof(szText) - 1, "%s (%s) - b m Æ” chπy tÌi", l.szName, szLoai);
	else
		nLen = _snprintf(szText, sizeof(szText) - 1, "%s - b m Æ” chπy tÌi", l.szName);
	if (nLen < 0)
