...
void KUiRankData::LoadScheme(const char* pScheme)
{
	...
	if (Ini.Load(Buff))
	{
		...
		for (i = 0; i < MISSION_STATNUM; i ++)
		{
			...
			m_TopTenPlayNameTxt[i].SetSize(strlen(Buff)*(m_nTitleFont/2) + 2,m_nTitleFont+1);//add by phong kiÒu fix tªn top 10 bÞ thiÕu
...
void KUiRankData::Breathe()
{
	...
	if(tmp.szMissionName[0])
	{
		...
		m_SelfCurGradeVal.SetIntText(tmp.nParam[6]);//®iÓm tÝch luü
		m_SelfCurDeathVal.SetIntText(tmp.nParam[4]);//sè lÇn chÕt
		m_SelfCurMaxKillVal.SetIntText(tmp.nParam[5]);//max liªn tr¶m

		m_SelfInfoObj[PkPlayer].SetIntText(tmp.nParam[2]);//kill player
		m_SelfInfoGrade[PkPlayer].SetIntText(tmp.nParam[13]);//tÝch luü kill player

		m_SelfInfoObj[NpcKill].SetIntText(tmp.nParam[3]);//kill npc
		m_SelfInfoGrade[NpcKill].SetIntText(tmp.nParam[14]);//tÝch luü kill npc

		m_SelfInfoObj[MaxKill].SetIntText(tmp.nParam[7]);//liªn tr¶m hiÖn t¹i
		m_SelfInfoGrade[MaxKill].SetIntText(tmp.nParam[15]);//tÝch luü liªn tr¶m hiÖn t¹i

		m_SelfInfoObj[GetItem].SetIntText(tmp.nParam[12]);//nhÆt b¶o vËt
		m_SelfInfoGrade[GetItem].SetIntText(tmp.nParam[16]);//tÝch luü nhÆt b¶o vËt

		m_SelfInfoObj[GetFlag].SetIntText(tmp.nParam[9]);//nhÆt cê
		m_SelfInfoGrade[GetFlag].SetIntText(tmp.nParam[17]);//tÝch luü nhÆt cê
		...
		for (int i = 0; i < MISSION_STATNUM; i ++)
		{
			...
			if(tmp.Name[0])
			{
				...
				tmp.nParam[MISSION_PARAM_AVAILABLE]?m_TopTenPlayNameTxt[i].SetText(tmp.Name):m_TopTenPlayNameTxt[i].Clear();//tªn player
				tmp.nParam[MISSION_PARAM_AVAILABLE]?m_TopTenGradeVal[i].SetIntText(tmp.nParam[6]):m_TopTenGradeVal[i].Clear();//tÝch luü
				tmp.nParam[MISSION_PARAM_AVAILABLE]?m_TopTenDeathVal[i].SetIntText(tmp.nParam[4]):m_TopTenDeathVal[i].Clear();//sè lÇn chÕt
				tmp.nParam[MISSION_PARAM_AVAILABLE]?m_TopTenMaxKillVal[i].SetIntText(tmp.nParam[5]):m_TopTenMaxKillVal[i].Clear();//max liªn tr¶m
				tmp.nParam[MISSION_PARAM_AVAILABLE]?m_TopTenPkPlayerVal[i].SetIntText(tmp.nParam[2]):m_TopTenPkPlayerVal[i].Clear();//kill player
				tmp.nParam[MISSION_PARAM_AVAILABLE]?m_TopTenKillNpcVal[i].SetIntText(tmp.nParam[3]):m_TopTenKillNpcVal[i].Clear();//kill npc
