...
KUiGetString* KUiGetString::OpenWindow(const char* pszTitle, const char* pszInitString, KWndWindow* pRequester, unsigned int uParam, int nMinLen, int nMaxLen, BOOL bNum)
{
	if (m_pSelf == NULL)
	{
...
int KUiGetString::WndProc(unsigned int uMsg, KUPARAM uParam, KNPARAM nParam)
{
	int nRet = 0;
	switch(uMsg)
	{
...
void KUiGetString::OnOk()
{
	...
	if (m_bNum)
	{
		...
		{
			UIMessageBox(((nLen < m_nMinLen) ? "Qu¸ Ýt ký tù" : "NhËp ký tù qu¸ dµi nªn kh«ng thÓ thùc hiÖn"));
		}
	}
	else
	{
