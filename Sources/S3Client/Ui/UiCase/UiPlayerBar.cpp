...
int KUiPlayerBar::ms_nLeftIni = 0;	// [NHOMTREN 13/09 X]
int KUiPlayerBar::ms_nDaDichX = 0;
extern "C" int JxSdl_AnToanTren(void);	// KSdlApp.cpp: mep tren VUNG AN TOAN (toa do khung ve)
extern "C" int JxSdl_AnToanTrai(void);	// [NHOMTREN 13/09 X] mep trai
...
{
	int nDichY = 0, nDichX = 0, nX = 0, nY = 0, i, nThem, nThemX;
	...
	// [NHOMTREN 13/09 X] khung [Main] neo GIUA ngang (bo cuc chu) nen tren man rong hon 1040 no doi sang phai -> dong ho, song,
	// nut an icon, day buff troi vao giua (anh Fold 7 13/09: dong ho o x = 200). Keo nhom ve mep trai nhu tren may ao.
	nDichX = nX - ms_nLeftIni - JxSdl_AnToanTrai();
	nThem = nDichY - ms_nDaDichY;
	nThemX = nDichX - ms_nDaDichX;
	if (nThem == 0 && nThemX == 0)
		return;
	ms_nDaDichY = nDichY;
	ms_nDaDichX = nDichX;
	...
	for (i = 0; i < MAX_BUTTON_STATE; i++)
	{
		...
		m_StateImg[i].SetPosition(nX - nThemX, nY - nThem);
		m_StateLife[i].GetPosition(&nX, &nY);
		m_StateLife[i].SetPosition(nX - nThemX, nY - nThem);
	}
}
