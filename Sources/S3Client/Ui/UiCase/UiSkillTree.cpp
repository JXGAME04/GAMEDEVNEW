...
void KUiSkillTree::LoadConfig(KIniFile* pIni)
{
	...
	for (i = 0; i < SKILLTREE_SHORTCUT_SKILL_COUNT; i++)
	{
		...
		pIni->GetStruct(SKILLTREE_SHORTCUT_SKILL_SAVE, szShortcutName, &ms_ShortcutSkills[i], sizeof(KUiSkillData));
		...
		if (ms_ShortcutSkills[i].uGenre != CGOG_NOTHING)
		{
			...
			for (j = 0; j < i; j++)
			{
				if (ms_ShortcutSkills[i].uId == ms_ShortcutSkills[j].uId && ms_ShortcutSkills[i].uGenre == ms_ShortcutSkills[j].uGenre && ms_ShortcutSkills[i].IS_LEFT_SKILL == ms_ShortcutSkills[j].IS_LEFT_SKILL)
				{
	...
	do
	{
		...
		if (bEncounter)
		{
			for (i = 0; i < SKILLTREE_SHORTCUT_SKILL_COUNT; i++)
			{
				if (ms_ShortcutSkills[i].uGenre == CGOG_NOTHING || (!bLeft) != (!ms_ShortcutSkills[i].IS_LEFT_SKILL))
					continue;
				for (j = 0; j < nNum; j++)
				{
					if (ms_ShortcutSkills[i].uId == Skills[j].uId && ms_ShortcutSkills[i].uGenre == Skills[j].uGenre)
					{
						break;
					}
				}
		...
		pIni->GetStruct("Player", bLeft ? "LeftSkill" : "RightSkill", &Skill, sizeof(KUiGameObject));
...
void KUiSkillTree::DirectSkillShortcutKey(int nSkillID)
{
	if (nSkillID <= 0)
		return;
	...
	int	x, y;
	Wnd_GetCursorPos(&x, &y);
	g_pCoreShell->UseSkill(x, y, nSkillID);
}

void KUiSkillTree::DirectHandleShortcutKeyCastB(int nIndex, int nNpcIdx)
{
	if (nIndex < 0 || nIndex >= SKILLTREE_SHORTCUT_SKILL_COUNT || nNpcIdx < 0)
		return;

	if (ms_ShortcutSkills[nIndex].uGenre != CGOG_NOTHING)
	{
		int	x, y;
		Wnd_GetCursorPos(&x, &y);
		g_pCoreShell->UseSkillCastB(x, y, ms_ShortcutSkills[nIndex].uId, nNpcIdx);
	}
}
