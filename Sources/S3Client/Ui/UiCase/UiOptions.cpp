...
void KUiOptions::StoreSetting()
{
	...
	if (pSetting)
	{
		for (int i = 0; i < OPTION_INDEX_COUNT; i++)
		{
			pSetting->WriteInteger(OPTIONS_SAVE_SECTION, ls_ToggleOptionName[i], m_ToggleItemList[i].bEnable);
		}
		//pSetting->WriteInteger(OPTIONS_SAVE_SECTION, ls_ToggleOptionName[OPTION_I_QUALITY], true);
...
void KUiOptions::ToggleOption(int nIndex)
{
	...
	case OPTION_I_PERSPECTIVE:	
		...
		break;
	case OPTION_I_QUALITY:
		if(KUiOptions2::GetIfVisible() == NULL)
			KUiOptions2::OpenWindow();
		else
			KUiOptions2::CloseWindow();
		break;
	}
