...
#define SCHEME_INI		"UiMsgSel4.ini"
#define	Image_NPC_List			"\\settings\\ImageNpcList.ini"

KUiMsgSel4* KUiMsgSel4::m_pSelf =NULL;

KUiMsgSel4* KUiMsgSel4::OpenWindow(KUiQuestionAndAnswer* pContent, int nImage)
{
	if (m_pSelf == NULL)
	{
		m_pSelf = new KUiMsgSel4;
		if (m_pSelf)
			m_pSelf->Initialize();
	}
	if (m_pSelf)
...
void KUiMsgSel4::CloseWindow(bool bDestroy)
{
	if (m_pSelf)
	{
...
void KUiMsgSel4::LoadScheme(const char* pScheme)
{
	if (m_pSelf == NULL)
		return;
	char		Buff[128];
...
void KUiMsgSel4::OnClickMsg(int nMsg)
{
	CloseWindow(false);
...
void KUiMsgSel4::ChangeCurSel(bool bNext)
{
...
void KUiMsgSel4::Show(KUiQuestionAndAnswer* pContent, int nImage)
{
	if (pContent && pContent->AnswerCount >= 0)
	{
		...
		if (pSetting)
		{
			...
			m_ImageNPC.SetImage(ISI_T_SPR, szImage);
		}
