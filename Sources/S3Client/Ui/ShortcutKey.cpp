...
int LuaOpenWindow(Lua_State * L)
{
	...
	if (strWindow)
	{
		...
		case 25:
			...
			break;
		case 26:
			if(KUiCityWar::GetIfVisible())
			{
				KUiCityWar::CloseWindow(false);
			}
			else
			{
...
char* l_StatusList[] =
{
	...
	"pk",		//3 PK
	"horse",	//4 Âí
	"showplayername",	//5 ÏÔÊ¾Íæ¼ÒÃû×Ö
	"showplayerlife",	//6 ÏÔÊ¾Íæ¼ÒÉúÃü
	"showplayermana",	//7 ÏÔÊ¾Íæ¼ÒÄÚÁ¦
...
int LuaSwitchStatus(Lua_State * L)
{
	...
	if (strStatus)
	{
		...
		case 5:	//hien thi ten player npc
			...
			break;
		case 6:	//hien thi thanh m¸u player
			if (g_pCoreShell)
			{
		...
		case 8: //hiÓn thÞ th«ng tin
			...
			break;
		case 9:    //thiÓn thÞ tªn d­íi ®Êt
			if (g_pCoreShell)
			{
				g_pCoreShell->OperationRequest(GOI_SHOW_OBJ_NAME, 0, 
                    !g_pCoreShell->GetGameData(GDI_SHOW_OBJ_NAME, 0, 0));// hien ten obj duoi dat edit by phong kieu
...
#define GAME_CONFIG_STRING_INI "\\Ui\\GameIni.ini"
...
int LuaSet3D(Lua_State * L)
{
	...
	if (nBool)
	{
		IniString.GetString("Config", "3D", "ÓÎÏ·ÉèÎª3DÄ£Ê½£¬ÖØÐÂÆô¶¯ÓÎÏ·ºóÉúÐ§¡£", Msg.szMessage, sizeof(Msg.szMessage));
		Ini.WriteInteger("Client", "Represent", SCREEN_MODE_3D);
	}
	else
	{
		IniString.GetString("Config", "2D", "ÓÎÏ·ÉèÎª2DÄ£Ê½£¬ÖØÐÂÆô¶¯ÓÎÏ·ºóÉúÐ§¡£", Msg.szMessage, sizeof(Msg.szMessage));
...
int LuaSetFullWindow(Lua_State * L)
{
	...
	if (nBool)
	{
		IniString.GetString("Config", "FullWindow", "ÓÎÏ·Éè¶¨¸Ä±äÎªÈ«ÆÁÔËÐÐ£¬½«ÔÚÏÂ´ÎÆô¶¯ÓÎÏ·µÄÊ±ºòÉúÐ§¡£", Msg.szMessage, sizeof(Msg.szMessage));
		Ini.WriteInteger("Client", "FullScreen", 1);
	}
	else
	{
		IniString.GetString("Config", "Window", "ÓÎÏ·Éè¶¨¸Ä±äÎª´°¿ÚÔËÐÐ£¬½«ÔÚÏÂ´ÎÆô¶¯ÓÎÏ·µÄÊ±ºòÉúÐ§¡£", Msg.szMessage, sizeof(Msg.szMessage));
...
BOOL KShortcutKeyCentre::InitScript()
{
	...
	{
		BOOL result = LoadScript(UI_AUTOEXEC_SETTING_FILE);
		return result;
	}
