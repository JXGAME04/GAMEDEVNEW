...
bool KNetConnectAgent::ProcessSwitchGameSvrMsg(void* pMsgData)
{
	...
	if (dwIp && nPort)
	{
		...
		if (ConnectToGameSvr((const unsigned char*)&dwIp, nPort, &guid))
		{
			...
			UpdateClientRequestTime(true);//UpdateClientRequestTime(false);//mac dinh fix by phong ki襲 8 gs
			//g_bDisconnect = true; //n誹 kh玭g 頲 th?phng 竛 n祔 k誸 n鑙 l筰 t?u nh?login khi chuy觧 gs
			//g_NetConnectAgent.TobeDisconnect();
			return true;
		}
	}
	else
	{
		...
		strcpy(Msg.szMessage, "Л阯g ph輆 trc kh玭g 甶 頲. ");
