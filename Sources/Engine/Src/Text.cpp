...
//					为前靠还是后靠，0: 向后靠; 非0: 向前靠。 FIX lai cac buoc nhay bang 1 la okay, mac dinh china 2 ky tu
//	注释：Chinese GBK编码版本，此字符串中字符全部视为显示字符，不包含控制字符 Loi go tieng viet trong nay
//--------------------------------------------------------------------------
extern "C" ENGINE_API
...
int	TEncodeText(char* pBuffer, int nCount)
{
	...
	if (pBuffer)
	{
		...
		while(nReadPos < nCount)
		{
			...
			else if (cCharacter == KTC_INLINE_PIC)//them vao fix icon kenh chat
			{
	...
	int& nFace, int& nLastPos, int& nTotalLen, int nSkipLine, int nNumLineLimit, BOOL bPicSingleLine/* = FALSE*/ )
{
	...
	while(nPos < nCount)
	{
		...
		{
			//Fong Ki襲 da den day [05-05-2013]
			nFace++;//
			nTotalLen+=3;//
			WORD wPicIndex = *((WORD*)(pBuffer + nPos + 1));
			nPos += 1 + sizeof(WORD);
