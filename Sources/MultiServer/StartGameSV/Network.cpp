...
bool CNetwork::Create()
{
	...
	sConfigFile = sAppPath + "StartGameSV.cfg";
	...
	m_nDbLoginUid.resize( MAX_STRING_LEN );
	DWORD dwLen2 = theConfigFile.ReadString( "Network", "DbLoginUid", const_cast< char * >( m_nDbLoginUid.c_str() ), MAX_STRING_LEN, "" );
	m_nDbLoginUid.resize( dwLen2 + 1 );

	m_nDbLoginPwd.resize( MAX_STRING_LEN );
	DWORD dwLen3 = theConfigFile.ReadString( "Network", "DbLoginPwd", const_cast< char * >( m_nDbLoginPwd.c_str() ), MAX_STRING_LEN, "" );
	m_nDbLoginPwd.resize( dwLen3 + 1 );*/
