...
LONG WINAPI ExeptionFillert(LPEXCEPTION_POINTERS lpExceptionInfo)
{
	...
	return GenerateMiniDump(NULL, lpExceptionInfo, L"goddess");
}
                     ...
                     int       nCmdShow)
{
	...
	::EnableWindow( GetDlgItem( hwndMain, IDC_EDIT_PORT ), FALSE );
	::EnableWindow( GetDlgItem( hwndMain, IDC_EDIT_MAXNUM_ROLE ), FALSE );
	::EnableWindow( GetDlgItem( hwndMain, IDC_EDIT_BACKUP_SLEEP_TIME ), FALSE );
	::SetWindowText( hwndMain, "Goddess - [Start up...]" );
	::PostMessage( hwndMain, WM_CREATE_ENGINE, 0L, 0L ); //auto start goddess

	BOOL bRet;
	MSG msg;
	//https://docs.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-registerhotkey
	//if (RegisterHotKey(NULL, 1, MOD_CONTROL, 0x53))  //0x42 is 'b' https://docs.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
    //{
    //    _tprintf(_T("Hotkey 'CTRL+S' registered, using MOD_NOREPEAT flag\n"));
    //}
	...
	//{
	//	if ( !IsWindow( hwndMain ) || !IsDialogMessage( hwndMain, &msg ) && !TranslateAccelerator(hwndMain, hAccel, &msg))
	//	{
