...
BOOL CChannelMgr::SomeoneChat(DWORD srcIP, const tagPlusSrcInfo& SrcInfo, const CHAT_SOMEONECHAT_CMD* pSomeoneChatCmd)
{
	...
	if (!g_ChatServer.FindPlayerByIpParam(NULL, srcIP, SrcInfo.lnID, NULL, NULL, &srcrole, NULL))
		return FALSE;
	...
	if (g_ChatServer.FindPlayerByRole(NULL, std::_tstring(pSomeoneChatCmd->someone), &hostconndup, NULL, &nameid, &param))
	{
	...
	*pSentlen = pSomeoneChatCmd->sentlen;
	...
	{	// [HAOHUU 04/09] bao nguoi gui biet loi nhan da duoc luu -- hien trong chinh cua so chat voi nguoi nhan
		...
		int nNotice = _snprintf(szNotice, sizeof(szNotice) - 1,
			"[H÷ thËng] %s hi÷n kh´ng tr˘c tuy’n, lÍi nhæn Æ∑ Æ≠Óc l≠u vµ sœ chuy”n khi %s Æ®ng nhÀp.",
	...
	memset(fkszLogChatM, 0, sizeof(fkszLogChatM));
	memcpy(fkszLogChatM, pSomeoneChatCmd + 1, pSomeoneChatCmd->sentlen);
	//
	SYSTEMTIME systm;
	::GetLocalTime(&systm);
	TCHAR buff[1024];
...
BOOL CChannelMgr::ChannelChat(DWORD srcIP, const tagPlusSrcInfo& SrcInfo, const CHAT_CHANNELCHAT_CMD* pChannelChatCmd)
{
	...
	if (!g_ChatServer.FindPlayerByIpParam(NULL, srcIP, SrcInfo.lnID, NULL, &srcacc, &srcrole, NULL))
		return FALSE;
	...
	for (IP2OPSMAP::iterator itIP = m_mapIp2Ops.begin(); itIP != m_mapIp2Ops.end(); itIP++)
	{
		...
		for (BLOCKOPLIST::iterator itOp = rOps.begin(); itOp != rOps.end(); itOp++)
		{
			...
			case _BLOCKCHANNELOP::op_querychannelid:
				...
				Block_QueryChannelID(pTheOp->channelname, ip, pTheOp->param, pTheOp->nameid, ip);
...
BOOL CChannelMgr::Block_QueryChannelID(const std::_tstring& channel, DWORD ip, unsigned long param, DWORD nameid, DWORD nServerip)
{
	CNetConnectDup conndup;
	std::_tstring rolename;
	rTRACE("Block_QueryChannelID Ip:%d - nServerip:%d", ip, nServerip);
	if (!g_HostServer.FindPlayerByIpParam(NULL, ip, param, &conndup, NULL, &rolename, NULL))
	{
		return FALSE;
	}
	std::_tstring channname = MakeChannelName(channel, nServerip);
	if (channname.empty())
	{
		rTRACE("Query ChannelID error : name is null :%s from (%08X,%08X) %s", channname.c_str(), nServerip, param);
		return FALSE;
	}
	DUMMY_AUTOLOCKWRITE(m_lockChannel);
	DWORD channid = -1;
	...
	{//channel exist
		...
		dTRACE("Query Channel exists: %s name:%s from (%08X, %08X, %08X)", channel.c_str(), channname.c_str(), nServerip, param, channid);
	}
	else
	{
		if (channname[0] != m_chChannEsc)
		...
		//create the channel
		channid = GenChannID();
		dTRACE("Query Channel Noexists and create: %s name:%s from (%08X, %08X, %08X)", channel.c_str(), channname.c_str(), nServerip, param, channid);
		if (channid != DWORD(-1))
		{
			m_mapChann2ID[channname] = channid;
	...
	if (channid != DWORD(-1))
	{
		if (pMapI2P)
		{
			PLAYERSMAP& rPlayers = (*pMapI2P)[nServerip];
...
BOOL CChannelMgr::Block_Subscribe(DWORD ip, unsigned long param, DWORD channelid)
{
	if (channelid == -1)
	{
		dTRACE("Subscribe Channel Error A <%08X> by (%08X,%08X)", channelid, ip, param);
		return FALSE;
	}
	...
	if (it == m_mapChannid2Info.end())
	{
		dTRACE("Subscribe Channel no Exists A <%08X> by (%08X,%08X)", channelid, ip, param);
		return FALSE;
	}
	...
	if (itRole == rPlayers.end())
	{
		dTRACE("Subscribe Channel no Exists B <%08X> by (%08X,%08X)", channelid, ip, param);
		return FALSE;
	}
...
BOOL CChannelMgr::Block_ClearPlayer(DWORD ip, unsigned long param)
{
	...
	while (itChann != m_mapChannid2Info.end())
	{
		...
		IP2PLAYERSMAP::iterator itIP = rChannInfo.mapIp2Players.find(ip);
		if (itIP == rChannInfo.mapIp2Players.end())	//√ª’“µΩ¥ÀGS
			continue;
		...
		if (!rPlayers.empty())	//¥ÀGS…œªπ”–»À
			continue;

		//GS…œŒﬁ»À£¨…æ≥˝¥ÀGS
		rChannInfo.mapIp2Players.erase(itIPMe);

		if (rChannInfo.stock || rChannInfo.gmuse)	//∆µµ¿≤ªø……æ
			continue;

		if (!rChannInfo.mapIp2Players.empty())	//»Áπ˚¥À∆µµ¿ªπ”–GS
			continue;
...
BOOL CChannelMgr::B_QueryChannelID(const std::_tstring& channel, DWORD ip, unsigned long param, DWORD nameid, DWORD nServerip)
{
	return Block_QueryChannelID(channel, ip, param, nameid, nServerip);
}
...
int CChannelMgr::DeliverOfflineMsgs(const char* role, DWORD ip, unsigned long param, DWORD nameid)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		int nPre = _snprintf(szText, 63, "[LÍi nhæn lÛc %02d/%02d %02d:%02d] ", r.day, r.mon, r.hour, r.minute);
		if (nPre < 0) nPre = 0;
		int nLen = r.textlen;
