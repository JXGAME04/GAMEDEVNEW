...
int	g_nTongPSSize[defTONG_PROTOCOL_SERVER_NUM] = 
{
	...
	sizeof(STONG_CHANGE_TITLE_COMMAND),			// enumC2S_TONG_CHANGE_TITLE
	sizeof(STONG_ACCEPT_TITLE_COMMAND),			// enumC2S_TONG_ACCEPT_TITLE
	sizeof(STONG_ACCEPT_SEX_TITLE_COMMAND),		// enumC2S_TONG_ACCEPT_SEX_TITLE
	sizeof(STONG_MONEY_COMMAND),
	sizeof(STONG_MONEY_COMMAND),
	sizeof(STONG_MONEY_COMMAND),
	sizeof(STONG_CHANGE_CAMP_COMMAND),			// enumC2S_TONG_CHANGE_CAMP
	sizeof(STONG_CHANGE_RECRUIT_COMMAND),		// enumC2S_TONG_CHANGE_RECRUIT
	sizeof(STONG_GET_EXTPOINT_COMMAND),			// enumC2S_GET_EXTPOINT
	...
	sizeof(STONG_JX2_ZHAOMU_COMMAND),		// enumC2S_TONG_JX2_ZHAOMU
};
...
void CTongConnect::Proc0_Normal(const void* pData, size_t size)
{
	EXTEND_HEADER* pHeader = (EXTEND_HEADER*)pData;
...
void CTongConnect::Proc1_Normal_EnterGame(const void* pData, size_t size)
{
	...
	rTRACE("Tong:Player Login Acc:%s Role:%s (IP:%08X, lnID:%08X, nSelServer:%d)", strAcc.c_str(), strRole.c_str(), GetIP(), pEnterGame2->lnID, pEnterGame2->nSelServer);
}

void CTongConnect::Proc1_Normal_LeaveGame(const void* pData, size_t size)
{
	...
	rTRACE("Tong player logout nSelServer:%d - Player:%s", pLeaveGame->nSelServer, strAcc.c_str());
}
...
void CTongConnect::Proc0_Tong(const void* pData, size_t size)
{
	...
	case enumC2S_TONG_JX2_TONG_OP:
		...
		break;
	case enumC2S_TONG_JX2_ZHAOMU:
		JX2_ProcZhaoMu(this, pData);
		break;
	...
	case enumC2S_TONG_ADD_MEMBER:
		{
			...
			rTRACE("gioi tinh da nhan: %d",pAdd->m_btSex);
			int nRet = g_cTongSet.AddMember(szPlayerName, szTongName, pAdd->m_btSex);
			// 成员添加成功
			if (nRet >= 0)
			{
...
//				rTRACE("Khong tim thay player");
			...
			{{
				...
				if (g_TongServer.FindPlayerByRole(NULL, std::_tstring(szName), &conndup, NULL, &nameid, &param))
				{
					...
					if (tongconndup.IsValid())
					{
						rTRACE("da tim thay player");
						STONG_CHECK_GET_TITLE_POWER_SYNC	sCheck;
						sCheck.ProtocolFamily	= pf_tong;
						sCheck.ProtocolID		= enumS2C_TONG_CHECK_CHANGE_TITLE_POWER;
						sCheck.m_dwTongNameID	= pChange->m_dwTongNameID;
						sCheck.m_btFigure		= pChange->m_btFigure;
						sCheck.m_btPos			= pChange->m_btPos;
						sCheck.m_dwParam		= param;
						memcpy(sCheck.m_szName, szName, sizeof(szName));
						memcpy(sCheck.m_szTitle, szTitle, sizeof(szTitle));
						
						tongconndup.SendPackage((const void *)&sCheck, sizeof(sCheck));
					}
					break;
				}
//				rTRACE("Loi khong the thay doi danh hieu");
				// 不在线 not find 或者 tong is not connect 当作不在线
				STONG_CHANGE_TITLE_FAIL_SYNC	sFail;
				sFail.ProtocolFamily	= pf_tong;
				sFail.ProtocolID		= enumS2C_TONG_CHANGE_TITLE_FAIL;
	...
	case enumC2S_TONG_ACCEPT_TITLE:
		{
			STONG_ACCEPT_TITLE_COMMAND	*pAccept = (STONG_ACCEPT_TITLE_COMMAND*)pData;
			g_cTongSet.AcceptTitle(pAccept);
		}
		break;
	case enumC2S_TONG_ACCEPT_SEX_TITLE:
		{
			STONG_ACCEPT_SEX_TITLE_COMMAND	*pAccept = (STONG_ACCEPT_SEX_TITLE_COMMAND*)pData;
			g_cTongSet.AcceptSexTitle(pAccept);
		}
		break;
	...
	case enumC2S_TONG_MONEY_SND:
		...
		break;
	case enumC2S_TONG_CHANGE_RECRUIT:
		{
			STONG_CHANGE_RECRUIT_COMMAND	*pTongChange = (STONG_CHANGE_RECRUIT_COMMAND*)pData;
			g_cTongSet.ChangeRecruit(pTongChange);
		}
		break;
	case enumC2S_TONG_CHANGE_LEVEL:
		{
			STONG_CHANGE_LEVEL_COMMAND	*pTongChange = (STONG_CHANGE_LEVEL_COMMAND*)pData;
	...
	case enumC2S_TONG_CHANGE_WAYEDIT:
		{
			...
			g_cTongSet.ChangeTongWayEdit(pTongChange);
		}
		break;
	case enumC2S_TONG_CHANGE_NEXTTARGER:
		{
			STONG_CHANGE_NEXTTARGET_COMMAND	*pTongChange = (STONG_CHANGE_NEXTTARGET_COMMAND*)pData;
			g_cTongSet.ChangeTongNextTarger(pTongChange);
		}
		break;
	case enumC2S_TONG_CHANGE_CAMP:
		{
			STONG_CHANGE_CAMP_COMMAND	*pTongChange = (STONG_CHANGE_CAMP_COMMAND*)pData;
			g_cTongSet.ChangeCamp(pTongChange);
		}
		break;
	case enumC2S_GET_EXTPOINT:
		{
...
void CTongConnect::Proc1_Friend_AskAddFriend(const void* pData, size_t size)
{
	...
	if (g_TongServer.FindPlayerByRole(NULL, dstrole, &dsthostconndup, NULL, &dstnameid, &dstparam))
	{
...
void CTongConnect::Proc1_Friend_Associate(const void* pData, size_t size)
{
	...
	DWORD nameid = 0;
	unsigned long param = 0;
	if (g_TongServer.FindPlayerByRole(NULL, role1, &conndup, NULL, &nameid, &param))
	{
	...
	if (pFa->bidir)
	{
		...
		if (g_TongServer.FindPlayerByRole(NULL, role2, &conndup2, NULL, &nameid2, &param2))
		{
...
void CTongConnect::Proc0_ServerCommunity(const void* pData, size_t size)
{
	EXTEND_HEADER* pHeader = (EXTEND_HEADER*)pData;
	if (pHeader->ProtocolID == relay_s2c_updateserver)
	{
		...
		rTRACE("---Tong start Okay nServerIndex:%d, nServerIndex:%08X ---", nServerIndex, nGsNumber);
	}
}

BOOL CTongConnect::FindPlayerByRole(const std::_tstring& role, std::_tstring* pAcc, DWORD* pNameID, unsigned long* pParam, std::_tstring* pRealRole)
{
...
BOOL CTongConnect::FindPlayerByParam(unsigned long param, std::_tstring* pAcc, std::_tstring* pRole, DWORD* pNameID)
{
	DUMMY_AUTOLOCKREAD(m_lockPlayer);
...
BOOL CTongConnect::FindPlayerByAcc(const std::_tstring& acc, std::_tstring* pRole, DWORD* pNameID, unsigned long* pParam, std::_tstring* pRealAcc)
{
	DUMMY_AUTOLOCKREAD(m_lockPlayer);
