...
#define		defTONG_MASTER_TITLE			"Bang chÒ"
#define		defTONG_DIRECTOR_TITLE			"Tr≠Îng l∑o"
#define		defTONG_MANAGER_TITLE			"ß≠Íng chÒ"
#define		defTONG_MEMBER_TITLE			"M´n Æ÷"
#define		defTONG_NAME_SAY_ON_CHANNEL		"C´ng bË"
...
BOOL	CTongControl::GetTongHeadInfo(STONG_HEAD_INFO_SYNC *pInfo)
{
	...
	// ========= add by Fong Ki“u =========
	pInfo->ms_nStatusGuide = m_nRecruit; //trπng th∏i tuy”n ng≠Íi bang hÈi
	...
	pInfo->ms_nTongLevel = m_nLevel; //Æºng c p bang hÈi
...
BOOL	CTongControl::Instate(STONG_INSTATE_COMMAND *pInstate, STONG_INSTATE_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		if (pSync->m_szTitle[0])
			sprintf(szMsg, "%s Æ≠Óc bÊ nhi÷m ch¯c vﬁ %s ", szName, pSync->m_szTitle);
		else
		{
			switch (pSync->m_btNewFigure)
			{
			case enumTONG_FIGURE_DIRECTOR:
				sprintf(szMsg, "%s Æ≠Óc phong ch¯c vﬁ %s ", szName, defTONG_DIRECTOR_TITLE);
				break;
			case enumTONG_FIGURE_MANAGER:
				sprintf(szMsg, "%s Æ≠Óc phong ch¯c vﬁ %s ", szName, defTONG_MANAGER_TITLE);
				break;
			case enumTONG_FIGURE_MEMBER:
				sprintf(szMsg, "%s Æ≠Óc phong ch¯c vﬁ M´n Æ÷ ", szName);
				break;
			}
		}
...
BOOL	CTongControl::Instate(STONG_INSTATE_COMMAND *pInstate, STONG_INSTATE_SYNC *pSync)
{
	...
	{
		if (pSync->m_szTitle[0])
			sprintf(szMsg, "%s±ª»Œ√¸Œ™%s£°", szName, pSync->m_szTitle);
		else
		{
			switch (pSync->m_btNewFigure)
			{
			case enumTONG_FIGURE_DIRECTOR:
				sprintf(szMsg, "%s±ª»Œ√¸Œ™≥§¿œ£°", szName);
				break;
			case enumTONG_FIGURE_MANAGER:
				sprintf(szMsg, "%s±ª»Œ√¸Œ™∂”≥§£°", szName);
				break;
			case enumTONG_FIGURE_MEMBER:
				sprintf(szMsg, "%s±ª»Œ√¸Œ™∆’Õ®∞Ô÷⁄£°", szName);
				break;
			}
		}
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string("π´∏Ê"), std::string(szMsg));
	}
...
BOOL	CTongControl::Kick(STONG_KICK_COMMAND *pKick, STONG_KICK_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg, "%s bi trÙc xu t kh·i bang hÈi ", szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(defTONG_NAME_SAY_ON_CHANNEL), std::string(szMsg));
	}
...
BOOL	CTongControl::Leave(STONG_LEAVE_COMMAND *pLeave, STONG_LEAVE_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg, "%s rÍi kh·i bang ph∏i.", szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(defTONG_NAME_SAY_ON_CHANNEL), std::string(szMsg));
	}
...
BOOL	CTongControl::AcceptMaster(STONG_ACCEPT_MASTER_COMMAND *pAccept)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg, "%s chuy”n nh≠Óng vﬁ tr› %s cho %s", szOldMaster, defTONG_MASTER_TITLE, szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(defTONG_NAME_SAY_ON_CHANNEL), std::string(szMsg));
	}
...
BOOL CTongControl::DBChangeTitle(STONG_ACCEPT_TITLE_COMMAND *pAccept)
{
	...
	if (pAccept->m_btFigure == enumTONG_FIGURE_DIRECTOR)
	{
		...
		sprintf(szMsg, "Thay ÆÊi danh hi÷u %s cho %s %s thµnh c´ng ", szTitle, defTONG_DIRECTOR_TITLE, m_szDirectorName[i]);
	}
	else if (pAccept->m_btFigure == enumTONG_FIGURE_MANAGER)
	{
		for (i = 0; i < defTONG_MAX_MANAGER; i++)
		...
		sprintf(szMsg, "Thay ÆÊi danh hi÷u %s cho %s %s thµnh c´ng ", szTitle, defTONG_MANAGER_TITLE, m_szManagerName[i]);
	}
	else
	{
		return FALSE;
...
//		rTRACE("title sex o s3relay 1");			// khong chay tu day
		...
		sprintf(szMsg, "Thay ÆÊi danh hi÷u n˜ trong bang thµnh %s", szTitle);
	}
	else
	{
		...
		sprintf(szMsg, "Thay ÆÊi danh hi÷u nam trong bang thµnh %s", szTitle);
	}
...
BOOL CTongControl::ChangeMoney( STONG_MONEY_COMMAND* pMoney, STONG_MONEY_SYNC *Sync)
{
	...
	case enumC2S_TONG_MONEY_SAVE:
		...
		if(channid > 0)
		{
			sprintf(szMsg, "%s Æ∑ cËng hi’n cho bang %d l≠Óng ", pMoney->m_szName, pMoney->m_dwMoney);
			WriteLogFile(szMsg);
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(defTONG_NAME_SAY_ON_CHANNEL), std::string(szMsg));
		}
		break;
	case enumC2S_TONG_MONEY_GET:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "%s Æ∑ rÛt %d l≠Óng!!!", pMoney->m_szName, pMoney->m_dwMoney);
			WriteLogFile(szMsg);
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(defTONG_NAME_SAY_ON_CHANNEL), std::string(szMsg));
		}
		break;
	case enumC2S_TONG_MONEY_SND:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "%s ph∏t %d l≠Óng cho t t c∂ bang.", pMoney->m_szName, pMoney->m_dwMoney);
			WriteLogFile(szMsg);
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(defTONG_NAME_SAY_ON_CHANNEL), std::string(szMsg));
		}
		break;
...
BOOL CTongControl::DBChangeTongLevel(STONG_CHANGE_LEVEL_COMMAND *pChange)
{
	...
	if (pChange->m_btFigure == enumTONG_FIGURE_MASTER || pChange->m_btFigure == enumTONG_FIGURE_DIRECTOR)
	{
		...
		sprintf(szMsg, "Bang hÈi thay ÆÊi Æºng c p thµnh %d.", pChange->m_nTongLevel);
...
BOOL CTongControl::DBChangeTongWayEdit(STONG_CHANGE_WAYEDIT_COMMAND *pChange)
{
	...
	if (pChange->m_btFigure == enumTONG_FIGURE_MASTER || pChange->m_btFigure == enumTONG_FIGURE_DIRECTOR)
	{
		...
		sprintf(szMsg, "%s thay ÆÊi ti™u ch› thµnh %s.", pChange->m_szName, pChange->m_szWayEdit);
...
BOOL CTongControl::DBChangeTongNextTarget(STONG_CHANGE_NEXTTARGET_COMMAND *pChange)
{
	...
	if (pChange->m_btFigure == enumTONG_FIGURE_MASTER || pChange->m_btFigure == enumTONG_FIGURE_DIRECTOR)
	{
		...
		sprintf(szMsg, "%s thay ÆÊi mÙc ti™u thµnh %s.", pChange->m_szName, pChange->m_szNextTarget);
...
BOOL CTongControl::DBChangeRecruit(STONG_CHANGE_RECRUIT_COMMAND *pChange)
{
	...
	if (pChange->m_btFigure == enumTONG_FIGURE_MASTER || pChange->m_btFigure == enumTONG_FIGURE_DIRECTOR)
	{
		...
		if (pChange->m_btRecruit == 0)
			sprintf(szMsg, "%s Æ„ng ch’ ÆÈ tuy”n thµnh vi™n bang.", pChange->m_szName);
		else
			sprintf(szMsg, "%s mÎ ch’ ÆÈ tuy”n thµnh vi™n bang.", pChange->m_szName);
...
BOOL CTongControl::DBChangeCamp(STONG_CHANGE_CAMP_COMMAND *pChange) //t›nh n®ng nµy Æ∑ lµm Æ≠Óc tr™n t t c∂ c∏c gs
{
	if (!pChange)
		return FALSE;
	...
	if (pChange->m_btFigure == enumTONG_FIGURE_MASTER || pChange->m_btFigure == enumTONG_FIGURE_DIRECTOR)
	{
		...
		if (pChange->m_btCamp == camp_justice)
			sprintf(szMsg, "%s thay ÆÊi phe bang thµnh lÀp chÒ ch›nh ph∏i.", pChange->m_szName);
		else if (pChange->m_btCamp == camp_evil)
			sprintf(szMsg, "%s thay ÆÊi phe bang thµnh lÀp chÒ tµ ph∏i.", pChange->m_szName);
		else if (pChange->m_btCamp == camp_balance)
			sprintf(szMsg, "%s thay ÆÊi phe bang thµnh lÀp chÒ trung lÀp.", pChange->m_szName);
