...
*/
	...
	strcpy(userlogin.Version, ACCOUNT_CURRENT_VERSION);
...
UINT CGamePlayer::VerifyAccount()
{
	...
	cprintf( "CGamePlayer::VerifyAccount Failed! [Already login]\n" );
			...
			_RemoveAccountLixiAn_ToGameServer(m_sAccountName.c_str());//#uy thac
...
bool CGamePlayer::_RemoveAccountLixiAn_ToGameServer(const char *szAccName)//#uy thac
{
	bool ok = false;
	...
	int fkMapIDForGS[16];
	memset(fkMapIDForGS, 0, sizeof(fkMapIDForGS));
	fkMapIDForGS[0] = 1;
	fkMapIDForGS[1] = 11;
	fkMapIDForGS[2] = 37;
	fkMapIDForGS[3] = 78;
	...
	for(int i = 0; i < 16; i++)
	{
		if(fkMapIDForGS[i] > 0)
		{
			pGServer = CGameServer::QueryServer(fkMapIDForGS[i]); //#uy thac o tat ca cac gs max 16
			if ( pGServer )
			{
				CGamePlayer::Add( m_sAccountName.c_str() , ( IPlayer * )this );
				pGServer->Attach( m_sAccountName.c_str() );
				m_nAttachServerID = pGServer->GetID();
				m_theDataQueue[enumOwnerPlayer].Empty();
				ok = pGServer->DispatchTask( CGameServer::enumRemoveAccountLixian,  m_sAccountName.c_str(), m_sAccountName.size(), 0);
				m_nExtPoint = -1;
			}
		}
	}
