...
int CBishopApp::Run()
{
	...
	{
		MessageBox(NULL, "Protocol Version is illegal", "Information",MB_OK | MB_ICONERROR);
		return 0;
	}
		...
		LPARAM lParam )
{
	...
	case WM_INITDIALOG:
		...
		EnterToAffirm(hwndDlg ); //auto start login bishop
...
*/
	...
	if ((int)(atoi(szText)) > KPROTOCOL_EXPIRATION_DATE)
	{
		MessageBox(hwndMain, "Protocol Version is illegal", "Information",MB_OK | MB_ICONERROR);
		return 0;
	}

	::EnableWindow( GetDlgItem( hwndMain, IDOK ), FALSE );
	::EnableWindow( GetDlgItem( hwndMain, IDC_EDIT_CLIENT_PORT ), FALSE );
	::EnableWindow( GetDlgItem( hwndMain, IDC_EDIT_GAMESVR_PORT ), FALSE );
	::EnableWindow( GetDlgItem( hwndMain, IDC_EDIT_ROLESVRIP ), FALSE );
	::EnableWindow( GetDlgItem( hwndMain, IDC_EDIT_ROLESVRPORT ), FALSE );
			
	::CheckDlgButton( hwndMain, IDC_CHECK_PLAYER, BST_CHECKED );
	::CheckDlgButton( hwndMain, IDC_CHECK_GAMESVR, BST_CHECKED );
			
	::CheckDlgButton( hwndMain, IDC_CHECK_ACCSVR, BST_CHECKED );
			
	UpdateVariable( hwndMain );

	if ( !m_pIntercessor->Create( hwndMain ) )
	{
	...
	{
		g_nTimer = ::SetTimer( hwndMain, g_nViewTimer, 5000, NULL );
		::SetWindowText( hwndMain, "Bishop - [Enable]" );
	}
