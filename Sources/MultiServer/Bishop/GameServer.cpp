...
bool CGameServer::DispatchTask( UINT nTask, const void *pData, size_t datalength, WORD nData )
{
	...
	case enumRemoveAccountLixian://#uy thac

		ok = _RemoveAccountLixian(pData, datalength);
...
bool CGameServer::_RequestSvrIP( const void *pData, size_t datalength )
{
	...
	DWORD dwIP = 0;
	IGServer *pGServer = CGameServer::QueryServer( pRSI->dwMapID, pRSI->nSerNoIndex);
...
bool CGameServer::ConsumeMoney( const char *pAccountName )
{
	...
	strcpy(user.Version, ACCOUNT_CURRENT_VERSION);
...
bool CGameServer::_RemoveAccountLixian(const void *pData, size_t datalength) //#uy thac
{
	tagGuidableInfo ginfo;
	ginfo.cProtocol = s2c_syncgamesvr_roleinfo_lixian;
	GenGuid( &( ginfo.guid ) );
	ginfo.nExtPoint = 0;
	ginfo.nChangePoint = 0;
	ginfo.datalength = datalength;

	m_thePackager.AddData( s2c_syncgamesvr_roleinfo_lixian, ( const char * )&ginfo, sizeof( tagGuidableInfo ) );

	m_thePackager.AddData( s2c_syncgamesvr_roleinfo_lixian, ( const char * )pData, datalength );

	CBuffer *pBuffer = m_thePackager.GetHeadPack( s2c_syncgamesvr_roleinfo_lixian );
	while ( pBuffer )
	{
		m_pGameSvrServer->SendData( m_lnIdentityID, pBuffer->GetBuffer(), pBuffer->GetUsed() );
		SAFE_RELEASE( pBuffer );
		pBuffer = m_thePackager.GetNextPack( s2c_syncgamesvr_roleinfo_lixian );		
	}
	SAFE_RELEASE( pBuffer );
	m_thePackager.DelData( s2c_syncgamesvr_roleinfo_lixian );
	return true;
}
...
IGServer *CGameServer::QueryServer( UINT nMapID, unsigned int nSerNoIndex)
{
	CCriticalSection::Owner locker( m_csMapIDAction );

	stdMapIDConvert::iterator it;

	if ( m_theMapIDConvert.end() != ( it = m_theMapIDConvert.find( nMapID ) ) ) //dang bi loi convert map nen khong vao
	{
		stdServerList& sl = ( *it ).second;
