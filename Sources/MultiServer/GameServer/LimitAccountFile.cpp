// LimitAccountFile.cpp: implementation of the LimitAccountFile class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "LimitAccountFile.h"
#include "assert.h"
#include <iostream>
#include <fstream>
#include <string>
...
CLimitAccountFile::CLimitAccountFile()
...
BOOL CLimitAccountFile::Initialize(const std::string& root, size_t threshold)
{
	if (m_threshold != 0)
		return FALSE;
...
BOOL CLimitAccountFile::Uninitialize()
{
...
size_t CLimitAccountFile::TraceLog(const void* pData, size_t size)
{
	if (m_threshold == 0)
		return 0;
...
BOOL CLimitAccountFile::PrepareFile()
{
	assert(m_threshold != 0);
	...
	sprintf(szDate, "\\limit",CH_SEPARATOR);
	std::string path;
...
ULONG CLimitAccountFile::BeginBatch()
{
	if (m_threshold == 0)
		return 0;
...
ULONG CLimitAccountFile::EndBatch()
{
	if (m_threshold == 0)
		return -1;
...
BOOL CLimitAccountFile::IsBatched()
{
	if (m_threshold == 0)
		return FALSE;
...
void CLimitAccountFile::KPIGetExePath( LPSTR lpExePath, DWORD dwSize )
{
	if ( lpExePath )
	{
...
BOOL CLimitAccountFile::OpenUniDirectory(const std::string& dirname, std::string* pNewdir)
{
...
HANDLE CLimitAccountFile::CreateUniFile(const std::string& filename, std::string* pNewfile)
{
...
int CLimitAccountFile::GetCountHWID_Login(char* sHWID)
{
	int res = 0;
	std::string myText;
	std::ifstream MyReadFile(c_strfile.c_str());
	//char* ucharptr = reinterpret_cast<char*>(const_cast<void*>(sHWID));
	sprintf(sHWID,"%s,in",sHWID);
	while (std::getline (MyReadFile, myText)) 
	{
		if(strcmp(myText.c_str(), sHWID) == 0)
		{
			res++;
		}
	}
...
int CLimitAccountFile::GetCountHWID_Logout(char* sHWID)
{
	int res = 0;
	std::string myText;
	std::ifstream MyReadFile(c_strfile.c_str());
	//char* ucharptr = reinterpret_cast<char*>(const_cast<void*>(sHWID));
	sprintf(sHWID,"%s,out",sHWID);
	while (std::getline (MyReadFile, myText)) 
	{
		if(strcmp(myText.c_str(),sHWID) == 0)
		{
...
int CLimitAccountFile::Replace_Line(const char *oldline, const char* newline)
{
	return 0;
}
