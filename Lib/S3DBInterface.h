...
// new add by king
	...
	int		ifuyuanvalue;
	int		irebornvalue;
	int		ithuhangvalue; //chua su dung
	int		ipassrole;
	int		irankrole;
	int		iexitemrole;
	int		iexboxrole;
	...
	char	szStringduphong1[32];//#MateName
	char	szStringduphong2[32];
	char	szStringduphong3[32];
	char	szStringduphong4[32];
	char	szStringduphong5[32];
	char	szStringduphong6[32];
	char	szStringduphong7[32];
	char	szStringduphong8[32];
	char	szStringduphong9[32];
};
...
struct TRoleData
{
	...
	DWORD	dwVersion;
	bool	bBaseNeedUpdate;//Í¨ÖªÊÇ·ñÐèÒª¸üÐÂ
	//µ±´æÈ¡Ê±Èç¹ûOffsetµÈÓÚ-1£¬±íÊ¾²»¸üÐÂ¸ÃÊý¾Ý
	//Additional Info
	short	nFightSkillCount;
	short	nLiveSkillCount;
	short		nStateSkillCount;
	BYTE	nTaskCount;
	short	nItemCount;
	short	nFriendCount;
	int		dwTaskOffset;
...
struct TDBItemData
{
	...
	int iiduphong1; //#phi phong
	int iiduphong2; //#sè lÇn sö dông item
	int iiduphong3; //GetItemGlowLight
	int iiduphong4;
	int iiduphong5;
	int iiduphong6;
	int iiduphong7;
