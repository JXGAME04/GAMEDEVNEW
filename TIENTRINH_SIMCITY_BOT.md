...
Bot đạt cấp 80 (mỗi lần lên cấp, gate `pb_TrangBiTheoCap`) tự gọi đúng đường NPC môn phái: `show_kynang90(m_nCurFaction+1)` trong `hocvocong.lua` — nhận trọn bộ skill 90 của phái + skill 120 + chiêu 210, log `[BotSkill90]`, ép chọn lại chiêu đánh. Gọi lặp sau nạp-bot-cũ vô hại vì `KSkillList::Add` chỉ NÂNG cấp chiêu, không bao giờ hạ (KSkillList.cpp:434-447). DLL md5 `65d26311...` (bản Xa Phu lùi `.bak_2008_0300`).
