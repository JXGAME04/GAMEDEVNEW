...
- Gương repo khớp cây sống **112/112 tệp, 0 lệch byte**; `core.autocrlf=false` + `.gitattributes * -text` ⇒ git không đổi byte.
- **13/13 tệp** qua `syncheck.exe` (Lua 4.0 của chính engine JX1).
...
## Việc CÒN LẠI (chưa làm, có chủ đích)

1. **F11 cho Săn Boss Sát Thủ** — cần thêm `BuildSatThuText()` C++ phía client (`S3Client`). Theo Gate 3 (không sửa cây client của chủ game) ⇒ để đợt client riêng. Feature chạy được không cần F11.
2. **Bảng thưởng chưa đưa ra config** — `HD3_*` mới phủ lịch/giới hạn/cấp; bảng thưởng vẫn nằm trong script Linux (`award.lua`, `lib_killlevel.lua`). Muốn chỉnh thưởng từ `cauhinh_hoatdong.lua` là **một đợt riêng**.
3. Một số khoá `HD3_*` hiện chỉ dùng cho menu admin/thông báo, chưa nối ngược vào hằng số trong script Linux (script Linux hardcode) — xem `A7_require.md`.
