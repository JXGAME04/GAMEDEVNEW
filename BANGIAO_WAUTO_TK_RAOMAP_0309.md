...
| Tệp | md5 | Cỡ | Ghi chú |
|---|---|---|---|
| `bin\client\CoreClient.dll.moi` | **`9e7a516d`** | `2.529.792` | **(04/09 đêm — ĐỢT 5 + 5b sau phản biện, xem mục 9)** hỏi máy chủ vị trí địch (`[TKDich]`) khi không thấy ai; tập cha của `5600d7a9` (đợt 4: khu xuất quân + luật ngựa mới, mục 8). **Đi cùng `CoreServer.dll.moi bdd9bb46`** (server cũ thì client vẫn chạy như đợt 4). |
| `bin\server\CoreServer.dll.moi` | **`bdd9bb46`** | `18.299.392` | **(04/09 đêm — ĐỢT 5 + 5b)** = mã máy chủ đang chạy `b68899b2` (S13k, d59340c4; main không đổi mã server từ đó) + nhánh trả `[TKDich]` trong `c2sNeedCount` (chặn 4 s/người, 9 địch) + đặt chỗ id. Swap bằng `ChayGameServer.bat` (khởi động lại máy chủ). |
...
## 9. ĐỢT 5 (04/09 đêm) — VỊ TRÍ ĐỊCH TỪ MÁY CHỦ → `CoreServer.dll.moi bdd9bb46` + `CoreClient.dll.moi 9e7a516d` (**swap CẢ HAI**; số trong bảng dưới là sau đợt 5b)

**Phản biện độc lập (tác tử đọc diff + mã quanh, 04/09 21:28–21:45) và quyết định:**

| # | Phát hiện | Xử lý (đợt 5b, `ReverseTools\goi_va_wauto_tk_dich_server_0309b.py`) |
|---|---|---|
| 1 | Server loại cả hai vòng hậu doanh, trong khi client/bot chỉ loại trại **địch** — sợ mất địch đang đánh sát cổng nhà | **Giữ nguyên**: địch không thể vào trong trại mình (trap "vào trại" chặn), vòng 1.440 mps chỉ phủ lõi trại, cổng trại cách tâm 132 ô → không mất địch nào; đổi sang theo thế trận phải đọc thêm biến nhiệm vụ, thêm rủi ro. |
| 2 | `TK_DichXa` không kiểm ô "Đánh người" → tắt ô đó vẫn bị dắt tới người chơi địch mà tầng đánh không nhận | **Sửa**: tắt "Đánh người" → không dắt đi, không hỏi. |
| 3 | `FindPath` thăm dò gọi `StopPath()`: mọi ứng viên fail → đường đang đi bị huỷ, tầng rảo đợi tới 2,5 s mới tính lại; `-2` (lưới đang nạp) bị coi là "bỏ mục" | **Sửa**: thăm dò thất bại → `RemoveFlag + uDTPath = 0` để tính lại ngay; `FindPath < 0` → thử nhịp sau, không bỏ mục. |
| 4 | Biên chặn sát: server 90 khung vs client 5,5 s → máy chủ chậm nhịp là cứ hai lần hỏi mất một | **Sửa**: server chặn `GAME_FPS*4` (4 s), client hỏi 6 s. |
| 5 | Mỗi tin mới reset mục đang đi → hai địch cách đều làm nhân vật đảo chiều mỗi tin; mục "tới mà không thấy" bị chọn lại ngay | **Sửa**: giữ mục cũ nếu còn trong tin mới (đổi khi mục khác gần hơn ≥ 25%, luật `TK_SanNguoi`); nhớ id đã bỏ 30 s qua các tin. |
| 6 | Chỉ 6 mục gần nhất theo đường chim bay; 6 mục bên kia sông/tường đều bị bỏ trong khi địch xa hơn tới được không bao giờ được báo | **Sửa**: 9 mục (8 + 9×21 = 197 byte ≤ 200). |
| 7 | Không tránh cụm cửa ra trại địch 25 ô như bot (chống cắm cửa) | Thông tin — chủ quyết; hiện **không** tránh (đúng yêu cầu "xác định vị trí địch để tới đánh"). |
| 8 | Client sửa đổi nào trong map 379 cũng rút được toạ độ địch mỗi 4 s | Bản chất thiết kế; chỉ có chặn nhịp giới hạn. |

Đã kiểm OK theo phản biện: chặn biên `nIndex`/`m_nIndex`/subworld; độ dài tin ≤ 197 < 200 < 255 (`sentlen` BYTE); `ExecuteScript(bGlobal=false)` không tái nhập; `m_dwID` NPC chỉ cấp ở `SetID`; `m_CurrentCamp` đúng trường với bot và client, `Revive` không xoá camp; bot máy chủ cũng nằm trong `Player[]` nên được liệt kê; tin bị ẩn khỏi chat, không vào vòng 4 khe (Dã Tẩu không nhiễu); tương thích hai chiều; chi phí ~50 µs CPU + ~200 byte mỗi 4 s/người.
...
| server | `KProtocolProcess.cpp` `c2sNeedCount` | nhánh `dwId == TK_DICH_ID`: chặn **4 s/người** theo chủ khe (`GAME_FPS*4`, reset khi đổi chủ khe như SapMap) → chỉ trả khi người hỏi ở subworld có `m_SubWorldID == 379` và `m_CurrentCamp` 1/2 → quét `Player[1..MAX_PLAYER)`: cùng subworld, camp đối phương (3 − camp mình), còn sống (`do_death/do_revive/life ≤ 0` loại), **ngoài 2 vòng hậu doanh R 1.440 mps** quanh (1229,3561)/(1689,3074) (trong đó có trap vào trại, không tới được) → giữ **9 gần nhất** (chèn vào mảng nhỏ đã sắp) → `"[TKDich] id:x:y ..."` toạ độ Ô, ≤ 197 byte (≤ trần 200 / BYTE 256 của kênh chat) → `ExecuteScript(mgs2player_from_c.lua)` = `Msg2Player`. |
...
| client | `CoreShell.cpp` | `TK_DichNhan` (mỗi nhịp `TK_Process`: chụp trả lời đúng lúc đến, parse `%u:%d:%d`, giữ mục đang đi nếu còn trong tin, đánh dấu mục đã bỏ 30 s, 0 mục → quên "điểm cuối thấy địch" + bỏ chuyến khu xuất quân); `TK_DichHoi` (hỏi mỗi **6 s**, chỉ khi không thấy địch và ô "Đánh người" bật); `TK_DichXa` xen vào `TKP_FIGHT` **trước `TK_RaoDi`**: chọn mục gần nhất chưa bỏ, không bị loại, `FindPath == 1` (2 → bỏ mục 30 s, thử mục khác; < 0 lưới đang nạp → thử nhịp sau; thăm dò thất bại → tính lại đường ngay) → `DT_WalkTo`; vào tới Tầm nhìn PK mà tầng săn/PK vẫn không thấy (địch đã dời) → bỏ mục, **hỏi lại ngay**; kẹt 3 s → bỏ mục; trả lời cũ > **15 s** → về rảo. Báo *"Máy chủ báo địch cách N ô - tiến tới."* (30 s/lần). Log `[TK-DICH]`. |

**Chi phí:** mỗi người hỏi ≤ 1 lần/4 s → máy chủ một vòng so sánh 1.500 khe (~30 µs) + một `ExecuteScript` (~20 µs, như SapMap) + tin ≤ 197 byte; 50 người dùng WAuto trong trận ≈ 12 vòng quét/giây, không đáng kể (phản biện ước 1.500 người hỏi cùng lúc cũng chỉ ~20 ms CPU/s). Client: parse chuỗi ngắn + tối đa 9 `FindPath` khi đổi mục.
...
**Swap:** (1) tắt `GameServer.exe` → `bin\server\ChayGameServer.bat` (đổi `CoreServer.dll.moi` → `.dll`, giữ `.truoc` = `b68899b2`); (2) thoát Game.exe → `ChoiGame.bat`. Kiểm: `grep -c "[TKDich]" CoreServer.dll` = 1, md5 server `bdd9bb46`, md5 client `9e7a516d`. **Nghiệm thu:** ra trại không thấy ai → trong ≤ 6 s có `[TK-DICH] server tra N dich (giu muc k): [TKDich] id:x:y ...` rồi `[TK-DICH] toi dich id=…` và nhân vật chạy thẳng tới; tới nơi có địch thì `[TK-SAN]`/`[PK-…]` tiếp quản. Server cũ → không có dòng `[TK-DICH] server tra`, auto vẫn rảo như trước.
