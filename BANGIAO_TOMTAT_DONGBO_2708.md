...
1. **Nghiệm thu S12 27/08 tối = TRƯỢT** (log 19:37-19:51: 2 thang S8-NAN 11+4 cú, S12-THEO=0) — GỐC: cửa sổ chỉ mở ở nhánh S8-NAN, còn LỆNH DẮT tới ngay lúc teleport (nhánh đặt-lại-mồ-côi, cửa chưa mở) → vứt; gác echo VÔ CAN (42 lệnh trong cửa sổ đều là echo thật). KÈM gốc mới triệu chứng "NPC chớp tắt/biến mất-hiện lại": `GetMapDisX` trả VOID_DIS khi self chưa có region → S6-VANH gỡ mỗi frame ↔ S6_XaQuaTam nhận hết (179 flap <500ms) + bão recenter theo từng cú snap (97 ca biến mất 2-60s). **Đọc mục 9.27** (chẩn đoán) + **9.28** (28/08: tái hiện lần 2 chốt gốc, ĐÃ THI CÔNG V1a/V1b/V2/V3 = client `33e9412d` + server `cdc783b7` CHỜ restart+relog; tái áp thêm `goi_va_S12b_cuaso_vanh.py` CUỐI chuỗi script).
