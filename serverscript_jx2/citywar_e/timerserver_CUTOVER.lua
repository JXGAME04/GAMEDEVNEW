...
Include("\\script\\tinhnang\\lien_dau\\lib_liendau.lua")
...
Include("\\script\\timerserver_ctc.lua")	-- DOT E (E6/E8): dong ho 5 pha cong thanh
...
function RunTime()
	...
	-- DOT E (E6): cong thanh JX2 - 5 pha 0h/18h/19h/20h (guard ngay trong ham;
	-- dat SAU cac call san co de loi khong keo sap Tong Kim/bao tri - KEHOACH 1.1)
...
if (nHr == 16 or nHr == 22) and nMi > 11 and nMi <= 54 then
	...
	for i = 1, GetCountPlayerMax() do
		PlayerIndex = i
		local w, _, _ = GetWorldPos()
		if w == 209 and GetFightState() == 1 then
			count = count + 1
			lastName = GetName()
		end
	end
