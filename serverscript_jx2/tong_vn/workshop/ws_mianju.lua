...
function ws_main(nTongID, nWorkshopID)
	...
	Say("<#>Tæng qu¶n DÞ dung ph­êng: Xem 72 phÐp biÕn hãa cña ta! ChØ cã nghÜ kh«ng tíi chø kh«ng cã viÖc biÕn kh«ng thµnh. \n c¸c lo¹i mÆt n¹, lo¹i g× còng cã, xem thö cho biÕt!", 3, "Ta muèn xem thö /#use_g_1_ok("..nTongID..","..nWorkshopID..")", 
...
function Mask_Team(nTongID, nWorkshopID)
	...
	if (nLevel < TEAMMASK_LEVEL)then
		Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ ®éi tr­ëng cã thÓ do ®éi tr­ëng hoÆc thµnh viªn trong ®éi sö dông. Sau khi sö dông th× tÊt c¶ c¸c thµnh viªn sÏ gièng hÖt nhau trong mét kho¶ng thêi gian. Cã ®iÒu ®¼ng cÊp DÞ dung ph­êng ®¹t cÊp <color=red>"..TEAMMASK_LEVEL.."<color> míi cã thÓ chÕ t¸c lo¹i mÆt n¹ nµy, hiÖn vÉn ch­a ®ñ cÊp",
			1, "Lóc kh¸c ta quay l¹i vËy./cancel")
		return
	end
	Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ ®éi tr­ëng cã thÓ do ®éi tr­ëng hoÆc thµnh viªn trong ®éi sö dông. Sau khi sö dông th× tÊt c¶ c¸c thµnh viªn sÏ gièng hÖt nhau trong mét kho¶ng thêi gian. ChÕ t¸c lo¹i mÆt n¹ nµy cÇn sö dông mét Ýt ®iÓm cèng hiÕn vµ 1 lo¹i mÆt n¹ bang héi ®Ó lµm mÉu, cã ®ång ý kh«ng?",
...
function Mask_Tong(nTongID, nWorkshopID)
	...
	if (nFigure ~= TONG_MASTER and nFigure ~= TONG_ELDER)then
		Say("Tæng qu¶n DÞ dung ph­êng: Bang qui chØ cho phÐp bang chñ vµ tr­ëng l·o ®Ó ta chÕ t¸c lo¹i mÆt n¹ nµy, ng­¬i th× kh«ng thÓ…", 1, 
...
function GetMask2(nTongID, nWorkshopID)
	GiveItemUI("ChÕ t¹o mÆt n¹ ®éi ngò","Tæng qu¶n DÞ dung ph­êng: H·y bá vµo mét lo¹i mÆt n¹ th­êng ®Ó lµm mÉu, chÕ t¹o thµnh c«ng sÏ mÊt ®i vËt mÉu nµy.", "MakeMask2", "onCancel" );
end

--°ï»áÃæ¾ß
function GetMask3(nTongID, nWorkshopID)
	GiveItemUI("ChÕ t¹o mÆt n¹ bang héi","Tæng qu¶n DÞ dung ph­êng: H·y bá vµo mét lo¹i mÆt n¹ th­êng ®Ó lµm mÉu, chÕ t¹o thµnh c«ng sÏ mÊt ®i vËt mÉu nµy.", "MakeMask3", "onCancel" );
end
...
function MakeMask2(nCount)		
	...
	if (not nPrice)then
		Say("Tæng qu¶n DÞ dung ph­êng: Kh«ng ®­îc råi, kh«ng thÓ dïng mÆt n¹ nµy lµm mÉu ®Ó chÕ t¹o mÆt n¹ ®éi ngò.", 1, "Ta ®æi mét c¸i vËy./cancel")
		return
	end	
	Say("Tæng qu¶n DÞ dung ph­êng: Dïng mÆt n¹ mÉu nµy ®Ó chÕ t¹o mÆt n¹ ®éi ngò cÇn tiªu phÝ <color=yellow>"..nPrice.."<color> ®iÓm cèng hiÕn, nÕu ng­¬i ®ång ý th× ta sÏ ®éng thñ vËy.",
...
function MakeMask3(nCount)		
	...
	Say("Tæng qu¶n DÞ dung ph­êng: Dïng mÆt n¹ mÉu nµy ®Ó chÕ t¹o mÆt n¹ ®éi ngò sÏ tiªu phÝ ng©n s¸ch kiÕn thiÕt<color=yellow> "..nPrice.."<color> ®iÓm cèng hiÕn, nÕu ng­¬i ®ång ý th× ta sÏ ®éng thñ vËy.",
...
function TeamMaskMake(nFeature, nPrice, nItemIdx)
	if (GetContribution() < nPrice)then
		Say("Tæng qu¶n DÞ dung ph­êng: Dïng mÆt n¹ nµy lµm mÉu cho mÆt n¹ ®éi ngò cÇn sö dông <color=yellow>"..nPrice.."<color> ®iÓm cèng hiÕn. §iÓm cèng hiÕn cña ng­¬i kh«ng ®ñ, h·y gãp søc cho bang héi tr­íc ®·.",
...
function USE_G_2(nTongID, nWorkshopID)
	Say("<#>Tæng qu¶n DÞ dung ph­êng: Nh÷ng mÆt n¹ phæ th«ng do bang ta chÕ t¹o cã thÓ dïng ®iÓm cèng hiÕn ®Ó ®æi.", 2, 
