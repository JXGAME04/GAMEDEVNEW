...
function WeekGoalPrice(nTongID)
	...
	Msg2Player("<#>§· hoµn thµnh môc tiªu tuÇn bang héi, nhËn ®­îc phÇn th­ëng ®iÓm cèng hiÕn:"..nPrice.." ®iÓm vinh dù")
	Msg2Player("<#>§· hoµn thµnh môc tiªu tuÇn bang héi, nhËn ®­îc phÇn th­ëng ®iÓm kinh nghiÖm: "..(nPrice*10000).." ®iÓm vinh dù")
...
--				"°ï»á°ïÖúĞÅÏ¢/#tong_help()",
	...
	if figure == TONG_MASTER then
		tinsert(aryDescribe, "NhËn th­ëng nhãm ¸p Tiªu Bang/#tong_yabiaoaward_say"..param);
	end
	tinsert(aryDescribe, "Nép R­¬ng ¸p Tiªu Bang/#tong_handyabiaobox"..param);
...
function GetDayPrice(nTongID)
	...
	if (nPrice > 0)then
		AddContribution(nPrice)
		Msg2Player("NhËn ®­îc <color=gold>"..nPrice.."<color>NhÊn vµo ®é cèng hiÕn")
	end
...
function tong_levelchoose(nTongID)
	...
	Say("<#>Møc ®é khã môc tiªu tuÇn cña bang héi lµ cÊp <color=yellow>"..nLevel..",\n\n ThiÕt lËp ®é khã môc tiªu tuÇn hiÖn t¹i sÏ cã t¸c dông vµo tuÇn sau, muèn thiÕt lËp l¹i ®é khã cña môc tiªu tuÇn kh«ng?", 3, "§é khã cÊp 1/#select_level(1)", "§é khã cÊp 2/#select_level(2)", "Ta kh«ng dïng ®Õn/tong_cancel")
end
...
function select_level(nLevel)
	...
	Say("<#>§é khã môc tiªu tuÇn hiÖn t¹i lµ: <color=yellow>"..nLevel.."", 0)
end
...
function LUP_PriceInfo(nLevel, nOrder)
	...
	elseif nOrder == nil then
		...
		function S:GenParam()
			self.i = self.i+1
			return "thø "..self.i.."phÇn th­ëng/#LUP_PriceInfo("..self.l..","..self.i..")"
		end
...
function LUP_GetPrice()
	...
	Say(szOrder..", hiÖn t¹i cã <color=yellow>"..nBossFu.."<color> c¸i <color=yellow>LÖnh bµi gäi Boss cÊp 10<color>, <color=yellow>"..nContributionPie.."<color> c¸i <color=yellow>Vâ ThÇn Ên<color> vµ <color=yellow> "..nFruit.."<color>qu¶ Huy Hoµng cao cÊp<color> cã thÓ nhËn, h·y quyÕt ®Şnh xem lÇn nµy muèn nhËn g× vµ sè l­îng bao nhiªu", 
		getn(tbSel), tbSel)
end

function LUP_GivePrice(nWhich, num)
	local aName = {"LÖnh bµi gäi Boss cÊp 10", "Vâ ThÇn Ên", "Qu¶ Huy Hoµng (cao) "}
	Say("§· nhËn tr­íc <color=yellow>"..num..""..aName[nWhich].."<color>, h·y b¶o ®¶m hµnh trang ®ñ chç trèng, nÕu cã rít ra ®Êt bŞ ng­êi kh¸c nhÆt ®­îc còng ®õng tr¸ch ta kh«ng nh¾c nhë.", 
...
function tong_yabiaoaward_say(nTongId)
	local szTongName, nTongId = GetTongName();
    if nTongId == 0 then
        return
    end
	local nAwardCount = TONG_GetTaskValue(nTongId, 1153);
	...
	local szNpcTitle = format("B¶o r­¬ng ¸p tiªu cña quı bang hiÖn t¹i lµ %d, x¸c nhËn nhËn?", nAwardCount)
	local szOption = format("X¸c nhËn/#tong_yabiaoaward(%d)", nTongId)
	local szReturn = format("Trë vÒ/main")
	CreateTaskSay({szNpcTitle, szOption, szReturn});
end

function tong_yabiaoaward(nTongId)
    local szScripPath = "\\script\\activitysys\\config\\129\\extend.lua";
    local szFunName = "pActivity:GetTongAward"
    --DynamicExecuteByPlayer(PlayerIndex, szScripPath, szFunName);
    pActivity:GetTongAward()
end

function tong_handyabiaobox(nTongId)
    local szScripPath = "\\script\\activitysys\\config\\129\\extend.lua";
    local szFunName = "pActivity:HandInBiaoCheBox"
    --DynamicExecuteByPlayer(PlayerIndex, szScripPath, szFunName);
    pActivity:HandInBiaoCheBox()
end
