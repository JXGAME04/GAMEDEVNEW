Include("\\script\\header\\cauhinh_hoatdong.lua")	-- [HD CONFIG 24/08] exp Hoang Chan Don chinh duoc
-- ÎÄ¼þÃû¡¡£ºchuangongdan.lua
-- ´´½¨Õß¡¡£ºzhongchaolong
-- ´´½¨Ê±¼ä£º2008-02-25 16:25:21
IncludeLib("SETTING");
--Include("\\script\\lib\\pay.lua");	-- [TONGWAR 23/08] tep khong co tren JX1, khong dung
--Include("\\script\\task\\task_addplayerexp.lua");	-- [TONGWAR 23/08] tep khong co tren JX1, khong dung
--Include("\\script\\event\\BeiDouChuanGong\\lib\\addaward.lua");	-- [TONGWAR 23/08] tep khong co tren JX1, khong dung
...
function main(nItemIndex)
	...
	if szMsg_Huang ~= nil then
		...
		szTitle	= format("Xin chµo <color=yellow>%s<color>, sö dông hoµng ch©n ®¬n cã thÓ nhËn ®­îc <color=yellow>%s<color> kinh nghiÖm. X¸c nhËn muèn sö dông?",GetName(), HD_CFG("HCD_EXP", 2000000000))
		tbOpt = 
		{
...
function BeiDouChuanGong_UseItem(nItemIndex)
	...
	if szMsg_Huang ~= nil then
		...
		if RemoveItemByIndex(nItemIndex) == 1 then
			tbAwardTemplet:GiveAwardByList({nExp_tl = HD_CFG("HCD_EXP", 2000000000);}, format("%s sö dông hoµng ch©n ®¬n", GetName()));
			SetTask(TSK_BeiDouChuanGong_USEDTIME_HUANG, GetCurServerTime());
			AddSkillState( 509, 1, 1, 180);	-- [TONGWAR 23/08] JX1: p3=1 ap skill that (Linux luon ap that)
		end
	end
end
...
function GetDesc(nItem)
	return format("\nChøa <color=yellow>%s<color> kinh nghiÖm", HD_CFG("HCD_EXP", 2000000000));
end
