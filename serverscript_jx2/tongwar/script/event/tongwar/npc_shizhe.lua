...
ONETONG_START_SIGNUP	= 20140407						-- ÌìÏÂµÚÒ»°ï¿ªÊ¼µÇ¼ÇÊ±¼ä
ONETONG_ENDLE_SIGNUP	= 20140417						-- ÌìÏÂµÚÒ»°ï½áÊøµÇ¼ÇÊ±¼ä

ONETONG_START_TITLE		= 20100118						-- ÌìÏÂÒ»°ïÁìÈ¡Í·ÏÎ¿ªÊ¼Ê±¼ä
ONETONG_ENDLE_TITLE		= 20100214						-- ÌìÏÂÒ»°ïÁìÈ¡Í·ÏÎ½áÊøÊ±¼ä

ONETONG_MAX_SIGNMEM		= 54
...
function onetong_showlistmem()
	...
	if (FALSE(n_lid)) then
		CreateTaskSay({"<dec><npc>ChØ cã bang chñ Vâ L©m §Ö NhÊt Bang lÇn 15 míi cã thÓ thùc hiÖn thao t¸c nµy.", "KÕt thóc ®èi tho¹i/OnCancel"});
		return 0;
	end;
...
function onetong_awardtitle()
	...
	if((nTongID == 0 or nTongID == -1) or szTongName ~= %SZTONGNAME_ONETONG or n_titletype == 1) then	-- [TONGWAR 23/08] bo 'vao bang >= 7 ngay' (DIEUKIEN #21)
