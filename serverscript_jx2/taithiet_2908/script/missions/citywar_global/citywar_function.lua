...
function take_tong_award(count)
	...
	local msg =	format("%s %s (%s) nhËn %s huyÒn tinh cÊp 5 tõ Kim s¬n ch­ëng m«n nh©n",GetLocalDate("[%y-%m-%d %H:%M] "),GetAccount(),GetName(),1)
		WriteLog(msg);
...
function take_tong_resaward()
	...
	-- [VACTC 30/08] bien `count` khong duoc khai o dau trong ham; cac dong
	-- lan can deu dung ten khac. format voi nil nem loi Lua, cat dut ham
	-- ngay tai day nen nguoi choi khong nhan duoc cau thong bao. (co bien nCount khai ngay tren)
	local msg =	format("%s %s (%s) nhËn %s huyÒn tinh cÊp 5 tõ Kim s¬n ch­ëng m«n nh©n",GetLocalDate("[%y-%m-%d %H:%M] "),GetAccount(),GetName(),nCount)
	WriteLog(msg);
