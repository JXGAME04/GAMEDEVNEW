...
Điều kiện: script nạp lúc boot → **khởi động lại máy chủ** là có phần Thế giới + phần Bang dạng "Tin bang" (DLL cũ: dòng vào khung chính qua `Msg2Tong`, vẫn chỉ tới thành viên). Muốn dòng bang vào **đúng thẻ bang** cần `CoreServer.dll` có `AUC_MsgTong`: **khe `bin\server\CoreServer.dll.moi` = f576ac9e** (16:25; gồm gỡ ROLECHK 9fb1f175 của wauto-go-rolechk + chienlenh a18b5eb8 + việc này; đã kiểm `RoleChk` = 0 và đủ dấu `CL_Cong` `st_ledger` `AUCWEB_ClaimRound` `SapNpc` `TKDich`; bản cũ của wauto-4c lưu `CoreServer.dll.moi.bot_b67f002f`). **ĐÃ CHẠY THẬT: chủ chạy bat lúc 16:20 → GameServer đang chạy f576ac9e + script mới**; 16:22 bang `TESTGAME` đưa món id 38 lên phiên bang, 16:23 web mở đợt 2 (id 39-41) — không một dòng `LOI Lua`; id kênh bang đã học lúc 16:21 (`\O305419896`, `\O441122550`). Việc của phiên khác chờ chủ quyết: `rainbow.dll.moi` (e8056284) + `bin\multiserver\heaven.dll.moi` (ad7e6985) của wauto-4c (vá RECV `origin/main` d028ea41 — build CoreServer sau phải gộp). Mã: `mail-0309` c8264cf5 → `origin/main` 4f7f2e2e. Chi tiết kỹ thuật: `BANGIAO_DAUGIA_WEB_THICONG_0409.md` mục 15.
