...
### 10.7 SỰ CỐ 19:34 — "mở GameServer lên bị dis" = SẬP LÚC BOOT, KHÔNG do bản BOTNOI
Chủ: *"mở gameserver lên bị dis"* · *"nếu không phải phiên bạn làm lỗi thì bạn xác định phiên nào thì báo bên đó để họ fix"*.

- **Hiện tượng:** 4 lần mở (19:34:22, 19:34:54, 19:35:22, 19:35:48) đều sập ngay sau `mysql_core.log Init OK`, chưa tới "Khoi dong hoan tat"; 4 dump 5,8 GB ghi tại `script\bando\dongbac\truongbachson\truongbachsoncuoc\obj\DumpInfo\` (CWD lúc sập = thư mục loader đang quét vì `LoadScriptInDirectory` `chdir` vào từng thư mục). Người chơi "bị dis" = máy chủ chết.
- **Stack** (PDB CoreServer của tôi + PDB Lua54Dll x64 17:58 ở `D:\GAMEDEVNEW_wt_lua54b`): `LoadScriptInDirectory` (KSortScript.cpp:434) → `LoadScriptToSortListA` (KSortScript.cpp:318 gọi `g_ScriptSet[n].Load`) → `lua4_compilebuffer` (lua4compat.c:1027) → `luaL_loadbufferx` → `luaY_parser` → `luaF_newLclosure` → `luaC_newobj` → `luaM_malloc_` (lmem.c:206) **ACCESS_VIOLATION**; `lua_State* L` = `0x2480C980061` / `0x1E066070061` / `0x21EE6970061` — hai byte thấp luôn `0x61 0x00` = **"a\0"** (đuôi ".lua").
- **Gốc:** `KLuaScript::Load` (Engine/Src/KLuaScript.cpp:107) làm `strcpy(m_szScriptName, Filename)` **không giới hạn** vào `char m_szScriptName[100]`; kế tiếp là `int m_UserTag` (+4 đệm) rồi `Lua_State* m_LuaState` (KLuaScript.h:90-92) → đường dẫn tương đối 100-107 ký tự đè `m_UserTag` (im lặng), **≥ 108 đè `m_LuaState`** → gọi Lua với con trỏ rác → sập. Tệp đầu tiên đủ dài theo thứ tự quét: `\script\bando\dongbac\truongbachson\truongbachsoncuoc\obj\lotieu_truongbachsoncuoc_to_truongbachsonbacloc.lua` = **109 ký tự** ('a' ở vị trí 108, NUL ở 109 → đúng `0x61 0x00`). `g_StrCpyLen(..., 100)` ở KSortScript.cpp:317 an toàn (strncpy + NUL ở [99]); lỗi engine có sẵn từ xưa, chỉ bùng khi tên dài.
- **Ai kích hoạt:** phiên **SAPXEP / Lua54** ("Lua 4 to 5.4 migration analysis", commit 9536efea / c92cf394 19:25): `gom_bando.py` đổi 1.316 tệp trap/obj/npc Hán sang tên Việt không dấu → **321 đường dẫn ≥ 100 ký tự, 153 ≥ 108** (taybac_bac 65, taynam_bac 25, taynam_nam 22, trungnguyen_bac 22, taybac_nam 14, dongbac 2, trungnguyen_nam 2, giangnam 1; dài nhất 140). Tên Hán cũ (GBK) ngắn hơn nên boot 19:03 vẫn ổn. Bản `CoreServer.dll` 6ab3dfbf (BOTNOI đợt 4) **không đụng** KSortScript / KLuaScript (đã so diff các commit của tôi) — không phải nguyên nhân. Đã nhắn phiên đó 2 tin (`send_message` 19:5x) kèm stack + hai hướng sửa.
- **Khắc phục tạm để mở máy chủ ngay (chủ tự quyết):** đổi tên `bin\server\script\bando` → `bin\server\script\_bando_tam` (loader bỏ qua tên bắt đầu `_`) rồi chạy `ChayGameServer.bat` — trap các map đó chết như trước 19:18 cho tới khi phiên SAPXEP rút ngắn tên (mọi đường dẫn tương đối tính cả `\script\` phải < 100 ký tự) hoặc vá `strcpy` → `g_StrCpyLen(m_szScriptName, Filename, sizeof)` trong Engine (không đổi bố cục lớp, chỉ build lại engine.dll; `KObj.cpp:439` cùng kiểu). Xoá 4 dump 23 GB trong `obj\DumpInfo` sau khi xong.
...
### 10.8 Phiên Lua54 đã sửa gốc sập boot; ĐỢT 5 — túi bot đầy bình thuốc nên chưa nhận lại vũ khí/ngựa
- Phiên Lua54 nhắn lại 20:5x: rút gọn 527 tệp bando (mọi đường dẫn ≤ 92 ký tự), vá `KLuaScript::Load` strcpy → strncpy (commit 0ef17789, chỉ engine.dll), dời 28 GB dump ra `bin\_luutru\0609\dump_1934`; máy chủ mở lại 20:45 và 20:57 với bộ `.moi` của họ: **CoreServer 7e41f1b7** (build từ origin/main 642eeaef, có đủ 14 nhãn BOTNOI đợt 1-4 — đã kiểm), engine 6457bc53, Lua54Dll 889d92e7. 1000 bot sống, không sập.
- **Vấn đề còn lại (đo bot.log 20:47-20:59):** 1.056 dòng `[BotMac] … tui DAY … -> giu do cu` trên **459 bot** (492 vũ khí nhập môn, 405 ngựa, 159 vũ khí cấp 10), **0** dòng `lam cho tui` → `pb_LamChoTui` không xoá được món nào = túi toàn đồ được "giữ". Gốc: bot nhặt **bình thuốc** rơi (`item_medicine`) và `pb_DonTui` giữ hết → `[BotUong] con 47..51 binh` = **47-51 chồng bình**, mỗi chồng 1 ô → túi đầy vĩnh viễn, không còn dải 1×3 / 2×3. Bot chỉ uống bình **đầu tiên** tìm thấy nên giữ vài chồng là đủ; bình nhặt lại liên tục.
- **Sửa** (`goi_va_botnoi_dot5_0609.py`): `pb_XoaBinhThua(nIdx, nGiu, nW, nH)` giữ nGiu chồng tốt nhất (bình HP detail 0 trước, chồng lớn trước) và xoá dần tới khi có dải ô; `pb_LamChoTui` gọi nó với `PB_BINH_GIU_KHAN` = 4 sau vòng xoá rác; `pb_DonTui` định kỳ cắt về `PB_BINH_GIU` = 12 chồng để túi không đầy lại; khi vẫn thất bại, `[BotMac]` ghi kèm thống kê túi theo genre (tối đa 60 dòng/phiên) để biết túi chứa gì.
- **Nhị phân đợt 5:** `bin\server\CoreServer.dll.moi` md5 **31f1537ca025787f8c3440828176129b** (18.463.232), build từ `origin/main` 478b0fec (= bản 7e41f1b7 của phiên Lua54 + đợt 5).
- **21:29 cập nhật:** phiên Lua54 đã THAY `.moi` bằng bản của họ **34a3fdd54f0b10c742db84e5ed4dce9a** (18.463.232, build `origin/main` 9bca93a6 = đợt 5 của tôi + sửa [DIALOG] callback hộp thoại trong Tống Kim). Tôi kiểm lại trên đĩa: đủ 14 nhãn BOTNOI + `chong binh` 2 → là superset, dùng bản này (bỏ 31f1537c). Nghiệm thu sau swap: `grep -c "lam cho tui" bot.log` > 0 và `grep -c "giu do cu" bot.log` giảm mạnh; `dang cam=1` / `len vu khi cap 10` / `cuoi ngua Chieu Da` xuất hiện cho ~459 bot; `[BotXoaDo] … chong binh thua` khi bot đầy túi.
